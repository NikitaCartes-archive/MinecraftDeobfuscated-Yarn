	...
	private void method_1760() {
		...
		if (lv != null) {
			...
			for(int x = class_3532.method_15386((f + (float)p) / 2.0F) - 1; x >= 0; --x) {
				...
				if (bl) {
					if (x * 2 + 1 < j) {
						this.method_1788(ab, ac, y + 54, 9 * ad, 9, 9);
					}

					if (x * 2 + 1 == j) {
						this.method_1788(ab, ac, y + 63, 9 * ad, 9, 9);
					}
				}

				if (u > 0) {
					if (u == p && p % 2 == 1) {
						this.method_1788(ab, ac, y + 153, 9 * ad, 9, 9);
						--u;
					} else {
						this.method_1788(ab, ac, y + 144, 9 * ad, 9, 9);
						u -= 2;
					}
				} else {
					if (x * 2 + 1 < i) {
						this.method_1788(ab, ac, y + 36, 9 * ad, 9, 9);
					}

					if (x * 2 + 1 == i) {
						this.method_1788(ab, ac, y + 45, 9 * ad, 9, 9);
					}
				}
			}
			...
			if (y == 0) {
				...
				for(int z = 0; z < 10; ++z) {
					...
					int ad = n - z * 8 - 9;
					this.method_1788(ad, aa, 16 + ac * 9, 27, 9, 9);
					if (z * 2 + 1 < k) {
						this.method_1788(ad, aa, ab + 36, 27, 9, 9);
					}

					if (z * 2 + 1 == k) {
						this.method_1788(ad, aa, ab + 45, 27, 9, 9);
					}
				}
