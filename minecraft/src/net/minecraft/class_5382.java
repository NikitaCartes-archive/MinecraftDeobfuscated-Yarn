...
public class class_5382<T> extends class_5379<T> {
	...
	private <E> DataResult<Supplier<E>> method_29763(
		...
		if (object != null) {
			return DataResult.success((Supplier)() -> object, Lifecycle.stable());
		} else {
			class_5382.class_5383<E> lv = this.method_29761(registryKey);
			DataResult<Supplier<E>> dataResult = (DataResult)lv.field_25513.get(registryKey2);
			if (dataResult != null) {
				return dataResult;
			} else {
				Supplier<E> supplier = Suppliers.memoize(() -> {
					E objectxx = mutableRegistry.get(registryKey2);
					if (objectxx == null) {
