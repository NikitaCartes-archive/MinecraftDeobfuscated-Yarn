...
public class class_3692 implements class_3696 {
	...
	public class_3692(Map<String, Long> map, Map<String, Long> map2, long l, int i, long m, int j) {
	...
	public List<class_3534> method_16067(String string) {
		...
		long l = this.field_16277.containsKey("root") ? this.field_16277.get("root") : 0L;
		long m = this.field_16277.getOrDefault(string, -1L);
		...
		for(String string3 : this.field_16277.keySet()) {
			if (string3.length() > string.length() && string3.startsWith(string) && string3.indexOf(".", string.length() + 1) < 0) {
				o += this.field_16277.get(string3);
			}
		}
		...
		Set<String> set = Sets.newHashSet(this.field_16277.keySet());
		...
		for(String string4 : set) {
			if (string4.length() > string.length() && string4.startsWith(string) && string4.indexOf(".", string.length() + 1) < 0) {
				long p = this.field_16277.getOrDefault(string4, 0L);
				...
				list.add(new class_3534(string5, d, e, q));
			}
		}

		for(String string4 : this.field_16277.keySet()) {
			this.field_16277.put(string4, this.field_16277.get(string4) * 999L / 1000L);
		}

		if ((float)o > f) {
			list.add(new class_3534("unspecified", (double)((float)o - f) * 100.0 / (double)o, (double)((float)o - f) * 100.0 / (double)l, n));
		}

		Collections.sort(list);
		list.add(0, new class_3534(string2, 100.0, (double)o * 100.0 / (double)l, n));
		return list;
	}
		...
		if (list.size() >= 3) {
			for(int j = 1; j < list.size(); ++j) {
				...
				stringBuilder.append(lv.field_15738)
					.append('(')
					.append(lv.field_19384)
					.append('/')
					.append(String.format(Locale.ROOT, "%.0f", (float)lv.field_19384 / (float)this.field_19383))
					.append(')')
					.append(" - ")
