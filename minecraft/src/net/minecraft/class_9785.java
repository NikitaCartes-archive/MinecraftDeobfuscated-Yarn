...
public class class_9785 extends DataFix {
	private static final Map<UUID, String> field_51992 = ImmutableMap.<UUID, String>builder()
		...
		.build();
	private static final Map<String, String> field_51993 = Map.of(
		"Random spawn bonus",
	...
	public class_9785(Schema schema) {
		super(schema, false);
	}
	...
	protected TypeRewriteRule makeRule() {
		...
		return TypeRewriteRule.seq(
			this.fixTypeEverywhereTyped(
				"AttributeIdFix (ItemStack)", type, typed -> typed.updateTyped(opticFinder, typedx -> typedx.update(DSL.remainderFinder(), class_9785::method_60686))
			),
			this.fixTypeEverywhereTyped("AttributeIdFix (Entity)", this.getInputSchema().getType(TypeReferences.ENTITY), class_9785::method_60679),
			this.fixTypeEverywhereTyped("AttributeIdFix (Player)", this.getInputSchema().getType(TypeReferences.PLAYER), class_9785::method_60679)
		);
	}

	private static Stream<Dynamic<?>> method_60683(Stream<?> stream) {
		return method_60687((Stream<Dynamic<?>>)stream);
	}

	private static Stream<Dynamic<?>> method_60687(Stream<Dynamic<?>> stream) {
		Map<String, Dynamic<?>> map = new Object2ObjectArrayMap<>();
		stream.forEach(dynamic -> {
			UUID uUID = method_60684(dynamic.get("uuid").asIntStream().toArray());
			String string = dynamic.get("name").asString("");
			String string2 = uUID != null ? (String)field_51992.get(uUID) : null;
			String string3 = (String)field_51993.get(string);
			if (string2 != null) {
	...
	private static Dynamic<?> method_60680(Dynamic<?> dynamic) {
	...
	private static Dynamic<?> method_60686(Dynamic<?> dynamic) {
		return dynamic.update(
			"attribute_modifiers",
			dynamicx -> dynamicx.update(
					"modifiers", dynamicxx -> DataFixUtils.orElse(dynamicxx.asStreamOpt().result().map(class_9785::method_60683).map(dynamicxx::createList), dynamicxx)
				)
		);
	}

	private static Dynamic<?> method_60688(Dynamic<?> dynamic) {
		return dynamic.renameField("Name", "id")
			...
			.renameAndFixField(
				...
				dynamic2 -> DataFixUtils.orElse(
						dynamic2.asStreamOpt().result().map(stream -> stream.map(class_9785::method_60680)).map(class_9785::method_60683).map(dynamic::createList), dynamic2
					)
			);
	}

	private static Typed<?> method_60679(Typed<?> typed) {
		return typed.update(
			DSL.remainderFinder(),
			dynamic -> dynamic.renameAndFixField(
					"Attributes",
					"attributes",
					dynamicx -> DataFixUtils.orElse(dynamicx.asStreamOpt().result().map(stream -> stream.map(class_9785::method_60688)).map(dynamicx::createList), dynamicx)
				)
		);
	}

	@Nullable
	public static UUID method_60684(int[] is) {
