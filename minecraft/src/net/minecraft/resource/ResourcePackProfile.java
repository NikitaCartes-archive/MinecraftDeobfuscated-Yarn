...
public class ResourcePackProfile implements AutoCloseable {
	...
	public static enum InsertionPosition {
		...
		public <T, P extends ResourcePackProfile> int insert(List<T> items, T item, Function<T, P> converter, boolean inverse) {
			ResourcePackProfile.InsertionPosition insertionPosition = inverse ? this.inverse() : this;
			if (insertionPosition == BOTTOM) {
				int i;
				for (i = 0; i < items.size(); i++) {
					P resourcePackProfile = (P)converter.apply(items.get(i));
