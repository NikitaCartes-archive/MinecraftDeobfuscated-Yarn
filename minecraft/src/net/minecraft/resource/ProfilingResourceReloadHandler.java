...
public class ProfilingResourceReloadHandler extends ResourceReloadHandler<ProfilingResourceReloadHandler.ProfilingInformation> {
	private static final Logger LOGGER = LogManager.getLogger();
	private final Stopwatch reloadTimer = Stopwatch.createUnstarted();

	public ProfilingResourceReloadHandler(
		...
		super(
			...
			resourceManager,
			list,
			(helper, resourceManagerx, resourceReloadListener, executor2x, executor3) -> {
				...
				CompletableFuture<java.lang.Void> completableFuturexx = resourceReloadListener.method_18222(
				...
				return completableFuturexx.thenApplyAsync(
					void_ -> new ProfilingResourceReloadHandler.ProfilingInformation(
							resourceReloadListener.getClass().getSimpleName(), profilerSystem.method_16064(), profilerSystem2.method_16064(), atomicLong, atomicLong2
						),
					executor2
				);
			},
	...
	private void finish(List<ProfilingResourceReloadHandler.ProfilingInformation> list) {
		this.reloadTimer.stop();
		int i = 0;
		LOGGER.info("Resource reload finished after " + this.reloadTimer.elapsed(TimeUnit.MILLISECONDS) + " ms");

		for(ProfilingResourceReloadHandler.ProfilingInformation profilingInformation : list) {
			ProfileResult profileResult = profilingInformation.prepareProfile;
			ProfileResult profileResult2 = profilingInformation.applyProfile;
			int j = (int)((double)profilingInformation.prepareTimeMs.get() / 1000000.0);
			int k = (int)((double)profilingInformation.applyTimeMs.get() / 1000000.0);
			int l = j + k;
			String string = profilingInformation.name;
	...
	public static class ProfilingInformation {
		...
		private ProfilingInformation(String string, ProfileResult profileResult, ProfileResult profileResult2, AtomicLong atomicLong, AtomicLong atomicLong2) {
			this.name = string;
