...
public class ProfilingResourceReloadHandlerImpl extends ResourceReloadHandlerImpl<ProfilingResourceReloadHandlerImpl.ProfilingInformation> {
	private static final Logger LOGGER = LogManager.getLogger();
	private final Stopwatch reloadTimer = Stopwatch.createUnstarted();

	public ProfilingResourceReloadHandlerImpl(
		...
		super(
			...
			(helper, resourceManagerx, resourceReloadListener, executor2x, executor3) -> {
				...
				CompletableFuture<java.lang.Void> completableFuturexx = resourceReloadListener.apply(
				...
				return completableFuturexx.thenApplyAsync(
					void_ -> new ProfilingResourceReloadHandlerImpl.ProfilingInformation(
		...
		this.completionFuture.thenAcceptAsync(this::finish, executor2);
	}

	private void finish(List<ProfilingResourceReloadHandlerImpl.ProfilingInformation> list) {
		this.reloadTimer.stop();
		int i = 0;
		LOGGER.info("Resource reload finished after " + this.reloadTimer.elapsed(TimeUnit.MILLISECONDS) + " ms");

		for(ProfilingResourceReloadHandlerImpl.ProfilingInformation profilingInformation : list) {
