...
class ResourcePath implements Path {
	...
	ResourcePath get(String name) {
		if (isSpecial(this.file)) {
			...
			ResourceFile resourcePath = this.file;
			if (resourcePath instanceof ResourceFile.Directory directory) {
				ResourcePath resourcePathx = (ResourcePath)directory.children().get(name);
				return resourcePathx != null ? resourcePathx : new ResourcePath(this.fileSystem, name, this, ResourceFile.EMPTY);
	...
	public Path toPath() {
		ResourceFile var2 = this.file;
		return var2 instanceof ResourceFile.File file ? file.contents() : null;
	}

	@Nullable
	public ResourceFile.Directory toDirectory() {
		ResourceFile var2 = this.file;
		return var2 instanceof ResourceFile.Directory directory ? directory : null;
	}
