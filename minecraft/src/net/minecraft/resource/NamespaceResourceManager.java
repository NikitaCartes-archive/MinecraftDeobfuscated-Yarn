...
public class NamespaceResourceManager implements ResourceManager {
	...
	public Map<Identifier, ResourceRef> findResources(String startingPath, Predicate<Identifier> allowedPathPredicate) {
		...
		for(Object2IntMap.Entry<Identifier> entry : Object2IntMaps.fastIterable(object2IntMap)) {
			...
			map.put(identifier2, new ResourceRef(string, () -> {
				Identifier identifier2xx = getMetadataPath(identifier2);
				InputStream inputStream = null;

				for(int j = this.packList.size() - 1; j >= k; --j) {
					NamespaceResourceManager.FilterablePack filterablePackxx = (NamespaceResourceManager.FilterablePack)this.packList.get(j);
					ResourcePack resourcePack2 = filterablePackxx.underlying;
					if (resourcePack2 != null && resourcePack2.contains(this.type, identifier2xx)) {
						inputStream = this.open(identifier2xx, resourcePack2);
						break;
					}

					if (filterablePackxx.isFiltered(identifier2xx)) {
						break;
					}
				}
	...
	private static void applyFilter(NamespaceResourceManager.FilterablePack pack, Map<Identifier, NamespaceResourceManager.EntryList> idToEntryList) {
		...
		while(iterator.hasNext()) {
			java.util.Map.Entry<Identifier, NamespaceResourceManager.EntryList> entry = (java.util.Map.Entry)iterator.next();
	...
	static record EntryList(Identifier metadataId, List<NamespaceResourceManager.Entry> entries) {
		final List<NamespaceResourceManager.Entry> entries;
