...
public class TagManagerLoader implements ResourceReloader {
	...
	public CompletableFuture<Void> reload(
		...
		return CompletableFuture.allOf((CompletableFuture[])list.toArray(i -> new CompletableFuture[i]))
			...
			.thenAcceptAsync(void_ -> this.registryTags = (List)list.stream().map(CompletableFuture::join).collect(Collectors.toUnmodifiableList()), applyExecutor);
	}
	...
	private <T> CompletableFuture<TagManagerLoader.RegistryTags<T>> buildRequiredGroup(
		...
		TagGroupLoader<RegistryEntry<T>> tagGroupLoader = new TagGroupLoader<>(registry::method_55841, getPath(registryKey));
