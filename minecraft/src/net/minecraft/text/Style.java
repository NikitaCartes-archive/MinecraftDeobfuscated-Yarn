...
public class Style {
	...
	private static final Style ROOT = new Style() {
		...
		@Override
		public Style setColor(TextFormat textFormat) {
	...
	public String getFormatString() {
		if (this.isEmpty()) {
			...
			if (this.isBold()) {
				stringBuilder.append(TextFormat.field_1067);
			}

			if (this.isItalic()) {
				stringBuilder.append(TextFormat.field_1056);
			}

			if (this.isUnderlined()) {
				stringBuilder.append(TextFormat.field_1073);
			}

			if (this.isObfuscated()) {
				stringBuilder.append(TextFormat.field_1051);
			}

			if (this.isStrikethrough()) {
				stringBuilder.append(TextFormat.field_1055);
			}
			...
			if (jsonElement.isJsonObject()) {
				...
				if (jsonObject == null) {
					...
					if (jsonObject.has("color")) {
						style.color = jsonDeserializationContext.deserialize(jsonObject.get("color"), TextFormat.class);
					}
