...
public record RawFilteredPair<T>(T raw, Optional<T> filtered) {
	...
	public static RawFilteredPair<String> of(FilteredMessage message) {
		return new RawFilteredPair((T)message.raw(), message.isFiltered() ? Optional.of(message.getString()) : Optional.empty());
	}
	...
	public <U> Optional<RawFilteredPair<U>> resolve(Function<T, Optional<U>> resolver) {
		Optional<U> optional = (Optional)resolver.apply(this.raw);
		if (optional.isEmpty()) {
			return Optional.empty();
		} else if (this.filtered.isPresent()) {
			Optional<U> optional2 = (Optional)resolver.apply(this.filtered.get());
			return optional2.isEmpty() ? Optional.empty() : Optional.of(new RawFilteredPair<Object>(optional.get(), optional2));
		} else {
			return Optional.of(new RawFilteredPair<Object>(optional.get(), Optional.empty()));
		}
	}
}
