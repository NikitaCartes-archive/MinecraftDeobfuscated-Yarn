	...
	private static Vec3d method_6371(MobEntityWithAi mobEntityWithAi, int i, int j, @Nullable Vec3d vec3d, boolean bl, double d) {
		...
		for(int n = 0; n < 10; ++n) {
			...
			if (blockPos != null) {
				...
				if ((!bl2 || mobEntityWithAi.isInAiRange(blockPos2)) && entityNavigation.isValidPosition(blockPos2)) {
					if (!bl) {
						blockPos2 = method_6372(blockPos2, mobEntityWithAi);
						if (method_6380(blockPos2, mobEntityWithAi)) {
							continue;
						}
					}

					float h = mobEntityWithAi.getPathfindingFavor(blockPos2);
					if (h > g) {
						g = h;
						k = o;
						l = p;
