...
public interface class_4069 extends class_364 {
	...
	default void method_20084(@Nullable class_364 arg) {
		class_364 lv = this.getFocused();
		if (lv != arg) {
			if (lv != null) {
				lv.onFocusChanged(true, false);
			}

			if (arg != null) {
				arg.onFocusChanged(true, true);
			}
	...
	default boolean method_20087(boolean bl) {
		...
		boolean bl2 = lv != null;
		if (bl2) {
			if (lv.method_20087(bl)) {
				return true;
			}

			lv.onFocusChanged(bl, false);
		}
		...
		while (booleanSupplier.getAsBoolean()) {
			...
			if (lv2.isPartOfFocusCycle()) {
				lv2.onFocusChanged(bl, true);
				this.setFocused(lv2);
				return true;
			}
		}
