...
public record DecoratableArgumentList<S>(List<DecoratableArgumentList.ParsedArgument<S>> arguments) {
	...
	public static <S> DecoratableArgumentList<S> of(ParseResults<S> parseResults) {
		...
		List<DecoratableArgumentList.ParsedArgument<S>> list;
		CommandContextBuilder<S> commandContextBuilder3;
		for(list = collectDecoratableArguments(commandContextBuilder);
	...
	private static <S> List<DecoratableArgumentList.ParsedArgument<S>> collectDecoratableArguments(CommandContextBuilder<S> contextBuilder) {
		...
		for(ParsedCommandNode<S> parsedCommandNode : contextBuilder.getNodes()) {
			...
			if (parsedArgument instanceof ArgumentCommandNode argumentCommandNode) {
				...
				if (var7 instanceof DecoratableArgumentType decoratableArgumentType) {
					com.mojang.brigadier.context.ParsedArgument<S, ?> parsedArgumentx = (com.mojang.brigadier.context.ParsedArgument)contextBuilder.getArguments()
						.get(argumentCommandNode.getName());
					if (parsedArgumentx != null) {
						list.add(new DecoratableArgumentList.ParsedArgument<S>(argumentCommandNode, parsedArgumentx, decoratableArgumentType));
					}
				}
			}
		}
