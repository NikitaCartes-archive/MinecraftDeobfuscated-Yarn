...
public class PackratParsing {
	public static <T, C, P> ArgumentParser<List<T>> createParser(PackratParsing.Callbacks<T, C, P> callbacks) {
		...
		parsingRules.set(
			...
			results -> (T)Optional.ofNullable(results.getAny(symbol4, symbol5))
		);
		parsingRules.set(symbol3, Literals.character('*'), results -> (T)Unit.INSTANCE);
		...
		parsingRules.set(symbol6, Term.sequence(Term.symbol(symbol7), Term.optional(Term.sequence(Literals.character(','), Term.symbol(symbol6)))), results -> {
			...
			return (T)((List)Optional.ofNullable((List)results.get(symbol6)).map(predicates -> Util.withPrepended(object, predicates)).orElse(List.of(object)));
		});
		parsingRules.set(symbol7, Term.sequence(Term.symbol(symbol8), Term.optional(Term.sequence(Literals.character('|'), Term.symbol(symbol7)))), results -> {
			...
			return (T)((List)Optional.ofNullable((List)results.get(symbol7)).map(predicates -> Util.withPrepended(object, predicates)).orElse(List.of(object)));
		});
		parsingRules.set(
			symbol8,
		...
		parsingRules.set(
			...
			(state, results) -> {
				...
				try {
					...
					state.getErrors().add(state.getCursor(), var9xx);
