				...
				if (j <= m) {
					if (!arg.method_16444()) {
						...
						for(Pair<BlockPos, CompoundTag> pair : list3) {
							...
							lv2.method_1049(blockPos4.getX() - q, blockPos4.getY() - r, blockPos4.getZ() - p, true, true);
						}

						lv2.method_1046((direction, mx, nx, ox) -> {
							BlockPos blockPosxx = new BlockPos(q + mx, r + nx, p + ox);
							BlockPos blockPos2 = blockPosxx.method_10093(direction);
							BlockState blockState = iWorld.getBlockState(blockPosxx);
							BlockState blockState2 = iWorld.getBlockState(blockPos2);
							BlockState blockState3xx = blockState.method_11578(direction, blockState2, iWorld, blockPosxx, blockPos2);
							if (blockState != blockState3xx) {
								iWorld.setBlockState(blockPosxx, blockState3xx, i & -2 | 16);
							}

							BlockState blockState4 = blockState2.method_11578(direction.getOpposite(), blockState3xx, iWorld, blockPos2, blockPosxx);
