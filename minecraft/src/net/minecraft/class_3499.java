				...
				if (j <= m) {
					if (!arg.method_16444()) {
						...
						for(Pair<BlockPos, CompoundTag> pair : list3) {
							...
							abstractVoxelShapeContainer.modify(blockPos4.getX() - q, blockPos4.getY() - r, blockPos4.getZ() - p, true, true);
						}

						abstractVoxelShapeContainer.method_1046((direction, mx, nx, ox) -> {
							BlockPos blockPosxx = new BlockPos(q + mx, r + nx, p + ox);
							...
							BlockState blockState3xx = blockState.getStateForNeighborUpdate(direction, blockState2, iWorld, blockPosxx, blockPos2);
							if (blockState != blockState3xx) {
								iWorld.setBlockState(blockPosxx, blockState3xx, i & -2 | 16);
							}

							BlockState blockState4 = blockState2.getStateForNeighborUpdate(direction.getOpposite(), blockState3xx, iWorld, blockPos2, blockPosxx);
