...
public class class_1188 extends DataFix {
	private static final Map<String, String> field_5684 = DataFixUtils.make(Maps.<String, String>newHashMap(), hashMap -> {
	...
	private static final Set<String> field_5683 = (Set<String>)field_5684.keySet()
		.stream()
		.map(string -> string.substring(0, string.indexOf(46)))
		.collect(Collectors.toSet());
	private static final Set<String> field_5682 = Sets.<String>newHashSet(
		"minecraft:bow",
	...
	public class_1188(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	public TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", type, typed -> {
			...
			if (!optional.isPresent()) {
				...
				String string = method_5042((String)((Pair)optional.get()).getSecond(), i);
				if (string != null) {
					typed2 = typed.set(opticFinder, Pair.of(class_1208.field_5713.typeName(), string));
				}

				if (field_5682.contains(((Pair)optional.get()).getSecond())) {
	...
	@Nullable
	public static String method_5042(@Nullable String string, int i) {
		if (field_5683.contains(string)) {
			String string2 = (String)field_5684.get(string + '.' + i);
			return string2 == null ? (String)field_5684.get(string + ".0") : string2;
		} else {
			return null;
		}
	}
