...
public class class_4401 extends RealmsScreen {
	...
	public void init() {
		...
		(new Thread("Realms-pending-invitations-fetcher") {
			public void run() {
				...
				try {
					...
					List<class_4401.class_4403> list2 = (List)list.stream().map(pendingInvite -> class_4401.this.new class_4403(pendingInvite)).collect(Collectors.toList());
		...
		this.buttonsAdd(
			this.field_19943 = new RealmsButton(2, this.width() / 2 + 74, this.height() - 32, 100, 20, getLocalizedString("mco.invites.button.reject")) {
				@Override
				public void onPress() {
		...
		public void selectItem(int i) {
			...
			if (i != -1) {
				...
				String string2 = Realms.joinNarrations(
					Arrays.asList(pendingInvite.worldName, pendingInvite.worldOwnerName, class_4401.method_21301(pendingInvite), string)
				);
