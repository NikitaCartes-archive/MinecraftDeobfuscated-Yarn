...
public record class_7644<S>(List<class_7644.class_7645<S>> arguments) {
	public static <S> class_7644<S> method_45043(ParseResults<S> parseResults) {
		...
		List<class_7644.class_7645<S>> list;
		CommandContextBuilder<S> commandContextBuilder3;
		for(list = method_45044(commandContextBuilder);
			...
			if (parsedArgument instanceof ArgumentCommandNode argumentCommandNode) {
				...
				if (var7 instanceof DecoratableArgumentType decoratableArgumentType) {
					...
					if (parsedArgumentx != null) {
						list.add(new class_7644.class_7645<S>(argumentCommandNode, parsedArgumentx, decoratableArgumentType));
					}
				}
			}
		}
