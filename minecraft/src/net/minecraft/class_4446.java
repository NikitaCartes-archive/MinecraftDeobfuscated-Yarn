			...
			Thread thread = new Thread("Realms Texture Downloader") {
				public void run() {
					...
					if (map.containsKey(Type.SKIN)) {
						...
						class_4446.field_20256.debug("Downloading http texture from {}", stringx);

						try {
							try {
								httpURLConnection = (HttpURLConnection)new URL(stringx).openConnection(Realms.getProxy());
								...
								class_4446.field_20256.error("Couldn't download http texture", var19);
								class_4446.field_20254.remove(string);
							}
						} finally {
