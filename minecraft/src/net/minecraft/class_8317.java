...
public class class_8317 extends class_8277 {
	...
	public Stream<class_8291> method_50118(MinecraftServer minecraftServer, Random random, int i) {
		...
		ObjectArrayList<Optional<ServerPlayerEntity>> objectArrayList = (ObjectArrayList)minecraftServer.getPlayerManager()
			.getPlayerList()
			.stream()
			.map(Optional::of)
		...
		public void method_50165(MinecraftServer minecraftServer) {
			if (this.field_43803.isPresent()) {
				SkullBlockEntity.loadProperties(((class_8345)this.field_43803.get()).method_50411(), gameProfile -> {
					for(ServerPlayerEntity serverPlayerEntityxx : minecraftServer.getPlayerManager().getPlayerList()) {
						serverPlayerEntityxx.editTransformation(transformationType -> transformationType.withSkin(Optional.of(gameProfile)));
					}
				});
			} else {
