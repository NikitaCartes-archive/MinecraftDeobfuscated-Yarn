...
public class HusbandryTabAdvancementGenerator implements Consumer<Consumer<SimpleAdvancement>> {
	...
	public void accept(Consumer<SimpleAdvancement> consumer) {
		...
		SimpleAdvancement simpleAdvancement5 = SimpleAdvancement.Builder.create()
			...
			.criterion(
				"broke_hoe",
				ItemDurabilityChangedCriterion.Conditions.method_8967(ItemPredicate.Builder.create().method_8977(Items.field_8527).build(), NumberRange.Integer.exactly(-1))
			)
	...
	private SimpleAdvancement.Builder method_10341(SimpleAdvancement.Builder builder) {
		for (Item item : field_11298) {
			builder.criterion(Registry.ITEM.getId(item).getPath(), ConsumeItemCriterion.Conditions.method_8828(item));
		}
	...
	private SimpleAdvancement.Builder method_10340(SimpleAdvancement.Builder builder) {
		for (Item item : field_11297) {
			builder.criterion(Registry.ITEM.getId(item).getPath(), FilledBucketCriterion.Conditions.create(ItemPredicate.Builder.create().method_8977(item).build()));
		}
