...
public abstract class RecipeProvider implements DataProvider {
	...
	private static final Map<BlockFamily.Variant, BiFunction<ItemConvertible, ItemConvertible, CraftingRecipeJsonBuilder>> VARIANT_FACTORIES = ImmutableMap.builder(
		...
		.put(
			BlockFamily.Variant.CHISELED, (BiFunction)(output, input) -> createChiseledBlockRecipe(RecipeCategory.BUILDING_BLOCKS, output, Ingredient.ofItems(input))
		)
		.put(
			BlockFamily.Variant.CUT,
			(BiFunction)(itemConvertible, itemConvertible2) -> createCutCopperRecipe(
					RecipeCategory.BUILDING_BLOCKS, itemConvertible, Ingredient.ofItems(itemConvertible2)
				)
		)
		.put(BlockFamily.Variant.DOOR, (BiFunction)(output, input) -> createDoorRecipe(output, Ingredient.ofItems(input)))
		.put(BlockFamily.Variant.CUSTOM_FENCE, (BiFunction)(output, input) -> createFenceRecipe(output, Ingredient.ofItems(input)))
	...
	protected static void generateFamily(RecipeExporter exporter, BlockFamily family) {
		family.getVariants().forEach((variant, block) -> {
			BiFunction<ItemConvertible, ItemConvertible, CraftingRecipeJsonBuilder> biFunction = (BiFunction)VARIANT_FACTORIES.get(variant);
			...
			if (biFunction != null) {
				...
				craftingRecipeJsonBuilder.criterion((String)family.getUnlockCriterionName().orElseGet(() -> hasItem(itemConvertible)), conditionsFromItem(itemConvertible));
	...
	private static AdvancementCriterion<InventoryChangedCriterion.Conditions> conditionsFromPredicates(ItemPredicate.Builder... predicates) {
		return conditionsFromItemPredicates((ItemPredicate[])Arrays.stream(predicates).map(ItemPredicate.Builder::build).toArray(i -> new ItemPredicate[i]));
	}
