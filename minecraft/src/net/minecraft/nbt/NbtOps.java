	...
	public DataResult<Tag> method_29155(Tag tag, List<Tag> list) {
		if (!(tag instanceof AbstractListTag) && !(tag instanceof EndTag)) {
			...
			AbstractListTag<?> abstractListTag = method_29144(
				tag instanceof AbstractListTag ? ((AbstractListTag)tag).getElementType() : 0, list.stream().findFirst().map(Tag::getType).orElse((byte)0)
			);
	...
	public DataResult<Tag> method_29154(Tag tag, MapLike<Tag> mapLike) {
		if (!(tag instanceof CompoundTag) && !(tag instanceof EndTag)) {
			...
			mapLike.entries().forEach(pair -> {
				Tag tagxx = (Tag)pair.getFirst();
				if (!(tagxx instanceof StringTag)) {
					list.add(tagxx);
				} else {
					compoundTag.put(tagxx.asString(), (Tag)pair.getSecond());
				}
			});
	...
	public DataResult<Consumer<BiConsumer<Tag, Tag>>> method_29162(Tag tag) {
		if (!(tag instanceof CompoundTag)) {
			...
			return DataResult.success(
				(Consumer)biConsumer -> compoundTag.getKeys().forEach(string -> biConsumer.accept(this.method_10639(string), compoundTag.get(string)))
			);
		}
	}
	...
	public Tag method_10665(Stream<Tag> stream) {
		...
		if (!peekingIterator.hasNext()) {
			...
			if (tag instanceof ByteTag) {
				...
				List<Integer> list = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((IntTag)tagx).getInt()));
				return new IntArrayTag(list);
			} else if (tag instanceof LongTag) {
				List<Long> list = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((LongTag)tagx).getLong()));
