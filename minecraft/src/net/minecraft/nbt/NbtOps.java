	...
	public DataResult<NbtElement> mergeToList(NbtElement nbtElement, List<NbtElement> list) {
		if (!(nbtElement instanceof AbstractNbtList) && !(nbtElement instanceof NbtEnd)) {
			...
			AbstractNbtList<?> abstractNbtList = createList(
				...
				list.stream().findFirst().map(NbtElement::getType).orElse((byte)0)
			);
	...
	public DataResult<NbtElement> mergeToMap(NbtElement nbtElement, MapLike<NbtElement> mapLike) {
		if (!(nbtElement instanceof NbtCompound) && !(nbtElement instanceof NbtEnd)) {
			...
			mapLike.entries().forEach(pair -> {
				NbtElement nbtElementxx = (NbtElement)pair.getFirst();
				if (!(nbtElementxx instanceof NbtString)) {
					list.add(nbtElementxx);
				} else {
					nbtCompound.put(nbtElementxx.asString(), (NbtElement)pair.getSecond());
				}
			});
	...
	public DataResult<Consumer<BiConsumer<NbtElement, NbtElement>>> getMapEntries(NbtElement nbtElement) {
		if (!(nbtElement instanceof NbtCompound)) {
			...
			return DataResult.success(
				(Consumer)entryConsumer -> nbtCompound.getKeys().forEach(key -> entryConsumer.accept(this.createString(key), nbtCompound.get(key)))
			);
		}
	}
	...
	public NbtElement createList(Stream<NbtElement> stream) {
		...
		if (!peekingIterator.hasNext()) {
			...
			if (nbtElement instanceof NbtByte) {
				List<Byte> list = Lists.newArrayList(Iterators.transform(peekingIterator, nbt -> ((NbtByte)nbt).byteValue()));
				return new NbtByteArray(list);
			} else if (nbtElement instanceof NbtInt) {
				List<Integer> list = Lists.newArrayList(Iterators.transform(peekingIterator, nbt -> ((NbtInt)nbt).intValue()));
				return new NbtIntArray(list);
			} else if (nbtElement instanceof NbtLong) {
				List<Long> list = Lists.newArrayList(Iterators.transform(peekingIterator, nbt -> ((NbtLong)nbt).longValue()));
