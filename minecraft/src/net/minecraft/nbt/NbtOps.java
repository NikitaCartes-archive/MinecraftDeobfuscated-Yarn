	...
	public DataResult<Consumer<BiConsumer<NbtElement, NbtElement>>> getMapEntries(NbtElement nbtElement) {
		if (!(nbtElement instanceof NbtCompound)) {
			...
			return DataResult.success(
				(Consumer)biConsumer -> nbtCompound.getKeys().forEach(string -> biConsumer.accept(this.createString(string), nbtCompound.get(string)))
			);
		}
	}
	...
	public NbtElement createList(Stream<NbtElement> stream) {
		...
		if (!peekingIterator.hasNext()) {
			...
			if (nbtElement instanceof NbtByte) {
				...
				List<Integer> list = Lists.newArrayList(Iterators.transform(peekingIterator, nbtElementx -> ((NbtInt)nbtElementx).intValue()));
				return new NbtIntArray(list);
			} else if (nbtElement instanceof NbtLong) {
				List<Long> list = Lists.newArrayList(Iterators.transform(peekingIterator, nbtElementx -> ((NbtLong)nbtElementx).longValue()));
