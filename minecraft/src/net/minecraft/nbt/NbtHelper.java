...
public final class NbtHelper {
		...
		switch(nbtElement.getType()) {
			...
			case 10:
				...
				for(int m = 0; m < list.size(); ++m) {
					...
					method_36114(i + 1, stringBuilder).append('"').append(string2).append('"').append(string, 0, string.length() - string2.length()).append(": ");
	...
	public static NbtCompound update(DataFixer fixer, DataFixTypes fixTypes, NbtCompound compound, int oldVersion, int targetVersion) {
		return fixer.update(fixTypes.getTypeReference(), new Dynamic<>(NbtOps.INSTANCE, compound), oldVersion, targetVersion).getValue();
	}
	...
	static NbtCompound method_32275(NbtCompound compound) {
		...
		Map<String, NbtElement> map = (Map)nbtList.stream()
