...
public class NbtCompound implements NbtElement {
	...
	}, nbtCompound -> new Dynamic<>(NbtOps.INSTANCE, nbtCompound));
	...
	public String toString() {
		...
		if (LOGGER.isDebugEnabled()) {
			List<String> list = Lists.<String>newArrayList(this.entries.keySet());
		...
		for (String string : collection) {
			...
			stringBuilder.append(escapeTagKey(string)).append(':').append(this.entries.get(string));
		}
		...
		if (this.entries.isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				MutableText mutableText2 = new LiteralText(Strings.repeat(indent, depth + 1))
					...
					.append(" ")
					.append(((NbtElement)this.entries.get(string)).toText(indent, depth + 1));
				if (iterator.hasNext()) {
