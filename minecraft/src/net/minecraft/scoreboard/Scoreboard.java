...
public class Scoreboard {
	...
	public ScoreboardObjective method_1168(
		...
		if (string.length() > 16) {
			...
			ScoreboardObjective scoreboardObjective = new ScoreboardObjective(this, string, scoreboardCriterion, textComponent, renderType);
	...
	public ScoreboardPlayerScore getPlayerScore(String string, ScoreboardObjective scoreboardObjective) {
		if (string.length() > 40) {
			...
			Map<ScoreboardObjective, ScoreboardPlayerScore> map = (Map)this.playerObjectives.computeIfAbsent(string, stringx -> Maps.newHashMap());
	...
	public void resetPlayerScore(String string, @Nullable ScoreboardObjective scoreboardObjective) {
		if (scoreboardObjective == null) {
			Map<ScoreboardObjective, ScoreboardPlayerScore> map = (Map)this.playerObjectives.remove(string);
			if (map != null) {
			...
			if (map != null) {
				...
				if (map.size() < 1) {
					Map<ScoreboardObjective, ScoreboardPlayerScore> map2 = (Map)this.playerObjectives.remove(string);
					if (map2 != null) {
						this.updatePlayerScore(string);
					}
	...
	public void removeObjective(ScoreboardObjective scoreboardObjective) {
		...
		List<ScoreboardObjective> list = (List)this.objectivesByCriterion.get(scoreboardObjective.getCriterion());
		if (list != null) {
