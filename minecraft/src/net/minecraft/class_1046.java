		...
		this.field_5213 = new Thread("Texture Downloader #" + field_5216.incrementAndGet()) {
			public void run() {
				...
				try {
					...
					if (httpURLConnection.getResponseCode() / 100 == 2) {
						...
						class_310.method_1551().execute(() -> {
							...
							try {
								...
								class_1011 lv2 = lv;
								class_310.method_1551().execute(() -> class_1046.this.method_4534(lv2));
							} catch (IOException var7xx) {
								class_1046.field_5212.warn("Error while loading the skin texture", var7xx);
							} finally {
								if (lv != null) {
									lv.close();
								}
					...
					class_1046.field_5212.error("Couldn't download http texture", var6);
					return;
				} finally {
