...
public class LootFunctions {
	...
	public static <T extends LootFunction> LootFunction.Factory<T> getFactory(T lootFunction) {
		LootFunction.Factory<T> factory = (LootFunction.Factory)byClass.get(lootFunction.getClass());
		if (factory == null) {
		...
		switch(biFunctions.length) {
			...
			default:
				return (itemStack, lootContext) -> {
					for(BiFunction<ItemStack, LootContext, ItemStack> biFunctionxx : biFunctions) {
