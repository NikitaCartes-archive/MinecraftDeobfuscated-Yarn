		...
		public <T> DataResult<ApplyBonusLootFunction.Formula> decode(DynamicOps<T> ops, MapLike<T> input) {
			...
			return object == null
				...
				: ApplyBonusLootFunction.TYPE_CODEC.decode(ops, object).flatMap(pair -> {
					T objectxx = (T)Objects.requireNonNullElseGet(input.get("parameters"), ops::emptyMap);
					return ((ApplyBonusLootFunction.Type)pair.getFirst()).codec().decode(ops, objectxx).map(Pair::getFirst);
				});
		}
