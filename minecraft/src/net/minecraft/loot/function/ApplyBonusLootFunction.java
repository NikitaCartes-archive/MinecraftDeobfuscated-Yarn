		...
		public <T> DataResult<ApplyBonusLootFunction.Formula> decode(DynamicOps<T> dynamicOps, MapLike<T> mapLike) {
			...
			return object == null
				...
				: ApplyBonusLootFunction.field_45807.decode(dynamicOps, object).flatMap(pair -> {
					T objectxx = (T)Objects.requireNonNullElseGet(mapLike.get("parameters"), dynamicOps::emptyMap);
					return ((ApplyBonusLootFunction.Type)pair.getFirst()).codec().decode(dynamicOps, objectxx).map(Pair::getFirst);
				});
		}
