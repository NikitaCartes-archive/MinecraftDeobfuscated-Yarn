...
public class LootManager implements ResourceReloader, LootDataLookup {
	...
	private static <T> CompletableFuture<?> load(
		...
		return CompletableFuture.runAsync(() -> {
			Map<Identifier, JsonElement> map2xx = new HashMap();
			JsonDataLoader.load(resourceManager, type.getId(), GSON, map2xx);
			map2xx.forEach((id, json) -> type.parse(id, registryOps, json).ifPresent(value -> map2.put(id, value)));
		}, executor);
	}
