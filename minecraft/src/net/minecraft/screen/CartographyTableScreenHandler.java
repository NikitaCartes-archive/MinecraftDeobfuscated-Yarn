...
public class CartographyTableScreenHandler extends ScreenHandler {
	...
	public CartographyTableScreenHandler(int syncId, PlayerInventory inventory, BlockContext context) {
			...
			public ItemStack takeStack(int amount) {
				...
				ItemStack itemStack2 = (ItemStack)context.run((BiFunction)((world, blockPos) -> {
					if (!CartographyTableScreenHandler.this.currentlyTakingItem && CartographyTableScreenHandler.this.inventory.getInvStack(1).getItem() == Items.GLASS_PANE) {
						ItemStack itemStack2xx = FilledMapItem.copyMap(world, CartographyTableScreenHandler.this.inventory.getInvStack(0));
						if (itemStack2xx != null) {
							itemStack2xx.setCount(1);
							return itemStack2xx;
						}
					}
			...
			if (invSlot == 2) {
				if (this.inventory.getInvStack(1).getItem() == Items.GLASS_PANE) {
					itemStack3 = (ItemStack)this.context.run((BiFunction)((world, blockPos) -> {
						ItemStack itemStack2xx = FilledMapItem.copyMap(world, this.inventory.getInvStack(0));
						if (itemStack2xx != null) {
							itemStack2xx.setCount(1);
							return itemStack2xx;
						} else {
