	...
	public Tag createList(Stream<Tag> stream) {
		...
		if (!peekingIterator.hasNext()) {
			...
			if (tag instanceof ByteTag) {
				ArrayList<Byte> arrayList = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((ByteTag)tagx).getByte()));
				return new ByteArrayTag(arrayList);
			} else if (tag instanceof IntTag) {
				ArrayList<Integer> arrayList = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((IntTag)tagx).getInt()));
				return new IntArrayTag(arrayList);
			} else if (tag instanceof LongTag) {
				ArrayList<Long> arrayList = Lists.newArrayList(Iterators.transform(peekingIterator, tagx -> ((LongTag)tagx).getLong()));
