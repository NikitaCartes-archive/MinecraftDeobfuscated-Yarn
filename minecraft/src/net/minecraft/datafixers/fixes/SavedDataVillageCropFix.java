...
public class SavedDataVillageCropFix extends DataFix {
	...
	private static <T> Dynamic<T> method_5157(Dynamic<T> dynamic) {
		return (Dynamic<T>)dynamic.getStream().map(SavedDataVillageCropFix::method_5151).map(dynamic::createList).orElse(dynamic);
	}
	...
	private static <T> Dynamic<T> method_5156(Dynamic<T> dynamic, String string) {
		return dynamic.get(string).flatMap(Dynamic::getNumberValue).isPresent()
			? dynamic.set(string, BlockStateFlattening.lookupState(dynamic.getInt(string) << 4))
			: dynamic;
	}
}
