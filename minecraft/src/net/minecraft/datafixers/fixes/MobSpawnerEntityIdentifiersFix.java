	...
	private Dynamic<?> method_5106(Dynamic<?> dynamic) {
		if (!"MobSpawner".equals(dynamic.getString("id"))) {
			...
			if (optional.isPresent()) {
				Dynamic<?> dynamic2 = DataFixUtils.orElse(dynamic.get("SpawnData"), dynamic.emptyMap());
			...
			if (optional2.isPresent()) {
				dynamic = dynamic.set("SpawnPotentials", dynamic.createList(((Stream)optional2.get()).map(dynamicx -> {
					Optional<String> optionalxx = dynamicx.get("Type").flatMap(Dynamic::getStringValue);
					if (optionalxx.isPresent()) {
						Dynamic<?> dynamic2 = DataFixUtils.orElse(dynamicx.get("Properties"), dynamicx.emptyMap()).set("id", dynamicx.createString((String)optionalxx.get()));
