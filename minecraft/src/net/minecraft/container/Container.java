...
public abstract class Container {
	...
	public ItemStack method_7593(int i, int j, SlotActionType slotActionType, PlayerEntity playerEntity) {
		...
		if (slotActionType == SlotActionType.field_7789) {
				...
				if (slot3 != null) {
					...
					if (itemStack3.isEmpty()) {
						...
						if (itemStack2.isEmpty()) {
							...
							int o = itemStack3.getAmount();
							if (o + itemStack2.getAmount() <= itemStack2.getMaxAmount()) {
								itemStack2.addAmount(o);
								itemStack3 = slot3.method_7671(o);
								if (itemStack3.isEmpty()) {
									slot3.method_7673(ItemStack.EMPTY);
								}
			...
			Slot slot3 = (Slot)this.slotList.get(i);
			ItemStack itemStack3 = playerInventory.method_5438(j);
			ItemStack itemStack2 = slot3.method_7677();
			if (!itemStack3.isEmpty() || !itemStack2.isEmpty()) {
				if (itemStack3.isEmpty()) {
					if (slot3.canTakeItems(playerEntity)) {
						playerInventory.method_5447(j, itemStack2);
						slot3.onTake(itemStack2.getAmount());
						slot3.method_7673(ItemStack.EMPTY);
						slot3.method_7667(playerEntity, itemStack2);
					}
				} else if (itemStack2.isEmpty()) {
					if (slot3.method_7680(itemStack3)) {
						int o = slot3.method_7676(itemStack3);
						if (itemStack3.getAmount() > o) {
							slot3.method_7673(itemStack3.split(o));
						} else {
							slot3.method_7673(itemStack3);
							playerInventory.method_5447(j, ItemStack.EMPTY);
						}
					}
				} else if (slot3.canTakeItems(playerEntity) && slot3.method_7680(itemStack3)) {
					int o = slot3.method_7676(itemStack3);
					if (itemStack3.getAmount() > o) {
						slot3.method_7673(itemStack3.split(o));
						slot3.method_7667(playerEntity, itemStack2);
						if (!playerInventory.method_7394(itemStack2)) {
							playerEntity.method_7328(itemStack2, true);
						}
					} else {
						slot3.method_7673(itemStack3);
						playerInventory.method_5447(j, itemStack2);
			...
			if (slot3 != null && slot3.hasStack()) {
				ItemStack itemStack3 = slot3.method_7677().copy();
			...
			if (slot3 != null && slot3.hasStack() && slot3.canTakeItems(playerEntity)) {
				ItemStack itemStack3 = slot3.method_7671(j == 0 ? 1 : slot3.method_7677().getAmount());
				slot3.method_7667(playerEntity, itemStack3);
				playerEntity.method_7328(itemStack3, true);
			}
		} else if (slotActionType == SlotActionType.field_7793 && i >= 0) {
			Slot slot3 = (Slot)this.slotList.get(i);
			ItemStack itemStack3 = playerInventory.method_7399();
			if (!itemStack3.isEmpty() && (slot3 == null || !slot3.hasStack() || !slot3.canTakeItems(playerEntity))) {
				...
				for(int p = 0; p < 2; ++p) {
					for(int q = l; q >= 0 && q < this.slotList.size() && itemStack3.getAmount() < itemStack3.getMaxAmount(); q += o) {
						...
						if (slot4.hasStack() && method_7592(slot4, itemStack3, true) && slot4.canTakeItems(playerEntity) && this.method_7613(itemStack3, slot4)) {
	...
	protected boolean method_7616(ItemStack itemStack, int i, int j, boolean bl) {
		...
		if (!itemStack.isEmpty()) {
			...
			while(bl ? k >= i : k < j) {
				Slot slot = (Slot)this.slotList.get(k);
				ItemStack itemStack2 = slot.method_7677();
				if (itemStack2.isEmpty() && slot.method_7680(itemStack)) {
					if (itemStack.getAmount() > slot.getMaxStackAmount()) {
						slot.method_7673(itemStack.split(slot.getMaxStackAmount()));
					} else {
						slot.method_7673(itemStack.split(itemStack.getAmount()));
					}
	...
	public static boolean method_7592(@Nullable Slot slot, ItemStack itemStack, boolean bl) {
		...
		if (!bl2 && itemStack.isEqualIgnoreTags(slot.method_7677()) && ItemStack.areTagsEqual(slot.method_7677(), itemStack)) {
			return slot.method_7677().getAmount() + (bl ? 0 : itemStack.getAmount()) <= itemStack.getMaxAmount();
		} else {
			return bl2;
		}
	}
