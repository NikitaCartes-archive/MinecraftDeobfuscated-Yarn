...
public abstract class Container {
	public DefaultedList<ItemStack> stackList = DefaultedList.create();
	public List<Slot> slotList = Lists.<Slot>newArrayList();
	public int syncId;
	...
	protected List<ContainerListener> listeners = Lists.<ContainerListener>newArrayList();
		...
		for(int i = 0; i < this.slotList.size(); ++i) {
			...
			if (!ItemStack.areEqual(itemStack2, itemStack)) {
				...
				for(int j = 0; j < this.listeners.size(); ++j) {
					((ContainerListener)this.listeners.get(j)).onContainerSlotUpdate(this, i, itemStack2);
				}
			}
		}
	}
			...
			if (!itemStack3.isEmpty() || !itemStack2.isEmpty()) {
				if (itemStack3.isEmpty()) {
					if (slot3.canTakeItems(playerEntity)) {
						...
						slot3.method_7672(itemStack2.getAmount());
						slot3.setStack(ItemStack.EMPTY);
			...
			if (!itemStack3.isEmpty() && (slot3 == null || !slot3.hasStack() || !slot3.canTakeItems(playerEntity))) {
				...
				for(int p = 0; p < 2; ++p) {
					for(int q = l; q >= 0 && q < this.slotList.size() && itemStack3.getAmount() < itemStack3.getMaxAmount(); q += o) {
						...
						if (slot4.hasStack() && canInsertItemIntoSlot(slot4, itemStack3, true) && slot4.canTakeItems(playerEntity) && this.method_7613(itemStack3, slot4)) {
	...
	protected void onCraftingContentChanged(World world, PlayerEntity playerEntity, Inventory inventory, CraftingResultInventory craftingResultInventory) {
		if (!world.isClient) {
			...
			serverPlayerEntity.networkHandler.sendPacket(new GuiSlotUpdateClientPacket(this.syncId, 0, itemStack));
		}
	}
}
