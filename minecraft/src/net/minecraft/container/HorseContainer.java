...
public class HorseContainer extends Container {
	...
	public HorseContainer(Inventory inventory, Inventory inventory2, HorseBaseEntity horseBaseEntity, PlayerEntity playerEntity) {
		...
		if (horseBaseEntity instanceof AbstractDonkeyEntity && ((AbstractDonkeyEntity)horseBaseEntity).hasChest()) {
			for (int k = 0; k < 3; k++) {
				for (int l = 0; l < ((AbstractDonkeyEntity)horseBaseEntity).method_6702(); l++) {
					this.addSlot(new Slot(inventory2, 2 + l + k * ((AbstractDonkeyEntity)horseBaseEntity).method_6702(), 80 + l * 18, 18 + k * 18));
				}
			}
		}
