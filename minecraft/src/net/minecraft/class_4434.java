		...
		public void run() {
			...
			for(int i = 0; i < 25; ++i) {
				...
				try {
					...
					class_4434.field_20211.error("Failed to close server", var5);
		...
		public void run() {
			...
			while(i < 25) {
				try {
					...
					class_4434.field_20211.error("Couldn't download world data", var6);
		...
		public void run() {
			...
			for(int i = 0; i < 25; ++i) {
				...
				try {
					...
					class_4434.field_20211.error("Failed to open server", var5);
			...
			for(int j = 0; j < 40 && !this.method_21065(); ++j) {
				try {
					...
					if (var11.field_19606 == 6002) {
						...
						class_4434.field_20211.error("Couldn't connect to world", var11);
					}
					break;
				} catch (IOException var12) {
					class_4434.field_20211.error("Couldn't parse response connecting to world", var12);
				} catch (Exception var13) {
					bl2 = true;
					class_4434.field_20211.error("Couldn't connect to world", var13);
		...
		public void run() {
			...
			while(i < 25) {
				try {
					...
					class_4434.field_20211.error("Couldn't restore backup", var5);
					...
					class_4434.field_20211.error("Couldn't restore backup", var6);
					this.method_21067(var6.getLocalizedMessage());
					return;
				}
			}
