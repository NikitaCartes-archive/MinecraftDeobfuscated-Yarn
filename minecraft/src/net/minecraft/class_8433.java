...
public class class_8433 {
	...
	private void method_50911() {
		...
		for(int j = 4; j >= i; --j) {
			...
			while(iterator.hasNext()) {
				Entry<BlockPos, Integer> entry = (Entry)iterator.next();
				if (entry.getValue() == j) {
	...
	private int method_50920() {
		...
		for(BlockPos blockPos : this.field_44259) {
			...
			while(this.field_44257.containsKey(lv.start)) {
				...
				i = Math.min(i, --j);
				this.field_44260.put(lv.pos, Math.min(this.field_44260.getOrDefault(lv.pos, j), j));
			}
		}
	...
	private void method_50905(StructureWorldAccess structureWorldAccess, class_8433.class_8434 arg, Random random, int i, boolean bl) {
		if (i < 0) {
			...
			if (bl) {
				structureWorldAccess.toServerWorld()
					.playSound(null, arg.pos, SoundEvents.BLOCK_SCULK_SPREAD, SoundCategory.BLOCKS, 0.6F, 0.5F + 0.2F * random.nextFloat());
			}
			...
			if (i == 4) {
				...
				BlockEntity var18 = structureWorldAccess.getBlockEntity(blockPos);
				if (var18 instanceof LootableContainerBlockEntity lootableContainerBlockEntity) {
					if (bl2) {
	...
	private List<class_8433.class_8434> method_50906(StructureWorldAccess structureWorldAccess, BlockPos blockPos, Random random, int i, int j, int k) {
		if (i > this.field_44256) {
			...
			List<Direction> list = new ArrayList(this.method_50898(i, random).filter(directionx -> {
				int ixx = random.nextInt(directionx == Direction.UP ? 3 : 4) + 3;
				map.put(directionx, ixx);

				for(int jxxx = 2; jxxx < ixx + 1; ++jxxx) {
					for(int kxx = -1; kxx < 2; ++kxx) {
						for(int lxx = -1; lxx < 2; ++lxx) {
							for(int mxx = -1; mxx < 2; ++mxx) {
								if (!structureWorldAccess.getBlockState(blockPos.offset(directionx, jxxx).add(kxx, lxx, mxx)).isAir()) {
									return false;
								}
							}
						}
				...
				for(int jxx = 3; jxx < ixx + 1; ++jxx) {
					for(Direction direction2 : Direction.values()) {
						if (!structureWorldAccess.getBlockState(blockPos.offset(directionx, jxx).offset(direction2, 2)).isAir()) {
							return false;
						}
					}
				}
				...
				return (List<class_8433.class_8434>)list.stream()
					.map(
						directionx -> new class_8433.class_8434(
								blockPos, blockPos.offset(directionx, map.get(directionx)), map.get(directionx), i + 1, j + map.get(directionx), directionx
							)
					)
				...
				() -> {
					...
					for(Direction directionxx : directions) {
						serverWorld.spawnEntity(
							new DisplayEntity.class_8398(
								serverWorld,
								Blocks.LIGHTNING_ROD.getDefaultState().with(LightningRodBlock.FACING, directionxx.getOpposite()),
								blockPos.up().up().offset(directionxx),
