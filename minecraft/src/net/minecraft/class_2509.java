	...
	public class_2520 method_10665(Stream<class_2520> stream) {
		...
		if (!peekingIterator.hasNext()) {
			...
			if (lv instanceof class_2481) {
				List<Byte> list = Lists.newArrayList(Iterators.transform(peekingIterator, arg -> ((class_2481)arg).method_10698()));
				return new class_2479(list);
			} else if (lv instanceof class_2497) {
				List<Integer> list = Lists.newArrayList(Iterators.transform(peekingIterator, arg -> ((class_2497)arg).method_10701()));
				return new class_2495(list);
			} else if (lv instanceof class_2503) {
				List<Long> list = Lists.newArrayList(Iterators.transform(peekingIterator, arg -> ((class_2503)arg).method_10699()));
