...
public class class_3846<T> implements class_3906<T>, AutoCloseable, Runnable {
	...
	private boolean method_16905() {
		if ((this.field_17041.get() & 1) != 0) {
			return false;
		} else {
			return !this.field_17039.method_16911();
		}
	}
	...
	private boolean method_16907() {
		if (!this.method_16906()) {
			...
			Runnable runnable = (Runnable)this.field_17039.method_16909();
			if (runnable == null) {
				return false;
			} else {
				runnable.run();
		...
		if (this.method_16905() && this.method_16903()) {
			try {
				...
				try {
					...
					field_17040.error("Cound not schedule mailbox", var3);
				}
			}
		}
	}
