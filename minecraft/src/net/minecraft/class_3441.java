...
public class class_3441 extends class_3439 {
	...
	public void method_14901(class_2487 arg) {
		...
		for (int i = 0; i < lv.size(); i++) {
			...
			Optional<? extends class_1860<?>> optional = this.field_15304.method_8130(lv2);
			if (!optional.isPresent()) {
				field_15303.error("Tried to load unrecognized recipe: {} removed now.", lv2);
			} else {
				this.method_14876((class_1860<?>)optional.get());
			}
		}

		class_2499 lv3 = arg.method_10554("toBeDisplayed", 8);

		for (int j = 0; j < lv3.size(); j++) {
			class_2960 lv4 = new class_2960(lv3.method_10608(j));
			Optional<? extends class_1860<?>> optional2 = this.field_15304.method_8130(lv4);
			if (!optional2.isPresent()) {
				field_15303.error("Tried to load unrecognized recipe: {} removed now.", lv4);
			} else {
				this.method_14885((class_1860<?>)optional2.get());
			}
		}
	}
