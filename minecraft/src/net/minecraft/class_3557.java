...
public class class_3557 extends DataFix {
	public class_3557(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	private <TE> TypeRewriteRule method_15506(Type<?> type, ListType<TE> listType) {
		...
		return TypeRewriteRule.seq(
			this.fixTypeEverywhere(
				...
				this.getInputSchema().findChoiceType(class_1208.field_5727),
				this.getOutputSchema().findChoiceType(class_1208.field_5727),
			...
			this.fixTypeEverywhereTyped(
				...
				typed -> {
					...
					int ixx = dynamic.get("xPos").asInt(0);
					...
					for(int k = 0; k < list2.size(); ++k) {
						...
						for(int n : stream::iterator) {
							if (416 == (n & 0xFF) << 4) {
								...
								map.put(dynamic2.createString("x"), dynamic2.createInt(o + (ixx << 4)));
