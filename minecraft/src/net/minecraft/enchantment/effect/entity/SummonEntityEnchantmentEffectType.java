...
public record SummonEntityEnchantmentEffectType(RegistryEntryList<EntityType<?>> entityTypes, boolean joinTeam) implements EnchantmentEntityEffectType {
	public static final MapCodec<SummonEntityEnchantmentEffectType> CODEC = RecordCodecBuilder.mapCodec(
		instance -> instance.group(
					RegistryCodecs.entryList(RegistryKeys.ENTITY_TYPE).fieldOf("entity").forGetter(SummonEntityEnchantmentEffectType::entityTypes),
					Codec.BOOL.optionalFieldOf("join_team", Boolean.valueOf(false)).forGetter(SummonEntityEnchantmentEffectType::joinTeam)
				)
				.apply(instance, SummonEntityEnchantmentEffectType::new)
	);
	...
	@Override
	public MapCodec<SummonEntityEnchantmentEffectType> getCodec() {
		return CODEC;
	}
}
