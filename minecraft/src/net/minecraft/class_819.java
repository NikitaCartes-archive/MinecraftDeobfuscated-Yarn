		...
		static class_815 method_3536(JsonObject jsonObject) {
			...
			if (set.isEmpty()) {
				...
				if (jsonObject.has("OR")) {
					List<class_815> list = (List)Streams.stream(class_3518.method_15261(jsonObject, "OR"))
						.map(jsonElement -> method_3536(jsonElement.getAsJsonObject()))
						.collect(Collectors.toList());
					return new class_821(list);
				} else if (jsonObject.has("AND")) {
					List<class_815> list = (List)Streams.stream(class_3518.method_15261(jsonObject, "AND"))
						.map(jsonElement -> method_3536(jsonElement.getAsJsonObject()))
						.collect(Collectors.toList());
				...
				return new class_812((Iterable<? extends class_815>)set.stream().map(class_819.class_820::method_3533).collect(Collectors.toList()));
			}
		}
