					...
					if (l != 3) {
						...
						if (blockView != null) {
							...
							if ((l != 1 || blockState.exceedsCube()) && (l != 2 || blockState.isOf(Blocks.MOVING_PISTON))) {
								VoxelShape voxelShapex = blockState.getCollisionShape(CollisionView.this, mutable, shapeContext);
								if (voxelShapex == VoxelShapes.fullCube()) {
									if (box.intersects((double)i, (double)j, (double)k, (double)i + 1.0, (double)j + 1.0, (double)k + 1.0)) {
										consumer.accept(voxelShapex.offset((double)i, (double)j, (double)k));
										return true;
									}
								} else {
									VoxelShape voxelShape2 = voxelShapex.offset((double)i, (double)j, (double)k);
