				...
				.forEach(
					(long_, blendingData) -> blendingData.method_39346(
							BiomeCoords.fromChunk(ChunkPos.getPackedX(long_)), BiomeCoords.fromChunk(ChunkPos.getPackedZ(long_)), j - 1, j + 1, (l, m, n, dx) -> {
								double exx = MathHelper.magnitude((double)(i - l), (double)((j - m) * 2), (double)(k - n));
								if (!(exx > 2.0)) {
									if (exx < mutableDouble3.doubleValue()) {
										mutableDouble3.setValue(exx);
									}
		...
		this.field_36343
			.forEach(
				(long_, blendingData) -> blendingData.method_40028(
						BiomeCoords.fromChunk(ChunkPos.getPackedX(long_)), y, BiomeCoords.fromChunk(ChunkPos.getPackedZ(long_)), (k, l, registryEntry) -> {
							double dxx = MathHelper.hypot((double)(x - k), (double)(i - l));
							if (!(dxx > (double)field_35502)) {
								if (dxx < mutableDouble.doubleValue()) {
									mutableObject.setValue(registryEntry);
									mutableDouble.setValue(dxx);
								}
							}
						}
					)
