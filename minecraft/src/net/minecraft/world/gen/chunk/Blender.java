...
public class Blender {
	...
	public static final Codec<Blender> field_35682 = RecordCodecBuilder.create(
			instance -> instance.group(
						Codec.BOOL.fieldOf("old_noise").forGetter(Blender::method_39566),
						field_35695.optionalFieldOf("heights")
							.forGetter(blender -> DoubleStream.of(blender.field_35692).anyMatch(d -> d != Double.MAX_VALUE) ? Optional.of(blender.field_35692) : Optional.empty())
					)
					.apply(instance, Blender::new)
		)
		.comapFlatMap(Blender::method_39573, Function.identity());

	private static DataResult<Blender> method_39573(Blender blender) {
		return blender.field_35692.length != field_35518 ? DataResult.error("heights has to be of length " + field_35518) : DataResult.success(blender);
	}

	private Blender(boolean bl, Optional<double[]> optional) {
		this.field_35689 = bl;
		this.field_35692 = (double[])optional.orElse(Util.make(new double[field_35518], ds -> Arrays.fill(ds, Double.MAX_VALUE)));
	...
	private void method_39572(Chunk chunk, Set<EightWayDirection> set) {
		if (!this.field_35690) {
			...
			for(int i = 0; i < this.field_35694.length; ++i) {
				...
				this.field_35694[i] = method_39350(chunk, mutable) ? 1.0 : -1.0;
			}
	...
	private void method_39347(int index, Chunk chunk, int x, int z) {
		if (!this.field_35691) {
			this.field_35692[index] = (double)getSurfaceHeight(chunk, x, z);
		}
	...
	private static double[] method_39354(Chunk chunk, int x, int z) {
		...
		for(int j = OLD_HEIGHT_LIMIT.getTopY() - 1; j >= OLD_HEIGHT_LIMIT.getBottomY(); --j) {
			double h = method_39350(chunk, mutable.set(x, j, z)) ? 1.0 : -1.0;
			int k = j % 8;
			if (k == 0) {
				double l = e / 15.0;
				int m = j / 8 + 1;
	...
	protected double method_39344(int i, int j, int k) {
		if (i == field_35685 || k == field_35685) {
			return this.field_35692[method_39582(i, k)];
		} else {
			return i != 0 && k != 0 ? Double.MAX_VALUE : this.field_35692[method_39578(i, k)];
		}
	}
	...
	public void method_39351(int i, int j, Blender.class_6751 arg) {
	...
	public void method_39346(int i, int j, int k, int l, Blender.class_6750 arg) {
		int m = method_39581();
		...
		if (m >= k && m <= l) {
			for(int p = 0; p < this.field_35694.length; ++p) {
				int t = this.method_39568(p);
				int q = this.method_39577(p);
