				...
				.forEach(
					(chunkPos, data) -> data.acceptHeights(
							BiomeCoords.fromChunk(ChunkPos.getPackedX(chunkPos)), BiomeCoords.fromChunk(ChunkPos.getPackedZ(chunkPos)), (biomeX, biomeZ, height) -> {
								double dxx = MathHelper.hypot((double)(i - biomeX), (double)(j - biomeZ));
								if (!(dxx > (double)BLENDING_BIOME_DISTANCE_THRESHOLD)) {
									if (dxx < mutableDouble3.doubleValue()) {
										mutableDouble3.setValue(dxx);
									}
			
									double exx = 1.0 / (dxx * dxx * dxx * dxx);
									mutableDouble2.add(height * exx);
									mutableDouble.add(exx);
								}
							}
						)
				);
				...
				.forEach(
					(chunkPos, data) -> data.acceptCollidableBlockDensities(
							...
							(biomeX, halfSectionY, biomeZ, collidableBlockDensity) -> {
								double dxx = MathHelper.magnitude((double)(i - biomeX), (double)((j - halfSectionY) * 2), (double)(k - biomeZ));
								if (!(dxx > 2.0)) {
									if (dxx < mutableDouble3.doubleValue()) {
										mutableDouble3.setValue(dxx);
									}
									...
									double exx = 1.0 / (dxx * dxx * dxx * dxx);
									mutableDouble2.add(collidableBlockDensity * exx);
									mutableDouble.add(exx);
								}
							}
						)
			...
			.forEach(
				(chunkPos, data) -> data.acceptBiomes(
						BiomeCoords.fromChunk(ChunkPos.getPackedX(chunkPos)), y, BiomeCoords.fromChunk(ChunkPos.getPackedZ(chunkPos)), (biomeX, biomeZ, biome) -> {
							double dxx = MathHelper.hypot((double)(x - biomeX), (double)(z - biomeZ));
							if (!(dxx > (double)BLENDING_BIOME_DISTANCE_THRESHOLD)) {
