...
public class Blender {
	...
	public static Blender getBlender(@Nullable ChunkRegion chunkRegion) {
		if (chunkRegion == null) {
			...
			List<Blender.ChunkBlendingData> list = Lists.newArrayList();
			List<Blender.ChunkBlendingData> list2 = Lists.newArrayList();
	...
	private Biome blendBiome(int x, int y, int z) {
		...
		for(Blender.ChunkBlendingData chunkBlendingData : this.field_35509) {
			chunkBlendingData.blendingData
				.method_39351(BiomeCoords.fromChunk(chunkBlendingData.chunkX), BiomeCoords.fromChunk(chunkBlendingData.chunkZ), (i, j, f) -> {
