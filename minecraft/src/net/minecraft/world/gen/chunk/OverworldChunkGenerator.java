...
public class OverworldChunkGenerator extends SurfaceChunkGenerator<OverworldChunkGeneratorConfig> {
	...
	public List<Biome.SpawnEntry> getEntitySpawnList(EntityCategory entityCategory, BlockPos blockPos) {
		if (Feature.field_13520.method_14029(this.world, blockPos)) {
			...
			if (Feature.field_16655.isApproximatelyInsideStructure(this.world, blockPos)) {
				return Feature.field_16655.getMonsterSpawns();
			}

			if (Feature.field_13588.isApproximatelyInsideStructure(this.world, blockPos)) {
	...
	public void spawnEntities(ServerWorld serverWorld, boolean bl, boolean bl2) {
		this.field_13245.spawn(serverWorld, bl, bl2);
		this.field_16650.spawn(serverWorld, bl, bl2);
