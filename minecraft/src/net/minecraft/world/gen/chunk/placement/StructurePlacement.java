...
public abstract class StructurePlacement {
	...
	public boolean shouldGenerate(ChunkGenerator chunkGenerator, NoiseConfig noiseConfig, long seed, int x, int z) {
		if (!this.isStartChunk(chunkGenerator, noiseConfig, seed, x, z)) {
			return false;
		} else if (this.frequency < 1.0F && !this.frequencyReductionMethod.shouldGenerate(seed, this.salt, x, z, this.frequency)) {
			return false;
		} else {
			return !this.exclusionZone.isPresent()
				|| !((StructurePlacement.ExclusionZone)this.exclusionZone.get()).shouldExclude(chunkGenerator, noiseConfig, seed, x, z);
		}
	}
	...
	public static record ExclusionZone(RegistryEntry<StructureSet> otherSet, int chunkCount) {
		public static final Codec<StructurePlacement.ExclusionZone> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
						RegistryElementCodec.of(Registry.STRUCTURE_SET_KEY, StructureSet.CODEC, false)
							.fieldOf("other_set")
