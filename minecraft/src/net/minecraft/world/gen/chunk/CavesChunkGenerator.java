...
public class CavesChunkGenerator extends SurfaceChunkGenerator<CavesChunkGeneratorConfig> {
	...
	public List<Biome.SpawnEntry> getEntitySpawnList(EntityCategory entityCategory, BlockPos blockPos) {
		if (entityCategory == EntityCategory.field_6302) {
			if (Feature.field_13569.isInsideStructure(this.world, blockPos)) {
				return Feature.field_13569.getMonsterSpawns();
			}

			if (Feature.field_13569.isApproximatelyInsideStructure(this.world, blockPos) && this.world.method_8320(blockPos.down()).getBlock() == Blocks.field_10266) {
				return Feature.field_13569.getMonsterSpawns();
			}
		}
