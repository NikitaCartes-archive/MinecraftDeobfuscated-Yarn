...
public class ChunkNoiseSampler {
	...
	public TerrainNoisePoint getTerrainNoisePoint(NoiseColumnSampler noiseColumnSampler, int x, int z) {
		...
		return i >= 0 && j >= 0 && i < k && j < k
			...
			: (TerrainNoisePoint)this.terrainNoisePoints
				.computeIfAbsent(
					ChunkPos.toLong(x, z),
					(Long2ObjectFunction)(l -> noiseColumnSampler.method_39330(ChunkPos.getPackedX(l), ChunkPos.getPackedZ(l), this.field_35487).terrainInfo())
				);
	}
