	...
	public void buildSurface(
		...
		for(int k = 0; k < 16; ++k) {
			for(int l = 0; l < 16; ++l) {
				...
				RegistryKey<Biome> registryKey = (RegistryKey)biomeRegistry.getKey(biome).orElseThrow(() -> new IllegalStateException("Unregistered biome: " + biome));
				...
				for(int aa = t; aa >= u && x < v; --aa) {
					...
					if (blockState.isAir()) {
						...
						RegistryKey<Biome> registryKey2 = (RegistryKey)biomeRegistry.getKey(biome2).orElseThrow(() -> new IllegalStateException("Unregistered biome: " + biome));
