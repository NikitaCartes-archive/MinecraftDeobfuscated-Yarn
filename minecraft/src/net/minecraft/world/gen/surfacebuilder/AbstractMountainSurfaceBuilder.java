...
public abstract class AbstractMountainSurfaceBuilder extends DefaultSurfaceBuilder {
	...
	protected BlockState getBlockFromNoise(
		double scale, int x, int z, BlockState outsideRangeState, BlockState insideRangeState, double noiseMin, double noiseMax
	) {
	...
	public boolean shouldPlaceSteepSlopeBlock(Chunk chunk, int x, int z, AbstractMountainSurfaceBuilder.SteepSlopeBlockConfig config) {
		...
		if (!config.isEast() && !config.isWest()) {
			...
			int n = Math.min(k + 1, 15);
			int o = chunk.sampleHeightmap(Heightmap.Type.WORLD_SURFACE_WG, m, l);
			int p = chunk.sampleHeightmap(Heightmap.Type.WORLD_SURFACE_WG, n, l);
			int q = o - p;
			if (config.isEast() && q > 3) {
				return true;
			} else {
				return config.isWest() && -q > 3;
			}
		}
	}
