...
public abstract class NewMountainSurfaceBuilder extends DefaultSurfaceBuilder {
	...
	public boolean method_37779(Chunk chunk, int i, int j, NewMountainSurfaceBuilder.class_6474 arg) {
		...
		if (!arg.method_37784() && !arg.method_37783()) {
			...
			int o = Math.max(m - 1, 0);
			int p = Math.min(m + 1, 15);
			int q = chunk.sampleHeightmap(Heightmap.Type.WORLD_SURFACE_WG, o, n);
			int r = chunk.sampleHeightmap(Heightmap.Type.WORLD_SURFACE_WG, p, n);
			int s = q - r;
			if (arg.method_37784() && s > 3) {
				return true;
			} else {
				return arg.method_37783() && -s > 3;
			}
		}
	}
