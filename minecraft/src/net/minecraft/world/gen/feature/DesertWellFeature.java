...
public class DesertWellFeature extends Feature<DefaultFeatureConfig> {
	...
	public boolean method_12977(
		...
		if (!CAN_GENERATE.apply(iWorld.getBlockState(blockPos))) {
			return false;
		} else {
			for(int i = -2; i <= 2; ++i) {
				for(int j = -2; j <= 2; ++j) {
			...
			for(int i = -1; i <= 0; ++i) {
				for(int j = -2; j <= 2; ++j) {
					for(int k = -2; k <= 2; ++k) {
						iWorld.setBlockState(blockPos.add(j, i, k), this.wall, 2);
					}
				}
			}
			...
			for(int i = -2; i <= 2; ++i) {
				for(int j = -2; j <= 2; ++j) {
					if (i == -2 || i == 2 || j == -2 || j == 2) {
						iWorld.setBlockState(blockPos.add(i, 1, j), this.wall, 2);
					}
				}
			}
			...
			for(int i = -1; i <= 1; ++i) {
				for(int j = -1; j <= 1; ++j) {
					if (i == 0 && j == 0) {
						iWorld.setBlockState(blockPos.add(i, 4, j), this.wall, 2);
					} else {
						iWorld.setBlockState(blockPos.add(i, 4, j), this.slab, 2);
					}
				}
			}
