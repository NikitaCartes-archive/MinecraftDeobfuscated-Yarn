	...
	public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
		...
		if (!CAN_GENERATE.test(structureWorldAccess.getBlockState(blockPos))) {
			...
			for(int i = -1; i <= 0; ++i) {
				for(int j = -2; j <= 2; ++j) {
					for(int k = -2; k <= 2; ++k) {
						structureWorldAccess.setBlockState(blockPos.add(j, i, k), this.wall, SetBlockStateFlags.NOTIFY_LISTENERS);
					}
				}
			}
			...
			for(int i = -2; i <= 2; ++i) {
				for(int j = -2; j <= 2; ++j) {
					if (i == -2 || i == 2 || j == -2 || j == 2) {
						structureWorldAccess.setBlockState(blockPos.add(i, 1, j), this.wall, SetBlockStateFlags.NOTIFY_LISTENERS);
					}
				}
			}
			...
			for(int i = -1; i <= 1; ++i) {
				for(int j = -1; j <= 1; ++j) {
					if (i == 0 && j == 0) {
						structureWorldAccess.setBlockState(blockPos.add(i, 4, j), this.wall, SetBlockStateFlags.NOTIFY_LISTENERS);
					} else {
						structureWorldAccess.setBlockState(blockPos.add(i, 4, j), this.slab, SetBlockStateFlags.NOTIFY_LISTENERS);
					}
				}
			}
