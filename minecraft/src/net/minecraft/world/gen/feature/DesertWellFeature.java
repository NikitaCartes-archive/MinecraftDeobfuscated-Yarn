	...
	public boolean method_12977(
		...
		if (!field_13450.apply(iWorld.getBlockState(blockPos))) {
			...
			for(int i = -1; i <= 0; ++i) {
				for(int j = -2; j <= 2; ++j) {
					for(int k = -2; k <= 2; ++k) {
						iWorld.setBlockState(blockPos.add(j, i, k), this.field_13451, 2);
					}
				}
			}
			...
			for(int i = -2; i <= 2; ++i) {
				for(int j = -2; j <= 2; ++j) {
					if (i == -2 || i == 2 || j == -2 || j == 2) {
						iWorld.setBlockState(blockPos.add(i, 1, j), this.field_13451, 2);
					}
				}
			}
			...
			for(int i = -1; i <= 1; ++i) {
				for(int j = -1; j <= 1; ++j) {
					if (i == 0 && j == 0) {
						iWorld.setBlockState(blockPos.add(i, 4, j), this.field_13451, 2);
					} else {
						iWorld.setBlockState(blockPos.add(i, 4, j), this.field_13452, 2);
					}
				}
			}
