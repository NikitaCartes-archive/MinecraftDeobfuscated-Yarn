			...
			for(int s = k; s <= l; ++s) {
				for(int t = 3; t >= -1; --t) {
					for(int u = p; u <= q; ++u) {
						...
						} else if (blockPos2.getY() >= 0 && !structureWorldAccess.getBlockState(blockPos2.method_10074()).getMaterial().isSolid()) {
							structureWorldAccess.setBlockState(blockPos2, AIR, 2);
						} else if (blockState.getMaterial().isSolid() && !blockState.isOf(Blocks.field_10034)) {
							if (t == -1 && random.nextInt(4) != 0) {
								structureWorldAccess.setBlockState(blockPos2, Blocks.field_9989.getDefaultState(), 2);
							} else {
