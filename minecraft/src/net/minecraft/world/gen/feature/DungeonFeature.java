			...
			for(int s = k; s <= l; ++s) {
				for(int t = 3; t >= -1; --t) {
					for(int u = p; u <= q; ++u) {
						...
						} else if (blockPos2.getY() >= 0 && !iWorld.method_8320(blockPos2.down()).method_11620().isSolid()) {
							iWorld.method_8652(blockPos2, AIR, 2);
						} else if (iWorld.method_8320(blockPos2).method_11620().isSolid() && iWorld.method_8320(blockPos2).getBlock() != Blocks.field_10034) {
