					...
					for (int s = 0; s < 16; s++) {
						for (int txx = 4; txx < 8; txx++) {
							if (bls[(j * 16 + s) * 8 + txx]) {
								...
								if (Block.isNaturalDirt(iWorld.method_8320(blockPos2).getBlock()) && iWorld.method_8314(LightType.SKY, blockPos.add(j, txx, s)) > 0) {
					...
					for (int j = 0; j < 16; j++) {
						for (int s = 0; s < 16; s++) {
							for (int txxx = 0; txxx < 8; txxx++) {
								...
								if (bl && (txxx < 4 || random.nextInt(2) != 0) && iWorld.method_8320(blockPos.add(j, txxx, s)).method_11620().method_15799()) {
