...
public class LakeFeature extends Feature<LakeFeatureConfig> {
	...
	public boolean method_13471(
		...
		while(blockPos.getY() > 5 && iWorld.method_8623(blockPos)) {
		...
		if (blockPos.getY() <= 4) {
			...
			if (!iWorld.method_16956(chunkPos.x, chunkPos.z, ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences(Feature.field_13587.getName()).isEmpty()) {
				return false;
			} else {
					...
					for(int s = 0; s < 16; ++s) {
						for(int t = 0; t < 8; ++t) {
							...
							if (bl) {
								...
								if (t < 4 && !material.method_15799() && iWorld.method_8320(blockPos.add(j, t, s)) != lakeFeatureConfig.state) {
									return false;
								}
							}
						}
				...
				for(int j = 0; j < 16; ++j) {
					for(int s = 0; s < 16; ++s) {
						for(int t = 0; t < 8; ++t) {
							if (bls[(j * 16 + s) * 8 + t]) {
								iWorld.method_8652(blockPos.add(j, t, s), t >= 4 ? CAVE_AIR : lakeFeatureConfig.state, 2);
							}
						}
					}
				}
					...
					for(int s = 0; s < 16; ++s) {
						for(int t = 4; t < 8; ++t) {
							if (bls[(j * 16 + s) * 8 + t]) {
								...
								if (Block.isNaturalDirt(iWorld.method_8320(blockPos2).getBlock()) && iWorld.method_8314(LightType.SKY, blockPos.add(j, t, s)) > 0) {
					...
					for(int j = 0; j < 16; ++j) {
						for(int s = 0; s < 16; ++s) {
							for(int t = 0; t < 8; ++t) {
								...
								if (bl && (t < 4 || random.nextInt(2) != 0) && iWorld.method_8320(blockPos.add(j, t, s)).method_11620().method_15799()) {
									iWorld.method_8652(blockPos.add(j, t, s), Blocks.field_10340.method_9564(), 2);
								}
							}
						}
					}
				...
				if (lakeFeatureConfig.state.method_11620() == Material.WATER) {
					for(int j = 0; j < 16; ++j) {
						for(int s = 0; s < 16; ++s) {
							...
							if (iWorld.method_8310(blockPos2).method_8685(iWorld, blockPos2, false)) {
