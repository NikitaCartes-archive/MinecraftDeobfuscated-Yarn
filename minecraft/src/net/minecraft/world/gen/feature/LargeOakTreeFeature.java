...
public class LargeOakTreeFeature extends AbstractTreeFeature<NormalTreeFeatureConfig> {
			...
			for (int j = 0; j <= i; j++) {
				...
				if (bl) {
					this.setBlockState(
						...
						blockPos4,
						normalTreeFeatureConfig.trunkProvider.getBlockState(random, blockPos4).with(LogBlock.AXIS, this.getLogAxis(blockPos, blockPos4)),
						blockBox
					);
					set.add(blockPos4);
			...
			for (; m >= 0; m--) {
				...
				if (!(f < 0.0F)) {
					for (int n = 0; n < k; n++) {
						...
						if (this.makeOrCheckBranch(modifiableTestableWorld, random, blockPos2, blockPos3, false, set, blockBox, normalTreeFeatureConfig) == -1) {
							...
							if (this.makeOrCheckBranch(modifiableTestableWorld, random, blockPos4, blockPos2, false, set, blockBox, normalTreeFeatureConfig) == -1) {
