...
public abstract class StructureFeature<C extends FeatureConfig> extends Feature<C> {
				...
				for (int m = -l; m <= l; m++) {
					...
					for (int n = -l; n <= l; n++) {
						...
						if (bl2 || bl3) {
							...
							StructureStart structureStart = world.method_16956(chunkPos.x, chunkPos.z, ChunkStatus.field_16423).method_12181(this.getName());
	...
	private List<StructureStart> getStructureStarts(IWorld iWorld, int i, int j) {
		...
		Chunk chunk = iWorld.method_16956(i, j, ChunkStatus.field_16422);
		LongIterator longIterator = chunk.getStructureReferences(this.getName()).iterator();

		while (longIterator.hasNext()) {
			long l = longIterator.nextLong();
			BlockViewWithStructures blockViewWithStructures = iWorld.method_16956(ChunkPos.getPackedX(l), ChunkPos.getPackedZ(l), ChunkStatus.field_16423);
