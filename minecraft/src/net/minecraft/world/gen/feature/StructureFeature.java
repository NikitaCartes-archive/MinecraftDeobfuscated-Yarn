...
public abstract class StructureFeature<C extends FeatureConfig> extends Feature<C> {
	...
	public boolean method_13151(IWorld iWorld, ChunkGenerator<? extends ChunkGeneratorConfig> chunkGenerator, Random random, BlockPos blockPos, C featureConfig) {
		if (!iWorld.method_8401().hasStructures()) {
			return false;
		} else {
			...
			int l = j << 4;
			boolean bl = false;
			LongIterator var11 = iWorld.method_8392(i, j).getStructureReferences(this.getName()).iterator();

			while (var11.hasNext()) {
				Long long_ = (Long)var11.next();
				ChunkPos chunkPos = new ChunkPos(long_);
				StructureStart structureStart = iWorld.method_8392(chunkPos.x, chunkPos.z).method_12181(this.getName());
	...
	protected StructureStart method_14025(IWorld iWorld, BlockPos blockPos, boolean bl) {
		for (StructureStart structureStart : this.getStructureStarts(iWorld, blockPos.getX() >> 4, blockPos.getZ() >> 4)) {
			if (structureStart.hasChildren() && structureStart.getBoundingBox().method_14662(blockPos)) {
				if (!bl) {
				...
				for (int m = -l; m <= l; m++) {
					...
					for (int n = -l; n <= l; n++) {
						...
						if (bl2 || bl3) {
							...
							StructureStart structureStart = world.method_16956(chunkPos.x, chunkPos.z, ChunkStatus.STRUCTURE_STARTS).method_12181(this.getName());
	...
	private List<StructureStart> getStructureStarts(IWorld iWorld, int i, int j) {
		...
		Chunk chunk = iWorld.method_16956(i, j, ChunkStatus.EMPTY);
		LongIterator longIterator = chunk.getStructureReferences(this.getName()).iterator();

		while (longIterator.hasNext()) {
			long l = longIterator.nextLong();
			BlockViewWithStructures blockViewWithStructures = iWorld.method_16956(ChunkPos.getPackedX(l), ChunkPos.getPackedZ(l), ChunkStatus.EMPTY);
			StructureStart structureStart = blockViewWithStructures.method_12181(this.getName());
