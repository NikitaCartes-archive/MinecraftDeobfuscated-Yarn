				...
				for(int r = -3; r <= 4; ++r) {
					for(int s = -3; s <= 4; ++s) {
						if ((r != -3 || s != -3) && (r != -3 || s != 4) && (r != 4 || s != -3) && (r != 4 || s != 4) && (Math.abs(r) < 3 || Math.abs(s) < 3)) {
							this.method_13873(modifiableTestableWorld, o + r, q, p + s);
						}
					}
				}
				...
				for(int r = -1; r <= 2; ++r) {
					for(int s = -1; s <= 2; ++s) {
						if ((r < 0 || r > 1 || s < 0 || s > 1) && random.nextInt(3) <= 0) {
							...
							for(int u = 0; u < t; ++u) {
								this.method_13874(set, modifiableTestableWorld, new BlockPos(j + r, q - u - 1, l + s));
							}

							for(int u = -1; u <= 1; ++u) {
								for(int v = -1; v <= 1; ++v) {
									this.method_13873(modifiableTestableWorld, o + r + u, q, p + s + v);
								}
							}
							...
							for(int u = -2; u <= 2; ++u) {
								for(int v = -2; v <= 2; ++v) {
									if (Math.abs(u) != 2 || Math.abs(v) != 2) {
										this.method_13873(modifiableTestableWorld, o + r + u, q - 1, p + s + v);
									}
								}
							}
						}
