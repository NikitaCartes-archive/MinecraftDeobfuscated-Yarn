...
public class BonusChestFeature extends Feature<DefaultFeatureConfig> {
	...
	public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
		...
		List<Integer> list = Util.copyShuffled(
			(List)IntStream.rangeClosed(chunkPos.getStartX(), chunkPos.getEndX()).boxed().collect(Collectors.toList()), abstractRandom
		);
		List<Integer> list2 = Util.copyShuffled(
			(List)IntStream.rangeClosed(chunkPos.getStartZ(), chunkPos.getEndZ()).boxed().collect(Collectors.toList()), abstractRandom
		);
