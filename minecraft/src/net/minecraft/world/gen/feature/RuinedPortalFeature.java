...
public class RuinedPortalFeature extends StructureFeature<RuinedPortalFeatureConfig> {
	...
	private static int method_27211(
		...
		List<BlockView> list2 = (List)list.stream().map(blockPos -> chunkGenerator.getColumnSample(blockPos.getX(), blockPos.getZ())).collect(Collectors.toList());
		Heightmap.Type type = verticalPlacement == RuinedPortalStructurePiece.VerticalPlacement.field_24031
			? Heightmap.Type.field_13195
			: Heightmap.Type.field_13194;
