...
public class RuinedPortalFeature extends StructureFeature<RuinedPortalFeatureConfig> {
	...
	private static int method_27211(
		...
		List<VerticalBlockSample> list2 = (List)list.stream()
			.map(blockPos -> chunkGenerator.getColumnSample(blockPos.getX(), blockPos.getZ(), heightLimitView))
	...
	public static class Start extends StructureStart<RuinedPortalFeatureConfig> {
		...
		public void init(
			...
			BlockRotation blockRotation = Util.getRandom((BlockRotation[])BlockRotation.values(), this.random);
			...
			int n = RuinedPortalFeature.method_27211(
				this.random, chunkGenerator, verticalPlacement, properties.airPocket, m, blockBox.getBlockCountY(), blockBox, heightLimitView
			);
