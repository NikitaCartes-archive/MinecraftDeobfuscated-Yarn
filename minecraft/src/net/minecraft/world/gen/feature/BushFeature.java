...
public class BushFeature extends Feature<BushFeatureConfig> {
	...
	public boolean method_12841(
		IWorld iWorld, ChunkGenerator<? extends ChunkGeneratorSettings> chunkGenerator, Random random, BlockPos blockPos, BushFeatureConfig bushFeatureConfig
	) {
		int i = 0;
		...
		for (int j = 0; j < 64; j++) {
			...
			if (iWorld.isAir(blockPos2) && (!iWorld.getDimension().method_12467() || blockPos2.getY() < 255) && blockState.canPlaceAt(iWorld, blockPos2)) {
