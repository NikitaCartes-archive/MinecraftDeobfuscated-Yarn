...
public class PlacedFeatureIndexer {
	...
	public static <T> List<PlacedFeatureIndexer.IndexedFeatures> collectIndexedFeatures(
		...
		for(T object : biomes) {
			...
			List<RegistryEntryList<PlacedFeature>> list2 = (List)biomesToPlacedFeaturesList.apply(object);
			...
			for(int j = 0; j < list.size(); ++j) {
				Set<IndexedFeature> set = (Set)map.computeIfAbsent((IndexedFeature)list.get(j), feature -> new TreeSet(comparator));
				if (j < list.size() - 1) {
		...
		for(IndexedFeature indexedFeature : map.keySet()) {
			...
			if (!set2.contains(indexedFeature) && TopologicalSorts.sort(map, set2, set3, list::add, (T)indexedFeature)) {
		...
		for(int j = 0; j < i; ++j) {
			int l = j;
			List<PlacedFeature> list4 = (List)list.stream().filter(feature -> feature.step() == l).map(IndexedFeature::feature).collect(Collectors.toList());
