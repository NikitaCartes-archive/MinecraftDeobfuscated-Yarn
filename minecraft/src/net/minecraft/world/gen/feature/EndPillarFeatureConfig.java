...
public class EndPillarFeatureConfig implements FeatureConfig {
	...
	public static <T> EndPillarFeatureConfig deserialize(Dynamic<T> dynamic) {
		List<EndSpikeFeature.Spike> list = (List)((Stream)dynamic.get("spikes").flatMap(Dynamic::getStream).orElse(Stream.empty()))
			.map(EndSpikeFeature.Spike::deserialize)
			.collect(Collectors.toList());
		List<Number> list2 = (List)((Stream)dynamic.get("crystalBeamTarget")
