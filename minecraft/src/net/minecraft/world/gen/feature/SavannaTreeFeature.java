			...
			if (!bl) {
				...
				for (int qx = -3; qx <= 3; qx++) {
					for (int r = -3; r <= 3; r++) {
						if (Math.abs(qx) != 3 || Math.abs(r) != 3) {
							this.addLeaves(set, modifiableTestableWorld, blockPos3.add(qx, 0, r), blockBox);
						}
					}
				}
				...
				for (int qx = -1; qx <= 1; qx++) {
					for (int rx = -1; rx <= 1; rx++) {
						this.addLeaves(set, modifiableTestableWorld, blockPos3.add(qx, 0, rx), blockBox);
					}
				}
					...
					if (o > 0) {
						...
						for (int t = -1; t <= 1; t++) {
							for (int ux = -1; ux <= 1; ux++) {
								this.addLeaves(set, modifiableTestableWorld, blockPos5.add(t, 0, ux), blockBox);
							}
						}
					}
				}
