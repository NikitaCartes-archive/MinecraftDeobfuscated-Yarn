...
public abstract class AbstractTreeFeature<T extends FeatureConfig> extends Feature<T> {
				...
				if (bl2 && !set.isEmpty()) {
					for(BlockPos blockPos2 : Lists.newArrayList(set)) {
						if (mutableIntBoundingBox.contains(blockPos2)) {
							voxelSet.set(
								blockPos2.getX() - mutableIntBoundingBox.minX,
								blockPos2.getY() - mutableIntBoundingBox.minY,
								blockPos2.getZ() - mutableIntBoundingBox.minZ,
								true,
								true
							);
						}
				...
				for(int k = 1; k < 6; ++k) {
					...
					for(BlockPos blockPos3 : set2) {
						if (mutableIntBoundingBox.contains(blockPos3)) {
							voxelSet.set(
								blockPos3.getX() - mutableIntBoundingBox.minX,
								blockPos3.getY() - mutableIntBoundingBox.minY,
								blockPos3.getZ() - mutableIntBoundingBox.minZ,
								true,
								true
							);
						}
	...
	private void generateBeeHive(
		...
		if (random.nextFloat() < f) {
			...
			BlockPos blockPos2 = blockPos.add(direction.getOffsetX(), i - 1 - blockPos.getY(), direction.getOffsetZ());
			if (iWorld.method_22347(blockPos2) && iWorld.method_22347(blockPos2.offset(Direction.SOUTH))) {
				this.setBlockState(iWorld, blockPos2, blockState);
