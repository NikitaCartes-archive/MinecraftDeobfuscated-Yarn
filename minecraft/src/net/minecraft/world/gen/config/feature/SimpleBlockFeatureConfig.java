...
public class SimpleBlockFeatureConfig implements FeatureConfig {
	...
	public static <T> SimpleBlockFeatureConfig make(Dynamic<T> dynamic) {
		...
		List<BlockState> list = (List)((Stream)dynamic.get("place_on").flatMap(Dynamic::getStream).orElseGet(Stream::empty))
			.map(BlockState::deserialize)
			.collect(Collectors.toList());
		List<BlockState> list2 = (List)((Stream)dynamic.get("place_in").flatMap(Dynamic::getStream).orElseGet(Stream::empty))
			.map(BlockState::deserialize)
			.collect(Collectors.toList());
		List<BlockState> list3 = (List)((Stream)dynamic.get("place_under").flatMap(Dynamic::getStream).orElseGet(Stream::empty))
			.map(BlockState::deserialize)
			.collect(Collectors.toList());
