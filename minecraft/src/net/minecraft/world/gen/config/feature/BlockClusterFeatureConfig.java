...
public class BlockClusterFeatureConfig implements FeatureConfig {
	public final BlockState state;
	public final int radius;
	public final int ySize;
	public final List<BlockState> targets;

	public BlockClusterFeatureConfig(BlockState blockState, int i, int j, List<BlockState> list) {
	...
	public static <T> BlockClusterFeatureConfig deserialize(Dynamic<T> dynamic) {
		...
		List<BlockState> list = (List)((Stream)dynamic.get("targets").flatMap(Dynamic::getStream).orElse(Stream.empty()))
			.map(BlockState::deserialize)
			.collect(Collectors.toList());
		return new BlockClusterFeatureConfig(blockState, i, j, list);
	}
}
