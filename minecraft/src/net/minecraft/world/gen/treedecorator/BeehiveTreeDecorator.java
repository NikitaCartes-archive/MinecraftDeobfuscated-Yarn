...
public class BeehiveTreeDecorator extends TreeDecorator {
	public static final Codec<BeehiveTreeDecorator> CODEC = Codec.floatRange(0.0F, 1.0F)
		...
		.codec();
	private static final Direction field_36932 = Direction.SOUTH;
	private static final Direction[] field_36933 = (Direction[])Direction.Type.HORIZONTAL
		.stream()
		.filter(direction -> direction != field_36932.getOpposite())
	...
	public void generate(
		...
		if (!(random.nextFloat() >= this.probability)) {
			...
			List<BlockPos> list = (List)logPositions.stream()
				.filter(pos -> pos.getY() == i)
				.flatMap(blockPos -> Stream.of(field_36933).map(blockPos::offset))
				.collect(Collectors.toList());
			if (!list.isEmpty()) {
				Collections.shuffle(list);
				Optional<BlockPos> optional = list.stream()
					.filter(blockPos -> Feature.isAir(world, blockPos) && Feature.isAir(world, blockPos.offset(field_36932)))
					.findFirst();
				if (!optional.isEmpty()) {
					replacer.accept((BlockPos)optional.get(), Blocks.BEE_NEST.getDefaultState().with(BeehiveBlock.FACING, field_36932));
