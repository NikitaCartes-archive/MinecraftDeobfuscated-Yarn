...
public class AttachedToLeavesTreeDecorator extends TreeDecorator {
	...
	public void generate(
		...
		List<BlockPos> list = new ArrayList(leavesPositions);
		Util.shuffle(list, random);

		for (BlockPos blockPos : list) {
			Direction direction = Util.getRandom(this.directions, random);
			BlockPos blockPos2 = blockPos.offset(direction);
			if (!set.contains(blockPos2) && random.nextFloat() < this.probability && this.meetsRequiredEmptyBlocks(world, blockPos, direction)) {
