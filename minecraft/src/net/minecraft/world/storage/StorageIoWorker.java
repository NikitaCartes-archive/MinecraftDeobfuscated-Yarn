...
public class StorageIoWorker implements AutoCloseable {
	...
	public CompletableFuture<Void> setResult(ChunkPos pos, CompoundTag nbt) {
		return this.run(() -> {
			StorageIoWorker.Result result = (StorageIoWorker.Result)this.results.computeIfAbsent(pos, chunkPos -> new StorageIoWorker.Result(nbt));
			result.nbt = nbt;
		...
		CompletableFuture<CompoundTag> completableFuture = this.run(() -> {
			...
			if (result != null) {
				...
				try {
					...
					LOGGER.warn("Failed to read chunk {}", pos, var4xx);
