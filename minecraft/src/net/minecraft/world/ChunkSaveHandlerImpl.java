...
public class ChunkSaveHandlerImpl {
	private static final Logger LOGGER = LogManager.getLogger();

	public static ProtoChunk writeSectionsTag(World world, StructureManager structureManager, ChunkPos chunkPos, CompoundTag compoundTag) {
		...
		if (compoundTag2.containsKey("Biomes", 11)) {
			...
			for(int i = 0; i < is.length; ++i) {
				...
				if (biomes[i] == null) {
					biomes[i] = biomeSource.getBiome(mutable.set((i & 15) + chunkPos.getXStart(), 0, (i >> 4 & 15) + chunkPos.getZStart()));
				}
			}
		} else {
			for(int j = 0; j < biomes.length; ++j) {
				biomes[j] = biomeSource.getBiome(mutable.set((j & 15) + chunkPos.getXStart(), 0, (j >> 4 & 15) + chunkPos.getZStart()));
			}
		}
		...
		for(int l = 0; l < listTag.size(); ++l) {
			...
			if (bl) {
				...
				if (bl2 && compoundTag3.containsKey("SkyLight", 7)) {
					chunkManager.getLightingProvider()
						.setSection(LightType.SKY_LIGHT, chunkPos.x, m, chunkPos.z, new ChunkNibbleArray(compoundTag3.getByteArray("SkyLight")));
				}
			}
		}

		long n = compoundTag2.getLong("InhabitedTime");
		ChunkStatus.ChunkType chunkType = getEntityStorageMode(compoundTag);
		Chunk chunk;
		if (chunkType == ChunkStatus.ChunkType.LEVELCHUNK) {
			...
			if (!bl && protoChunk.getStatus().isAfter(ChunkStatus.LIGHT)) {
				for(BlockPos blockPos : BlockPos.iterateBoxPositions(chunkPos.getXStart(), 0, chunkPos.getZStart(), chunkPos.getXEnd(), 255, chunkPos.getZEnd())) {
	...
	public static CompoundTag saveChunk(World world, Chunk chunk) {
		...
		compoundTag2.put("Structures", writeStructuresTag(chunkPos, chunk.getStructureStarts(), chunk.getStructureReferences()));
		return compoundTag;
	}

	public static ChunkStatus.ChunkType getEntityStorageMode(@Nullable CompoundTag compoundTag) {
		if (compoundTag != null) {
	...
	private static CompoundTag writeStructuresTag(ChunkPos chunkPos, Map<String, StructureStart> map, Map<String, LongSet> map2) {
		CompoundTag compoundTag = new CompoundTag();
		CompoundTag compoundTag2 = new CompoundTag();
