...
public class ChunkSaveHandlerImpl implements ChunkSaveHandler {
	...
	public class_2839 method_12411(IWorld iWorld, int i, int j) {
		try {
			...
			if (compoundTag == null) {
				...
				if (!bl) {
					...
					ChunkTickScheduler<Fluid> chunkTickScheduler2 = new ChunkTickScheduler<>(
						fluid -> fluid == null || fluid == Fluids.field_15906,
						Registry.FLUID::getId,
						Registry.FLUID::get,
						chunkPos,
						compoundTag2.getList("LiquidsToBeTicked", 9)
					);
			...
			field_13001.error("Couldn't load chunk", var38);
			return null;
		}
	}
	...
	public void method_12410(World world, Chunk chunk) throws IOException, MinecraftException {
		...
		try {
			...
			field_13001.error("Failed to save chunk", var21);
		}
	}
	...
	public boolean method_12412() {
		...
		if (!iterator.hasNext()) {
			...
			Entry<ChunkPos, CompoundTag> entry = (Entry)iterator.next();
			iterator.remove();
			...
			if (compoundTag == null) {
				...
				try {
					...
					field_13001.error("Failed to save chunk", var6);
				}
	...
	private Map<String, class_3449> method_12392(ChunkGenerator<?> chunkGenerator, class_3485 arg, BiomeSource biomeSource, CompoundTag compoundTag) {
		Map<String, class_3449> map = Maps.newHashMap();
	...
	protected static Entity method_12379(CompoundTag compoundTag, World world) {
		try {
			...
			field_13001.warn("Exception loading entity: ", var3);
			return null;
		}
	}
