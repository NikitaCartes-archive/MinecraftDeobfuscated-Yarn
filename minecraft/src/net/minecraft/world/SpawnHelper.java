...
public final class SpawnHelper {
	...
	private static final EntityCategory[] SPAWNABLE_CATEGORIES = (EntityCategory[])Stream.of(EntityCategory.values())
		...
		.toArray(i -> new EntityCategory[i]);
	...
	private static boolean isAcceptableSpawnPosition(ServerWorld world, Chunk chunk, BlockPos.Mutable pos, double squaredDistance) {
		if (squaredDistance <= 576.0) {
			return false;
		} else if (world.method_27911()
			.isWithinDistance(new Vec3d((double)((float)pos.getX() + 0.5F), (double)pos.getY(), (double)((float)pos.getZ() + 0.5F)), 24.0)) {
			return false;
		} else {
	...
	private static boolean canSpawn(
		...
		if (entityType.getCategory() == EntityCategory.MISC) {
			...
			if (!canSpawn(location, world, pos, entityType)) {
				return false;
			} else if (!SpawnRestriction.canSpawn(entityType, world, SpawnType.NATURAL, pos, world.random)) {
				return false;
			} else {
				return world.doesNotCollide(entityType.createSimpleBoundingBox((double)((float)pos.getX() + 0.5F), (double)pos.getY(), (double)((float)pos.getZ() + 0.5F)));
			}
		} else {
			return false;
		}
	...
	private static boolean isValidSpawn(ServerWorld world, MobEntity entity, double squaredDistance) {
		if (squaredDistance > (double)(entity.getType().getImmediateDespawnRange() * entity.getType().getImmediateDespawnRange())
			...
			switch(location) {
				...
				default:
					...
					if (!blockState2.allowsSpawning(world, blockPos2, entityType)) {
						...
						return isClearForSpawn(world, pos, blockState, fluidState)
							&& isClearForSpawn(world, blockPos, world.getBlockState(blockPos), world.getFluidState(blockPos));
					}
			}
		} else {
			return false;
