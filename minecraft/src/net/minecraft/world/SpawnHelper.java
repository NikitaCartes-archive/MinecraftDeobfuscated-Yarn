					...
					while(true) {
						label109: {
							...
							if (s < q) {
								...
								if (spawnEntry.type.getCategory() == EntityCategory.MISC) {
									break label109;
								}
								...
								if (playerEntity.squaredDistanceTo((double)f, (double)k, (double)g) > 16384.0
									...
									|| !mobEntity.canSpawn(world, SpawnType.NATURAL)
			...
			switch(location) {
				...
				default:
					...
					if (!blockState2.allowsSpawning(viewableWorld, blockPos3, entityType)) {
						...
						return isClearForSpawn(viewableWorld, blockPos, blockState, fluidState)
