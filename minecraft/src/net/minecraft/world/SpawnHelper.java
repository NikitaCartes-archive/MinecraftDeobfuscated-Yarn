...
public final class SpawnHelper {
	...
	private static boolean method_24934(
		ServerWorld serverWorld, ChunkGenerator<?> chunkGenerator, Biome.SpawnEntry spawnEntry, BlockPos.Mutable mutable, double d
	) {
		...
		if (entityType.getCategory() == EntityCategory.MISC) {
			...
			if (!canSpawn(location, serverWorld, mutable, entityType)) {
				return false;
			} else if (!SpawnRestriction.canSpawn(entityType, serverWorld, SpawnType.NATURAL, mutable, serverWorld.random)) {
				return false;
			} else {
				return serverWorld.doesNotCollide(
			...
			switch(location) {
				...
				default:
					...
					if (!blockState2.allowsSpawning(worldView, blockPos2, entityType)) {
						...
						return isClearForSpawn(worldView, pos, blockState, fluidState)
	...
	private static BlockPos getEntitySpawnPos(WorldView worldView, @Nullable EntityType<?> entityType, int x, int z) {
		...
		return worldView.getBlockState(blockPos2).canPathfindThrough(worldView, blockPos2, NavigationType.LAND) ? blockPos2 : blockPos;
	}
}
