...
public final class SpawnHelper {
	...
	private static boolean canSpawn(
		...
		if (entityType.getSpawnGroup() == SpawnGroup.MISC) {
			...
			if (!canSpawn(location, world, pos, entityType)) {
				...
				return world.doesNotCollide(entityType.createSimpleBoundingBox((double)pos.getX() + 0.5, (double)pos.getY(), (double)pos.getZ() + 0.5));
			}
		} else {
			return false;
		}
	...
	private static boolean isValidSpawn(ServerWorld world, MobEntity entity, double squaredDistance) {
		if (squaredDistance > (double)(entity.getType().getSpawnGroup().getImmediateDespawnRange() * entity.getType().getSpawnGroup().getImmediateDespawnRange())
			...
			switch(location) {
				...
				default:
					...
					if (!blockState2.allowsSpawning(world, blockPos2, entityType)) {
						...
						return isClearForSpawn(world, pos, blockState, fluidState, entityType)
			...
			while(random.nextFloat() < biome.getMaxSpawnChance()) {
				...
				for(int p = 0; p < k; ++p) {
					...
					for(int q = 0; !bl && q < 4; ++q) {
						BlockPos blockPos = getEntitySpawnPos(arg, spawnEntry.type, l, m);
						if (spawnEntry.type.isSummonable() && canSpawn(SpawnRestriction.getLocation(spawnEntry.type), arg, blockPos, spawnEntry.type)) {
							...
							if (!arg.doesNotCollide(spawnEntry.type.createSimpleBoundingBox(d, (double)blockPos.getY(), e))
								|| !SpawnRestriction.canSpawn(spawnEntry.type, arg, SpawnReason.CHUNK_GENERATION, new BlockPos(d, (double)blockPos.getY(), e), arg.getRandom())) {
								continue;
							}
							...
							if (entity instanceof MobEntity) {
								MobEntity mobEntity = (MobEntity)entity;
								if (mobEntity.canSpawn(arg, SpawnReason.CHUNK_GENERATION) && mobEntity.canSpawn(arg)) {
