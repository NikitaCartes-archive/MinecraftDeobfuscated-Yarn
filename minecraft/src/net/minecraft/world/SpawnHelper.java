...
public final class SpawnHelper {
	...
	private static boolean canSpawn(
		...
		if (entityType.getSpawnGroup() == SpawnGroup.field_17715) {
			...
			if (!canSpawn(location, world, pos, entityType)) {
				return false;
			} else if (!SpawnRestriction.canSpawn(entityType, world, SpawnReason.field_16459, pos, world.random)) {
				return false;
			} else {
	...
	private static boolean isValidSpawn(ServerWorld world, MobEntity entity, double squaredDistance) {
		if (squaredDistance > (double)(entity.getType().getSpawnGroup().getImmediateDespawnRange() * entity.getType().getSpawnGroup().getImmediateDespawnRange())
			...
			return entity.canSpawn(world, SpawnReason.field_16459) && entity.canSpawn(world);
		}
	}
	...
	public static boolean isClearForSpawn(BlockView blockView, BlockPos pos, BlockState state, FluidState fluidState, EntityType<?> entityType) {
		if (state.isFullCube(blockView, pos)) {
			...
			return false;
		} else if (state.isIn(BlockTags.field_24459)) {
			return false;
		} else {
