		...
		if (registry.size() != BASE_DIMENSIONS.size()) {
			...
			if (!optional.isEmpty() && !optional2.isEmpty() && !optional3.isEmpty()) {
				if (!((DimensionOptions)optional.get()).getDimensionTypeSupplier().value().equals(DimensionType.OVERWORLD)
					...
					return false;
				} else if (((DimensionOptions)optional2.get()).getChunkGenerator() instanceof NoiseChunkGenerator noiseChunkGenerator
					&& ((DimensionOptions)optional3.get()).getChunkGenerator() instanceof NoiseChunkGenerator noiseChunkGenerator2) {
