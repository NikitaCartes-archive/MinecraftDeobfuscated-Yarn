...
public abstract class Biome {
	...
	public float getNoiseDistance(Biome.MixedNoisePoint from) {
		return this.noisePoints.stream().map(mixedNoisePoint2 -> mixedNoisePoint2.calculateDistanceTo(from)).min(Float::compare).orElse(Float.POSITIVE_INFINITY);
	}
		...
		public boolean equals(Object object) {
			if (this == object) {
				...
				if (Float.compare(mixedNoisePoint.temperature, this.temperature) != 0) {
					...
					return false;
				} else if (Float.compare(mixedNoisePoint.hilliness, this.hilliness) != 0) {
					return false;
				} else {
