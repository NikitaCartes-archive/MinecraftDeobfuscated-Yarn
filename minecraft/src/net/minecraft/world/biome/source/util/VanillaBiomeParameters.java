		...
		for(int i = 0; i < this.TEMPERATURE_PARAMETERS.length; ++i) {
			...
			for(int j = 0; j < this.HUMIDITY_PARAMETERS.length; ++j) {
				...
				if (weirdness.getMax() < 0.0F) {
					this.writeBiomeParameters(
						consumer, parameterRange, parameterRange2, this.SHORE_CONTINENTALNESS, this.EROSION_PARAMETERS[4], weirdness, 0.0F, registryKey6
					);
					this.writeBiomeParameters(
						consumer,
						parameterRange,
				...
				if (weirdness.getMax() < 0.0F) {
					this.writeBiomeParameters(
						consumer, parameterRange, parameterRange2, this.SHORE_CONTINENTALNESS, this.EROSION_PARAMETERS[6], weirdness, 0.0F, registryKey6
					);
				} else {
