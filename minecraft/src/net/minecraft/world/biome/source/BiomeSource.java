...
public abstract class BiomeSource {
	...
	protected BiomeSource(List<Biome> list) {
		this.biomes = list;
		this.topMaterials = (ImmutableSet)list.stream()
		...
		Map<class_6543, Set<class_6543>> map = Maps.newHashMap();
		int i = 0;
		...
		for(Biome biome : list) {
			...
			for(int j = 0; j < list2.size(); ++j) {
				Set<class_6543> set = (Set)map.computeIfAbsent((class_6543)list2.get(j), arg -> Sets.newHashSet());
				if (j < list2.size() - 1) {
		...
		Set<class_6543> set2 = Sets.newHashSet();
		Set<class_6543> set3 = Sets.newHashSet();
		List<class_6543> list2 = Lists.newArrayList();
