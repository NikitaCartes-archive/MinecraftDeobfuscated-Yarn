...
public abstract class BiomeSource implements BiomeAccess.Storage {
	...
	protected BiomeSource(List<Biome> list) {
		...
		Map<Pair<Integer, ConfiguredFeature<?, ?>>, Set<Pair<Integer, ConfiguredFeature<?, ?>>>> map = Maps.newHashMap();
		int i = 0;
		...
		for(int j = 0; j < i; ++j) {
			int k = j;
			builder3.add((List)list2.stream().filter(pair -> pair.getFirst() == k).map(Pair::getSecond).collect(Collectors.toList()));
		}
