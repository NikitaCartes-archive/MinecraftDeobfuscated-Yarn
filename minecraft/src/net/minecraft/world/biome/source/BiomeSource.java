...
public abstract class BiomeSource implements BiomeSupplier {
	...
	private List<BiomeSource.IndexedFeatures> method_39525(List<RegistryEntry<Biome>> biomes, boolean bl) {
		...
		for(RegistryEntry<Biome> registryEntry : biomes) {
			...
			List<class_6543> list = Lists.newArrayList();
			...
			for(int j = 0; j < list.size(); ++j) {
				Set<class_6543> set = (Set)map.computeIfAbsent((class_6543)list.get(j), arg -> new TreeSet(comparator));
				if (j < list.size() - 1) {
		...
		List<class_6543> list3 = Lists.newArrayList();
			...
			if (!set2.contains(lv) && TopologicalSorts.sort(map, set2, set3, list3::add, lv)) {
				...
				do {
					...
					while(listIterator.hasNext()) {
						RegistryEntry<Biome> registryEntry3 = (RegistryEntry)listIterator.next();
						listIterator.remove();
		...
		for(int l = 0; l < i; ++l) {
			List<PlacedFeature> list5 = (List)list3.stream().filter(arg -> arg.step() == l).map(class_6543::feature).collect(Collectors.toList());
			int m = list5.size();
