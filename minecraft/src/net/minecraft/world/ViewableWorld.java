		...
		Stream<VoxelShape> stream = StreamSupport.stream(BlockPos.iterateBoxPositions(i, k, m, j - 1, l - 1, n - 1).spliterator(), false).map(blockPos -> {
			...
			if (World.isHeightInvalid(p)) {
				...
				if ((!bl3 || !bl4) && (!bl4 || !bl5) && (!bl5 || !bl3) && chunk != null) {
					VoxelShape voxelShape2xx;
					if (bl && !bl2 && !worldBorder.contains(blockPos)) {
						voxelShape2xx = VoxelShapes.fullCube();
					} else {
						voxelShape2xx = chunk.getBlockState(blockPos).getCollisionShape(this, blockPos, verticalEntityPosition);
					}

					VoxelShape voxelShape3 = voxelShape2.method_1096((double)(-o), (double)(-p), (double)(-q));
					if (VoxelShapes.compareShapes(voxelShape3, voxelShape2xx, BooleanBiFunction.AND)) {
						return VoxelShapes.empty();
					} else if (voxelShape2xx == VoxelShapes.fullCube()) {
						abstractVoxelShapeContainer.modify(o - i, p - k, q - m, true, true);
						return VoxelShapes.empty();
					} else {
						return voxelShape2xx.method_1096((double)o, (double)p, (double)q);
					}
				} else {
					return VoxelShapes.empty();
				}
