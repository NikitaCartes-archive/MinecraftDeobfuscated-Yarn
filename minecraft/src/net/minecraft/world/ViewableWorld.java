...
public interface ViewableWorld extends ExtendedBlockView {
	...
	default Stream<VoxelShape> getCollidingBoundingBoxesForEntity(@Nullable Entity entity, BoundingBox boundingBox, Set<Entity> set) {
		...
		Predicate<VoxelShape> predicate = voxelShape2 -> !voxelShape2.isEmpty() && VoxelShapes.compareShapes(voxelShape, voxelShape2, BooleanBiFunction.AND);
		...
		Stream<VoxelShape> stream2 = BlockPos.getBlocksInCuboid(i, k, m, j - 1, l - 1, n - 1).map(blockPos -> {
			...
			if (World.isHeightInvalid(p)) {
				...
				if ((!bl || !bl2) && (!bl2 || !bl3) && (!bl3 || !bl) && chunk != null) {
					VoxelShape voxelShapexx = chunk.getBlockState(blockPos).getCollisionShape(this, blockPos, verticalEntityPosition);
					VoxelShape voxelShape2 = VoxelShapes.empty().offset((double)(-o), (double)(-p), (double)(-q));
					if (VoxelShapes.compareShapes(voxelShape2, voxelShapexx, BooleanBiFunction.AND)) {
						return VoxelShapes.empty();
					} else if (voxelShapexx == VoxelShapes.fullCube()) {
						abstractVoxelShapeContainer.modify(o - i, p - k, q - m, true, true);
						return VoxelShapes.empty();
					} else {
						return voxelShapexx.offset((double)o, (double)p, (double)q);
					}
				} else {
					return VoxelShapes.empty();
				}
