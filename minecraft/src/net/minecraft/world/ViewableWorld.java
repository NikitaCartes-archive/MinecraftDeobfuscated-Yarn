...
public interface ViewableWorld extends ExtendedBlockView {
	...
	default Stream<VoxelShape> method_8600(@Nullable Entity entity, BoundingBox boundingBox, Set<Entity> set) {
		...
		Predicate<VoxelShape> predicate = voxelShape2 -> !voxelShape2.isEmpty() && VoxelShapes.method_1074(voxelShape, voxelShape2, BooleanBiFunction.AND);
		...
		Stream<VoxelShape> stream2 = BlockPos.getBlocksInCuboid(i, k, m, j - 1, l - 1, n - 1).map(blockPos -> {
			...
			if (World.isHeightInvalid(p)) {
				...
				if ((!bl || !bl2) && (!bl2 || !bl3) && (!bl3 || !bl) && chunk != null) {
					VoxelShape voxelShapexx = chunk.method_8320(blockPos).method_16337(this, blockPos, verticalEntityPosition);
					VoxelShape voxelShape2 = VoxelShapes.method_1073().offset((double)(-o), (double)(-p), (double)(-q));
					if (VoxelShapes.method_1074(voxelShape2, voxelShapexx, BooleanBiFunction.AND)) {
						return VoxelShapes.method_1073();
					} else if (voxelShapexx == VoxelShapes.method_1077()) {
