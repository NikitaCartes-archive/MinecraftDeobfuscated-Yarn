		...
		return Streams.concat(StreamSupport.stream(new AbstractSpliterator<VoxelShape>(Long.MAX_VALUE, 0) {
			...
			public boolean tryAdvance(Consumer<? super VoxelShape> consumer) {
				if (!this.field_19296) {
					...
					VoxelShape voxelShapex = ViewableWorld.this.getWorldBorder().asVoxelShape();
					boolean bl = VoxelShapes.matchesAnywhere(voxelShapex, VoxelShapes.cuboid(entity.getBoundingBox().contract(1.0E-7)), BooleanBiFunction.AND);
					boolean bl2 = VoxelShapes.matchesAnywhere(voxelShapex, VoxelShapes.cuboid(entity.getBoundingBox().expand(1.0E-7)), BooleanBiFunction.AND);
					if (!bl && bl2) {
				...
				while(cuboidBlockIterator.step()) {
					int ix = cuboidBlockIterator.getX();
					int jx = cuboidBlockIterator.getY();
					int kx = cuboidBlockIterator.getZ();
					int lx = 0;
					if (ix == i || ix == j) {
						++lx;
					}
					...
					if (lx < 3) {
						int mx = ix >> 4;
						int nx = kx >> 4;
						Chunk chunk = ViewableWorld.this.getChunk(mx, nx, ViewableWorld.this.getLeastChunkStatusForCollisionCalculation(), false);
						if (chunk != null) {
							mutable.set(ix, jx, kx);
							BlockState blockState = chunk.getBlockState(mutable);
							if ((lx != 1 || blockState.method_17900()) && (lx != 2 || blockState.getBlock() == Blocks.field_10008)) {
								...
								VoxelShape voxelShape3 = voxelShape2.offset((double)ix, (double)jx, (double)kx);
