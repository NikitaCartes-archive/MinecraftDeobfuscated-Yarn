...
public class class_4180<R extends class_4213> extends RegionFileCache {
	...
	protected void method_19290(BooleanSupplier booleanSupplier) {
		while(!this.field_18693.isEmpty() && booleanSupplier.getAsBoolean()) {
			...
			for(int i = 0; i < 16; ++i) {
				...
				Optional<R> optional = (Optional)this.field_18692.get(l);
			...
			try {
				...
				field_18691.error("Error writing data to disk", var8);
			}
		}
	}
	...
	protected Optional<R> method_19293(long l) {
		return (Optional<R>)this.field_18692.get(l);
	}
			...
			for(int i = 0; i < 16; ++i) {
				...
				if (compoundTag != null) {
					try {
						...
						field_18691.error("Error reading data from disk", var7);
					}
				}
			}
		} catch (IOException var8) {
			field_18691.error("Error reading data from disk", var8);
		}
	}
