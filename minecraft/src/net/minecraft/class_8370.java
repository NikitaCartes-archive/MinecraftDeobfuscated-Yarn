...
public record class_8370(UUID id, class_8376 vote, class_8384 results) {
	...
	private List<class_8384.class_8385> method_50484() {
		...
		List<class_8384.class_8385> list = (List)this.results
			.options()
			.stream()
	...
	private static void method_50476(Consumer<Text> consumer, Map<UUID, class_8388> map) {
		map.entrySet()
			...
			.sorted(field_43982)
			.forEach(
				entry -> consumer.accept(Text.translatable("vote.voter", ((class_8388)entry.getValue()).displayName(), ((class_8388)entry.getValue()).voteCount()))
			);
	}
	...
	private List<class_8384.class_8385> method_50482(Consumer<Text> consumer, class_8370.class_8372 arg, List<class_8384.class_8385> list) {
		...
		if (list2.isEmpty() && this.method_50465(arg)) {
			...
			ObjectArrayList<class_8384.class_8385> objectArrayList = (ObjectArrayList)list.stream()
				.sorted(field_43983)
	...
	private List<class_8384.class_8385> method_50474(Consumer<Text> consumer, List<class_8384.class_8385> list, class_8370.class_8372 arg) {
		Map<Integer, List<class_8384.class_8385>> map = (Map)list.stream()
		...
		for(Entry<Integer, List<class_8384.class_8385>> entry : map.entrySet()
			.stream()
			.filter(entryx -> entryx.getKey() >= i)
			.sorted(Entry.comparingByKey(comparator))
			.toList()) {
			List<class_8384.class_8385> list4 = (List)entry.getValue();
			if (!arg.onlyApplyOptionsWithVotes || entry.getKey() != 0) {
				if (list4.size() > 1) {
	...
	private void method_50478(Consumer<class_8376.class_8377> consumer, Consumer<Text> consumer2, List<class_8384.class_8385> list) {
		record class_8371(class_8373 id, class_8376.class_8378 option) {
			final class_8373 id;
			final class_8376.class_8378 option;
		}
	...
	public static record class_8372(
		...
		final Random random;
		final boolean requireAtLeastOnePlayer;
		final int playerCount;
		final float quorum;
		final float absoluteMajorityVotes;
		final boolean showTotals;
		final boolean showVoters;
		final boolean pickRandomOnFail;
		final boolean selectTopVotes;
		final boolean onlyApplyOptionsWithVotes;
		final int maxSelectedOptions;
		final class_8383 tieStrategy;
