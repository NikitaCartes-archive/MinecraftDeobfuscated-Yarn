...
public class EntityUuidFix extends AbstractUuidFix {
	...
	private Dynamic<?> updateFox(Dynamic<?> root) {
		Optional<Dynamic<?>> optional = root.get("TrustedUUIDs")
			.map(dynamic2 -> root.createList(dynamic2.asStream().map(dynamic -> (Dynamic)createArrayFromCompoundUuid(dynamic).orElseGet(() -> {
					...
					dynamic2.asStream()
						.map(
							dynamic -> dynamic.update(
									"Modifiers",
									dynamic2x -> dynamic.createList(dynamic2x.asStream().map(dynamicxx -> (Dynamic)updateRegularMostLeast(dynamicxx, "UUID", "UUID").orElse(dynamicxx)))
								)
						)
				)
		);
