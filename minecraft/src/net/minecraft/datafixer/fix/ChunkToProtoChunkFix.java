				...
				typed -> typed.updateTyped(
						...
						typedx -> {
							...
							if (bl) {
								...
								List<Dynamic<?>> list = (List)IntStream.range(0, 16).mapToObj(ix -> dynamic2.createList(Stream.empty())).collect(Collectors.toList());
								if (optional.isPresent()) {
									((Stream)optional.get()).forEach(dynamic2x -> {
										int ixx = dynamic2x.get("x").asInt(0);
										int j = dynamic2x.get("y").asInt(0);
										int k = dynamic2x.get("z").asInt(0);
										short s = method_15675(ixx, j, k);
