					...
					dynamicOps -> pair -> {
							...
							while(true) {
								Either<List<NewEntityTree>, Unit> either = DataFixUtils.orElse(
									optional.map(
										pairx -> {
											Typed<NewEntityTree> typed = (Typed)type2.pointTyped(dynamicOps).orElseThrow(() -> new IllegalStateException("Could not create new entity tree"));
											...
											return Either.left(ImmutableList.<NewEntityTree>of(object));
										}
									),
								...
								pair2 = (Pair)new Typed<>(type, dynamicOps, (OldEntityTree)optional2.get())
