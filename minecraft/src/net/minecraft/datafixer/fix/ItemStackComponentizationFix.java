...
public class ItemStackComponentizationFix extends DataFix {
	...
	private static void fixStack(ItemStackComponentizationFix.StackData data, Dynamic<?> dynamic) {
		...
		data.moveToComponent("BlockStateTag", "minecraft:block_state");
		...
		data.applyFixer("BlockEntityTag", false, blockEntityTagDynamic -> {
			...
			Dynamic<?> dynamicxx = blockEntityTagDynamic.remove("id");
			return dynamicxx.equals(blockEntityTagDynamic.emptyMap()) ? dynamicxx : blockEntityTagDynamic;
		});
		data.moveToComponent("BlockEntityTag", "minecraft:block_entity_data");
		if (FixUtil.getBoolean(data.getAndRemove("Unbreakable"), false)) {
		...
		if (data.itemEquals("minecraft:filled_map")) {
			...
			Map<? extends Dynamic<?>, ? extends Dynamic<?>> map = (Map)data.getAndRemove("Decorations")
				.asStream()
		...
		return switch(blockEntityId) {
			...
			case "minecraft:shulker_box" -> {
				List<Dynamic<T>> list = dynamic.get("Items")
					.asList(
						itemsDynamic -> itemsDynamic.emptyMap().set("slot", itemsDynamic.createInt(itemsDynamic.get("Slot").asInt(0))).set("item", itemsDynamic.remove("Slot"))
					);
	...
	private static Dynamic<?> fixAttributeModifier(Dynamic<?> dynamic) {
		Dynamic<?> dynamic2 = dynamic.emptyMap();
		dynamic2 = FixUtil.copyValue(dynamic, "AttributeName", dynamic2, "type");
		dynamic2 = FixUtil.copyValue(dynamic, "Slot", dynamic2, "slot");
		dynamic2 = FixUtil.copyValue(dynamic, "UUID", dynamic2, "uuid");
		dynamic2 = FixUtil.copyValue(dynamic, "Name", dynamic2, "name");
		dynamic2 = FixUtil.copyValue(dynamic, "Amount", dynamic2, "amount");
		return FixUtil.copyAndTransformValue(dynamic, "Operation", dynamic2, "operation", operationDynamic -> {
	...
	private static void fixLodestoneTarget(ItemStackComponentizationFix.StackData data, Dynamic<?> dynamic) {
		...
		boolean bl = FixUtil.getBoolean(data.getAndRemove("LodestoneTracked"), true);
		if (optional.isPresent() && optional2.isPresent()) {
			...
			data.setComponent("minecraft:lodestone_target", dynamic2);
		}
	}
	...
	private static Dynamic<?> fixExplosion(Dynamic<?> dynamic) {
		dynamic = FixUtil.replaceKey(dynamic, "Type", "shape", (UnaryOperator<Dynamic<?>>)(typeDynamic -> {
			return typeDynamic.createString(switch(typeDynamic.asInt(0)) {
				case 1 -> "large_ball";
				case 2 -> "star";
				case 3 -> "creeper";
				case 4 -> "burst";
		...
		dynamic = FixUtil.renameKey(dynamic, "Colors", "colors");
		dynamic = FixUtil.renameKey(dynamic, "FadeColors", "fade_colors");
		dynamic = FixUtil.renameKey(dynamic, "Trail", "has_trail");
		return FixUtil.renameKey(dynamic, "Flicker", "has_twinkle");
	}

	private static Dynamic<?> createProfileDynamic(Dynamic<?> dynamic) {
