		...
		if (!"MobSpawner".equals(tag.get("id").asString(""))) {
			...
			if (optional2.isPresent()) {
				tag = tag.set("SpawnPotentials", tag.createList(((Stream)optional2.get()).map(dynamic -> {
					Optional<String> optionalxx = dynamic.get("Type").asString();
					if (optionalxx.isPresent()) {
						Dynamic<?> dynamic2 = DataFixUtils.orElse(dynamic.get("Properties").get(), dynamic.emptyMap()).set("id", dynamic.createString((String)optionalxx.get()));
