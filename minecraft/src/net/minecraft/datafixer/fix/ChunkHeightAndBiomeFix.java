...
public class ChunkHeightAndBiomeFix extends DataFix {
					...
					level -> {
						...
						return level.update(
							...
							level2 -> {
								...
								return fixLevel(
									level2, bl, mutableBoolean.booleanValue(), "minecraft:noise".equals(string2), (Supplier<ProtoChunkTickListFix.class_6741>)mutableObject.getValue()
								);
							}
						);
					}
	...
	private static Dynamic<?>[] fixBiomes(Dynamic<?> level, boolean overworld, int i, MutableBoolean heightAlreadyUpdated) {
		...
		if (is != null && is.length == 1536) {
			...
			for(int j = 0; j < 24; ++j) {
				dynamics[j] = fixBiomes(level, sectionY -> getClamped(is, j * 64 + sectionY));
			}
	...
	private static Dynamic<?> fixChunkSectionList(Dynamic<?> level, String key) {
		List<Dynamic<?>> list = (List)level.get(key).orElseEmptyList().asStream().collect(Collectors.toCollection(ArrayList::new));
		if (list.size() == 24) {
			return level;
		} else {
