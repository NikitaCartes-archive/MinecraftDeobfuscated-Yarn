...
public class ProtoChunkTickListFix extends DataFix {
						...
						typedx.getOptionalTyped(opticFinder2)
							.ifPresent(
								typedxx -> typedxx.getAllTyped(opticFinder3)
										.forEach(
											typedxxx -> {
												Dynamic<?> dynamicxx = typedxxx.get(DSL.remainderFinder());
												int ixx = dynamicxx.get("Y").asInt(Integer.MAX_VALUE);
												if (ixx != Integer.MAX_VALUE) {
													if (typedxxx.getOptionalTyped(opticFinder5).isPresent()) {
														mutableInt.setValue(Math.min(ixx, mutableInt.getValue()));
													}
													...
													typedxxx.getOptionalTyped(opticFinder4)
														.ifPresent(
															typedxxxx -> int2ObjectMap.put(
																	...
																	Suppliers.memoize(
																		() -> {
																			List<? extends Dynamic<?>> list = (List)typedxxxx.getOptionalTyped(opticFinder6)
																				.map(
	...
	private Dynamic<?> fixToBeTicked(
		...
		for(int k = 0; k < list.size(); ++k) {
			int l = k + b;
			Supplier<ProtoChunkTickListFix.class_6741> supplier = (Supplier)int2ObjectMap.get(l);
