...
public final class SpawnHelper {
							...
							if (!(d <= 576.0) && !(blockPos.squaredDistanceTo((double)f, (double)k, (double)g) < 576.0)) {
								...
								if (Objects.equals(chunkPos, worldChunk.getPos())) {
									...
									if (entityType.isSummonable() && method_8659(chunkGenerator, entityCategory, spawnEntry, mutable)) {
										...
										if (location != null && canSpawn(location, world, mutable, entityType)) {
											Entity entity;
											try {
											...
											entity.setPositionAndAngles((double)f, (double)k, (double)g, world.random.nextFloat() * 360.0F, 0.0F);
					...
					for (int s = 0; !bl && s < 4; s++) {
						...
						if (spawnEntry.type.isSummonable() && canSpawn(SpawnRestriction.Location.field_6317, iWorld, blockPos, spawnEntry.type)) {
							...
							try {
								...
								LOGGER.warn("Failed to create mob", (Throwable)var25);
								continue;
							}

							double d = MathHelper.clamp((double)n, (double)k + (double)entity.width, (double)k + 16.0 - (double)entity.width);
							double e = MathHelper.clamp((double)o, (double)l + (double)entity.width, (double)l + 16.0 - (double)entity.width);
	...
	private static BlockPos method_8658(ViewableWorld viewableWorld, @Nullable EntityType<?> entityType, int i, int j) {
		...
		return viewableWorld.getBlockState(blockPos2).canPlaceAtSide(viewableWorld, blockPos2, PlacementEnvironment.field_50) ? blockPos2 : blockPos;
	}
}
