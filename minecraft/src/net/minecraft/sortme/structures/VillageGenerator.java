	...
	public static class class_3452 extends VillageGenerator.class_3465 {
		...
		public boolean method_14931(IWorld iWorld, Random random, MutableIntBoundingBox mutableIntBoundingBox, ChunkPos chunkPos) {
			...
			for(int i = 1; i <= 7; ++i) {
				...
				int q = p / 3;
				this.addBlock(
					iWorld, this.field_15331.with(cropBlock.getAgeProperty(), Integer.valueOf(MathHelper.nextInt(random, q, p))), 10, 1, i, mutableIntBoundingBox
				);
				this.addBlock(
					iWorld, this.field_15331.with(cropBlock.getAgeProperty(), Integer.valueOf(MathHelper.nextInt(random, q, p))), 11, 1, i, mutableIntBoundingBox
				);
			}
	...
	public static class class_3463 extends VillageGenerator.class_3465 {
		...
		public boolean method_14931(IWorld iWorld, Random random, MutableIntBoundingBox mutableIntBoundingBox, ChunkPos chunkPos) {
			...
			for(int i = 4; i >= 1; --i) {
				...
				for(int j = 8 - i; j <= 10; ++j) {
					this.addBlock(iWorld, blockState11, i, 2 + i, j, mutableIntBoundingBox);
				}
			}
			...
			for(int i = 6; i <= 8; ++i) {
				for(int j = 5; j <= 10; ++j) {
					this.addBlock(iWorld, blockState10, i, 12 - i, j, mutableIntBoundingBox);
				}
			}
			...
			for(int i = 0; i < 5; ++i) {
				for(int j = 0; j < 9; ++j) {
					this.method_14920(iWorld, j, 7, i, mutableIntBoundingBox);
					this.method_14936(iWorld, blockState, j, -1, i, mutableIntBoundingBox);
				}
			}
