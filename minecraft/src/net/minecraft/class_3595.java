...
public class class_3595 extends DataFix {
	...
	public class_3595(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	public TypeRewriteRule makeRule() {
		...
		Type<Pair<Either<Pair<String, Either<Integer, String>>, Unit>, Dynamic<?>>> type = DSL.and(
			DSL.optional(DSL.field("inTile", DSL.named(class_1208.field_5731.typeName(), DSL.or(DSL.intType(), DSL.namespacedString())))), DSL.remainderType()
		);
		Function<Typed<?>, Typed<?>> function3 = typed -> typed.update(type.finder(), DSL.remainderType(), Pair::getSecond);
		return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(class_1208.field_5729), schema2.getType(class_1208.field_5729), typed -> {
	...
	private Typed<?> method_15695(Typed<?> typed) {
		Type<Either<Pair<String, Either<Integer, String>>, Unit>> type = DSL.optional(
			DSL.field("Block", DSL.named(class_1208.field_5731.typeName(), DSL.or(DSL.intType(), DSL.namespacedString())))
		);
		Type<Either<Pair<String, Dynamic<?>>, Unit>> type2 = DSL.optional(DSL.field("BlockState", DSL.named(class_1208.field_5720.typeName(), DSL.remainderType())));
		Dynamic<?> dynamic = typed.get(DSL.remainderFinder());
		return typed.update(type.finder(), type2, either -> {
			int i = either.map(pair -> ((Either)pair.getSecond()).map(integer -> integer, class_3595::method_15686), unit -> {
			...
			return Either.left(Pair.of(class_1208.field_5720.typeName(), class_3580.method_15594(i << 4 | j)));
	...
	private Typed<?> method_15687(Typed<?> typed, String string, String string2, String string3) {
		Type<Pair<String, Either<Integer, String>>> type = DSL.field(
			string, DSL.named(class_1208.field_5731.typeName(), DSL.or(DSL.intType(), DSL.namespacedString()))
		);
		Type<Pair<String, Dynamic<?>>> type2 = DSL.field(string3, DSL.named(class_1208.field_5720.typeName(), DSL.remainderType()));
		Dynamic<?> dynamic = typed.getOrCreate(DSL.remainderFinder());
		return typed.update(type.finder(), type2, pair -> {
			int i = ((Either)pair.getSecond()).map(integer -> integer, class_3595::method_15686);
			int j = dynamic.get(string2).asInt(0) & 15;
			return Pair.of(class_1208.field_5720.typeName(), class_3580.method_15594(i << 4 | j));
	...
	private Typed<?> method_15682(Typed<?> typed, String string, Function<Typed<?>, Typed<?>> function) {
		Type<?> type = this.getInputSchema().getChoiceType(class_1208.field_5729, string);
		Type<?> type2 = this.getOutputSchema().getChoiceType(class_1208.field_5729, string);
