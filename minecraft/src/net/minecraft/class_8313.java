...
public class class_8313 extends class_8275<Integer, Integer> {
	...
	protected void method_50136(Integer integer) {
		this.field_43767.remove(integer);
	}
		...
		return Stream.generate(() -> {
			if ((double)random.nextFloat() < 0.7) {
				...
				if (random.nextBoolean()) {
					ixxx = random.nextBetween(65, 90);
					j = Character.toLowerCase(ixxx);
				} else {
					ixxx = random.nextBetween(97, 122);
					j = Character.toUpperCase(ixxx);
				}

				return this.field_43767.containsKey(ixxx) ? Optional.empty() : Optional.of(new class_8275.class_8276(ixxx, j));
			} else {
				int ixxx = random.nextBetween(32, 126);
				if (this.field_43767.containsKey(ixxx)) {
					return Optional.empty();
				} else {
					int j = random.nextBetween(32, 126);
					return ixxx == j ? Optional.empty() : Optional.of(new class_8275.class_8276(ixxx, j));
				}
			}
