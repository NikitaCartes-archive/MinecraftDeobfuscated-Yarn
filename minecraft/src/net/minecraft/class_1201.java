...
public class class_1201 extends DataFix {
	private static final Int2ObjectMap<String> field_5706 = DataFixUtils.make(new Int2ObjectOpenHashMap(), int2ObjectOpenHashMap -> {
	...
	public class_1201(Schema schema, boolean bl) {
		super(schema, bl);
	}
							...
							.map(map -> dynamic.createMap((Map<? extends Dynamic<?>, ? extends Dynamic<?>>)map.entrySet().stream().map(entry -> {
									if (((Dynamic)entry.getKey()).asString("").startsWith("key_")) {
										...
										if (i < 0) {
											...
											String string2 = (String)field_5706.getOrDefault(i, "key.unknown");
