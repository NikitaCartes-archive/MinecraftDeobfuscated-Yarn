	...
	static {
		...
		if (inputStream != null) {
			try {
				...
				LOGGER.warn("Failed to read rgb.txt", var7);
			}
		} else {
		...
		FROM_NAME = (Map)NAMED_COLORS.stream().collect(ImmutableMap.toImmutableMap(NamedColors.NamedColor::name, namedColor -> namedColor));
