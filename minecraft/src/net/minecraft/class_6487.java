...
public class class_6487 {
	...
	public void method_37882() {
		...
		if (this.field_34365 != null) {
			try {
				...
				field_34360.error("Failed to close remote control server socket", var2);
			}
	...
	public void method_37884() {
		try {
			...
			if (this.field_34364) {
				field_34360.error("Remote control server closed because of an IO exception", var13);
			}
		} finally {
			if (this.field_34365 != null && !this.field_34365.isClosed()) {
				try {
					...
					field_34360.warn("Failed to close remote control server socket", var11);
				}
			}
		}
	...
	private void method_37881(Socket socket) {
		try {
			...
			field_34360.info("Remote control client broadcast socket got an IO exception and will be closed", var14);
		} finally {
			try {
				if (!socket.isClosed()) {
					socket.close();
				...
				field_34360.warn("Failed to close remote control client socket", var12);
			}
		}
