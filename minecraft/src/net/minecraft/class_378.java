...
public class class_378 implements ResourceReloadListener {
	...
	public void onResourceReload(ResourceManager resourceManager) {
		...
		Map<Identifier, List<class_390>> map = Maps.newHashMap();
		...
		for(Identifier identifier : resourceManager.findResources("font", stringx -> stringx.endsWith(".json"))) {
			...
			List<class_390> list = (List)map.computeIfAbsent(identifier2, identifierx -> Lists.<class_390>newArrayList(new class_376()));
		...
		Stream.concat(this.field_2259.keySet().stream(), map.keySet().stream())
			...
			.forEach(
				identifierx -> {
					List<class_390> listxx = (List)map.getOrDefault(identifierx, Collections.emptyList());
