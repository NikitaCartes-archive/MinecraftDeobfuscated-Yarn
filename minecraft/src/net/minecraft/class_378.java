...
public class class_378 implements AutoCloseable {
	...
	private final class_3302 field_18215 = new class_4080<Map<class_2960, List<class_390>>>() {
		protected Map<class_2960, List<class_390>> method_18638(class_3300 arg, class_3695 arg2) {
			...
			Map<class_2960, List<class_390>> map = Maps.newHashMap();
			...
			for(class_2960 lv : arg.method_14488("font", stringx -> stringx.endsWith(".json"))) {
				...
				List<class_390> list = (List)map.computeIfAbsent(lv2, argx -> Lists.<class_390>newArrayList(new class_376()));
				...
				try {
					for(class_3298 lv3 : arg.method_14489(lv)) {
						...
						try {
							...
							try {
								...
								for(int i = jsonArray.size() - 1; i >= 0; --i) {
									...
									try {
										...
										class_378.field_2261.warn("Unable to read definition '{}' in fonts.json in resourcepack: '{}': {}", lv2, lv3.method_14480(), var29.getMessage());
									}
								}
							...
							class_378.field_2261.warn("Unable to load font '{}' in fonts.json in resourcepack: '{}': {}", lv2, lv3.method_14480(), var32.getMessage());
						}

						arg2.method_15407();
					}
				} catch (IOException var33) {
					class_378.field_2261.warn("Unable to load font '{}' in fonts.json: {}", lv2, var33.getMessage());
				}
		...
		protected void method_18635(Map<class_2960, List<class_390>> map, class_3300 arg, class_3695 arg2) {
			...
			Stream.concat(class_378.this.field_2259.keySet().stream(), map.keySet().stream())
				...
				.forEach(
					argx -> {
						List<class_390> list = (List)map.getOrDefault(argx, Collections.emptyList());
	...
	public void method_2018(boolean bl, Executor executor, Executor executor2) {
		if (bl != this.field_2258) {
			...
			this.field_18215.reload(lv2, lv, class_3694.field_16280, class_3694.field_16280, executor, executor2);
		}
	}
