...
public class class_378 implements class_3302 {
	...
	public void method_14491(class_3300 arg) {
		...
		Stream.concat(this.field_2259.keySet().stream(), map.keySet().stream()).distinct().forEach(argx -> {
			List<class_390> listxx = (List)map.getOrDefault(argx, Collections.emptyList());
			Collections.reverse(listxx);
			((class_327)this.field_2259.computeIfAbsent(argx, argxx -> new class_327(this.field_2260, new class_377(this.field_2260, argxx)))).method_1715(listxx);
		});
	}
