...
public class ZombieSiegeManager {
	...
	private Vec3d getSpawnVector(ServerWorld world, BlockPos pos) {
		for (int i = 0; i < 10; i++) {
			...
			int l = world.getTop(Heightmap.Type.WORLD_SURFACE, j, k);
			BlockPos blockPos = new BlockPos(j, l, k);
			if (world.isNearOccupiedPointOfInterest(blockPos) && HostileEntity.method_20680(EntityType.ZOMBIE, world, SpawnType.EVENT, blockPos, world.random)) {
