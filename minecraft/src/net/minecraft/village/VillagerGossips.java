...
public class VillagerGossips {
	...
	private Collection<VillagerGossips.GossipEntry> pickGossips(Random random, int i) {
		List<VillagerGossips.GossipEntry> list = (List)this.entries().collect(Collectors.toList());
		if (list.isEmpty()) {
	...
	public void shareGossipFrom(VillagerGossips villagerGossips, Random random, int i) {
		...
		collection.forEach(gossipEntry -> {
			int ixx = gossipEntry.value - gossipEntry.type.value;
			if (ixx >= 2) {
				this.getReputationFor(gossipEntry.target).associatedGossip.mergeInt(gossipEntry.type, ixx, VillagerGossips::max);
			}
		});
	}
	...
	static class Reputation {
		...
		public void method_20652() {
			...
			while(objectIterator.hasNext()) {
				Entry<VillageGossipType> entry = (Entry)objectIterator.next();
				int i = entry.getIntValue() - ((VillageGossipType)entry.getKey()).field_19354;
				if (i < 2) {
