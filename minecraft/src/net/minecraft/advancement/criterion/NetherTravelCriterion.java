	...
	public void trigger(ServerPlayerEntity serverPlayerEntity, Vec3d vec3d) {
		this.test(
			...
			conditions -> conditions.matches(
					serverPlayerEntity.getServerWorld(), vec3d, serverPlayerEntity.getX(), serverPlayerEntity.getY(), serverPlayerEntity.getZ()
				)
		);
	}
	...
	public static class Conditions extends AbstractCriterionConditions {
		...
		public boolean matches(ServerWorld serverWorld, Vec3d vec3d, double d, double e, double f) {
			if (!this.enteredPos.test(serverWorld, vec3d.x, vec3d.y, vec3d.z)) {
				return false;
			} else if (!this.exitedPos.test(serverWorld, d, e, f)) {
				return false;
			} else {
