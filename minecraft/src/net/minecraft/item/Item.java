...
public class Item implements ItemProvider {
	...
	@Nullable
	protected HitResult getHitResult(World world, PlayerEntity playerEntity, boolean bl) {
		float f = playerEntity.pitch;
		float g = playerEntity.yaw;
		double d = playerEntity.x;
		double e = playerEntity.y + (double)playerEntity.getEyeHeight();
		double h = playerEntity.z;
		...
		return world.rayTrace(vec3d, vec3d2, bl ? FluidRayTraceMode.field_1345 : FluidRayTraceMode.NONE, false, false);
	}
