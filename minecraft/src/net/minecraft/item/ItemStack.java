...
public final class ItemStack {
	...
	private BlockProxy lastCheckedCanHarvestBlock;
	private boolean lastCheckedCanHarvestResult;
	private BlockProxy lastCheckedCanPlaceBlock;
	...
	public ActionResult useOnBlock(ItemUsageContext itemUsageContext) {
		...
		BlockProxy blockProxy = new BlockProxy(itemUsageContext.getWorld(), blockPos, false);
		if (playerEntity != null && !playerEntity.abilities.allowModifyWorld && !this.getCustomCanPlace(itemUsageContext.getWorld().getTagManager(), blockProxy)) {
	...
	public List<TextComponent> getTooltipText(@Nullable PlayerEntity playerEntity, TooltipOptions tooltipOptions) {
		...
		if (this.hasTag() && this.tag.containsKey("CanDestroy", 9) && (i & 8) == 0) {
			...
			if (!listTag.isEmpty()) {
				...
				for(int j = 0; j < listTag.size(); ++j) {
					list.addAll(method_7937(listTag.getString(j)));
				}
			}
		}
	...
	private static Collection<TextComponent> method_7937(String string) {
		try {
			...
			BlockState blockState = lv.method_9669();
	...
	private static boolean areBlockProxiesEqual(BlockProxy blockProxy, @Nullable BlockProxy blockProxy2) {
		if (blockProxy2 == null || blockProxy.getBlockState() != blockProxy2.getBlockState()) {
			return false;
		} else if (blockProxy.getBlockEntity() == null && blockProxy2.getBlockEntity() == null) {
			return true;
		} else {
			return blockProxy.getBlockEntity() != null && blockProxy2.getBlockEntity() != null
				? Objects.equals(blockProxy.getBlockEntity().toTag(new CompoundTag()), blockProxy2.getBlockEntity().toTag(new CompoundTag()))
				: false;
		}
	}

	public boolean getCustomCanHarvest(TagManager tagManager, BlockProxy blockProxy) {
		if (areBlockProxiesEqual(blockProxy, this.lastCheckedCanHarvestBlock)) {
			return this.lastCheckedCanHarvestResult;
		} else {
			this.lastCheckedCanHarvestBlock = blockProxy;
			if (this.hasTag() && this.tag.containsKey("CanDestroy", 9)) {
				...
				for(int i = 0; i < listTag.size(); ++i) {
					...
					try {
						Predicate<BlockProxy> predicate = BlockPredicateArgumentType.create().method_9642(new StringReader(string)).create(tagManager);
	...
	public boolean getCustomCanPlace(TagManager tagManager, BlockProxy blockProxy) {
		if (areBlockProxiesEqual(blockProxy, this.lastCheckedCanPlaceBlock)) {
			return this.lastCheckedCanPlaceResult;
		} else {
			this.lastCheckedCanPlaceBlock = blockProxy;
