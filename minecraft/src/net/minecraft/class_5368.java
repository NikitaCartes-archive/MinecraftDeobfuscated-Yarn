			...
			new Function<Runnable, class_5369<?>>() {
				...
				public class_5369<?> apply(Runnable runnable) {
					...
					return new class_5369(
						...
						list3,
						(listx, list2x, bl) -> {
							List<String> list3xx = (List)Lists.reverse(listx).stream().map(ResourcePackProfile::getName).collect(ImmutableList.toImmutableList());
							List<String> list4 = (List)list2x.stream().map(ResourcePackProfile::getName).collect(ImmutableList.toImmutableList());
							this.field_25450 = new class_5359(list3xx, list4);
							if (!bl) {
								this.field_25451.setEnabledProfiles(list3xx);
