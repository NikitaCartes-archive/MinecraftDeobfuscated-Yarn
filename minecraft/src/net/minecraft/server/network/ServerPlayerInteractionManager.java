...
public class ServerPlayerInteractionManager {
	...
	public void update() {
		...
		if (this.field_14001) {
			...
			if (blockState2.isAir()) {
				...
				float f = blockState2.calcBlockBreakingDelta(this.player, this.player.world, this.field_14004) * (float)(k + 1);
				int j = (int)(f * 10.0F);
				if (j != this.field_14009) {
		...
		if (this.isCreative()) {
			...
			if (this.gameMode.shouldLimitWorldModification()) {
				...
				if (!this.player.canModifyWorld()) {
					...
					BlockProxy blockProxy = new BlockProxy(this.world, blockPos, false);
					if (!itemStack.getCustomCanHarvest(this.world.getTagManager(), blockProxy)) {
						return;
					}
				}
			}
	...
	public ActionResult interactBlock(
		...
		if (this.gameMode == GameMode.field_9219) {
			...
			if (blockEntity instanceof LockableContainer) {
				Block block = blockState.getBlock();
				LockableContainer lockableContainer = (LockableContainer)blockEntity;
				if (lockableContainer instanceof ChestBlockEntity && block instanceof ChestBlock) {
					lockableContainer = ((ChestBlock)block).getContainer(blockState, world, blockPos, false);
				}
