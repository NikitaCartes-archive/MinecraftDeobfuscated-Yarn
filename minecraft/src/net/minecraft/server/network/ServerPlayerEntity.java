...
public class ServerPlayerEntity extends PlayerEntity {
			...
			this.networkHandler
				.sendPacket(
					new GameMessageS2CPacket(
						Text.translatable("multiplayer.message_not_delivered", text).formatted(Formatting.RED), this.getMessageTypeId(MessageType.SYSTEM)
					)
				);
		}
	}
	...
	private int getMessageTypeId(RegistryKey<MessageType> typeKey) {
		...
		return registry.getRawId((MessageType)registry.get(typeKey));
	}
