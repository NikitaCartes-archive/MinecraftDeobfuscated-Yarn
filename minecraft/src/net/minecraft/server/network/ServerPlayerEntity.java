...
public class ServerPlayerEntity extends PlayerEntity {
	...
	public void readCustomDataFromNbt(NbtCompound nbt) {
		...
		if (nbt.contains("SpawnX", NbtElement.NUMBER_TYPE) && nbt.contains("SpawnY", NbtElement.NUMBER_TYPE) && nbt.contains("SpawnZ", NbtElement.NUMBER_TYPE)) {
			...
			if (nbt.contains("SpawnDimension")) {
				this.spawnPointDimension = (RegistryKey)World.CODEC
	...
	public void tick() {
		...
		if (this.getBlockPos().getY() < -32 && !this.world.getDimension().isBedWorking()) {
			Entity minecraftServer = this.getVehicle();
			if (minecraftServer instanceof EnderDragonEntity enderDragonEntity) {
				MinecraftServer minecraftServerx = this.world.getServer();
				if (minecraftServerx != null) {
					World world = minecraftServerx.getOverworld();
