...
public class DebugInfoSender {
	...
	private static void writeBrain(LivingEntity entity, PacketByteBuf buf) {
		...
		Set<String> set = (Set)brain.getRunningTasks().stream().map(Task::getName).collect(Collectors.toSet());
		...
		if (entity instanceof VillagerEntity) {
			Set<BlockPos> set2 = (Set)Stream.of(MemoryModuleType.JOB_SITE, MemoryModuleType.HOME, MemoryModuleType.MEETING_POINT)
		...
		if (entity instanceof VillagerEntity) {
			Set<BlockPos> set2 = (Set)Stream.of(MemoryModuleType.POTENTIAL_JOB_SITE)
				.map(brain::getOptionalRegisteredMemory)
				.flatMap(Optional::stream)
				.map(GlobalPos::getPos)
				.collect(Collectors.toSet());
