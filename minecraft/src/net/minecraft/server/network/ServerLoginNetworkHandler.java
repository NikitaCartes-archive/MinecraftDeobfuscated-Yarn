	...
	public void onKey(LoginKeyC2SPacket packet) {
		...
		Thread thread = new Thread("User Authenticator #" + NEXT_AUTHENTICATOR_THREAD_ID.incrementAndGet()) {
			public void run() {
				...
				try {
					GameProfile gameProfile = ServerLoginNetworkHandler.this.server.getSessionService().hasJoinedServer(stringx, string, this.getClientAddress());
					if (gameProfile != null) {
						...
						ServerLoginNetworkHandler.this.startVerify(ServerLoginNetworkHandler.createOfflineProfile(stringx));
					} else {
