...
public class ServerPlayNetworkHandler implements EntityTrackingListener, TickablePacketListener, ServerPlayPacketListener {
	...
	public void onRenameItem(RenameItemC2SPacket packet) {
		NetworkThreadUtils.forceMainThread(packet, this, this.player.getWorld());
		ScreenHandler string = this.player.currentScreenHandler;
		if (string instanceof AnvilScreenHandler anvilScreenHandler) {
			if (!anvilScreenHandler.canUse(this.player)) {
				LOGGER.debug("Player {} interacted with invalid menu {}", this.player, anvilScreenHandler);
				return;
			}

			String stringx = SharedConstants.stripInvalidChars(packet.getName());
			if (stringx.length() <= 50) {
				anvilScreenHandler.setNewItemName(stringx);
			}
		}
	}
	...
	private void addBook(FilteredMessage<String> title, List<FilteredMessage<String>> pages, int slotId) {
		...
		if (itemStack.isOf(Items.WRITABLE_BOOK)) {
			...
			if (this.player.shouldFilterText()) {
				itemStack2.setSubNbt("title", NbtString.of((String)title.filteredOrElse("")));
			} else {
				itemStack2.setSubNbt("filtered_title", NbtString.of((String)title.filteredOrElse("")));
				itemStack2.setSubNbt("title", NbtString.of((String)title.raw()));
			}
	...
	private void setTextToBook(List<FilteredMessage<String>> messages, UnaryOperator<String> postProcessor, ItemStack book) {
		...
		if (this.player.shouldFilterText()) {
			messages.stream().map(message -> NbtString.of((String)postProcessor.apply((String)message.filteredOrElse("")))).forEach(nbtList::add);
		} else {
			NbtCompound nbtCompound = new NbtCompound();
			int i = 0;

			for(int j = messages.size(); i < j; ++i) {
				FilteredMessage<String> filteredMessage = (FilteredMessage)messages.get(i);
				String string = (String)filteredMessage.raw();
				nbtList.add(NbtString.of((String)postProcessor.apply(string)));
				if (filteredMessage.isFiltered()) {
					nbtCompound.putString(String.valueOf(i), (String)postProcessor.apply((String)filteredMessage.filteredOrElse("")));
				}
			}
	...
	private CompletableFuture<Void> handleMessage(SignedMessage signedMessage, FilteredMessage<String> filteredMessage) {
		...
		return decoratedContents.isDecorated()
			...
			: messageDecorator.decorate(this.player, Text.literal((String)filteredMessage.raw()))
	...
	private CompletableFuture<Text> decorateCommand(ServerCommandSource serverCommandSource, class_7644<ServerCommandSource> arg) {
		...
		if (list.isEmpty()) {
			...
			for(int i = list.size() - 1; i >= 0; --i) {
				class_7644.class_7645<ServerCommandSource> lv = (class_7644.class_7645)list.get(i);
	...
	public void onClientCommand(ClientCommandC2SPacket packet) {
		...
		switch(packet.getMode()) {
			...
			case OPEN_INVENTORY:
				...
				if (i instanceof RideableInventory rideableInventory) {
