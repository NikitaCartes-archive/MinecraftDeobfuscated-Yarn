...
public abstract class PlayerManager {
	...
	private final Map<UUID, class_3442> statisticsMap = Maps.newHashMap();
	private final Map<UUID, net.minecraft.advancement.ServerAdvancementManager> advancementManagerMap = Maps.newHashMap();
	...
	public void method_14598(ServerPlayerEntity serverPlayerEntity, DimensionType dimensionType) {
		...
		serverPlayerEntity.networkHandler
			.method_14363(serverPlayerEntity.x, serverPlayerEntity.y, serverPlayerEntity.z, serverPlayerEntity.yaw, serverPlayerEntity.pitch);
	...
	public List<ServerPlayerEntity> getPlayersByIp(String string) {
		...
		for(ServerPlayerEntity serverPlayerEntity : this.players) {
			if (serverPlayerEntity.method_14209().equals(string)) {
				list.add(serverPlayerEntity);
			}
		}
