...
public abstract class PlayerManager {
	...
	public void remove(ServerPlayerEntity player) {
		...
		if (player.hasVehicle()) {
			...
			if (entity.hasPlayerRider()) {
				...
				player.stopRiding();
				entity.streamPassengersAndSelf().forEach(entity -> entity.setRemoved(Entity.RemovalReason.UNLOADED_WITH_PLAYER));
			}
		}
	...
	public void broadcast(FilteredMessage<SignedChatMessage> message, ServerCommandSource source, RegistryKey<MessageType> typeKey) {
		...
		if (serverPlayerEntity != null) {
			...
			this.broadcast((SignedChatMessage)message.raw(), source.getChatMessageSender(), typeKey);
		}
	}
	...
	public void broadcast(FilteredMessage<SignedChatMessage> message, ServerPlayerEntity sender, RegistryKey<MessageType> typeKey) {
		this.broadcast((SignedChatMessage)message.raw(), player -> (SignedChatMessage)message.getFilterableFor(sender, player), sender.asMessageSender(), typeKey);
	}
