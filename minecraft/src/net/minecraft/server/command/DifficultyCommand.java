...
public class DifficultyCommand {
	...
	public static void register(CommandDispatcher<ServerCommandSource> commandDispatcher) {
		...
		commandDispatcher.register(literalArgumentBuilder.requires(serverCommandSource -> serverCommandSource.hasPermissionLevel(2)).executes(commandContext -> {
			Difficulty difficultyxx = commandContext.getSource().method_9225().getDifficulty();
			commandContext.getSource().method_9226(new TranslatableTextComponent("commands.difficulty.query", difficultyxx.method_5463()), false);
