...
public class LootCommand {
			...
			method_13206(
				...
				(argumentBuilder, arg) -> argumentBuilder.then(
							ServerCommandManager.literal("fish")
								.then(
									ServerCommandManager.argument("loot_table", ResourceLocationArgumentType.create())
											...
											ServerCommandManager.argument("pos", BlockPosArgumentType.create())
												.executes(
													commandContext -> method_13199(
															commandContext,
															ResourceLocationArgumentType.method_9443(commandContext, "loot_table"),
						...
						.then(
							ServerCommandManager.literal("loot")
								.then(
									ServerCommandManager.argument("loot_table", ResourceLocationArgumentType.create())
										.suggests(field_13605)
										.executes(commandContext -> method_13197(commandContext, ResourceLocationArgumentType.method_9443(commandContext, "loot_table"), arg))
								)
						)
						.then(
						...
						.then(
							ServerCommandManager.literal("mine")
								.then(
									ServerCommandManager.argument("pos", BlockPosArgumentType.create())
										.executes(commandContext -> method_13219(commandContext, BlockPosArgumentType.method_9696(commandContext, "pos"), ItemStack.EMPTY, arg))
										.then(
											ServerCommandManager.argument("tool", ItemStackArgumentType.create())
												.executes(
										...
										.then(
											ServerCommandManager.literal("mainhand")
												.executes(
													commandContext -> method_13219(
															commandContext, BlockPosArgumentType.method_9696(commandContext, "pos"), method_13178(commandContext.getSource(), EquipmentSlot.HAND_MAIN), arg
														)
												)
										)
										.then(
											ServerCommandManager.literal("offhand")
												.executes(
													commandContext -> method_13219(
															commandContext, BlockPosArgumentType.method_9696(commandContext, "pos"), method_13178(commandContext.getSource(), EquipmentSlot.HAND_OFF), arg
														)
												)
										)
								)
									...
									.then(
										arg.construct(
												...
												(commandContext, list, argx) -> method_13209(
														...
														BlockPosArgumentType.method_9696(commandContext, "targetPos"),
														ItemSlotArgumentType.getSlotArgument(commandContext, "slot"),
														list.size(),
														list,
														argx
			...
			.then(
				ServerCommandManager.literal("insert")
					.then(
						arg.construct(
							ServerCommandManager.argument("targetPos", BlockPosArgumentType.create()),
							(commandContext, list, argx) -> method_13196(commandContext.getSource(), BlockPosArgumentType.method_9696(commandContext, "targetPos"), list, argx)
						)
					)
			)
			.then(
	...
	private static boolean method_13223(Inventory inventory, ItemStack itemStack) {
		...
		for(int i = 0; i < inventory.getInvSize() && !itemStack.isEmpty(); ++i) {
			ItemStack itemStack2 = inventory.method_5438(i);
			if (inventory.method_5437(i, itemStack)) {
	...
	private static int method_13209(ServerCommandSource serverCommandSource, BlockPos blockPos, int i, int j, List<ItemStack> list, LootCommand.class_3040 arg) throws CommandSyntaxException {
		...
		if (i >= 0 && i < k) {
			...
			for(int l = 0; l < j; ++l) {
				...
				if (inventory.method_5437(m, itemStack)) {
					inventory.method_5447(m, itemStack);
					list2.add(itemStack);
				}
			}
	...
	private static boolean method_13218(ItemStack itemStack, ItemStack itemStack2) {
		return itemStack.getItem() == itemStack2.getItem()
			...
			&& Objects.equals(itemStack.method_7969(), itemStack2.method_7969());
	}
	...
	private static int method_13201(Collection<ServerPlayerEntity> collection, List<ItemStack> list, LootCommand.class_3040 arg) throws CommandSyntaxException {
		...
		for(ItemStack itemStack : list) {
			for(ServerPlayerEntity serverPlayerEntity : collection) {
				if (serverPlayerEntity.inventory.method_7394(itemStack.copy())) {
					list2.add(itemStack);
				}
			}
		}
	...
	private static void method_13213(ServerCommandSource serverCommandSource, List<ItemStack> list) {
		if (list.size() == 1) {
			...
			serverCommandSource.method_9226(new TranslatableTextComponent("commands.drop.success.single", itemStack.getAmount(), itemStack.method_7954()), false);
		} else {
			serverCommandSource.method_9226(new TranslatableTextComponent("commands.drop.success.multiple", list.size()), false);
		}
	}
	...
	private static void method_13212(ServerCommandSource serverCommandSource, List<ItemStack> list, Identifier identifier) {
		if (list.size() == 1) {
			...
			serverCommandSource.method_9226(
				new TranslatableTextComponent("commands.drop.success.single_with_table", itemStack.getAmount(), itemStack.method_7954(), identifier), false
			);
		} else {
			serverCommandSource.method_9226(new TranslatableTextComponent("commands.drop.success.multiple_with_table", list.size(), identifier), false);
		}
	}
	...
	private static ItemStack method_13178(ServerCommandSource serverCommandSource, EquipmentSlot equipmentSlot) throws CommandSyntaxException {
		...
		if (entity instanceof LivingEntity) {
			return ((LivingEntity)entity).method_6118(equipmentSlot);
		} else {
			throw NOHELD_ITEMS_EXCEPTION.create(entity.method_5476());
		}
	}
	...
	private static int method_13219(CommandContext<ServerCommandSource> commandContext, BlockPos blockPos, ItemStack itemStack, LootCommand.class_3041 arg) throws CommandSyntaxException {
		...
		List<ItemStack> list = blockState.method_11612(builder);
		return arg.accept(commandContext, list, listx -> method_13212(serverCommandSource, listx, blockState.getBlock().method_9580()));
	}
	...
	private static int method_13189(CommandContext<ServerCommandSource> commandContext, Entity entity, LootCommand.class_3041 arg) throws CommandSyntaxException {
		if (!(entity instanceof LivingEntity)) {
			throw NOLOOTTABLE_EXCEPTION.create(entity.method_5476());
		} else {
			Identifier identifier = ((LivingEntity)entity).method_5989();
			ServerCommandSource serverCommandSource = commandContext.getSource();
			LootContext.Builder builder = new LootContext.Builder(serverCommandSource.method_9225());
			...
			builder.method_312(LootContextParameters.field_1232, new BlockPos(serverCommandSource.method_9222()));
			LootSupplier lootSupplier = serverCommandSource.getMinecraftServer().getLootManager().method_367(identifier);
			List<ItemStack> list = lootSupplier.getDrops(builder.method_309(LootContextTypes.ENTITY));
	...
	private static int method_13197(CommandContext<ServerCommandSource> commandContext, Identifier identifier, LootCommand.class_3041 arg) throws CommandSyntaxException {
		...
		LootContext.Builder builder = new LootContext.Builder(serverCommandSource.method_9225())
			.method_306(LootContextParameters.field_1226, serverCommandSource.getEntity())
			.method_312(LootContextParameters.field_1232, new BlockPos(serverCommandSource.method_9222()));
		return method_13180(commandContext, identifier, builder.method_309(LootContextTypes.CHEST), arg);
	}
	...
	private static int method_13199(
		...
		ServerCommandSource serverCommandSource = commandContext.getSource();
		LootContext lootContext = new LootContext.Builder(serverCommandSource.method_9225())
			.method_312(LootContextParameters.field_1232, blockPos)
			.method_312(LootContextParameters.field_1229, itemStack)
			.method_309(LootContextTypes.FISHING);
		return method_13180(commandContext, identifier, lootContext, arg);
	}

	private static int method_13180(
		CommandContext<ServerCommandSource> commandContext, Identifier identifier, LootContext lootContext, LootCommand.class_3041 arg
	) throws CommandSyntaxException {
		ServerCommandSource serverCommandSource = commandContext.getSource();
		LootSupplier lootSupplier = serverCommandSource.getMinecraftServer().getLootManager().method_367(identifier);
