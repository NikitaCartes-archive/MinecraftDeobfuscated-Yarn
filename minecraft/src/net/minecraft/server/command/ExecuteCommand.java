...
public class ExecuteCommand {
							...
							ServerCommandManager.argument("rot", RotationArgumentType.create())
								.redirect(
									...
									commandContext -> commandContext.getSource()
											.withRotation(RotationArgumentType.getRotationArgument(commandContext, "rot").method_9709(commandContext.getSource()))
								)
						)
						.then(
							ServerCommandManager.literal("as")
			...
			ServerCommandManager.literal("bossbar")
				.then(
					ServerCommandManager.argument("id", ResourceLocationArgumentType.create())
						.suggests(BossBarCommand.field_13482)
						.then(
							ServerCommandManager.literal("value")
								.redirect(literalCommandNode, commandContext -> method_13297(commandContext.getSource(), BossBarCommand.method_13054(commandContext), true, bl))
						)
						.then(
							ServerCommandManager.literal("max")
								.redirect(literalCommandNode, commandContext -> method_13297(commandContext.getSource(), BossBarCommand.method_13054(commandContext), false, bl))
						)
				)
		);
	...
	private static ServerCommandSource method_13297(ServerCommandSource serverCommandSource, class_3002 arg, boolean bl, boolean bl2) {
							...
							.then(
								method_13310(
									...
									commandContext -> BlockPredicateArgumentType.getPredicateArgument(commandContext, "block")
											.test(new BlockProxy(commandContext.getSource().getWorld(), BlockPosArgumentType.getValidPosArgument(commandContext, "pos"), true))
								)
							)
					)
			)
				...
				ServerCommandManager.literal("score")
					.then(
						ServerCommandManager.argument("target", ScoreHolderArgumentType.method_9447())
							.suggests(ScoreHolderArgumentType.SUGGESTION_PROVIDER)
							.then(
								((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)ServerCommandManager.argument(
	...
	private static ArgumentBuilder<ServerCommandSource, ?> method_13310(
		...
		return argumentBuilder.fork(commandNode, commandContext -> method_13319(commandContext, bl, arg.test(commandContext))).executes(commandContext -> {
			if (bl == arg.test(commandContext)) {
				((ServerCommandSource)commandContext.getSource()).sendFeedback(new TranslatableTextComponent("commands.execute.conditional.pass"), false);
				return 1;
			} else {
				throw CONDITIONAL_FAIL_EXCEPTION.create();
			}
