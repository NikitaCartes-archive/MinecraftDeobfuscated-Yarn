...
public class ExecuteCommand {
					...
					CommandManager.argument("id", IdentifierArgumentType.create())
						...
						.then(
							CommandManager.literal("value")
								.redirect(
									literalCommandNode, commandContext -> executeStoreBossbar(commandContext.getSource(), BossBarCommand.createBossBar(commandContext), true, bl)
								)
						)
						.then(
	...
	private static boolean testScoreMatch(CommandContext<ServerCommandSource> commandContext, NumberRange.IntRange intRange) throws CommandSyntaxException {
		...
		return !scoreboard.playerHasObjective(string, scoreboardObjective)
			? false
			: intRange.test(scoreboard.getPlayerScore(string, scoreboardObjective).getScore());
	}
