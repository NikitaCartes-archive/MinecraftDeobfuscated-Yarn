	...
	private static ArgumentBuilder<ServerCommandSource, ?> createDestinationArgs(
		...
		return CommandManager.argument("destination", BlockPosArgumentType.blockPos())
			.executes(
				context -> execute(
						context.getSource(),
						(CloneCommand.DimensionalPos)argumentGetter.apply(context),
						(CloneCommand.DimensionalPos)argumentGetter2.apply(context),
						(CloneCommand.DimensionalPos)argumentGetter3.apply(context),
		...
		return builder.then(
				CommandManager.literal("force")
					.executes(
						context -> execute(
								context.getSource(),
								(CloneCommand.DimensionalPos)beginPosGetter.apply(context),
								(CloneCommand.DimensionalPos)endPosGetter.apply(context),
								(CloneCommand.DimensionalPos)destinationPosGetter.apply(context),
								(Predicate<CachedBlockPosition>)filterGetter.apply(context),
			...
			if (i > j) {
				...
				for(CloneCommand.BlockInfo blockInfo2 : list2) {
					...
					if (blockInfo2.blockEntityNbt != null && blockEntity4 != null) {
						blockEntity4.readNbt(blockInfo2.blockEntityNbt, serverWorld2.getRegistryManager());
