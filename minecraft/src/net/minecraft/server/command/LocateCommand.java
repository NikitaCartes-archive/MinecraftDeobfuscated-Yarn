...
public class LocateCommand {
	...
	private static int execute(ServerCommandSource source, class_7066.class_7068<ConfiguredStructureFeature<?, ?>> arg) throws CommandSyntaxException {
		...
		RegistryEntryList<ConfiguredStructureFeature<?, ?>> registryEntryList = (RegistryEntryList)((Optional)arg.method_41173()
				.map(registryKey -> registry.getEntry(registryKey).map(registryEntry -> RegistryEntryList.of(registryEntry)), registry::getEntryList))
		...
		String string = arg.method_41173()
			.map(
				...
				tagKey -> "#"
						+ tagKey.id()
						+ " ("
						+ (String)pair.getSecond().getKey().map(registryKey -> registryKey.getValue().toString()).orElse("[unregistered]")
						+ ")"
			);
