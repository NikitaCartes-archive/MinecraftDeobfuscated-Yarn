...
public class DebugCommand {
	...
	private static int method_13159(ServerCommandSource serverCommandSource) throws CommandSyntaxException {
		...
		if (disableableProfiler.getController().isEnabled()) {
			...
			serverCommandSource.sendFeedback(
				new TranslatableTextComponent("commands.debug.started", "Started the debug profiler. Type '/debug stop' to stop it."), true
			);
			return 0;
		}
	}
	...
	private static int method_13158(ServerCommandSource serverCommandSource) throws CommandSyntaxException {
		...
		if (!disableableProfiler.getController().isEnabled()) {
			...
			serverCommandSource.sendFeedback(
				new TranslatableTextComponent("commands.debug.stopped", String.format(Locale.ROOT, "%.2f", f), profileResult.getTickSpan(), String.format("%.2f", g)),
				true
			);
