		...
		Consumer<ChunkHolder> consumer = chunkHolder -> chunkHolder.collectFuturesByStatus()
				.forEach(
					pair -> {
						...
						CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> completableFuture = (CompletableFuture)pair.getSecond();
						...
						try {
							...
							if (chunk.getStatus().isAtLeast(requiredStatus)) {
								completableFuturexx = requiredStatus.runLoadTask(
									this.world, this.structureTemplateManager, this.lightingProvider, chunk -> this.convertToFullChunk(holder), chunk
								);
							} else {
								completableFuturexx = requiredStatus.runGenerationTask(
									executor, this.world, this.chunkGenerator, this.structureTemplateManager, this.lightingProvider, chunk -> this.convertToFullChunk(holder), chunks
								);
							}
