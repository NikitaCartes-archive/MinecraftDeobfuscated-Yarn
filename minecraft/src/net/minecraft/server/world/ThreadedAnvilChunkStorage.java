...
public class ThreadedAnvilChunkStorage extends VersionedRegionFileCache implements ChunkHolder.PlayersWatchingChunkProvider {
	...
	private final Queue<Runnable> field_18240 = Queues.newConcurrentLinkedQueue();
					...
					completableFuture.thenAcceptAsync(chunk -> {
						if (chunk != null) {
							...
							for(int ixx = 0; ixx < 16; ++ixx) {
								this.serverLightingProvider.scheduleChunkLightUpdate(class_4076.method_18681(chunk.getPos(), ixx), true);
							}
	...
	private void save(Chunk chunk) {
		if (chunk.needsSaving()) {
			...
			try {
				...
				this.writeChunkTag(chunkPos, compoundTag);
