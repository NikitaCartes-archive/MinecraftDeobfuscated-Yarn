...
public class ThreadedAnvilChunkStorage extends VersionedChunkStorage implements ChunkHolder.PlayersWatchingChunkProvider {
	...
	public ThreadedAnvilChunkStorage(
		...
		if (chunkGenerator instanceof NoiseChunkGenerator noiseChunkGenerator) {
			this.noiseConfig = NoiseConfig.create(
				(ChunkGeneratorSettings)noiseChunkGenerator.getSettings().value(), dynamicRegistryManager.getWrapperOrThrow(RegistryKeys.NOISE_PARAMETERS), l
			);
		} else {
			this.noiseConfig = NoiseConfig.create(
	...
	public String getChunkLoadingDebugInfo(ChunkPos chunkPos) {
		...
		if (chunkHolder == null) {
			...
			string = string + 167 + chunkLevelType.ordinal() + chunkLevelType;
			return string + "§r";
		}
	}
		...
		if (margin == 0) {
			...
			CompletableFuture<OptionalChunk<List<Chunk>>> completableFuture3 = completableFuture2.thenApply(
				chunks -> {
					...
					return OptionalChunk.of(listxx);
				}
			);
		...
		Consumer<ChunkHolder> consumer = chunkHolder -> chunkHolder.collectFuturesByStatus()
				.forEach(
					pair -> {
						...
						CompletableFuture<OptionalChunk<Chunk>> completableFuture = (CompletableFuture)pair.getSecond();
	...
	private CompletableFuture<OptionalChunk<Chunk>> upgradeChunk(ChunkHolder holder, ChunkStatus requiredStatus) {
		...
		return completableFuture.thenComposeAsync(optionalChunk -> {
			List<Chunk> list = (List)optionalChunk.orElse(null);
			if (list == null) {
				...
				try {
					...
					CompletableFuture<Chunk> completableFuturexx;
					if (chunk.getStatus().isAtLeast(requiredStatus)) {
						completableFuturexx = requiredStatus.runLoadTask(this.field_49171, chunk -> this.convertToFullChunk(holder, chunk), chunk);
					} else {
						completableFuturexx = requiredStatus.runGenerationTask(this.field_49171, executor, chunk -> this.convertToFullChunk(holder, chunk), list);
					}

					this.worldGenerationProgressListener.setChunkStatus(chunkPos, requiredStatus);
					return completableFuturexx.thenApply(OptionalChunk::of);
	...
	private static String getFutureStatus(CompletableFuture<OptionalChunk<WorldChunk>> future) {
		try {
			OptionalChunk<WorldChunk> optionalChunk = (OptionalChunk)future.getNow(null);
			if (optionalChunk != null) {
	...
	private void sendWatchPackets(ServerPlayerEntity player) {
		...
		if (var5 instanceof ChunkFilter.Cylindrical cylindrical && cylindrical.center().equals(chunkPos) && cylindrical.viewDistance() == i) {
			return;
		}
