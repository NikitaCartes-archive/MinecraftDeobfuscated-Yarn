...
public class ServerChunkManager extends ChunkManager {
	...
	public Chunk getChunk(int x, int z, ChunkStatus leastStatus, boolean create) {
		if (Thread.currentThread() != this.serverThread) {
			...
			OptionalChunk<Chunk> optionalChunk = (OptionalChunk)completableFuture.join();
	...
	public WorldChunk getWorldChunk(int chunkX, int chunkZ) {
		if (Thread.currentThread() != this.serverThread) {
			...
			if (chunkHolder == null) {
				...
				OptionalChunk<Chunk> optionalChunk = (OptionalChunk)chunkHolder.getValidFutureFor(ChunkStatus.FULL).getNow(null);
	...
	private void tickChunks() {
		...
		if (!this.world.isDebugWorld()) {
			...
			List<ServerChunkManager.ChunkWithHolder> list = Lists.newArrayListWithCapacity(this.threadedAnvilChunkStorage.getLoadedChunkCount());
