...
public class ServerChunkManager extends ChunkManager {
	...
	public Chunk getChunk(int x, int z, ChunkStatus leastStatus, boolean create) {
		if (Thread.currentThread() != this.serverThread) {
			...
			this.method_21738(l, chunk, leastStatus);
			return chunk;
		}
	}
	...
	public WorldChunk getWorldChunk(int chunkX, int chunkZ) {
		if (Thread.currentThread() != this.serverThread) {
			...
			if (chunkHolder == null) {
				...
				Either<Chunk, ChunkHolder.Unloaded> either = (Either)chunkHolder.method_21737(ChunkStatus.FULL).getNow(null);
				if (either == null) {
					return null;
				} else {
	...
	private boolean method_20585(long pos, Function<ChunkHolder, CompletableFuture<Either<WorldChunk, ChunkHolder.Unloaded>>> function) {
		...
		if (chunkHolder == null) {
			...
			Either<WorldChunk, ChunkHolder.Unloaded> either = (Either)((CompletableFuture)function.apply(chunkHolder)).getNow(ChunkHolder.UNLOADED_WORLD_CHUNK);
