...
public class ServerChunkManager extends ChunkManager {
	...
	public WorldChunk getWorldChunk(int chunkX, int chunkZ) {
		if (Thread.currentThread() != this.serverThread) {
			...
			if (chunkHolder == null) {
				...
				Either<Chunk, ChunkHolder.Unloaded> either = (Either)chunkHolder.getNowFuture(ChunkStatus.FULL).getNow(null);
				if (either == null) {
					return null;
				} else {
	...
	private void ifChunkLoaded(long pos, Consumer<WorldChunk> chunkConsumer) {
		ChunkHolder chunkHolder = this.getChunkHolder(pos);
		if (chunkHolder != null) {
			((Either)chunkHolder.getBorderFuture().getNow(ChunkHolder.UNLOADED_WORLD_CHUNK)).left().ifPresent(chunkConsumer);
		}
	}
