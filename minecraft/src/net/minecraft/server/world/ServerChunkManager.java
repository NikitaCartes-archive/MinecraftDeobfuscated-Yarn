...
public class ServerChunkManager extends ChunkManager {
	...
	private final Queue<Runnable> genQueue = Queues.newConcurrentLinkedQueue();
	...
	public boolean method_16165() {
		if (this.method_16155()) {
			...
			if (runnable != null) {
				...
				Long2ObjectLinkedOpenHashMap<List<Runnable>> long2ObjectLinkedOpenHashMap = (Long2ObjectLinkedOpenHashMap)this.field_13924.get(this.field_13925);
				List<Runnable> list = (List)long2ObjectLinkedOpenHashMap.removeFirst();
	...
	public boolean method_12125(Entity entity) {
		...
		if (serverChunkManagerEntry == null) {
			...
			Either<WorldChunk, ServerChunkManagerEntry.Unloaded> either = (Either)serverChunkManagerEntry.method_14003()
	...
	private CompletableFuture<Either<List<Chunk>, ServerChunkManagerEntry.Unloaded>> method_16154(ChunkPos chunkPos, int i, IntFunction<ChunkStatus> intFunction) {
		List<CompletableFuture<Either<Chunk, ServerChunkManagerEntry.Unloaded>>> list = Lists.newArrayList();
		int j = chunkPos.x;
		int k = chunkPos.z;
	...
	private ServerChunkManagerEntry method_14150(long l, int i, @Nullable ServerChunkManagerEntry serverChunkManagerEntry, int j) {
		if (i > field_13922) {
			...
			if (serverChunkManagerEntry != null) {
				Long2ObjectMap<List<Runnable>> long2ObjectMap = (Long2ObjectMap)this.field_13924.get(j);
				List<Runnable> list = (List)long2ObjectMap.remove(l);
	...
	private void method_14106(Chunk chunk, boolean bl) {
		try {
			...
			LOGGER.error("Couldn't save chunk", var4);
		} catch (MinecraftException var5) {
			LOGGER.error("Couldn't save chunk; already in use by another instance of Minecraft?", var5);
		}
	}
					...
					completableFuture.thenAcceptAsync(chunk -> {
						if (chunk != null) {
							...
							for(int ixx = 0; ixx < 16; ++ixx) {
								this.lightProvider.method_15551(chunk.getPos().x, ixx, chunk.getPos().z, true);
							}
						}
					}, this.genQueue::add);
				}
	...
	private void doMobSpawning() {
		...
		if (!bl) {
			...
			while(objectBidirectionalIterator.hasNext()) {
				Entry<ServerChunkManagerEntry> entry = (Entry)objectBidirectionalIterator.next();
	...
	public void applyViewDistance(int i) {
		...
		if (j != this.maxWatchDistance) {
			for(ServerChunkManagerEntry serverChunkManagerEntry : this.loadedChunkMap.values()) {
				...
				Packet<?>[] packets = new Packet[2];
				this.players.getPlayersWatchingChunk(chunkPos.toLong()).forEach(serverPlayerEntity -> {
					int jxx = getWatchDistance(chunkPos, serverPlayerEntity);
					boolean bl = jxx <= this.maxWatchDistance;
					boolean bl2 = jxx <= j;
	...
	public boolean method_14154(ServerPlayerEntity serverPlayerEntity, int i, int j) {
		...
		if (serverChunkManagerEntry == null) {
			return false;
		} else if (serverChunkManagerEntry.getChunk() != null) {
			return getWatchDistance(chunkPos, serverPlayerEntity) <= this.maxWatchDistance;
		} else {
			return false;
		}
	}
