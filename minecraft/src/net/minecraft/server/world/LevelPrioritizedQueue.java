...
public class LevelPrioritizedQueue<T> {
	...
	protected void updateLevel(int fromLevel, ChunkPos pos, int toLevel) {
		if (fromLevel < LEVEL_COUNT) {
			...
			if (list != null && !list.isEmpty()) {
				((List)((Long2ObjectLinkedOpenHashMap)this.levelToPosToElements.get(toLevel))
						.computeIfAbsent(pos.toLong(), (Long2ObjectFunction)(l -> Lists.newArrayList())))
					.addAll(list);
	...
	protected void add(Optional<T> element, long pos, int level) {
		((List)((Long2ObjectLinkedOpenHashMap)this.levelToPosToElements.get(level)).computeIfAbsent(pos, (Long2ObjectFunction)(l -> Lists.newArrayList())))
			.add(element);
