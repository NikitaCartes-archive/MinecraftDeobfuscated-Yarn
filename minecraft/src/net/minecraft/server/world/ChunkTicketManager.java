...
public abstract class ChunkTicketManager {
	...
	protected void setChunkForced(ChunkPos chunkPos, boolean forced) {
		ChunkTicket<ChunkPos> chunkTicket = new ChunkTicket<>(ChunkTicketType.FORCED, 31, chunkPos);
		long l = chunkPos.toLong();
		if (forced) {
			this.addTicket(l, chunkTicket);
			this.simulationDistanceTracker.add(l, chunkTicket);
	...
	protected String getTicket(long pos) {
		SortedArraySet<ChunkTicket<?>> sortedArraySet = (SortedArraySet)this.ticketsByPosition.get(pos);
	...
	class TicketDistanceLevelPropagator extends ChunkPosDistanceLevelPropagator {
		...
		@Override
		protected int getInitialLevel(long id) {
			SortedArraySet<ChunkTicket<?>> sortedArraySet = (SortedArraySet)ChunkTicketManager.this.ticketsByPosition.get(id);
