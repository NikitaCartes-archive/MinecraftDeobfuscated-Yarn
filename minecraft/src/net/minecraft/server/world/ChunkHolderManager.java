					...
					completableFuture.thenAcceptAsync(chunk -> {
						if (chunk != null) {
							...
							for(int ixx = 0; ixx < 16; ++ixx) {
								this.serverLightingProvider.scheduleChunkLightUpdate(chunk.getPos().x, ixx, chunk.getPos().z, true);
							}
		...
		if (chunkStatus == ChunkStatus.EMPTY) {
			return CompletableFuture.supplyAsync(() -> {
				try {
					...
					LOGGER.error("Couldn't load chunk", var6);
				}
			...
			return completableFuture.thenComposeAsync(
				either -> (CompletableFuture)either.map(
						list -> {
							try {
								CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> completableFuturexx = chunkStatus.runTask(
	...
	private void save(Chunk chunk, boolean bl) {
		...
		if (chunk.needsSaving()) {
			try {
				...
				LOGGER.error("Couldn't save chunk; already in use by another instance of Minecraft?", var9);
				return;
			}
			...
			try {
				...
				CompoundTag compoundTag = ChunkSaveHandlerImpl.saveChunk(this.world, chunk);
