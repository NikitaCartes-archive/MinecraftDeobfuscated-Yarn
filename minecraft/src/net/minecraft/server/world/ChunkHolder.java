...
public class ChunkHolder {
	...
	public CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> getFuture(ChunkStatus chunkStatus) {
		CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> completableFuture = (CompletableFuture)this.futuresByStatus.get(chunkStatus.getIndex());
	...
	protected void tick(ThreadedAnvilChunkStorage threadedAnvilChunkStorage) {
		...
		this.field_19238 |= chunkStatus2.isAtLeast(ChunkStatus.field_12803);
		...
		if (bl) {
			...
			for(int j = bl2 ? chunkStatus2.getIndex() + 1 : 0; j <= chunkStatus.getIndex(); ++j) {
				CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> completableFuture = (CompletableFuture)this.futuresByStatus.get(j);
				if (completableFuture != null) {
