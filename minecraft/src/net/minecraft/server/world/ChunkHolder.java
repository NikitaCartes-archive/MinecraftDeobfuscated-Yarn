...
public class ChunkHolder {
	...
	public CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> getChunkForStatus(ChunkStatus chunkStatus) {
		CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> completableFuture = (CompletableFuture)this.chunkByStatus.get(chunkStatus.getIndex());
	...
	public void flushUpdates(WorldChunk worldChunk) {
		if (this.blockUpdateCount != 0 || this.blockLightUpdateBits != 0 || this.skyLightUpdateBits != 0) {
			...
			if (this.blockUpdateCount == 1) {
				...
				this.sendPacketToPlayersWatching(new BlockUpdateClientPacket(world, blockPos), false);
			...
			if (this.blockLightUpdateBits != 0 || this.skyLightUpdateBits != 0) {
				...
				int i = this.blockLightUpdateBits & this.lightSentWithBlocksBits;
				int j = this.skyLightUpdateBits & this.lightSentWithBlocksBits;
				if (i != 0 || j != 0) {
					this.sendPacketToPlayersWatching(new LightUpdateClientPacket(worldChunk.getPos(), this.lightingProvider, i, j), false);
				}

				this.blockLightUpdateBits = 0;
				this.skyLightUpdateBits = 0;
				this.lightSentWithBlocksBits &= ~(this.blockLightUpdateBits & this.skyLightUpdateBits);
			}
		}
	}
	...
	private void sendBlockEntityUpdatePacket(World world, BlockPos blockPos) {
		...
		if (blockEntity != null) {
			...
			if (blockEntityUpdateClientPacket != null) {
				this.sendPacketToPlayersWatching(blockEntityUpdateClientPacket, false);
			}
		}
	}
	...
	public CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> getChunk(ChunkStatus chunkStatus, ThreadedAnvilChunkStorage threadedAnvilChunkStorage) {
		...
		CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> completableFuture = (CompletableFuture)this.chunkByStatus.get(i);
	...
	protected void update(ThreadedAnvilChunkStorage threadedAnvilChunkStorage) {
		...
		if (bl) {
			...
			for(int j = bl2 ? chunkStatus2.getIndex() + 1 : 0; j <= chunkStatus.getIndex(); ++j) {
				CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> completableFuture = (CompletableFuture)this.chunkByStatus.get(j);
				if (completableFuture != null) {
