...
public class ServerChunkManagerEntry {
	...
	public CompletableFuture<Either<Chunk, ServerChunkManagerEntry.Unloaded>> getChunkForStatus(ChunkStatus chunkStatus) {
		CompletableFuture<Either<Chunk, ServerChunkManagerEntry.Unloaded>> completableFuture = (CompletableFuture)this.statusToChunk.get(chunkStatus.getIndex());
	...
	public WorldChunk getChunk() {
		...
		Either<WorldChunk, ServerChunkManagerEntry.Unloaded> either = (Either)completableFuture.getNow(null);
	...
	public void flushUpdates(WorldChunk worldChunk) {
		if (this.blockUpdateCount != 0 || this.field_13870 != 0 || this.field_13871 != 0) {
			...
			if (this.field_13870 != 0 || this.field_13871 != 0) {
				...
				if (i != 0 || j != 0) {
					this.sendPacket(new LightUpdateClientPacket(worldChunk.getPos(), this.lightingProvider, i, j), false);
				}
	...
	public CompletableFuture<Either<Chunk, ServerChunkManagerEntry.Unloaded>> method_13993(ChunkStatus chunkStatus, class_3898 arg) {
		...
		CompletableFuture<Either<Chunk, ServerChunkManagerEntry.Unloaded>> completableFuture = (CompletableFuture)this.statusToChunk.get(i);
		if (completableFuture != null) {
			Either<Chunk, ServerChunkManagerEntry.Unloaded> either = (Either)completableFuture.getNow(null);
	...
	protected void method_14007(class_3898 arg) {
		...
		if (this.field_16432 <= ServerChunkManager.FULL_CHUNK_LEVEL) {
			...
			for(int j = chunkStatus2.getIndex() + 1; j <= chunkStatus.getIndex(); ++j) {
				CompletableFuture<Either<Chunk, ServerChunkManagerEntry.Unloaded>> completableFuture = (CompletableFuture)this.statusToChunk.get(j);
				if (completableFuture != null) {
