...
public class ServerEntityManager<T extends EntityLike> implements AutoCloseable {
	...
	private void method_31810(long l) {
		ServerEntityManager.class_5581 lv = (ServerEntityManager.class_5581)this.field_27268.get(l);
	...
	private boolean method_31812(long l, Consumer<T> consumer) {
		...
		if (lv == ServerEntityManager.class_5581.field_27276) {
			...
			List<T> list = (List)this.field_27265.method_31782(l).flatMap(arg -> arg.method_31766().filter(EntityLike::shouldSave)).collect(Collectors.toList());
			if (list.isEmpty()) {
	...
	private void method_31853() {
		class_5566<T> lv;
		while((lv = (class_5566)this.loadingQueue.poll()) != null) {
	...
	public void method_31826(Writer writer) throws IOException {
		...
		this.field_27265.method_31770().forEach(l -> {
			...
			this.field_27265.method_31772(l).forEach(lx -> {
				class_5572<T> lvxx = this.field_27265.method_31785(lx);
				if (lvxx != null) {
					try {
						csvWriter.printRow(ChunkSectionPos.unpackX(lx), ChunkSectionPos.unpackY(lx), ChunkSectionPos.unpackZ(lx), lvxx.method_31768(), lv, lvxx.method_31769());
		...
		public void remove(Entity.RemovalReason reason) {
			if (!this.field_27274.method_31767(this.entity)) {
				ServerEntityManager.LOGGER
					.warn("Entity {} wasn't found in section {} (destroying due to {})", this.entity, ChunkSectionPos.from(this.field_27273), reason);
			}
