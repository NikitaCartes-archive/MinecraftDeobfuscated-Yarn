...
public abstract class MinecraftServer extends class_4093<ServerTask> implements SnooperListener, CommandOutput, AutoCloseable, Runnable {
	...
	public static void main(String[] strings) {
		...
		try {
			...
			LOGGER.fatal("Failed to start the minecraft server", var28);
		}
	}
	...
	private void reloadDataPacks(LevelProperties levelProperties) {
		...
		for(ResourcePackContainer resourcePackContainer : this.field_4595.getAlphabeticallyOrderedContainers()) {
			if (!levelProperties.getDisabledDataPacks().contains(resourcePackContainer.getName()) && !list.contains(resourcePackContainer)) {
				...
				resourcePackContainer.getSortingDirection().locate(list, resourcePackContainer, resourcePackContainer -> resourcePackContainer, false);
			}
		}

		this.field_4595.setEnabled(list);
		List<ResourcePack> list2 = Lists.<ResourcePack>newArrayList();
		this.field_4595.getEnabledContainers().forEach(resourcePackContainer -> list2.add(resourcePackContainer.createResourcePack()));
		...
		this.field_4595.getEnabledContainers().forEach(resourcePackContainer -> levelProperties.getEnabledDataPacks().add(resourcePackContainer.getName()));
		this.field_4595.getAlphabeticallyOrderedContainers().forEach(resourcePackContainer -> {
			if (!this.field_4595.getEnabledContainers().contains(resourcePackContainer)) {
