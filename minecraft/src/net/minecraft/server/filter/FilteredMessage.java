...
public record FilteredMessage<T>(T raw, @Nullable T filtered) {
	public static final FilteredMessage<String> EMPTY = permitted((T)"");
	...
	public static <T> FilteredMessage<T> censored(T filterable) {
		return new FilteredMessage<>(filterable, (T)null);
	}

	public <U> FilteredMessage<U> map(Function<T, U> mapper) {
		return new FilteredMessage<>((U)mapper.apply(this.raw), Util.map(this.filtered, mapper));
	}
	...
	public T filteredOrElse(T filterable) {
		return (T)(this.filtered != null ? this.filtered : filterable);
	}

	@Nullable
	public T getFilterableFor(ServerPlayerEntity sender, ServerPlayerEntity receiver) {
		return (T)(sender.shouldFilterMessagesSentTo(receiver) ? this.filtered : this.raw);
	}
	...
	public T getFilterableFor(ServerCommandSource source, ServerPlayerEntity receiver) {
		...
		return (T)(serverPlayerEntity != null ? this.getFilterableFor(serverPlayerEntity, receiver) : this.raw);
	}
}
