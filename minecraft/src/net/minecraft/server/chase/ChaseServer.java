...
public class ChaseServer {
	...
	public void stop() {
		...
		if (this.socket != null) {
			try {
				...
				LOGGER.error("Failed to close remote control server socket", var2);
			}
	...
	public void run() {
		try {
			...
			if (this.running) {
				LOGGER.error("Remote control server closed because of an IO exception", var13);
			}
		} finally {
			if (this.socket != null && !this.socket.isClosed()) {
				try {
					...
					LOGGER.warn("Failed to close remote control server socket", var11);
				}
			}
		}
	...
	private void streamCommand(Socket socket) {
		try {
			...
			LOGGER.info("Remote control client broadcast socket got an IO exception and will be closed", var14);
		} finally {
			try {
				if (!socket.isClosed()) {
					socket.close();
				...
				LOGGER.warn("Failed to close remote control client socket", var12);
			}
		}
