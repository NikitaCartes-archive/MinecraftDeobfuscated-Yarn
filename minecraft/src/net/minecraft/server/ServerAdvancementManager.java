...
public class ServerAdvancementManager implements ResourceReloadListener {
	...
	public static final int pathPrefixLength = "advancements/".length();
	public static final int fileExtensionLength = ".json".length();
	...
	private Map<Identifier, SimpleAdvancement.Builder> method_12894(ResourceManager resourceManager) {
		...
		for (Identifier identifier : resourceManager.findResources("advancements", stringx -> stringx.endsWith(".json"))) {
			...
			Identifier identifier2 = new Identifier(identifier.getNamespace(), string.substring(pathPrefixLength, string.length() - fileExtensionLength));
	...
	public void onResourceReload(ResourceManager resourceManager) {
		...
		MANAGER.method_711(map);

		for (SimpleAdvancement simpleAdvancement : MANAGER.method_715()) {
			if (simpleAdvancement.getDisplay() != null) {
				class_194.method_852(simpleAdvancement);
			}
		}
	}
}
