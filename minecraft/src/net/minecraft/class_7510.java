...
public class class_7510 {
	public static <T> List<class_7510.IndexedFeatures> method_44210(List<T> list, Function<T, List<RegistryEntryList<PlacedFeature>>> function, boolean bl) {
		...
		for(T object : list) {
			List<class_6543> list2 = Lists.newArrayList();
			List<RegistryEntryList<PlacedFeature>> list3 = (List)function.apply(object);
			...
			for(int j = 0; j < list2.size(); ++j) {
				Set<class_6543> set = (Set)map.computeIfAbsent((class_6543)list2.get(j), arg -> new TreeSet(comparator));
				if (j < list2.size() - 1) {
		...
		for(class_6543 lv : map.keySet()) {
			...
			if (!set2.contains(lv) && TopologicalSorts.sort(map, set2, set3, list2::add, (T)lv)) {
				if (!bl) {
		...
		for(int j = 0; j < i; ++j) {
			int l = j;
			List<PlacedFeature> list5 = (List)list2.stream().filter(arg -> arg.step() == l).map(class_6543::feature).collect(Collectors.toList());
