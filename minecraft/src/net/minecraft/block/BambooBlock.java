...
public class BambooBlock extends Block implements Fertilizable {
	...
	public void onScheduledTick(BlockState blockState, World world, BlockPos blockPos, Random random) {
		...
		} else if (blockState.get(field_9916) == 0) {
	...
	public BlockState getStateForNeighborUpdate(
		...
		if (direction == Direction.field_11036 && blockState2.getBlock() == Blocks.field_10211 && blockState2.get(field_9914) > blockState.get(field_9914)) {
	...
	public boolean isFertilizable(BlockView blockView, BlockPos blockPos, BlockState blockState, boolean bl) {
		...
		return i + j + 1 < 16 && blockView.getBlockState(blockPos.up(i)).get(field_9916) != 1;
	}
	...
	public void grow(World world, Random random, BlockPos blockPos, BlockState blockState) {
		...
		for(int m = 0; m < l; ++m) {
			...
			if (k >= 16 || blockState2.get(field_9916) == 1 || !world.isAir(blockPos2.up())) {
				return;
			}
