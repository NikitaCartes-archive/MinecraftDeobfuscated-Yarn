...
public class StemBlock extends PlantBlock implements Fertilizable {
	...
	public void scheduledTick(BlockState blockState, World world, BlockPos blockPos, Random random) {
		...
		if (world.getLightLevel(blockPos.up(), 0) >= 9) {
			...
			if (random.nextInt((int)(25.0F / f) + 1) == 0) {
				int i = blockState.get(field_11584);
				if (i < 7) {
					...
					if (world.getBlockState(blockPos2).isAir()
						...
						world.setBlockState(blockPos, this.field_11585.method_10680().getDefaultState().with(HorizontalFacingBlock.field_11177, direction));
					}
				}
			}
		}
	...
	public boolean isFertilizable(BlockView blockView, BlockPos blockPos, BlockState blockState, boolean bl) {
		return blockState.get(field_11584) != 7;
	}
	...
	public void grow(World world, Random random, BlockPos blockPos, BlockState blockState) {
		int i = Math.min(7, blockState.get(field_11584) + MathHelper.nextInt(world.random, 2, 5));
