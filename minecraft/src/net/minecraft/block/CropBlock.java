...
public class CropBlock extends PlantBlock implements Fertilizable {
	...
	protected int method_9829(BlockState blockState) {
		return blockState.method_11654(this.method_9824());
	}
	...
	public boolean method_9825(BlockState blockState) {
		return blockState.method_11654(this.method_9824()) >= this.getMaxAge();
	}
		...
		for(int i = -1; i <= 1; ++i) {
			for(int j = -1; j <= 1; ++j) {
				...
				if (blockState.getBlock() == Blocks.field_10362) {
					g = 1.0F;
					if (blockState.method_11654(FarmlandBlock.field_11009) > 0) {
						g = 3.0F;
					}
				}
	...
	public boolean method_9558(BlockState blockState, ViewableWorld viewableWorld, BlockPos blockPos) {
		return (viewableWorld.getLightLevel(blockPos, 0) >= 8 || viewableWorld.isSkyVisible(blockPos)) && super.method_9558(blockState, viewableWorld, blockPos);
	}
