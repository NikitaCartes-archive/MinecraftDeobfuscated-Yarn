...
public class AbstractLichenBlock extends Block {
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
		...
		return (BlockState)Arrays.stream(ctx.getPlacementDirections())
			.map(direction -> this.addDirection(blockState, world, blockPos, direction))
	...
	public boolean canSpreadRandomly(BlockState state, ServerWorld world, BlockPos pos, Random random) {
		...
		return list.stream().filter(from -> hasDirection(state, from)).anyMatch(to -> this.canSpreadRandomly(state, world, pos, to, random));
	}
	...
	public boolean canSpreadRandomly(BlockState state, WorldAccess world, BlockPos pos, Direction from, Random random) {
		...
		return list.stream().anyMatch(to -> this.canSpreadTo(state, world, pos, from, to));
	}
	...
	public boolean canSpreadTo(BlockState state, WorldAccess world, BlockPos pos, Direction from, Direction to) {
		if (to.getAxis() == from.getAxis() || !hasDirection(state, from) || hasDirection(state, to)) {
			return false;
		} else if (this.addDirection(world, pos, to)) {
			return true;
		} else {
			return this.addDirection(world, pos.offset(to), from) ? true : this.addDirection(world, pos.offset(to).offset(from), to.getOpposite());
		}
	}
	...
	private boolean addDirection(WorldAccess world, BlockPos pos, Direction direction) {
		...
		if (!this.canGrowIn(blockState)) {
			...
			return blockState2 != null ? world.setBlockState(pos, blockState2, 2) : false;
		}
	}
