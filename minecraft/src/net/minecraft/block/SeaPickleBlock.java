...
public class SeaPickleBlock extends PlantBlock implements Fertilizable, Waterloggable {
	...
	public int getLuminance(BlockState blockState) {
		return this.method_10506(blockState) ? 0 : super.getLuminance(blockState) + 3 * blockState.get(field_11472);
	}
	...
	public BlockState getPlacementState(ItemPlacementContext itemPlacementContext) {
		...
		if (blockState.getBlock() == this) {
			return blockState.with(field_11472, Integer.valueOf(Math.min(4, blockState.get(field_11472) + 1)));
		} else {
	...
	private boolean method_10506(BlockState blockState) {
		return !blockState.get(field_11475);
	}
