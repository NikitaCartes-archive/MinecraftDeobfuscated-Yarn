...
public class CoralParentBlock extends Block implements Waterloggable {
	...
	protected static boolean method_9431(BlockState blockState, BlockView blockView, BlockPos blockPos) {
		if (blockState.method_11654(field_9940)) {
			return true;
		} else {
			for(Direction direction : Direction.values()) {
				if (blockView.method_8316(blockPos.offset(direction)).matches(FluidTags.field_15517)) {
					return true;
				}
			}
	...
	public BlockState method_9605(ItemPlacementContext itemPlacementContext) {
		...
		return this.method_9564().method_11657(field_9940, Boolean.valueOf(fluidState.matches(FluidTags.field_15517) && fluidState.getLevel() == 8));
	}
