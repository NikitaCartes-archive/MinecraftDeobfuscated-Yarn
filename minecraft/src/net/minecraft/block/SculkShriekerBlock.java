...
public class SculkShriekerBlock extends BlockWithEntity implements Waterloggable {
	...
	public static boolean canShriek(ServerWorld world, BlockPos pos, BlockState state) {
		return !state.get(SHRIEKING)
			&& (
				!state.get(CAN_SUMMON)
					|| getClosestPlayerWarningManager(world, pos).map(warningManager -> warningManager.canIncreaseWarningLevel(world, pos)).orElse(false)
			);
	}

	public static void shriek(ServerWorld world, BlockState state, BlockPos pos) {
		if (canShriek(world, pos, state)) {
			if (!state.get(CAN_SUMMON) || getClosestPlayerWarningManager(world, pos).filter(warningManager -> warningManager.warnNearbyPlayers(world, pos)).isPresent()
				)
			 {
