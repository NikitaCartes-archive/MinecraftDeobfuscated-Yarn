...
public class SweetBerryBushBlock extends PlantBlock implements Fertilizable {
	...
	public VoxelShape getOutlineShape(BlockState state, BlockView view, BlockPos pos, EntityContext ePos) {
		if (state.get(AGE) == 0) {
			...
			return state.get(AGE) < 3 ? LARGE_SHAPE : super.getOutlineShape(state, view, pos, ePos);
		}
	}
	...
	public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
		if (entity instanceof LivingEntity && entity.getType() != EntityType.FOX && entity.getType() != EntityType.BEE) {
			...
			if (!world.isClient && state.get(AGE) > 0 && (entity.prevRenderX != entity.getX() || entity.prevRenderZ != entity.getZ())) {
