...
public abstract class AbstractRedstoneGateBlock extends HorizontalFacingBlock {
	...
	protected int getPower(World world, BlockPos pos, BlockState state) {
		...
		if (i >= 15) {
			...
			return Math.max(i, blockState.isOf(Blocks.field_10091) ? blockState.get(RedstoneWireBlock.POWER) : 0);
		}
	}
	...
	protected int getInputLevel(WorldView world, BlockPos pos, Direction dir) {
		...
		if (this.isValidInput(blockState)) {
			if (blockState.isOf(Blocks.field_10002)) {
				...
				return blockState.isOf(Blocks.field_10091) ? blockState.get(RedstoneWireBlock.POWER) : world.getStrongRedstonePower(pos, dir);
			}
		} else {
			return 0;
		}
