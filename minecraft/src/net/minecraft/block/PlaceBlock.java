			...
			placeBlock(
				...
				stack -> {
					if (stack.isEmpty()) {
						...
						boolean var10000;
						label19: {
							Item var6xx = stack.getItem();
							if (var6xx instanceof BlockItem blockItem
								&& blockItem.place(
	...
	public static boolean placeBlock(World world, BlockPos pos, Direction side, Function<ItemStack, Boolean> placer) {
		for(Inventory inventory : findInventories(world, pos)) {
			boolean bl = HopperBlockEntity.getAvailableSlots(inventory, side).anyMatch(slot -> {
				ItemStack itemStackxx = inventory.removeStack(slot, 1);
				if (!itemStackxx.isEmpty()) {
					boolean blxx = placer.apply(itemStackxx.copy());
					if (blxx) {
						inventory.markDirty();
					} else {
						inventory.setStack(slot, itemStackxx);
					}
		...
		if (itemEntity != null) {
			...
			if (!itemStack.isEmpty()) {
				boolean bl = placer.apply(itemStack.copyWithCount(1));
				if (bl) {
