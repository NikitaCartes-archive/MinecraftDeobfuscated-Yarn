...
public class PistonHeadBlock extends FacingBlock {
	...
	private VoxelShape getArmShape(BlockState blockState) {
		boolean bl = blockState.get(SHORT);
		switch((Direction)blockState.get(FACING)) {
			case DOWN:
			default:
	...
	public void onBlockRemoved(BlockState blockState, World world, BlockPos blockPos, BlockState blockState2, boolean bl) {
		if (blockState.getBlock() != blockState2.getBlock()) {
			...
			if ((blockState3.getBlock() == Blocks.PISTON || blockState3.getBlock() == Blocks.STICKY_PISTON) && blockState3.get(PistonBlock.EXTENDED)) {
	...
	public boolean canPlaceAt(BlockState blockState, class_4538 arg, BlockPos blockPos) {
		Block block = arg.getBlockState(blockPos.offset(((Direction)blockState.get(FACING)).getOpposite())).getBlock();
