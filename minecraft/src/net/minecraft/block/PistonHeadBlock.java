...
public class PistonHeadBlock extends FacingBlock {
	...
	private static VoxelShape[] getHeadShapes(boolean shortHead) {
		return (VoxelShape[])Arrays.stream(Direction.values()).map(direction -> getHeadShape(direction, shortHead)).toArray(i -> new VoxelShape[i]);
	}
	...
	private boolean method_26980(BlockState state, BlockState blockState) {
		...
		return blockState.isOf(block) && blockState.get(PistonBlock.EXTENDED) && blockState.get(FACING) == state.get(FACING);
	}
