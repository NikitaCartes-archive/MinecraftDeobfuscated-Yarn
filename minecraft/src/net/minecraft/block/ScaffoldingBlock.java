...
public class ScaffoldingBlock extends Block implements Waterloggable {
	...
	public boolean canPlaceAt(BlockState blockState, ViewableWorld viewableWorld, BlockPos blockPos) {
		...
		if (direction == null) {
			...
			return blockState2.getBlock() != this || blockState2.get(field_16495) + 1 <= 7;
		}
	}
	...
	public void scheduledTick(BlockState blockState, World world, BlockPos blockPos, Random random) {
		...
		if (blockState2.get(field_16495) == 7) {
			if (blockState.get(field_16495) == 7) {
		...
		if (blockState.getBlock() == this) {
			...
			for(Direction direction2 : Direction.class_2353.HORIZONTAL) {
				...
				if (blockState2.getBlock() == this) {
					int j = blockState2.get(field_16495);
					if (j < i) {
						i = j;
	...
	private int method_16371(BlockView blockView, BlockPos blockPos) {
		...
		if (direction == null) {
			...
			return blockState.getBlock() == this ? Math.min(7, blockState.get(field_16495) + (direction.getAxis() == Direction.Axis.Y ? 0 : 1)) : 0;
		}
	}
