...
public class ScaffoldingBlock extends Block implements Waterloggable {
			...
			if (state.get(DISTANCE) == 7) {
				world.spawnEntity(
					new FallingBlockEntity(
						world, (double)pos.getX() + 0.5, (double)pos.getY(), (double)pos.getZ() + 0.5, blockState.with(WATERLOGGED, Boolean.valueOf(false))
					)
				);
			} else {
	...
	private boolean shouldBeBottom(BlockView world, BlockPos pos, int distance) {
		return distance > 0 && world.getBlockState(pos.method_10074()).getBlock() != this;
	}
