...
public class ScaffoldingBlock extends Block implements Waterloggable {
	...
	public boolean canPlaceAt(BlockState blockState, ViewableWorld viewableWorld, BlockPos blockPos) {
		...
		if (direction == null) {
			...
			return blockState2.getBlock() != this || blockState2.get(DISTANCE) + 1 <= 7;
		}
	}
		...
		if (blockState.getBlock() == this) {
			...
			for(Direction direction2 : Direction.Type.HORIZONTAL) {
				...
				if (blockState2.getBlock() == this) {
					int j = blockState2.get(DISTANCE);
					if (j < i) {
						i = j;
	...
	private int method_16371(BlockView blockView, BlockPos blockPos) {
		...
		if (direction == null) {
			...
			return blockState.getBlock() == this ? Math.min(7, blockState.get(DISTANCE) + (direction.getAxis() == Direction.Axis.Y ? 0 : 1)) : 0;
		}
	}
