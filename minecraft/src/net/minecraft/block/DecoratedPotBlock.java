...
public class DecoratedPotBlock extends BlockWithEntity implements Waterloggable {
	...
	protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		if (itemStack instanceof DecoratedPotBlockEntity decoratedPotBlockEntity) {
			if (world.isClient) {
				...
				if (!stack.isEmpty()
					&& (itemStackx.isEmpty() || ItemStack.areItemsAndComponentsEqual(itemStackx, stack) && itemStackx.getCount() < itemStackx.getMaxCount())) {
					...
					if (decoratedPotBlockEntity.isEmpty()) {
						...
						itemStackx.increment(1);
						f = (float)itemStackx.getCount() / (float)itemStackx.getMaxCount();
					}
	...
	public ItemStack getPickStack(WorldView world, BlockPos pos, BlockState state) {
		...
		return var5 instanceof DecoratedPotBlockEntity decoratedPotBlockEntity ? decoratedPotBlockEntity.asStack() : super.getPickStack(world, pos, state);
	}
