...
public class BlockPatternBuilder {
	...
	private final Map<Character, Predicate<BlockProxy>> charMap = Maps.<Character, Predicate<BlockProxy>>newHashMap();
	private int height;
	private int width;
	...
	public BlockPatternBuilder where(char c, Predicate<BlockProxy> predicate) {
	...
	private Predicate<BlockProxy>[][][] bakePredicates() {
		this.validate();
		Predicate<BlockProxy>[][][] predicates = (Predicate<BlockProxy>[][][])Array.newInstance(
		...
		for (int i = 0; i < this.aisles.size(); i++) {
			for (int j = 0; j < this.height; j++) {
				for (int k = 0; k < this.width; k++) {
					predicates[i][j][k] = (Predicate<BlockProxy>)this.charMap.get(((String[])this.aisles.get(i))[j].charAt(k));
				}
			}
		}
	...
	private void validate() {
		...
		for (Entry<Character, Predicate<BlockProxy>> entry : this.charMap.entrySet()) {
