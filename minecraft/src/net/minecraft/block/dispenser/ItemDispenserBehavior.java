...
public class ItemDispenserBehavior implements DispenserBehavior {
	...
	public static void dispenseItem(World world, ItemStack itemStack, int i, Direction direction, Position position) {
		...
		itemEntity.velocityX += world.random.nextGaussian() * 0.0075F * (double)i;
		itemEntity.velocityY += world.random.nextGaussian() * 0.0075F * (double)i;
		itemEntity.velocityZ += world.random.nextGaussian() * 0.0075F * (double)i;
	...
	protected void spawnParticles(BlockPointer blockPointer, Direction direction) {
		blockPointer.getWorld().playEvent(2000, blockPointer.getBlockPos(), direction.getId());
	}
}
