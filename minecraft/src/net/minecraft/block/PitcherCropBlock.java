...
public class PitcherCropBlock extends TallPlantBlock implements Fertilizable, Crop {
	...
	public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
		return state.get(HALF) == DoubleBlockHalf.UPPER ? UPPER_OUTLINE_SHAPES[Math.abs(4 - (state.get(AGE) + 1))] : LOWER_OUTLINE_SHAPES[state.get(AGE)];
	}
	...
	public boolean isFertilizable(WorldView world, BlockPos pos, BlockState state, boolean isClient) {
		return !this.isFullyGrown(state)
			&& (state.get(AGE) < 2 || world instanceof ServerWorld serverWorld && canGrowAt(serverWorld, isLowerHalf(state) ? pos.up() : pos));
	}
