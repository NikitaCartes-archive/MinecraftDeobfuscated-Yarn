...
public class EndGatewayBlockEntity extends EndPortalBlockEntity implements Tickable {
	...
	private void createPortal() {
		...
		if (this.exitPortalPos == null) {
			...
			Feature.END_ISLAND
				.generate(this.world, this.world.getChunkManager().getChunkGenerator(), new Random(this.exitPortalPos.asLong()), this.exitPortalPos, FeatureConfig.DEFAULT);
		} else {
			...
			for(int j = -searchRadius; j <= searchRadius; ++j) {
				if (i != 0 || j != 0 || bl) {
					for(int k = 255; k > (blockPos == null ? 0 : blockPos.getY()); --k) {
						...
						if (blockState.method_21743(world, blockPos2) && (bl || blockState.getBlock() != Blocks.BEDROCK)) {
							blockPos = blockPos2;
							break;
						}
					}
	...
	private void createPortal(BlockPos blockPos) {
		Feature.END_GATEWAY
			.generate(this.world, this.world.getChunkManager().getChunkGenerator(), new Random(), blockPos, EndGatewayFeatureConfig.createConfig(this.getPos(), false));
	}
