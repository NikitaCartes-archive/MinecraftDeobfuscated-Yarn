...
public abstract class AbstractFurnaceBlockEntity extends LockableContainerBlockEntity implements SidedInventory, RecipeUnlocker, RecipeInputProvider, Tickable {
	...
	protected int getCookTime() {
		return this.world.getRecipeManager().method_8132(this.recipeType, this, this.world).map(AbstractCookingRecipe::getCookTime).orElse(200);
	}
	...
	public boolean canPlayerUseInv(PlayerEntity playerEntity) {
		if (this.world.method_8321(this.pos) != this) {
			return false;
		} else {
	...
	public void dropExperience(PlayerEntity playerEntity) {
		...
		for(Entry<Identifier, Integer> entry : this.recipesUsed.entrySet()) {
			playerEntity.field_6002.getRecipeManager().get((Identifier)entry.getKey()).ifPresent(recipe -> {
				list.add(recipe);
				dropExperience(playerEntity, entry.getValue(), ((AbstractCookingRecipe)recipe).getExperience());
			});
		}
