...
public class SculkCatalystBlockEntity extends BlockEntity implements GameEventListener {
	...
	private final SculkSpreadManager spreadManager = new SculkSpreadManager();
	...
	public boolean listen(World world, GameEvent event, @Nullable Entity sourceEntity, Vec3d pos) {
		if (event == GameEvent.ENTITY_DYING && !world.isClient() && sourceEntity instanceof LivingEntity livingEntity) {
	...
	public static void tick(World world, BlockPos pos, BlockState state, SculkCatalystBlockEntity blockEntity) {
		blockEntity.spreadManager.tick(world, pos, world.getRandom());
	}
