...
public class SignBlockEntity extends BlockEntity {
	...
	public boolean isPlayerFacingFront(PlayerEntity player) {
		Block d = this.getCachedState().getBlock();
		if (d instanceof AbstractSignBlock abstractSignBlock) {
			double dx = player.getX() - ((double)this.getPos().getX() + 0.5);
			double e = player.getZ() - ((double)this.getPos().getZ() + 0.5);
			float f = abstractSignBlock.getRotationDegrees(this.getCachedState());
			float g = (float)(MathHelper.atan2(e, dx) * 180.0F / (float)Math.PI) - 90.0F;
