...
public class SignBlockEntity extends BlockEntity {
	...
	public boolean isPlayerFacingFront(PlayerEntity player) {
		Block vec3d = this.getCachedState().getBlock();
		if (vec3d instanceof AbstractSignBlock abstractSignBlock) {
			Vec3d vec3dx = abstractSignBlock.getCenter(this.getCachedState());
			double d = player.getX() - ((double)this.getPos().getX() + vec3dx.x);
			double e = player.getZ() - ((double)this.getPos().getZ() + vec3dx.z);
