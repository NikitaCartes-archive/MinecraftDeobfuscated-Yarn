...
public class TrapdoorBlock extends HorizontalFacingBlock implements Waterloggable {
	...
	public boolean canPlaceAtSide(BlockState blockState, BlockView blockView, BlockPos blockPos, BlockPlacementEnvironment blockPlacementEnvironment) {
		switch(blockPlacementEnvironment) {
			case field_50:
				return blockState.get(field_11631);
			case field_48:
				return blockState.get(field_11626);
			case field_51:
				return blockState.get(field_11631);
			default:
				return false;
	...
	public boolean activate(BlockState blockState, World world, BlockPos blockPos, PlayerEntity playerEntity, Hand hand, BlockHitResult blockHitResult) {
		if (this.material == Material.METAL) {
			...
			if (blockState.get(field_11626)) {
				world.getFluidTickScheduler().schedule(blockPos, Fluids.WATER, Fluids.WATER.method_15789(world));
			}

			this.method_10740(playerEntity, world, blockPos, blockState.get(field_11631));
			return true;
		}
	}
	...
	public void neighborUpdate(BlockState blockState, World world, BlockPos blockPos, Block block, BlockPos blockPos2) {
		if (!world.isClient) {
			...
			if (bl != blockState.get(field_11629)) {
				if (blockState.get(field_11631) != bl) {
	...
	public BlockState getPlacementState(ItemPlacementContext itemPlacementContext) {
		...
		if (!itemPlacementContext.method_7717() && direction.getAxis().isHorizontal()) {
			blockState = blockState.with(field_11177, direction)
				.with(field_11625, itemPlacementContext.method_17698().y - (double)itemPlacementContext.getPos().getY() > 0.5 ? BlockHalf.TOP : BlockHalf.BOTTOM);
		} else {
