...
public class WitherSkullBlock extends SkullBlock {
			...
			if (result != null) {
				for(int i = 0; i < blockPattern.getWidth(); ++i) {
					for(int j = 0; j < blockPattern.getHeight(); ++j) {
						world.setBlockState(result.translate(i, j, 0).getPos(), Blocks.field_10124.getDefaultState(), 2);
					}
				}
				...
				for(int k = 0; k < blockPattern.getWidth(); ++k) {
					for(int l = 0; l < blockPattern.getHeight(); ++l) {
						world.updateNeighbors(result.translate(k, l, 0).getPos(), Blocks.field_10124);
					}
				}
			}
		}
	...
	public static boolean method_10899(World world, BlockPos blockPos, ItemStack itemStack) {
		if (itemStack.getItem() == Items.field_8791 && blockPos.getY() >= 2 && world.getDifficulty() != Difficulty.PEACEFUL && !world.isRemote) {
	...
	protected static BlockPattern getWitherBossPattern() {
		if (field_11765 == null) {
			field_11765 = BlockPatternBuilder.start()
				.aisle("^^^", "###", "~#~")
				.where('#', BlockProxy.method_11678(BlockStatePredicate.forBlock(Blocks.field_10114)))
				.where('^', BlockProxy.method_11678(BlockStatePredicate.forBlock(Blocks.field_10177).or(BlockStatePredicate.forBlock(Blocks.field_10101))))
