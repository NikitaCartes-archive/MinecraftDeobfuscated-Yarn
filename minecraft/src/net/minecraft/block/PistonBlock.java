...
public class PistonBlock extends FacingBlock {
		...
		if (type == 0) {
			...
			if (this.sticky) {
				...
				if (blockState3.isOf(Blocks.MOVING_PISTON)) {
					...
					if (blockEntity2 instanceof PistonBlockEntity pistonBlockEntity && pistonBlockEntity.getFacing() == direction && pistonBlockEntity.isExtending()) {
	...
	public boolean sticksTo(World world, BlockPos pos, BlockState state, BlockPos otherPos, BlockState otherState, Direction face, Direction otherFace) {
		return this.sticky && !state.get(EXTENDED) && state.get(FACING) == face && !otherState.getCullingFace(world, otherPos, face.getOpposite()).isEmpty();
	}
}
