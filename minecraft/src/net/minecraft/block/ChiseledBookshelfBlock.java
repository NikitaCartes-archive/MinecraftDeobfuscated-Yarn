...
public class ChiseledBookshelfBlock extends BlockWithEntity {
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		if (itemStack instanceof ChiseledBookshelfBlockEntity chiseledBookshelfBlockEntity) {
			if (world.isClient()) {
				return ActionResult.SUCCESS;
			} else {
				ItemStack itemStackx = player.getStackInHand(hand);
				return itemStackx.isIn(ItemTags.BOOKSHELF_BOOKS)
					? tryAddBook(state, world, pos, player, chiseledBookshelfBlockEntity, itemStackx)
