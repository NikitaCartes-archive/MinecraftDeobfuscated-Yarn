...
public class ChiseledBookshelfBlock extends BlockWithEntity {
	...
	public ChiseledBookshelfBlock(AbstractBlock.Settings settings) {
		super(settings);
		BlockState blockState = this.stateManager
			.getDefaultState()
			.with(HorizontalFacingBlock.FACING, Direction.NORTH)
			.with(Properties.LAST_INTERACTION_BOOK_SLOT, Integer.valueOf(0));
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		if (optional instanceof ChiseledBookshelfBlockEntity chiseledBookshelfBlockEntity) {
			Optional<Vec2f> optionalx = getHitPos(hit, state.get(HorizontalFacingBlock.FACING));
			if (optionalx.isEmpty()) {
				return ActionResult.PASS;
			} else if (world.isClient()) {
				return ActionResult.SUCCESS;
			} else {
				int i = getSlotForHitPos((Vec2f)optionalx.get());
