...
public class FungusBlock extends PlantBlock implements Fertilizable {
	...
	public boolean isFertilizable(WorldView world, BlockPos pos, BlockState state, boolean isClient) {
		...
		if (optional.isPresent()) {
			FeatureConfig block = ((ConfiguredFeature)((RegistryEntry)optional.get()).value()).config();
			if (block instanceof HugeFungusFeatureConfig hugeFungusFeatureConfig) {
				Block blockx = hugeFungusFeatureConfig.validBaseBlock.getBlock();
				BlockState blockState = world.getBlockState(pos.down());
				return blockState.isOf(blockx);
			}
		}
