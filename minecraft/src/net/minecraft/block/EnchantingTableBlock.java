...
public class EnchantingTableBlock extends BlockWithEntity {
				...
				if (random.nextInt(16) == 0) {
					for (int k = 0; k <= 1; k++) {
						...
						if (world.method_8320(blockPos2).getBlock() == Blocks.field_10504) {
							if (!world.method_8623(blockPos.add(i / 2, 0, j / 2))) {
								break;
							}
	...
	public NameableContainerProvider method_17454(BlockState blockState, World world, BlockPos blockPos) {
		...
		if (blockEntity instanceof EnchantingTableBlockEntity) {
			TextComponent textComponent = ((Nameable)blockEntity).method_5476();
			return new ClientDummyContainerProvider(
				(i, playerInventory, playerEntity) -> new EnchantingTableContainer(i, playerInventory, BlockContext.method_17392(world, blockPos)), textComponent
			);
		} else {
			return null;
		}
