...
public class CauldronBlock extends Block {
	...
	protected static final VoxelShape field_10746 = VoxelShapes.method_1072(VoxelShapes.fullCube(), field_10747, BooleanBiFunction.ONLY_FIRST);
	...
	public void onEntityCollision(BlockState blockState, World world, BlockPos blockPos, Entity entity) {
		int i = blockState.get(field_10745);
		float f = (float)blockPos.getY() + (6.0F + (float)(3 * i)) / 16.0F;
		if (!world.isRemote && entity.isOnFire() && i > 0 && entity.getBoundingBox().minY <= (double)f) {
			entity.extinguish();
		...
		if (itemStack.isEmpty()) {
			...
			if (item == Items.field_8705) {
				...
				if (i > 0 && item instanceof DyeableArmorItem) {
					...
					if (dyeableArmorItem.hasColor(itemStack) && !world.isRemote) {
				...
				if (i > 0 && item instanceof BannerItem) {
					if (BannerBlockEntity.getPatternCount(itemStack) > 0 && !world.isRemote) {
					...
					if (block instanceof ShulkerBoxBlock && !world.isRemote()) {
	...
	public void onRainTick(World world, BlockPos blockPos) {
		if (world.random.nextInt(20) == 1) {
			...
			if (!(f < 0.15F)) {
				...
				if (blockState.get(field_10745) < 3) {
	...
	public int getComparatorOutput(BlockState blockState, World world, BlockPos blockPos) {
		return blockState.get(field_10745);
	}
