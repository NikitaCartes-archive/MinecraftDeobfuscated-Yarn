...
public class CrafterBlock extends BlockWithEntity {
	...
	protected int getComparatorOutput(BlockState state, World world, BlockPos pos) {
		...
		return blockEntity instanceof CrafterBlockEntity crafterBlockEntity ? crafterBlockEntity.getComparatorOutput() : 0;
	}
	...
	protected void craft(BlockState state, ServerWorld world, BlockPos pos) {
		...
		if (optional instanceof CrafterBlockEntity crafterBlockEntity) {
			Optional<RecipeEntry<CraftingRecipe>> optionalx = getCraftingRecipe(world, crafterBlockEntity);
			if (optionalx.isEmpty()) {
				world.syncWorldEvent(WorldEvents.CRAFTER_FAILS, pos, 0);
			} else {
				RecipeEntry<CraftingRecipe> recipeEntry = (RecipeEntry)optionalx.get();
