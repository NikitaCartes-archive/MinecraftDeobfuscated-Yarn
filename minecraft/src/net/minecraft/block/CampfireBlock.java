...
public class CampfireBlock extends BlockWithEntity implements Waterloggable {
	...
	public void onEntityCollision(BlockState blockState, World world, BlockPos blockPos, Entity entity) {
		if (!entity.isFireImmune() && blockState.get(LIT) && entity instanceof LivingEntity && !EnchantmentHelper.hasFrostWalker((LivingEntity)entity)) {
	...
	public boolean tryFillWithFluid(IWorld iWorld, BlockPos blockPos, BlockState blockState, FluidState fluidState) {
		if (!blockState.get(Properties.WATERLOGGED) && fluidState.getFluid() == Fluids.WATER) {
			...
			if (bl) {
				if (iWorld.isClient()) {
					for(int i = 0; i < 20; ++i) {
						spawnSmokeParticle(iWorld.getWorld(), blockPos, blockState.get(SIGNAL_FIRE), true);
					}
				} else {
	...
	public void onProjectileHit(World world, BlockState blockState, BlockHitResult blockHitResult, Entity entity) {
		if (!world.isClient && entity instanceof ProjectileEntity) {
			...
			if (projectileEntity.isOnFire() && !blockState.get(LIT) && !blockState.get(WATERLOGGED)) {
