...
public class CampfireBlock extends BlockWithEntity implements Waterloggable {
	...
	public boolean tryFillWithFluid(IWorld world, BlockPos pos, BlockState state, FluidState fluidState) {
		if (!state.get(Properties.WATERLOGGED) && fluidState.getFluid() == Fluids.WATER) {
			...
			if (bl) {
				if (world.isClient()) {
					for(int i = 0; i < 20; ++i) {
						spawnSmokeParticle(world.getWorld(), pos, state.get(SIGNAL_FIRE), true);
					}
				} else {
	...
	public void onProjectileHit(World world, BlockState state, BlockHitResult hitResult, Entity entity) {
		if (!world.isClient) {
			...
			if (bl) {
				...
				if (bl2 && !state.get(LIT) && !state.get(WATERLOGGED)) {
	...
	private static boolean method_23896(BlockState blockState) {
		return blockState.getBlock() == Blocks.CAMPFIRE && blockState.get(LIT);
	}
