...
public class class_4838 {
	...
	protected static boolean method_24730(class_4836 arg, ItemStack itemStack) {
		...
		if (arg.method_24706() && !method_24714(arg) && (!method_24752(item) || !method_24789(arg))) {
			return field_22394.contains(item) || method_24735(item) || method_24744(arg, itemStack);
		} else {
			return false;
		}
	}
	...
	public static void method_24733(PlayerEntity playerEntity) {
		if (method_24757(playerEntity)) {
			...
			list.stream()
				.filter(class_4838::method_24766)
				.filter(arg -> LookTargetUtil.method_24565(arg, playerEntity))
				.forEach(arg -> method_24750(arg, playerEntity));
		}
	}
