...
public class class_860 implements RenderDebug.DebugRenderer {
		...
		if (this.field_4512 != null) {
			...
			for (Entry<ChunkPos, String> entry : this.field_4512.field_4515.entrySet()) {
				...
				for (String string2 : strings) {
					RenderDebug.method_3712(string2, (double)((chunkPos.x << 4) + 8), e + (double)i, (double)((chunkPos.z << 4) + 8), f, -1, 0.15F);
					i -= 2;
				}
			}
			...
			this.field_4514 = integratedServer.executeFuture(() -> {
				...
				for (int kx = i - 12; kx <= i + 12; kx++) {
					for (int lx = j - 12; lx <= j + 12; lx++) {
						...
						builderx.put(chunkPosx, "Server: " + serverChunkManager.method_17294(chunkPosx));
					}
				}
