...
public class class_8471 {
	...
	public void method_51071(UUID uUID, class_8367 arg) {
		...
		List<List<class_8471.class_8473>> list = (List)arg.options()
			.values()
			.stream()
			.filter(argx -> !argx.irregular())
			.map(
				argx -> {
					List<class_8471.class_8473> listxx = new ArrayList();
	...
	public static class class_8474 {
		...
		public void method_51078(int i, class_8375 arg) {
			((Map)this.field_44433.computeIfAbsent(i, (Int2ObjectFunction)(ix -> new HashMap()))).putAll(arg.voters());
		}
		...
		private int method_51084(UUID uUID, class_8373 arg) {
			Map<UUID, class_8388> map = (Map)this.field_44433.get(arg.index());
			if (map == null) {
				return 0;
			} else {
