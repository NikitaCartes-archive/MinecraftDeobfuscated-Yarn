...
public class class_869 implements class_863.class_864 {
	...
	public void method_3870(long l, class_2338 arg) {
		Map<class_2338, Integer> map = (Map)this.field_4623.get(l);
		if (map == null) {
			map = Maps.newHashMap();
	...
	public void method_3715(long l) {
		...
		Map<class_2338, Integer> map = Maps.newHashMap();
		Iterator<Entry<Long, Map<class_2338, Integer>>> iterator = this.field_4623.entrySet().iterator();

		while(iterator.hasNext()) {
			Entry<Long, Map<class_2338, Integer>> entry = (Entry)iterator.next();
			Long long_ = (Long)entry.getKey();
			Map<class_2338, Integer> map2 = (Map)entry.getValue();
			long n = m - long_;
			if (n > 200L) {
				iterator.remove();
			} else {
