...
public class class_4157 implements class_4213 {
	...
	private final Map<class_4158, Set<class_4156>> field_18498 = Maps.newHashMap();
	...
	public <T> class_4157(Runnable runnable, Dynamic<T> dynamic) {
		...
		try {
			...
			field_18496.error("Failed to load POI chunk", var4);
			this.method_20395();
	...
	public <T> T method_19508(DynamicOps<T> dynamicOps) {
		T object = dynamicOps.createList(this.field_18497.values().stream().map(arg -> arg.<T>method_19508(dynamicOps)));
