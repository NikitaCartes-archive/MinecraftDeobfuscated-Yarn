...
public class class_2861 implements AutoCloseable {
	...
	public class_2861(File file) throws IOException {
		...
		this.field_13030 = Lists.newArrayListWithCapacity(i);
		...
		for(int j = 0; j < 1024; ++j) {
			int k = this.field_13025.readInt();
			this.field_13026[j] = k;
		}
	}

	@Nullable
	public synchronized DataInputStream method_12421(class_1923 arg) {
		try {
	...
	protected synchronized void method_12428(class_1923 arg, byte[] bs, int i) {
		try {
			...
			if (k != 0 && l == m) {
				...
				if (n != -1) {
					for(int p = n; p < this.field_13030.size(); ++p) {
						if (o != 0) {
							if (this.field_13030.get(p)) {
								++o;
							} else {
								o = 0;
							}
						} else if (this.field_13030.get(p)) {
							n = p;
							o = 1;
						}
