...
public class class_8307 extends class_8285<Block> {
	...
	public Stream<class_8291> method_50118(MinecraftServer minecraftServer, Random random, int i) {
		...
		for(int j = 0; j < 30; ++j) {
			RegistryEntry.Reference<Block> reference = (RegistryEntry.Reference)registry.getRandom(random).orElse(null);
			RegistryEntry.Reference<Block> reference2 = (RegistryEntry.Reference)registry.getRandom(random).orElse(null);
			if (reference != null
	...
	private static <T extends Comparable<T>> BlockState method_50312(
		...
		try {
			Property<T> property = stateManager.getProperty(string);
			Property<T> property2 = stateManager2.getProperty(string);
	...
	static record class_8308(RegistryEntry.Reference<Block> source, RegistryEntry.Reference<Block> target) {
		final RegistryEntry.Reference<Block> target;
	}
