...
public class BlockLeakParticle extends Particle {
	...
	public void update() {
		...
		if (this.onGround) {
			if (this.field_3789.matches(FluidTags.field_15517)) {
				this.markDead();
				this.world.method_8406(ParticleTypes.field_11202, this.posX, this.posY, this.posZ, 0.0, 0.0, 0.0);
			} else {
		...
		if (fluidState.getFluid() == this.field_3789) {
			double d = (double)((float)MathHelper.floor(this.posY) + fluidState.method_15763());
			if (this.posY < d) {
				this.markDead();
			}
		}
