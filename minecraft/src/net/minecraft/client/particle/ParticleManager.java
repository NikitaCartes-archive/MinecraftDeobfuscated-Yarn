			...
			try {
				...
				try {
					...
					if (list == null) {
						...
						result.put(
							id,
							(List)list.stream().map(identifierx -> new Identifier(identifierx.getNamespace(), "particle/" + identifierx.getPath())).collect(Collectors.toList())
						);
					}
			...
			if (item != null) {
				...
				if (!itemStack.isEmpty()) {
					this.method_42998(
						...
						voxelShape,
						0.25,
						(clientWorld, dx, e, f, g, h, i, blockPosx) -> new CrackParticle(clientWorld, dx, e, f, g * 0.1, h * 0.1, i * 0.1, itemStack)
					);
					return;
				}
			}
