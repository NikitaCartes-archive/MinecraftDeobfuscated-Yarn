...
public class JsonEffectGlShader implements GlShader, AutoCloseable {
	...
	public JsonEffectGlShader(ResourceManager resource, String name) throws IOException {
		...
		try {
			...
			this.vertexShader = getShader(resource, Program.Type.VERTEX, string);
			this.fragmentShader = getShader(resource, Program.Type.FRAGMENT, string2);
	...
	public static Program getShader(ResourceManager resourceManager, Program.Type type, String name) throws IOException {
		Program program = (Program)type.getLoadedShaders().get(name);
		if (program == null) {
