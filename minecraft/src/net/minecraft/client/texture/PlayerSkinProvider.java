...
public class PlayerSkinProvider {
	...
	public CompletableFuture<SkinTextures> fetchSkinTextures(GameProfile profile) {
		return (CompletableFuture<SkinTextures>)this.cache.getUnchecked(new PlayerSkinProvider.Key(profile));
	}
	...
	static record Key(GameProfile profile) {
		public boolean equals(Object o) {
			if (!(o instanceof PlayerSkinProvider.Key)) {
				return false;
			} else {
				PlayerSkinProvider.Key key = (PlayerSkinProvider.Key)o;
				return this.profile.getId().equals(key.profile.getId()) && Objects.equals(this.getTextureEntry(), key.getTextureEntry());
			}
		}
