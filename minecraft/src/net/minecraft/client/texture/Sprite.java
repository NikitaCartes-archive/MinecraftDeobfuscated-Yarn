...
public class Sprite implements AutoCloseable {
	...
	protected Sprite(
		SpriteAtlasTexture spriteAtlasTexture, Sprite.Info info, int maxLevel, int atlasWidth, int atlasHeight, int x, int y, NativeImage nativeImage
	) {
		...
		if (animationResourceMetadata.getFrameCount() > 0) {
			int m = animationResourceMetadata.getFrameIndexSet().stream().max(Integer::compareTo).get() + 1;
