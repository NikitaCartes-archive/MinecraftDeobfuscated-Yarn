...
public class PlayerSkinTexture extends ResourceTexture {
				...
				this.field_20844 = CompletableFuture.runAsync(() -> {
					...
					try {
						...
						if (httpURLConnection.getResponseCode() / 100 == 2) {
							...
							MinecraftClient.getInstance().execute(() -> {
								NativeImage nativeImagexx = this.method_22795(inputStream);
								if (nativeImagexx != null) {
									this.method_4534(nativeImagexx);
								}
							});
							return;
						}
	...
	private static void method_22794(NativeImage nativeImage, int i, int j, int k, int l) {
		...
		for(int m = i; m < k; ++m) {
			for(int n = j; n < l; ++n) {
				nativeImage.setPixelRgba(m, n, nativeImage.getPixelRgba(m, n) & 16777215);
			}
		}
	}
