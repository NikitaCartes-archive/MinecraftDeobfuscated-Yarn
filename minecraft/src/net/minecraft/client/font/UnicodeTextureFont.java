	...
	public UnicodeTextureFont(ResourceManager resourceManager, byte[] sizes, String template) {
		...
		resourceManager.findResources(string, set::contains).forEach((id, resource) -> map.put(id, CompletableFuture.supplyAsync(() -> {
				try {
					...
					NativeImage var3x;
					try {
						var3x = NativeImage.read(NativeImage.Format.RGBA, inputStream);
		...
		for(int k = 0; k < 256; ++k) {
			...
			CompletableFuture<NativeImage> completableFuture = (CompletableFuture)map.get(identifier);
			if (completableFuture != null) {
				list.add(completableFuture.thenAcceptAsync(image -> {
					if (image != null) {
						if (image.getWidth() == 256 && image.getHeight() == 256) {
							for(int kxx = 0; kxx < 256; ++kxx) {
								byte b = sizes[l + kxx];
		...
		CompletableFuture.allOf((CompletableFuture[])list.toArray(i -> new CompletableFuture[i])).join();
	}

	private static String getCommonPath(Set<Identifier> ids) {
		String string = StringUtils.getCommonPrefix((String[])ids.stream().map(Identifier::getPath).toArray(ix -> new String[ix]));
