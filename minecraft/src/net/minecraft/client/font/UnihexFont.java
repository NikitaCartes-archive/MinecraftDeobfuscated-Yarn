...
public class UnihexFont implements Font {
	...
	public static class Loader implements FontLoader {
		...
		private UnihexFont loadHexFile(InputStream stream) throws IOException {
			GlyphContainer<UnihexFont.BitmapGlyph> glyphContainer = new GlyphContainer<>(
				ix -> new UnihexFont.BitmapGlyph[ix], rows -> new UnihexFont.BitmapGlyph[rows][]
			);
			...
			try {
				...
				GlyphContainer<UnihexFont.UnicodeTextureGlyph> glyphContainer2 = new GlyphContainer<>(
					ix -> new UnihexFont.UnicodeTextureGlyph[ix], ix -> new UnihexFont.UnicodeTextureGlyph[ix][]
				);
				...
				glyphContainer.forEachGlyph((codePoint, glyph) -> {
					int ixx = glyph.getPackedDimensions();
					int jxx = UnihexFont.Dimensions.getLeft(ixx);
					int kxx = UnihexFont.Dimensions.getRight(ixx);
					glyphContainer2.put(codePoint, new UnihexFont.UnicodeTextureGlyph(glyph, jxx, kxx));
				});
