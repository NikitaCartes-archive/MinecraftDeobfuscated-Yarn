...
public class FontStorage implements AutoCloseable {
	...
	private final GlyphContainer<FontStorage.GlyphPair> glyphCache = new GlyphContainer(
		i -> new FontStorage.GlyphPair[i], rowCount -> new FontStorage.GlyphPair[rowCount][]
	);
	...
	public void setFonts(List<Font> fonts) {
		...
		intSet.forEach(
			codePoint -> {
				for(Font fontxx : fonts) {
					Glyph glyph = fontxx.getGlyph(codePoint);
					if (glyph != null) {
						set.add(fontxx);
	...
	public Glyph getGlyph(int codePoint, boolean validateAdvance) {
		return ((FontStorage.GlyphPair)this.glyphCache.computeIfAbsent(codePoint, this::findGlyph)).getGlyph(validateAdvance);
	}
