...
public class FontManager implements AutoCloseable {
	...
	private final ResourceReloader resourceReloadListener = new SinglePreparationResourceReloader<Map<Identifier, List<Font>>>() {
		protected Map<Identifier, List<Font>> prepare(ResourceManager resourceManager, Profiler profiler) {
			...
			for(Entry<Identifier, List<Resource>> entry : FontManager.FINDER.findAllResources(resourceManager).entrySet()) {
				...
				List<Font> list = (List)map.computeIfAbsent(identifier2, id -> Lists.<Font>newArrayList(new BlankFont()));
				...
				intSet.forEach(codePoint -> {
					if (codePoint != 32) {
						for(Font fontxx : Lists.reverse(list)) {
	...
	public TextRenderer createAdvanceValidatingTextRenderer() {
		return new TextRenderer(id -> (FontStorage)this.fontStorages.getOrDefault(this.idOverrides.getOrDefault(id, id), this.missingStorage), true);
	}
