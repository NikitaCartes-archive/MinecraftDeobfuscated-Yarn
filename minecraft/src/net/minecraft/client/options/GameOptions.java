...
public class GameOptions {
	...
	public KeyBinding keyForward = new KeyBinding("key.forward", 87, "key.categories.movement");
	public KeyBinding keyLeft = new KeyBinding("key.left", 65, "key.categories.movement");
	public KeyBinding keyBack = new KeyBinding("key.back", 83, "key.categories.movement");
	public KeyBinding keyRight = new KeyBinding("key.right", 68, "key.categories.movement");
	public KeyBinding keyJump = new KeyBinding("key.jump", 32, "key.categories.movement");
	public KeyBinding keySneak = new KeyBinding("key.sneak", 340, "key.categories.movement");
	public KeyBinding keySprint = new KeyBinding("key.sprint", 341, "key.categories.movement");
	public KeyBinding keyInventory = new KeyBinding("key.inventory", 69, "key.categories.inventory");
	public KeyBinding keySwapHands = new KeyBinding("key.swapHands", 70, "key.categories.inventory");
	public KeyBinding keyDrop = new KeyBinding("key.drop", 81, "key.categories.inventory");
	public KeyBinding keyUse = new KeyBinding("key.use", InputUtil.Type.field_1672, 1, "key.categories.gameplay");
	public KeyBinding keyAttack = new KeyBinding("key.attack", InputUtil.Type.field_1672, 0, "key.categories.gameplay");
	public KeyBinding keyPickItem = new KeyBinding("key.pickItem", InputUtil.Type.field_1672, 2, "key.categories.gameplay");
	public KeyBinding keyChat = new KeyBinding("key.chat", 84, "key.categories.multiplayer");
	public KeyBinding keyPlayerList = new KeyBinding("key.playerlist", 258, "key.categories.multiplayer");
	public KeyBinding keyCommand = new KeyBinding("key.command", 47, "key.categories.multiplayer");
	public KeyBinding keyScreenshot = new KeyBinding("key.screenshot", 291, "key.categories.misc");
	public KeyBinding keyTogglePerspective = new KeyBinding("key.togglePerspective", 294, "key.categories.misc");
	public KeyBinding keySmoothCamera = new KeyBinding("key.smoothCamera", InputUtil.UNKNOWN_KEYCODE.getKeyCode(), "key.categories.misc");
	public KeyBinding keyFullscreen = new KeyBinding("key.fullscreen", 300, "key.categories.misc");
	public KeyBinding keySpectatorOutlines = new KeyBinding("key.spectatorOutlines", InputUtil.UNKNOWN_KEYCODE.getKeyCode(), "key.categories.misc");
	public KeyBinding keyAdvancements = new KeyBinding("key.advancements", 76, "key.categories.misc");
	public KeyBinding[] keysHotbar = new KeyBinding[]{
	...
	public KeyBinding keySaveToolbarActivator = new KeyBinding("key.saveToolbarActivator", 67, "key.categories.creative");
	public KeyBinding keyLoadToolbarActivator = new KeyBinding("key.loadToolbarActivator", 88, "key.categories.creative");
	public KeyBinding[] keysAll = ArrayUtils.addAll(
		new KeyBinding[]{
			this.keyAttack,
			this.keyUse,
			this.keyForward,
	...
	public void method_1625(GameOptions.Option option, double d) {
		...
		if (option == GameOptions.Option.CHAT_OPACITY) {
			this.chatOpacity = d;
			this.client.hudInGame.getHudChat().reset();
		}
	...
	public void updateOption(GameOptions.Option option, int i) {
		...
		if (option == GameOptions.Option.GUI_SCALE) {
			this.guiScale = Integer.remainderUnsigned(this.guiScale + i, this.client.window.method_4476(0, this.client.forcesUnicodeFont()) + 1);
		}
	...
	public float getSoundVolume(SoundCategory soundCategory) {
		return this.soundVolumeLevels.containsKey(soundCategory) ? this.soundVolumeLevels.get(soundCategory) : 1.0F;
	}
	...
	public void method_1627(ResourcePackContainerManager<ClientResourcePackContainer> resourcePackContainerManager) {
		...
		resourcePackContainerManager.resetEnabled(set);
	}
