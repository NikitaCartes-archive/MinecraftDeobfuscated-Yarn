...
public class ChatAbuseReport {
		...
		this.selections.forEach(i -> {
			...
			for(UUID uUID : set) {
				this.streamHeadersFrom(log, int2ObjectMap, uUID).forEach(entry -> {
					...
					if (headerEntry instanceof ReceivedMessage.ChatMessage chatMessagexx) {
						int2ObjectSortedMap.putIfAbsent(entry.index(), this.toReportChatMessage(entry.index(), chatMessagexx));
					} else {
	...
	private static IntCollection collectIndicesUntilLastSeen(ChatLog log, int selectedIndex, SignedMessage message) {
		Set<MessageSignatureData> set = (Set)message.signedBody()
			.lastSeenMessages()
			.entries()
			.stream()
