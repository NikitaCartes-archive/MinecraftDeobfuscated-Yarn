...
public final class SimpleOption<T> {
	...
	public static final SimpleOption.PotentialValuesBasedCallbacks<Boolean> BOOLEAN = new SimpleOption.PotentialValuesBasedCallbacks(
		ImmutableList.of((T)Boolean.TRUE, (T)Boolean.FALSE), Codec.BOOL
	);
	...
	public static SimpleOption<Boolean> ofBoolean(
		...
		return new SimpleOption(key, tooltipFactoryGetter, (optionText, value) -> value ? ScreenTexts.ON : ScreenTexts.OFF, BOOLEAN, (T)defaultValue, changeCallback);
	}
	...
	public ClickableWidget createButton(GameOptions options, int x, int y, int width) {
		SimpleOption.TooltipFactory<T> tooltipFactory = (SimpleOption.TooltipFactory)this.tooltipFactoryGetter.apply(MinecraftClient.getInstance());
	...
	interface IntSliderCallbacks extends SimpleOption.SliderCallbacks<Integer> {
		...
		default <R> SimpleOption.SliderCallbacks<R> withModifier(
			...
			return new SimpleOption.SliderCallbacks<R>() {
				@Override
				public Optional<R> validate(R value) {
					return IntSliderCallbacks.this.validate((T)Integer.valueOf(valueToSliderProgressValue.applyAsInt(value))).map(sliderProgressValueToValue::apply);
				}
	...
	interface TypeChangeableCallbacks<T> extends SimpleOption.CyclingCallbacks<T>, SimpleOption.SliderCallbacks<T> {
		...
		default Function<SimpleOption<T>, ClickableWidget> getButtonCreator(
			...
			return this.isCycling()
				? SimpleOption.CyclingCallbacks.super.getButtonCreator(tooltipFactory, gameOptions, x, y, width)
				: SimpleOption.SliderCallbacks.super.getButtonCreator(tooltipFactory, gameOptions, x, y, width);
		}
	}
