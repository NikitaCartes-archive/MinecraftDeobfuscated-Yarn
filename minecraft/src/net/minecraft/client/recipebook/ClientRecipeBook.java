...
public class ClientRecipeBook extends RecipeBook {
	...
	public void reload(Iterable<Recipe<?>> iterable) {
		...
		map.forEach((recipeBookGroup, list) -> list.stream().map(RecipeResultCollection::new).peek(builder::add));
	...
	private static Map<RecipeBookGroup, List<List<Recipe<?>>>> method_30283(Iterable<Recipe<?>> iterable) {
		...
		for(Recipe<?> recipe : iterable) {
			if (!recipe.isIgnoredInRecipeBook()) {
				...
				if (string.isEmpty()) {
					((List)map.computeIfAbsent(recipeBookGroup, recipeBookGroupx -> Lists.newArrayList())).add(ImmutableList.<Recipe<?>>of(recipe));
				} else {
