...
public class LoomGui extends ContainerGui implements InventoryListener {
	...
	private static final ArrayList<DyeColor> field_2959 = Lists.newArrayList(field_2964, field_2956);
	private final PlayerInventory field_2960;
	private final LoomContainer field_2971;
	...
	public LoomGui(PlayerInventory playerInventory) {
		super(new LoomContainer(playerInventory, null));
		this.field_2960 = playerInventory;
		this.field_2971 = (LoomContainer)this.container;
		((BasicInventory)this.field_2971.inv).addListener(this);
	}
	...
	public void update() {
		...
		if (this.field_2969 < BannerPattern.COUNT) {
			...
			this.field_2972[this.field_2969] = TextureCache.BANNER.get(string + string2, Lists.newArrayList(BannerPattern.BASE, bannerPattern), field_2959);
			++this.field_2969;
		}
	}
	...
	protected void drawBackground(float f, int i, int j) {
		...
		Slot slot = this.field_2971.getSlot(0);
		Slot slot2 = this.field_2971.getSlot(1);
		Slot slot3 = this.field_2971.getSlot(2);
		Slot slot4 = this.field_2971.getSlot(3);
		if (!slot.hasStack()) {
		...
		if (this.field_2965) {
			...
			for(int q = this.field_2970; q < p && q < this.field_2972.length - 4; ++q) {
				...
				if (q == this.field_2971.method_7647()) {
					u += 14;
			...
			int p = this.field_2971.method_7647();
	...
	public boolean mouseClicked(double d, double e, int i) {
		...
		if (this.field_2965) {
			...
			for(int m = this.field_2970; m < l; ++m) {
				...
				if (f >= 0.0 && g >= 0.0 && f < 14.0 && g < 14.0 && this.field_2971.onButtonClick(this.client.player, m)) {
					MinecraftClient.getInstance().getSoundLoader().play(PositionedSoundInstance.master(SoundEvents.field_14920, 1.0F));
					this.client.interactionManager.clickButton(this.field_2971.syncId, m);
					return true;
				}
			}
	...
	public void onInvChange(Inventory inventory) {
		ItemStack itemStack = inventory.getInvStack(3);
		...
		ItemStack itemStack2 = inventory.getInvStack(0);
		ItemStack itemStack3 = inventory.getInvStack(1);
		ItemStack itemStack4 = inventory.getInvStack(2);
