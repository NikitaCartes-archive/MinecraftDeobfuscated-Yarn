...
public class BeaconGui extends ContainerGui<BeaconContainer> {
	...
	public BeaconGui(BeaconContainer beaconContainer, PlayerInventory playerInventory, TextComponent textComponent) {
		...
		beaconContainer.addListener(new ContainerListener() {
			...
			public void onContainerPropertyUpdate(Container container, int i, int j) {
				BeaconGui.this.field_17412 = beaconContainer.method_17374();
				BeaconGui.this.field_17413 = beaconContainer.method_17375();
				BeaconGui.this.canConsumeGem = true;
			}
		});
	}
	...
	public void update() {
		...
		if (this.canConsumeGem && i >= 0) {
			...
			for(int n = 0; n < l - 1; ++n) {
				StatusEffect statusEffect = BeaconBlockEntity.EFFECTS_BY_LEVEL[3][n];
				BeaconGui.WidgetButtonIconEffect widgetButtonIconEffect = new BeaconGui.WidgetButtonIconEffect(
					j++, this.left + 167 + n * 24 - m / 2, this.top + 47, statusEffect, false
				);
				this.addButton(widgetButtonIconEffect);
				if (3 >= i) {
					widgetButtonIconEffect.enabled = false;
	...
	class WidgetButtonIconDone extends BeaconGui.WidgetButtonIcon {
		...
		public void onPressed(double d, double e) {
			BeaconGui.this.client
				.getNetworkHandler()
				.sendPacket(new UpdateBeaconServerPacket(StatusEffect.getRawId(BeaconGui.this.field_17412), StatusEffect.getRawId(BeaconGui.this.field_17413)));
			BeaconGui.this.client.player.networkHandler.sendPacket(new GuiCloseServerPacket(BeaconGui.this.client.player.container.syncId));
			BeaconGui.this.client.openGui(null);
		}
	...
	class WidgetButtonIconEffect extends BeaconGui.WidgetButtonIcon {
		...
		public void onPressed(double d, double e) {
			if (!this.isDisabled()) {
				if (this.field_17416) {
					BeaconGui.this.field_17412 = this.effect;
				} else {
					BeaconGui.this.field_17413 = this.effect;
				}
