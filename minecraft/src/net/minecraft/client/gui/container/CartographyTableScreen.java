...
public class CartographyTableScreen extends ContainerScreen<CartographyTableContainer> {
	private static final Identifier field_17421 = new Identifier("textures/gui/container/cartography_table.png");
	...
	protected void drawBackground(float f, int i, int j) {
		...
		this.client.method_1531().method_4618(field_17421);
		int k = this.left;
		int l = this.top;
		this.drawTexturedRect(k, l, 0, 0, this.width, this.height);
		Item item = this.container.method_7611(1).method_7677().getItem();
		...
		if (itemStack.getItem() == Items.field_8204) {
			mapState = FilledMapItem.method_7997(itemStack, this.client.field_1687);
			if (mapState != null) {
	...
	private void drawMap(@Nullable MapState mapState, boolean bl, boolean bl2, boolean bl3, boolean bl4) {
		...
		int j = this.top;
		if (bl2 && !bl4) {
			this.drawTexturedRect(i + 67, j + 13, this.width, 66, 66, 66);
			this.drawMap(mapState, i + 85, j + 31, 0.226F);
		} else if (bl) {
			this.drawTexturedRect(i + 67 + 16, j + 13, this.width, 132, 50, 66);
			this.drawMap(mapState, i + 86, j + 16, 0.34F);
			this.client.method_1531().method_4618(field_17421);
			GlStateManager.pushMatrix();
			GlStateManager.translatef(0.0F, 0.0F, 1.0F);
			this.drawTexturedRect(i + 67, j + 13 + 16, this.width, 132, 50, 66);
			this.drawMap(mapState, i + 70, j + 32, 0.34F);
			GlStateManager.popMatrix();
		} else if (bl3) {
			this.drawTexturedRect(i + 67, j + 13, this.width, 0, 66, 66);
			this.drawMap(mapState, i + 71, j + 17, 0.45F);
			this.client.method_1531().method_4618(field_17421);
			GlStateManager.pushMatrix();
			GlStateManager.translatef(0.0F, 0.0F, 1.0F);
			this.drawTexturedRect(i + 66, j + 12, 0, this.height, 66, 66);
			GlStateManager.popMatrix();
		} else {
			this.drawTexturedRect(i + 67, j + 13, this.width, 0, 66, 66);
			this.drawMap(mapState, i + 71, j + 17, 0.45F);
