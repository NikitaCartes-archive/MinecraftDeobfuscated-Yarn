...
public abstract class ContainerGui<T extends Container> extends Gui implements ContainerProvider<T> {
	...
	public ContainerGui(T container, PlayerInventory playerInventory, TextComponent textComponent) {
	...
	private void drawItem(ItemStack itemStack, int i, int j, String string) {
		...
		this.itemRenderer.renderItemAndGlowInGui(itemStack, i, j);
		this.itemRenderer.renderItemOverlaysInGUIWithText(this.fontRenderer, itemStack, i, j - (this.field_2782.isEmpty() ? 0 : 8), string);
		this.zOffset = 0.0F;
	...
	private void drawSlot(Slot slot) {
		...
		if (!bl2) {
			...
			GlStateManager.enableDepthTest();
			this.itemRenderer.renderItemInGui(this.client.player, itemStack, i, j);
			this.itemRenderer.renderItemOverlaysInGUIWithText(this.fontRenderer, itemStack, i, j, string);
		}
		...
		if (!itemStack.isEmpty() && this.field_2794) {
			if (this.field_2790 == 2) {
				...
				for(Slot slot : this.slots) {
					...
					this.field_2803 -= itemStack2.getAmount() - i;
				}
			}
		}
	}
		...
		if (this.field_2783 && slot != null && i == 0 && this.container.method_7613(ItemStack.EMPTY, slot)) {
			...
			if (this.field_2777 != null && this.client.options.touchscreen) {
				...
				for(Slot slot2 : this.slots) {
					this.onMouseClick(slot2, slot2.id, Container.method_7591(1, this.field_2790), SlotActionType.field_7789);
				}
