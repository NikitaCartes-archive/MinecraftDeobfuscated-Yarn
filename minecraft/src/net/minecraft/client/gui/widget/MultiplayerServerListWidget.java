...
public class MultiplayerServerListWidget extends AlwaysSelectedItemListWidget<MultiplayerServerListWidget.Entry> {
	...
	@Environment(EnvType.CLIENT)
	public abstract static class Entry extends AlwaysSelectedItemListWidget.class_4281<MultiplayerServerListWidget.Entry> {
	}
	...
	public class ServerItem extends MultiplayerServerListWidget.Entry {
		...
		public void render(int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
			...
			} else if (this.server.field_3754 && this.server.ping != -2L) {
			...
			if (t >= l - 15 && t <= l - 5 && u >= 0 && u <= 8) {
				this.screen.method_2528(string3);
			} else if (t >= l - q - 15 - 2 && t <= l - 15 - 2 && u >= 0 && u <= 8) {
				this.screen.method_2528(string2);
			}
		...
		public boolean mouseClicked(double d, double e, int i) {
			...
			if (f <= 32.0) {
				...
				if (f < 16.0 && g < 16.0 && j > 0) {
					...
					this.screen.serverListWidget.method_20125(this.screen.method_2529());
					return true;
				}
