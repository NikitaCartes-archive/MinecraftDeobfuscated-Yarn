...
public class TextFieldWidget extends Drawable implements GuiEventListener {
	...
	public void render(int i, int j, float f) {
		if (this.isVisible()) {
			...
			if (bl2) {
				if (bl3) {
					Drawable.drawRect(q, o - 1, q + 1, o + 1 + this.fontRenderer.fontHeight, -3092272);
				} else {
			...
			if (m != l) {
				...
				this.method_1886(q, o - 1, r - 1, o + 1 + this.fontRenderer.fontHeight);
			}
		}
	}
	...
	public void method_1884(int i) {
		...
		if (this.fontRenderer != null) {
			...
			if (this.cursorMin == this.field_2103) {
				this.field_2103 -= this.fontRenderer.method_1711(this.text, k, true).length();
			}
