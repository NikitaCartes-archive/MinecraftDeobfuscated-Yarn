...
public class ScoreboardHud extends DrawableHelper {
	private static final Ordering<PlayerListEntry> ENTRY_ORDERING = Ordering.from(new ScoreboardHud.EntryOrderComparator());
	...
	public TextComponent method_1918(PlayerListEntry playerListEntry) {
		return playerListEntry.getDisplayName() != null
			...
			: Team.method_1142(playerListEntry.getScoreboardTeam(), new StringTextComponent(playerListEntry.getProfile().getName()));
	}
	...
	public void method_1919(int i, Scoreboard scoreboard, @Nullable ScoreboardObjective scoreboardObjective) {
		...
		List<PlayerListEntry> list = ENTRY_ORDERING.sortedCopy(clientPlayNetworkHandler.getScoreboardEntries());
		int j = 0;
		int k = 0;
		...
		for (PlayerListEntry playerListEntry : list) {
			...
			j = Math.max(j, l);
			if (scoreboardObjective != null && scoreboardObjective.method_1118() != ScoreboardCriterion.RenderType.HEARTS) {
		...
		if (scoreboardObjective != null) {
			if (scoreboardObjective.method_1118() == ScoreboardCriterion.RenderType.HEARTS) {
				o = 90;
			} else {
				o = k;
			}
		...
		if (this.field_2153 != null) {
			list2 = this.client.textRenderer.wrapStringToWidthAsList(this.field_2153.getFormattedText(), i - 50);
		...
		if (this.field_2154 != null) {
			list3 = this.client.textRenderer.wrapStringToWidthAsList(this.field_2154.getFormattedText(), i - 50);
	...
	private void method_1922(ScoreboardObjective scoreboardObjective, int i, String string, int j, int k, PlayerListEntry playerListEntry) {
		...
		if (scoreboardObjective.method_1118() == ScoreboardCriterion.RenderType.HEARTS) {
			...
			if (this.field_2152 == playerListEntry.method_2976()) {
			...
			if (m - playerListEntry.method_2974() > 1000L || this.field_2152 != playerListEntry.method_2976()) {
				playerListEntry.method_2972(l);
				playerListEntry.method_2965(l);
				playerListEntry.method_2978(m);
			}

			playerListEntry.method_2964(this.field_2152);
			playerListEntry.method_2972(l);
