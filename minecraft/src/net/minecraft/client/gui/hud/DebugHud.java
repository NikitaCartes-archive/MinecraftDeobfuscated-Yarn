...
public class DebugHud extends DrawableHelper {
		...
		if (this.client.hasReducedDebugInfo()) {
			...
			if (serverWorld != null) {
				...
				if (info != null) {
					...
					int k = info.getSpawningChunkCount();
					list.add(
						"SC: "
							+ k
							+ ", "
	...
	private World getWorld() {
		return DataFixUtils.orElse(
			Optional.ofNullable(this.client.getServer())
				.flatMap(integratedServer -> Optional.ofNullable(integratedServer.getWorld(this.client.world.getRegistryKey()))),
			this.client.world
		);
	}
