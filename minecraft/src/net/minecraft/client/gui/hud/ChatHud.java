		...
		if (!this.isChatHidden()) {
			...
			if (j > 0) {
				...
				for(int n = 0; n + this.scrolledLines < this.visibleMessages.size() && n < i; ++n) {
					ChatHudLine<OrderedText> chatHudLine = (ChatHudLine)this.visibleMessages.get(n + this.scrolledLines);
					if (chatHudLine != null) {
				...
				if (!this.messageQueue.isEmpty()) {
					...
					RenderSystem.enableBlend();
					matrices.translate(0.0, 0.0, 50.0);
					this.client.textRenderer.drawWithShadow(matrices, new TranslatableText("chat.queue", this.messageQueue.size()), 0.0F, 1.0F, 16777215 + (n << 24));
					matrices.pop();
					RenderSystem.disableBlend();
				}
