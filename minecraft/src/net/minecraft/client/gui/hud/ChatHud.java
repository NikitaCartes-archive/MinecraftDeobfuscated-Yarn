		...
		if (!this.isChatHidden()) {
			...
			if (j > 0) {
				...
				for(int m = 0; m + this.scrolledLines < this.visibleMessages.size() && m < i; ++m) {
					ChatHudLine<class_5481> chatHudLine = (ChatHudLine)this.visibleMessages.get(m + this.scrolledLines);
					if (chatHudLine != null) {
				...
				if (!this.field_23934.isEmpty()) {
					...
					RenderSystem.enableBlend();
					matrices.translate(0.0, 0.0, 50.0);
					this.client.textRenderer.method_30881(matrices, new TranslatableText("chat.queue", this.field_23934.size()), 0.0F, 1.0F, 16777215 + (m << 24));
					matrices.pop();
					RenderSystem.disableAlphaTest();
					RenderSystem.disableBlend();
				}
			...
			if (!(d < 0.0) && !(e < 0.0)) {
				...
				if (d <= (double)MathHelper.floor((double)this.getWidth() / this.getChatScale()) && e < (double)(9 * i + i)) {
					...
					if (j >= 0 && j < this.visibleMessages.size()) {
						ChatHudLine<class_5481> chatHudLine = (ChatHudLine)this.visibleMessages.get(j);
