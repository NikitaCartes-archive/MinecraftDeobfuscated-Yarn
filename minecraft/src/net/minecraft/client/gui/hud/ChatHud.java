...
public class ChatHud extends Drawable {
	...
	public void method_1805(int i) {
		if (this.client.options.chatVisibility != PlayerEntity.ChatVisibility.HIDDEN) {
			...
			if (k > 0) {
				...
				if (bl) {
					int n = this.client.fontRenderer.FONT_HEIGHT;
					...
					if (s != o) {
						...
						drawRect(0, -t, 2, -t - u, q + (p << 24));
						drawRect(2, -t, 1, -t - u, 13421772 + (p << 24));
					}
				}
	...
	public void addMessage(TextComponent textComponent, int i) {
		this.addMessage(textComponent, i, this.client.hudInGame.getTicks(), false);
	...
	private void addMessage(TextComponent textComponent, int i, int j, boolean bl) {
		...
		List<TextComponent> list = class_341.method_1850(textComponent, k, this.client.fontRenderer, false, false);
					...
					if (j >= 0 && j < this.field_2064.size()) {
						...
						for(TextComponent textComponent : chatHudLine.getContents()) {
							if (textComponent instanceof StringTextComponent) {
								k += this.client.fontRenderer.getStringWidth(class_341.method_1849(((StringTextComponent)textComponent).getTextField(), false));
								if ((double)k > g) {
