...
public class ResourcePackOptionsScreen extends class_5375 {
	public ResourcePackOptionsScreen(
		Screen parent, GameOptions gameOptions, ResourcePackManager<ClientResourcePackProfile> resourcePackManager, Runnable runnable
	) {
		super(parent, new TranslatableText("resourcePack.title"), runnable2 -> {
			...
			return new class_5369(runnable2, ClientResourcePackProfile::drawIcon, Lists.reverse(list), list2, (listx, list2x, bl) -> {
				...
				List<String> list4 = (List)list3.stream().map(ResourcePackProfile::getName).collect(ImmutableList.toImmutableList());
