	...
	private void setPlayers(Collection<UUID> playerUuids, Map<UUID, SocialInteractionsPlayerListEntry> entriesByUuids) {
		...
		for(UUID uUID : playerUuids) {
			...
			if (playerListEntry != null) {
				...
				entriesByUuids.put(
					uUID2,
					new SocialInteractionsPlayerListEntry(this.client, this.parent, uUID2, playerListEntry.getProfile().getName(), playerListEntry::getSkinTexture, bl)
				);
			}
		}
	}
		...
		for(GameProfile gameProfile : this.client.getAbuseReportContext().chatLog().streamBackward().collectSenderProfiles()) {
			...
			if (includeOffline) {
				socialInteractionsPlayerListEntry = (SocialInteractionsPlayerListEntry)entries.computeIfAbsent(
					gameProfile.getId(),
					uuid -> {
						SocialInteractionsPlayerListEntry socialInteractionsPlayerListEntryxx = new SocialInteractionsPlayerListEntry(
							this.client,
							this.parent,
							gameProfile.getId(),
							gameProfile.getName(),
							Suppliers.memoize(() -> this.client.getSkinProvider().loadSkin(gameProfile)),
							true
						);
						socialInteractionsPlayerListEntryxx.setOffline(true);
						return socialInteractionsPlayerListEntryxx;
					}
				);
			} else {
