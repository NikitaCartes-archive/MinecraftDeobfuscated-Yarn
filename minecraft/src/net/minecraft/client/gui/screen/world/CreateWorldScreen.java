...
public class CreateWorldScreen extends Screen {
	...
	void openPackScreen(DataConfiguration dataConfiguration) {
		...
		if (pair != null) {
			this.client.setScreen(new PackScreen(this, pair.getSecond(), this::applyDataPacks, (Path)pair.getFirst(), Text.translatable("dataPack.title")));
		}
	}
		...
		SaveLoading.load(
				...
				context -> {
					if (context.worldGenRegistryManager().get(RegistryKeys.WORLD_PRESET).size() == 0) {
						...
						WorldGenSettings worldGenSettings = (WorldGenSettings)dataResult.flatMap(json -> WorldGenSettings.CODEC.parse(dynamicOps2, json))
	...
	class GameTab extends GridScreenTab {
		...
		GameTab() {
			...
			CyclingButtonWidget<WorldCreator.Mode> cyclingButtonWidget = adder.add(
				CyclingButtonWidget.builder(value -> value.name)
	...
	class WorldTab extends GridScreenTab {
			...
			CyclingButtonWidget<WorldCreator.WorldType> cyclingButtonWidget = adder.add(
				CyclingButtonWidget.builder(WorldCreator.WorldType::getName)
					...
					.build(
						...
						(cyclingButtonWidgetx, worldType) -> CreateWorldScreen.this.worldCreator.setWorldType(worldType)
					)
			);
		...
		private static MutableText getWorldTypeNarrationMessage(CyclingButtonWidget<WorldCreator.WorldType> worldTypeButton) {
			return ((WorldCreator.WorldType)worldTypeButton.getValue()).isAmplified()
