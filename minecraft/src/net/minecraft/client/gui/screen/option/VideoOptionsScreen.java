...
public class VideoOptionsScreen extends GameOptionsScreen {
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
		if (Screen.hasControlDown()) {
			...
			SimpleOption.Callbacks i = simpleOption.getCallbacks();
			if (i instanceof SimpleOption.MaxSuppliableIntCallbacks maxSuppliableIntCallbacks) {
				int ix = simpleOption.getValue() + (int)Math.signum(verticalAmount);
				if (ix != 0 && ix <= maxSuppliableIntCallbacks.maxInclusive()) {
					CyclingButtonWidget<Integer> cyclingButtonWidget = (CyclingButtonWidget)this.list.getWidgetFor(simpleOption);
					if (cyclingButtonWidget != null) {
						simpleOption.setValue(ix);
						cyclingButtonWidget.setValue(ix);
					}

					if (simpleOption.getValue() == ix) {
						this.client.onResolutionChanged();
