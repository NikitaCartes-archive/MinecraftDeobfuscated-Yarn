...
public class PoisonousPotatoCutterScreen extends HandledScreen<PoisonousPotatoCutterScreenHandler> {
	...
	public PoisonousPotatoCutterScreen(PoisonousPotatoCutterScreenHandler handler, PlayerInventory inventory, Text title) {
	...
	protected void drawMouseoverTooltip(DrawContext context, int x, int y) {
		...
		if (this.canCraft) {
			...
			List<RecipeEntry<PoisonousPotatoCuttingRecipe>> list = this.handler.getAvailableRecipes();
			...
			for (int l = this.scrollOffset; l < k && l < this.handler.getAvailableRecipeCount(); l++) {
				...
				if (x >= n && x < n + 16 && y >= o && y < o + 18) {
					context.drawItemTooltip(
						this.textRenderer, ((PoisonousPotatoCuttingRecipe)((RecipeEntry)list.get(l)).value()).getResult(this.client.world.getRegistryManager()), x, y
					);
				}
			}
		}
	...
	private void renderRecipeIcons(DrawContext context, int x, int y, int scrollOffset) {
		...
		for (int i = this.scrollOffset; i < scrollOffset && i < this.handler.getAvailableRecipeCount(); i++) {
			...
			int l = j / 4;
			int m = y + l * 18 + 2;
			context.drawItem(((PoisonousPotatoCuttingRecipe)((RecipeEntry)list.get(i)).value()).getResult(this.client.world.getRegistryManager()), k, m);
		}
	}
