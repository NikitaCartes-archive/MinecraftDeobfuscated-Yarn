...
public abstract class ScreenWithHandler<T extends ScreenHandler> extends Screen implements ScreenHandlerProvider<T> {
	...
	public ScreenWithHandler(T handler, PlayerInventory inventory, Text name) {
		super(name);
	...
	private void drawSlot(Slot slot) {
		...
		if (itemStack.isEmpty() && slot.doDrawHoveringEffect()) {
			...
			if (pair != null) {
				...
				blit(i, j, this.getZOffset(), 16, 16, sprite);
				bl2 = true;
			}
		}
