...
public class BeaconScreen extends HandledScreen<BeaconScreenHandler> {
	...
	public void tick() {
		...
		if (this.consumeGem && i >= 0) {
			...
			for(int m = 0; m < k - 1; ++m) {
				StatusEffect statusEffect = BeaconBlockEntity.EFFECTS_BY_LEVEL[3][m];
				BeaconScreen.EffectButtonWidget effectButtonWidget = new BeaconScreen.EffectButtonWidget(this.x + 167 + m * 24 - l / 2, this.y + 47, statusEffect, false);
				this.addButton(effectButtonWidget);
				if (3 >= i) {
					effectButtonWidget.active = false;
	...
	protected void drawForeground(MatrixStack matrixStack, int i, int j) {
		this.drawCenteredString(matrixStack, this.textRenderer, I18n.translate("block.minecraft.beacon.primary"), 62, 10, 14737632);
		this.drawCenteredString(matrixStack, this.textRenderer, I18n.translate("block.minecraft.beacon.secondary"), 169, 10, 14737632);
	...
	class CancelButtonWidget extends BeaconScreen.IconButtonWidget {
		...
		@Override
		public void onPress() {
			BeaconScreen.this.client.player.networkHandler.sendPacket(new GuiCloseC2SPacket(BeaconScreen.this.client.player.currentScreenHandler.syncId));
