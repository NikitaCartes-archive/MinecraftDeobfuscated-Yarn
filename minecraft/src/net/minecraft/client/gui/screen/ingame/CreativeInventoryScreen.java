...
public class CreativeInventoryScreen extends AbstractInventoryScreen<CreativeInventoryScreen.CreativeContainer> {
		...
		if (slot == null && selectedTab != ItemGroup.INVENTORY.getIndex() && slotActionType != SlotActionType.QUICK_CRAFT) {
			...
			if (slot == this.deleteItemSlot && bl) {
				...
				if (Container.unpackButtonId(button) == 2) {
					...
					this.minecraft.interactionManager.clickCreativeStack(itemStack2, slot.id - this.container.slots.size() + 9 + 36);
					int k = 45 + button;
					if (slotActionType == SlotActionType.SWAP) {
	...
	private void setSelectedTab(ItemGroup group) {
		...
		if (group == ItemGroup.INVENTORY) {
			...
			for(int j = 0; j < container.slots.size(); ++j) {
				Slot slot = new CreativeInventoryScreen.CreativeSlot((Slot)container.slots.get(j), j);
				...
				if (j >= 5 && j < 9) {
					...
					slot.xPosition = 35;
					slot.yPosition = 20;
				} else if (j < container.slots.size()) {
					int k = j - 9;
					int l = k % 9;
					int m = k / 9;
	...
	public void render(int mouseX, int mouseY, float delta) {
		...
		for(ItemGroup itemGroup : ItemGroup.GROUPS) {
			if (this.method_2471(itemGroup, mouseX, mouseY)) {
				break;
			}
		}
