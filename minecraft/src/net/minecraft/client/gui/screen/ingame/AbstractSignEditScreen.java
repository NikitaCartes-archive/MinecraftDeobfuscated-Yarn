...
public abstract class AbstractSignEditScreen extends Screen {
	...
	public AbstractSignEditScreen(SignBlockEntity blockEntity, boolean filtered, Text title) {
		...
		this.text = (String[])IntStream.range(0, 4).mapToObj(row -> blockEntity.getTextOnRow(row, filtered)).map(Text::getString).toArray(i -> new String[i]);
	...
	protected void translateForRender(MatrixStack matrices, BlockState state) {
		matrices.translate((double)((float)this.width / 2.0F), 90.0, 50.0);
	}
			...
			if (string != null) {
				...
				if (n == this.currentRow && j >= 0 && bl) {
					...
					if (j >= string.length()) {
						this.client
							.textRenderer
							.draw("_", (float)p, (float)m, i, false, matrix4f, vertexConsumers, false, 0, LightmapTextureManager.MAX_LIGHT_COORDINATE, false);
					}
				}
			}
		}
