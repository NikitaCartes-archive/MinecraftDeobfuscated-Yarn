	...
	private BookEditScreen.PageContent createPageContent() {
		...
		if (string.isEmpty()) {
			...
			textHandler.wrapLines(string, 114, Style.EMPTY, true, (style, ix, jx) -> {
				...
				int lxx = k * 9;
				BookEditScreen.Position positionxx = this.method_27590(new BookEditScreen.Position(0, lxx));
				intList.add(ix);
				list.add(new BookEditScreen.Line(style, string3, positionxx.x, positionxx.y));
			});
