	...
	private BookEditScreen.PageContent createPageContent() {
		...
		if (string.isEmpty()) {
			...
			textHandler.wrapLines(string, 114, Style.EMPTY, true, (style, start, end) -> {
				int ixx = mutableInt.getAndIncrement();
				String stringxx = string.substring(start, end);
				mutableBoolean.setValue(stringxx.endsWith("\n"));
				String string2xx = StringUtils.stripEnd(stringxx, " \n");
				int jxx = ixx * 9;
				BookEditScreen.Position positionxx = this.absolutePositionToScreenPosition(new BookEditScreen.Position(0, jxx));
				intList.add(start);
				list.add(new BookEditScreen.Line(style, string2xx, positionxx.x, positionxx.y));
			});
