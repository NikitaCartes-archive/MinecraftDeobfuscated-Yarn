...
public abstract class Screen extends DrawableContainer implements class_4068, YesNoCallback {
	...
	public static boolean isControlPressed() {
		if (MinecraftClient.IS_SYSTEM_MAC) {
			return InputUtil.isKeyPressed(MinecraftClient.getInstance().window.getHandle(), 343)
				|| InputUtil.isKeyPressed(MinecraftClient.getInstance().window.getHandle(), 347);
		} else {
			return InputUtil.isKeyPressed(MinecraftClient.getInstance().window.getHandle(), 341)
