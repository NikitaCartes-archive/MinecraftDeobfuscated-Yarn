...
import net.minecraft.client.hotbar.HotbarEntry;
...
public class CreativeInventoryGui extends AbstractGuiInventory {
	private static final Identifier TEXTURE = new Identifier("textures/gui/container/creative_inventory/tabs.png");
	private static final BasicInventory inventory = new BasicInventory(new StringTextComponent("tmp"), 45);
	...
	public CreativeInventoryGui(PlayerEntity playerEntity) {
		super(new CreativeInventoryGui.ContainerCreative(playerEntity));
	...
	public void update() {
		if (!this.client.interactionManager.hasCreativeInventory()) {
			this.client.openGui(new InventoryGui(this.client.player));
	...
	@Override
	protected void onMouseClick(@Nullable Slot slot, int i, int j, ActionTypeSlot actionTypeSlot) {
		if (this.method_2470(slot)) {
			this.searchBox.method_1872();
			this.searchBox.method_1884(0);
		}

		boolean bl = actionTypeSlot == ActionTypeSlot.SHIFT_CLICK;
		actionTypeSlot = i == -999 && actionTypeSlot == ActionTypeSlot.CLICK ? ActionTypeSlot.DROP : actionTypeSlot;
		if (slot == null && selectedTab != ItemGroup.INVENTORY.getId() && actionTypeSlot != ActionTypeSlot.DRAG) {
			...
			if (slot == this.deleteItemSlot && bl) {
				...
				if (slot == this.deleteItemSlot) {
				...
				} else if (actionTypeSlot == ActionTypeSlot.DROP && slot != null && slot.hasStack()) {
					...
					this.client.interactionManager.method_2909(itemStack2, ((CreativeInventoryGui.CreativeSlot)slot).slot.id);
				} else if (actionTypeSlot == ActionTypeSlot.DROP && !this.client.player.inventory.getCursorStack().isEmpty()) {
					...
					this.client
						.player
						.containerPlayer
						.onSlotClick(slot == null ? i : ((CreativeInventoryGui.CreativeSlot)slot).slot.id, j, actionTypeSlot, this.client.player);
					this.client.player.containerPlayer.sendContentUpdates();
				}
			} else if (actionTypeSlot != ActionTypeSlot.DRAG && slot.inventory == inventory) {
				PlayerInventory playerInventory = this.client.player.inventory;
				ItemStack itemStack2 = playerInventory.getCursorStack();
				ItemStack itemStack3 = slot.getStack();
				if (actionTypeSlot == ActionTypeSlot.HOTBAR_KEY) {
				...
				if (actionTypeSlot == ActionTypeSlot.MIDDLE_CLICK) {
				...
				if (actionTypeSlot == ActionTypeSlot.DROP) {
				...
				this.container.onSlotClick(slot == null ? i : slot.id, j, actionTypeSlot, this.client.player);
				if (Container.method_7594(j) == 2) {
					for(int l = 0; l < 9; ++l) {
						this.client.interactionManager.method_2909(this.container.getSlot(45 + l).getStack(), 36 + l);
					}
				} else if (slot != null) {
					ItemStack itemStack2 = this.container.getSlot(slot.id).getStack();
					this.client.interactionManager.method_2909(itemStack2, slot.id - this.container.slotList.size() + 9 + 36);
					int m = 45 + j;
					if (actionTypeSlot == ActionTypeSlot.HOTBAR_KEY) {
						this.client.interactionManager.method_2909(itemStack, m - this.container.slotList.size() + 9 + 36);
					} else if (actionTypeSlot == ActionTypeSlot.DROP && !itemStack.isEmpty()) {
	...
	protected void method_2476() {
		...
		if (this.searchBox != null && this.left != i) {
			this.searchBox.method_16872(this.left + 82);
		}
	}
	...
	protected void onInitialized() {
		if (this.client.interactionManager.hasCreativeInventory()) {
			...
			this.searchBox = new TextFieldWidget(0, this.fontRenderer, this.left + 82, this.top + 6, 80, this.fontRenderer.FONT_HEIGHT);
	...
	public boolean keyPressed(int i, int j, int k) {
		...
		if (selectedTab != ItemGroup.SEARCH.getId()) {
			if (this.client.options.keyChat.matches(i, j)) {
	...
	private void method_2464() {
		CreativeInventoryGui.ContainerCreative containerCreative = (CreativeInventoryGui.ContainerCreative)this.container;
		containerCreative.itemList.clear();
		this.field_16201.clear();
		String string = this.searchBox.getText();
		if (string.isEmpty()) {
			for(Item item : Registry.ITEM) {
				item.addStacksForDisplay(ItemGroup.SEARCH, containerCreative.itemList);
			}
		} else {
			Searchable<ItemStack> searchable;
			if (string.startsWith("#")) {
				string = string.substring(1);
				searchable = this.client.getSearchableContainer(SearchManager.ITEMS_TAG);
				this.method_15871(string);
			} else {
				searchable = this.client.getSearchableContainer(SearchManager.ITEMS_TOOLTIP);
			}

			containerCreative.itemList.addAll(searchable.findAll(string.toLowerCase(Locale.ROOT)));
		}

		this.scrollPosition = 0.0F;
		containerCreative.method_2473(0.0F);
	}
	...
	public boolean mouseClicked(double d, double e, int i) {
		if (i == 0) {
			...
			for(ItemGroup itemGroup : ItemGroup.GROUPS) {
				if (this.method_2463(itemGroup, f, g)) {
					return true;
				}
			}
	...
	private boolean doRenderScrollBar() {
		return selectedTab != ItemGroup.INVENTORY.getId()
			&& ItemGroup.GROUPS[selectedTab].useScrollBar()
			&& ((CreativeInventoryGui.ContainerCreative)this.container).method_2474();
	}
	...
	private void setSelectedTab(ItemGroup itemGroup) {
		...
		if (itemGroup == ItemGroup.HOTBAR) {
			CreativeHotbarStorage creativeHotbarStorage = this.client.getCreativeHotbarStorage();

			for(int j = 0; j < 9; ++j) {
				HotbarEntry hotbarEntry = creativeHotbarStorage.getSavedHotbar(j);
				if (hotbarEntry.isEmpty()) {
					for(int k = 0; k < 9; ++k) {
						if (k == j) {
							ItemStack itemStack = new ItemStack(Items.field_8407);
							itemStack.getOrCreateSubCompoundTag("CustomCreativeLock");
							String string = this.client.options.keysHotbar[j].method_16007();
							String string2 = this.client.options.keySaveToolbarActivator.method_16007();
					...
					containerCreative.itemList.addAll(hotbarEntry);
				}
			}
		} else if (itemGroup != ItemGroup.SEARCH) {
			itemGroup.getStacksForDisplay(containerCreative.itemList);
		}

		if (itemGroup == ItemGroup.INVENTORY) {
			Container container = this.client.player.containerPlayer;
			if (this.slots == null) {
				this.slots = containerCreative.slotList;
			}

			containerCreative.slotList = Lists.<Slot>newArrayList();

			for(int j = 0; j < container.slotList.size(); ++j) {
				Slot slot = new CreativeInventoryGui.CreativeSlot((Slot)container.slotList.get(j), j);
				containerCreative.slotList.add(slot);
				if (j >= 5 && j < 9) {
					int k = j - 5;
					int l = k / 2;
					int m = k % 2;
			...
			containerCreative.slotList.add(this.deleteItemSlot);
		} else if (i == ItemGroup.INVENTORY.getId()) {
			containerCreative.slotList = this.slots;
			this.slots = null;
		}
	...
	public boolean mouseScrolled(double d) {
		if (!this.doRenderScrollBar()) {
			...
			int i = (((CreativeInventoryGui.ContainerCreative)this.container).itemList.size() + 9 - 1) / 9 - 5;
			this.scrollPosition = (float)((double)this.scrollPosition - d / (double)i);
			this.scrollPosition = MathHelper.clamp(this.scrollPosition, 0.0F, 1.0F);
			((CreativeInventoryGui.ContainerCreative)this.container).method_2473(this.scrollPosition);
			return true;
		}
	}
	...
	protected boolean isClickInContainerBounds(double d, double e, int i, int j, int k) {
		...
		this.field_2887 = bl && !this.method_2463(ItemGroup.GROUPS[selectedTab], d, e);
	...
	protected void drawBackground(float f, int i, int j) {
		...
		if (itemGroup == ItemGroup.INVENTORY) {
			InventoryGui.drawEntity(this.left + 88, this.top + 45, 20, (float)(this.left + 88 - i), (float)(this.top + 45 - 30 - j), this.client.player);
		}
	}

	protected boolean method_2463(ItemGroup itemGroup, double d, double e) {
	...
	protected void method_2468(ItemGroup itemGroup) {
		...
		this.itemRenderer.renderItemAndGlowInGui(itemStack, l, m);
		this.itemRenderer.renderItemOverlaysInGUI(this.fontRenderer, itemStack, l, m);
		GlStateManager.disableLighting();
	...
	public static void method_2462(MinecraftClient minecraftClient, int i, boolean bl, boolean bl2) {
		...
		CreativeHotbarStorage creativeHotbarStorage = minecraftClient.getCreativeHotbarStorage();
		HotbarEntry hotbarEntry = creativeHotbarStorage.getSavedHotbar(i);
		if (bl) {
			for(int j = 0; j < PlayerInventory.getHotbarSize(); ++j) {
				ItemStack itemStack = hotbarEntry.get(j).copy();
			...
			for(int j = 0; j < PlayerInventory.getHotbarSize(); ++j) {
				hotbarEntry.set(j, clientPlayerEntity.inventory.getInvStack(j).copy());
			}

			String string = minecraftClient.options.keysHotbar[i].method_16007();
			String string2 = minecraftClient.options.keyLoadToolbarActivator.method_16007();
			minecraftClient.hudInGame.setOverlayMessage(new TranslatableTextComponent("inventory.hotbarSaved", string2, string), false);
	...
	public static class ContainerCreative extends Container {
		public DefaultedList<ItemStack> itemList = DefaultedList.create();

		public ContainerCreative(PlayerEntity playerEntity) {
			PlayerInventory playerInventory = playerEntity.inventory;

			for(int i = 0; i < 5; ++i) {
				for(int j = 0; j < 9; ++j) {
					this.addSlot(new CreativeInventoryGui.class_482(CreativeInventoryGui.inventory, i * 9 + j, 9 + j * 18, 18 + i * 18));
				}
			}
			...
			for(int k = 0; k < 5; ++k) {
				for(int l = 0; l < 9; ++l) {
					...
					if (m >= 0 && m < this.itemList.size()) {
						CreativeInventoryGui.inventory.setInvStack(l + k * 9, this.itemList.get(m));
					} else {
						CreativeInventoryGui.inventory.setInvStack(l + k * 9, ItemStack.EMPTY);
					}
				}
			}
		}
		...
		public boolean method_7613(ItemStack itemStack, Slot slot) {
			return slot.yPosition > 90;
		}

		@Override
		public boolean method_7615(Slot slot) {
			return slot.inventory instanceof PlayerInventory || slot.yPosition > 90 && slot.xPosition <= 162;
		}
	}
	...
	class CreativeSlot extends Slot {
		...
		public boolean matches(Inventory inventory, int i) {
			return this.slot.matches(inventory, i);
		}
