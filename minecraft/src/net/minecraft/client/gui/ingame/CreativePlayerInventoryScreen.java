...
public class CreativePlayerInventoryScreen extends AbstractPlayerInventoryScreen<CreativePlayerInventoryScreen.CreativeContainer> {
	...
	public CreativePlayerInventoryScreen(PlayerEntity playerEntity) {
		super(new CreativePlayerInventoryScreen.CreativeContainer(playerEntity), playerEntity.inventory, new TextComponent(""));
			...
			if (slot == this.deleteItemSlot && bl) {
				...
				if (slot == this.deleteItemSlot) {
					...
					this.minecraft
						.player
						.playerContainer
						.onSlotClick(slot == null ? i : ((CreativePlayerInventoryScreen.CreativeSlot)slot).slot.id, j, slotActionType, this.minecraft.player);
				...
				if (Container.unpackButtonId(j) == 2) {
					...
					this.minecraft.interactionManager.clickCreativeStack(itemStack2, slot.id - this.container.slotList.size() + 9 + 36);
					int m = 45 + j;
					if (slotActionType == SlotActionType.field_7791) {
	...
	protected void init() {
		if (this.minecraft.interactionManager.hasCreativeInventory()) {
			...
			this.field_2891 = new CreativePlayerInventoryContainerListener(this.minecraft);
	...
	private void setSelectedTab(ItemGroup itemGroup) {
		...
		if (itemGroup == ItemGroup.INVENTORY) {
			...
			for(int j = 0; j < container.slotList.size(); ++j) {
				Slot slot = new CreativePlayerInventoryScreen.CreativeSlot((Slot)container.slotList.get(j), j);
		...
		if (selectedTab == ItemGroup.SEARCH.getIndex()) {
			...
			this.field_16201.forEach((identifier, tag) -> {
				if (tag.contains(item)) {
					list2.add(1, "" + ChatFormat.field_1067 + ChatFormat.field_1064 + "#" + identifier);
				}
			});
			if (itemGroup != null) {
				list2.add(1, "" + ChatFormat.field_1067 + ChatFormat.field_1078 + I18n.translate(itemGroup.getTranslationKey()));
			}
			...
			for(int k = 0; k < list2.size(); ++k) {
				if (k == 0) {
					...
					list2.set(k, ChatFormat.field_1080 + (String)list2.get(k));
				}
			}
	...
	protected void drawBackground(float f, int i, int j) {
		...
		if (itemGroup == ItemGroup.INVENTORY) {
			PlayerInventoryScreen.drawEntity(this.left + 88, this.top + 45, 20, (float)(this.left + 88 - i), (float)(this.top + 45 - 30 - j), this.minecraft.player);
		}
	}
