		...
		if (slot == null && selectedTab != ItemGroup.INVENTORY.getIndex() && slotActionType != SlotActionType.field_7789) {
			...
			if (slot == this.deleteItemSlot && bl) {
				...
				if (Container.unpackButtonId(j) == 2) {
					...
					ItemStack itemStack2 = this.container.method_7611(slot.id).method_7677();
					this.client.field_1761.method_2909(itemStack2, slot.id - this.container.slotList.size() + 9 + 36);
					int m = 45 + j;
					if (slotActionType == SlotActionType.field_7791) {
	...
	static class class_482 extends Slot {
		...
		public boolean canTakeItems(PlayerEntity playerEntity) {
			if (super.canTakeItems(playerEntity) && this.hasStack()) {
				return this.method_7677().method_7941("CustomCreativeLock") == null;
			} else {
