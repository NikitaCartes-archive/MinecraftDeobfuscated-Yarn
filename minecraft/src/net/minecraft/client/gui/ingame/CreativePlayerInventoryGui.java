...
public class CreativePlayerInventoryGui extends AbstractPlayerInventoryGui {
	...
	public CreativePlayerInventoryGui(PlayerEntity playerEntity) {
		super(new CreativePlayerInventoryGui.CreativeContainer(playerEntity));
	...
	public void update() {
		if (!this.client.interactionManager.hasCreativeInventory()) {
			this.client.openGui(new PlayerInventoryGui(this.client.player));
		...
		if (slot == null && selectedTab != ItemGroup.INVENTORY.getId() && slotActionType != SlotActionType.field_7789) {
			...
			if (slot == this.deleteItemSlot && bl) {
				...
				if (slot == this.deleteItemSlot) {
					...
					this.client.interactionManager.method_2909(itemStack2, ((CreativePlayerInventoryGui.CreativeSlot)slot).slot.id);
					...
					this.client
						.player
						.containerPlayer
						.method_7593(slot == null ? i : ((CreativePlayerInventoryGui.CreativeSlot)slot).slot.id, j, slotActionType, this.client.player);
				...
				this.container.method_7593(slot == null ? i : slot.id, j, slotActionType, this.client.player);
	...
	protected void onInitialized() {
		if (this.client.interactionManager.hasCreativeInventory()) {
			...
			this.searchBox = new TextFieldWidget(0, this.fontRenderer, this.left + 82, this.top + 6, 80, this.fontRenderer.fontHeight);
	...
	private void method_2464() {
		CreativePlayerInventoryGui.CreativeContainer creativeContainer = (CreativePlayerInventoryGui.CreativeContainer)this.container;
		creativeContainer.itemList.clear();
		this.field_16201.clear();
		String string = this.searchBox.getText();
		if (string.isEmpty()) {
			for (Item item : Registry.ITEM) {
				item.addStacksForDisplay(ItemGroup.SEARCH, creativeContainer.itemList);
			}
		} else {
			...
			creativeContainer.itemList.addAll(searchable.findAll(string.toLowerCase(Locale.ROOT)));
		}

		this.scrollPosition = 0.0F;
		creativeContainer.method_2473(0.0F);
	}
	...
	private boolean doRenderScrollBar() {
		return selectedTab != ItemGroup.INVENTORY.getId()
			...
			&& ((CreativePlayerInventoryGui.CreativeContainer)this.container).method_2474();
	}
				...
				if (hotbarStorageEntry.isEmpty()) {
					for (int k = 0; k < 9; k++) {
						if (k == j) {
							...
							String string = this.client.field_1690.keysHotbar[j].method_16007();
							String string2 = this.client.field_1690.keySaveToolbarActivator.method_16007();
							itemStack.setDisplayName(new TranslatableTextComponent("inventory.hotbarInfo", string2, string));
							creativeContainer.itemList.add(itemStack);
						} else {
							creativeContainer.itemList.add(ItemStack.EMPTY);
						}
					}
				} else {
					creativeContainer.itemList.addAll(hotbarStorageEntry);
				}
			}
		} else if (itemGroup != ItemGroup.SEARCH) {
			itemGroup.getStacksForDisplay(creativeContainer.itemList);
		}

		if (itemGroup == ItemGroup.INVENTORY) {
			Container container = this.client.player.containerPlayer;
			if (this.slots == null) {
				this.slots = creativeContainer.slotList;
			}

			creativeContainer.slotList = Lists.<Slot>newArrayList();

			for (int jx = 0; jx < container.slotList.size(); jx++) {
				Slot slot = new CreativePlayerInventoryGui.CreativeSlot((Slot)container.slotList.get(jx), jx);
				creativeContainer.slotList.add(slot);
				if (jx >= 5 && jx < 9) {
					int kx = jx - 5;
					int l = kx / 2;
					int m = kx % 2;
			...
			creativeContainer.slotList.add(this.deleteItemSlot);
		} else if (i == ItemGroup.INVENTORY.getId()) {
			creativeContainer.slotList = this.slots;
			this.slots = null;
		}
	...
	protected void drawBackground(float f, int i, int j) {
		...
		if (itemGroup == ItemGroup.INVENTORY) {
			PlayerInventoryGui.drawEntity(this.left + 88, this.top + 45, 20, (float)(this.left + 88 - i), (float)(this.top + 45 - 30 - j), this.client.player);
		}
	}
	...
	public static void method_2462(MinecraftClient minecraftClient, int i, boolean bl, boolean bl2) {
		...
		if (bl) {
			...
			String string = minecraftClient.field_1690.keysHotbar[i].method_16007();
			String string2 = minecraftClient.field_1690.keyLoadToolbarActivator.method_16007();
	...
	public static class CreativeContainer extends Container {
		...
		public CreativeContainer(PlayerEntity playerEntity) {
			...
			for (int i = 0; i < 5; i++) {
				for (int j = 0; j < 9; j++) {
					this.addSlot(new CreativePlayerInventoryGui.class_482(CreativePlayerInventoryGui.inventory, i * 9 + j, 9 + j * 18, 18 + i * 18));
				}
			}
			...
			for (int k = 0; k < 5; k++) {
				for (int l = 0; l < 9; l++) {
					...
					if (m >= 0 && m < this.itemList.size()) {
						CreativePlayerInventoryGui.inventory.setInvStack(l + k * 9, this.itemList.get(m));
					} else {
						CreativePlayerInventoryGui.inventory.setInvStack(l + k * 9, ItemStack.EMPTY);
					}
				}
			}
		}
