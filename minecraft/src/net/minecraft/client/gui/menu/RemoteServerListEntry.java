...
public class RemoteServerListEntry extends ServerListGuiWidget.Entry {
	...
	protected RemoteServerListEntry(MultiplayerGui multiplayerGui, ServerEntry serverEntry) {
	...
	public void draw(int i, int j, int k, int l, boolean bl, float f) {
		...
		if (!this.field_3061.field_3754) {
			...
			PING_THREAD_POOL.submit(() -> {
				try {
					...
					this.field_3061.label = TextFormat.DARK_RED + I18n.translate("multiplayer.status.cannot_resolve");
				} catch (Exception var3) {
					this.field_3061.ping = -1L;
					this.field_3061.label = TextFormat.DARK_RED + I18n.translate("multiplayer.status.cannot_connect");
				}
			});
		}
		...
		boolean bl4 = bl2 || bl3;
		this.client.fontRenderer.draw(this.field_3061.name, (float)(n + 32 + 3), (float)(m + 1), 16777215);
		List<String> list = this.client.fontRenderer.wrapStringToWidthAsList(this.field_3061.label, i - 32 - 2);

		for (int o = 0; o < Math.min(list.size(), 2); o++) {
			this.client.fontRenderer.draw((String)list.get(o), (float)(n + 32 + 3), (float)(m + 12 + this.client.fontRenderer.fontHeight * o), 8421504);
		}

		String string = bl4 ? TextFormat.DARK_RED + this.field_3061.version : this.field_3061.playerCountLabel;
