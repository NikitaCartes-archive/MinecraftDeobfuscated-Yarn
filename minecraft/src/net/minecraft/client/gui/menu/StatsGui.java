...
public class StatsGui extends Gui implements class_452 {
	protected final Gui field_2648;
	...
	public void method_2270() {
		this.field_2644 = new StatsGui.class_448(this.client);
		this.field_2642 = new StatsGui.class_449(this.client);
		this.field_2646 = new StatsGui.class_451(this.client);
	}
	...
	public void method_2267() {
		...
		ButtonWidget buttonWidget = this.addButton(new ButtonWidget(3, this.width / 2 - 40, this.height - 52, 80, 20, I18n.translate("stat.itemsButton")) {
			@Override
			public void onPressed(double d, double e) {
				StatsGui.this.field_2643 = StatsGui.this.field_2642;
			}
		});
		ButtonWidget buttonWidget2 = this.addButton(new ButtonWidget(4, this.width / 2 + 40, this.height - 52, 80, 20, I18n.translate("stat.mobsButton")) {
			@Override
			public void onPressed(double d, double e) {
				StatsGui.this.field_2643 = StatsGui.this.field_2646;
			}
		});
	...
	class class_448 extends AbstractListWidget {
		...
		public class_448(MinecraftClient minecraftClient) {
			super(minecraftClient, StatsGui.this.width, StatsGui.this.height, 32, StatsGui.this.height - 64, 10);
		...
		protected void drawEntry(int i, int j, int k, int l, int m, int n, float f) {
			...
			Stat<Identifier> stat = (Stat)this.field_2650.next();
	...
	class class_449 extends AbstractListWidget {
		...
		protected final Comparator<Item> field_2661 = new StatsGui.class_449.class_450();
		@Nullable
		protected StatType<?> field_2659;
		protected int field_2657;

		public class_449(MinecraftClient minecraftClient) {
			super(minecraftClient, StatsGui.this.width, StatsGui.this.height, 32, StatsGui.this.height - 64, 20);
			...
			for(Block block : Registry.BLOCK) {
				...
				for(StatType<Block> statType : this.field_2656) {
					if (statType.method_14958(block) && StatsGui.this.field_2647.getStat(statType.method_14956(block)) > 0) {
						bl = true;
					}
				}
		...
		protected void method_1940(int i, int j, Tessellator tessellator) {
			...
			for(int k = 0; k < this.field_2654.length; ++k) {
				StatsGui.this.method_2282(i + StatsGui.this.method_2285(k) - 18, j + 1, 0, this.field_2653 == k ? 0 : 18);
			}

			if (this.field_2659 != null) {
				int k = StatsGui.this.method_2285(this.method_2294(this.field_2659)) - 36;
				int l = this.field_2657 == 1 ? 2 : 1;
				StatsGui.this.method_2282(i + k, j + 1, 18 * l, 0);
			}

			for(int k = 0; k < this.field_2654.length; ++k) {
				int l = this.field_2653 == k ? 1 : 0;
				StatsGui.this.method_2282(i + StatsGui.this.method_2285(k) - 18 + l, j + 1 + l, 18 * this.field_2654[k], 18);
			}
		}
		...
		protected void drawEntry(int i, int j, int k, int l, int m, int n, float f) {
			...
			StatsGui.this.method_2289(j + 40, k, item);
			...
			for(int o = 0; o < this.field_2656.size(); ++o) {
				...
				this.method_2292(stat, j + StatsGui.this.method_2285(o), k, i % 2 == 0);
			}
		...
		protected void method_1929(int i, int j) {
			...
			for(int k = 0; k < this.field_2654.length; ++k) {
				int l = i - StatsGui.this.method_2285(k);
				if (l >= -36 && l <= 0) {
					this.field_2653 = k;
					break;
				}
			...
			if (j >= this.y1 && j <= this.y2) {
				...
				if (k >= 0) {
					...
					for(int n = 0; n < this.field_2654.length; ++n) {
						int o = StatsGui.this.method_2285(n);
						if (m >= o - 18 && m <= o) {
		...
		protected void method_2293(@Nullable TextComponent textComponent, int i, int j) {
			if (textComponent != null) {
				...
				int k = i + 12;
				int l = j - 12;
				int m = StatsGui.this.fontRenderer.getStringWidth(string);
				this.drawGradientRect(k - 3, l - 3, k + m + 3, l + 8 + 3, -1073741824, -1073741824);
				StatsGui.this.fontRenderer.drawWithShadow(string, (float)k, (float)l, -1);
			}
		}
		...
		class class_450 implements Comparator<Item> {
			...
			public int method_2297(Item item, Item item2) {
				...
				if (class_449.this.field_2659 == null) {
					...
					StatType<Block> statType = class_449.this.field_2659;
					i = item instanceof BlockItem ? StatsGui.this.field_2647.method_15024(statType, ((BlockItem)item).getBlock()) : -1;
					j = item2 instanceof BlockItem ? StatsGui.this.field_2647.method_15024(statType, ((BlockItem)item2).getBlock()) : -1;
				} else {
					StatType<Item> statType = class_449.this.field_2659;
					i = StatsGui.this.field_2647.method_15024(statType, item);
					j = StatsGui.this.field_2647.method_15024(statType, item2);
				}
	...
	class class_451 extends AbstractListWidget {
		...
		public class_451(MinecraftClient minecraftClient) {
			super(minecraftClient, StatsGui.this.width, StatsGui.this.height, 32, StatsGui.this.height - 64, 9 * 4);
			this.method_1943(false);
