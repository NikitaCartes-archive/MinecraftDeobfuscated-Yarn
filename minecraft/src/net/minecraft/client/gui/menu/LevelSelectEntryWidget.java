...
public final class LevelSelectEntryWidget extends EntryListWidget.Entry<LevelSelectEntryWidget> implements AutoCloseable {
	...
	public void draw(int i, int j, int k, int l, boolean bl, float f) {
		...
		this.client.fontRenderer.draw(string2, (float)(n + 32 + 3), (float)(m + this.client.fontRenderer.fontHeight + 3), 8421504);
		this.client
			.fontRenderer
			.draw(string3, (float)(n + 32 + 3), (float)(m + this.client.fontRenderer.fontHeight + this.client.fontRenderer.fontHeight + 3), 8421504);
	...
	public void loadLevel() {
		if (this.levelSummary.isOutdatedLevel() || this.levelSummary.isLegacyCustomizedWorld()) {
			...
			this.client.openGui(new BackupPromptGui(this.guiLevelSelect, bl -> {
				if (bl) {
					String stringxx = this.levelSummary.getName();
					BackupLevelGui.backupLevel(this.client.getLevelStorage(), stringxx);
				}
