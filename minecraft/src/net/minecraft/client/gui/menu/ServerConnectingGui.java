		...
		Thread thread = new Thread("Server Connector #" + field_2408.incrementAndGet()) {
			public void run() {
				...
				try {
					...
					ServerConnectingGui.this.connection = ClientConnection.connect(inetAddress, i, ServerConnectingGui.this.client.options.shouldUseNativeTransport());
					...
					ServerConnectingGui.LOGGER.error("Couldn't connect to server", var4);
					ServerConnectingGui.this.client
						.execute(
							() -> ServerConnectingGui.this.client
									.openGui(
					...
					ServerConnectingGui.LOGGER.error("Couldn't connect to server", var5);
