...
public class AudioSettingsGui extends Gui {
	private final Gui parent;
	private final GameOptions settings;
	protected String title = "Options";
	private String field_2616;

	public AudioSettingsGui(Gui gui, GameOptions gameOptions) {
		this.parent = gui;
	...
	protected void onInitialized() {
		...
		int i = 0;
		this.addButton(
			new AudioSettingsGui.class_444(
		...
		for (SoundCategory soundCategory : SoundCategory.values()) {
			if (soundCategory != SoundCategory.field_15250) {
				this.addButton(
					new AudioSettingsGui.class_444(soundCategory.ordinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 - 12 + 24 * (i >> 1), soundCategory, false)
				);
				i++;
			}
		}
	...
	class class_444 extends ButtonWidget {
		...
		public class_444(int i, int j, int k, SoundCategory soundCategory, boolean bl) {
			...
			this.text = this.field_2621 + ": " + AudioSettingsGui.this.method_2256(soundCategory);
			this.field_2620 = (double)AudioSettingsGui.this.settings.getSoundVolume(soundCategory);
		}
		...
		public void onReleased(double d, double e) {
			if (this.field_2623) {
				AudioSettingsGui.this.client.getSoundLoader().play(PositionedSoundInstance.master(SoundEvents.field_15015, 1.0F));
			}
