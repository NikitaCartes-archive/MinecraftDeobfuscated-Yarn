		...
		this.addButton(new ButtonWidget(this.screenWidth / 2 + 4, this.screenHeight - 48, 150, 20, I18n.translate("gui.done"), buttonWidget -> {
			if (this.field_3155) {
				...
				for(ResourcePackListEntry resourcePackListEntry : this.selectedList.getInputListeners()) {
					listxx.add(resourcePackListEntry.getResourcePack());
				}

				Collections.reverse(listxx);
				this.client.method_1520().setEnabled(listxx);
				...
				for(ClientResourcePackContainer clientResourcePackContainerxx : listxx) {
					if (!clientResourcePackContainerxx.sortsTillEnd()) {
						this.client.options.resourcePacks.add(clientResourcePackContainerxx.getName());
						if (!clientResourcePackContainerxx.getCompatibility().isCompatible()) {
							this.client.options.incompatibleResourcePacks.add(clientResourcePackContainerxx.getName());
						}
					}
				}
