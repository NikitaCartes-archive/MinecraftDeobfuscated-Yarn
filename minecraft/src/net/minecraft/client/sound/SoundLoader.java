...
public class SoundLoader {
					...
					try {
						...
						try {
							...
							try {
								...
								try {
									audioStream = new OggAudioStream(inputStream);
									Throwable var7 = null;
	...
	public CompletableFuture<?> loadStatic(Collection<Sound> collection) {
		return CompletableFuture.allOf(
			(CompletableFuture[])collection.stream().map(sound -> this.loadStatic(sound.getLocation())).toArray(i -> new CompletableFuture[i])
		);
	}
}
