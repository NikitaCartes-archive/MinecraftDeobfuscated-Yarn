...
public class SoundSystem {
		...
		if (this.started) {
			if (soundCategory == SoundCategory.field_15250) {
				...
				this.sources.forEach((soundInstance, sourceManager) -> {
					float fxx = this.getAdjustedVolume(soundInstance);
	...
	public boolean isPlaying(SoundInstance soundInstance) {
		if (!this.started) {
			...
			return this.soundEndTicks.containsKey(soundInstance) && this.soundEndTicks.get(soundInstance) <= this.ticks
				? true
				: this.sources.containsKey(soundInstance);
		}
	}
	...
	public void stopSounds(@Nullable Identifier identifier, @Nullable SoundCategory soundCategory) {
		if (soundCategory != null) {
			...
			for(SoundInstance soundInstance : this.sources.keySet()) {
				if (soundInstance.getId().equals(identifier)) {
					this.stop(soundInstance);
				}
			}
		}
