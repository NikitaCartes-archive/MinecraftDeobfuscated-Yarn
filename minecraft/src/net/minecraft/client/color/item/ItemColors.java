	...
	public static ItemColors create(BlockColors blockColors) {
		...
		itemColors.register(
			(stack, tintIndex) -> tintIndex > 0
					? -1
					: ColorHelper.Argb.fullAlpha(((PotionContentsComponent)stack.getOrDefault(DataComponentTypes.POTION_CONTENTS, PotionContentsComponent.DEFAULT)).getColor()),
			Items.POTION,
			Items.SPLASH_POTION,
		...
		itemColors.register(
			(stack, tintIndex) -> tintIndex == 0
					? -1
					: ColorHelper.Argb.fullAlpha(((MapColorComponent)stack.getOrDefault(DataComponentTypes.MAP_COLOR, MapColorComponent.DEFAULT)).rgb()),
			Items.FILLED_MAP
		);
		return itemColors;
	}
