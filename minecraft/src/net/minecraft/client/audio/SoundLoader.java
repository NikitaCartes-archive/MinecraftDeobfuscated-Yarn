...
public class SoundLoader {
	...
	private final Map<Identifier, CompletableFuture<SoundData>> loadedSounds = Maps.newHashMap();
	...
	public CompletableFuture<?> method_19741(Collection<Sound> collection) {
		return CompletableFuture.allOf(
			(CompletableFuture[])collection.stream().map(sound -> this.method_19743(sound.getLocation())).toArray(i -> new CompletableFuture[i])
		);
	}
}
