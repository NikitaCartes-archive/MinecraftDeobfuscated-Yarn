...
public class SoundManager {
	...
	private final Map<SoundInstance, Integer> field_5566 = Maps.newHashMap();
	private final Map<String, Integer> field_5554 = Maps.newHashMap();
	private final List<ListenerSoundInstance> listeners = Lists.<ListenerSoundInstance>newArrayList();
	private final List<String> pausedSoundIds = Lists.newArrayList();
	...
	public void update() {
		...
		while(iterator.hasNext()) {
			Entry<String, SoundInstance> entry = (Entry)iterator.next();
			...
			if (!this.system.playing(string)) {
				int i = this.field_5554.get(string);
				if (i <= this.ticks) {
		...
		while(iterator2.hasNext()) {
			...
			if (this.ticks >= entry2.getValue()) {
				...
				if (soundInstance instanceof TickableSoundInstance) {
					((TickableSoundInstance)soundInstance).method_16896();
				}
	...
	public boolean isPlaying(SoundInstance soundInstance) {
		if (!this.initialized) {
			...
			if (string == null) {
				...
				return this.system.playing(string) || this.field_5554.containsKey(string) && this.field_5554.get(string) <= this.ticks;
			}
		}
	}
