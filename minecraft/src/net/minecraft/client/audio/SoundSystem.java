...
public class SoundSystem {
	...
	private final Map<SoundInstance, Integer> field_18952 = Maps.newHashMap();
	...
	private void tick() {
		...
		while(iterator.hasNext()) {
			...
			if (h <= 0.0F) {
				...
				int i = this.field_18952.get(soundInstance);
				if (i <= this.ticks) {
	...
	public boolean isPlaying(SoundInstance soundInstance) {
		if (!this.initialized) {
			...
			return this.field_18952.containsKey(soundInstance) && this.field_18952.get(soundInstance) <= this.ticks
				? true
				: this.field_18950.containsKey(soundInstance);
		}
	}
	...
	public void stopSounds(@Nullable Identifier identifier, @Nullable SoundCategory soundCategory) {
		if (soundCategory != null) {
			...
			for(SoundInstance soundInstance : this.field_18950.keySet()) {
