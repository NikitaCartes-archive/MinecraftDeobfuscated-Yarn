		...
		if (property == null) {
			...
			if (list.isEmpty()) {
				throw new RuntimeException(
					String.format("Empty value '%s' for property '%s' on '%s'", this.valueString, this.key, stateFactory.getBaseObject().toString())
				);
			} else {
				...
				if (list.size() == 1) {
					...
					List<Predicate<BlockState>> list2 = (List)list.stream().map(stringx -> this.method_3525(stateFactory, property, stringx)).collect(Collectors.toList());
