...
public class AdvancementToast implements Toast {
	...
	public Toast.Visibility method_1986(ToastManager toastManager, long l) {
		...
		if (advancementDisplay != null) {
			List<String> list = toastManager.getGame().field_1772.wrapStringToWidthAsList(advancementDisplay.getTitle().asFormattedString(), 125);
			int i = advancementDisplay.getFrame() == AdvancementFrame.field_1250 ? 16746751 : 16776960;
			if (list.size() == 1) {
				toastManager.getGame().field_1772.draw(I18n.translate("advancements.toast." + advancementDisplay.getFrame().getId()), 30.0F, 7.0F, i | 0xFF000000);
				toastManager.getGame().field_1772.draw(advancementDisplay.getTitle().asFormattedString(), 30.0F, 18.0F, -1);
			} else {
				int j = 1500;
				float f = 300.0F;
				if (l < 1500L) {
					int k = MathHelper.floor(MathHelper.clamp((float)(1500L - l) / 300.0F, 0.0F, 1.0F) * 255.0F) << 24 | 67108864;
					toastManager.getGame().field_1772.draw(I18n.translate("advancements.toast." + advancementDisplay.getFrame().getId()), 30.0F, 11.0F, i | k);
				} else {
			...
			if (!this.field_2206 && l > 0L) {
				...
				if (advancementDisplay.getFrame() == AdvancementFrame.field_1250) {
					toastManager.getGame().method_1483().play(PositionedSoundInstance.master(SoundEvents.field_15195, 1.0F, 1.0F));
				}
			}
