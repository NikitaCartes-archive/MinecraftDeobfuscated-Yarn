...
public class FirstPersonRenderer {
	...
	public void renderFirstPersonItem(AbstractClientPlayerEntity abstractClientPlayerEntity, float f, float g, Hand hand, float h, ItemStack itemStack, float i) {
		...
		if (itemStack.isEmpty()) {
			...
			boolean bl3 = abstractClientPlayerEntity.isUsingItem() && abstractClientPlayerEntity.getActiveHand() == hand;
			boolean bl4 = !bl3 && hand == Hand.field_5810 && abstractClientPlayerEntity.method_6039();
			if (bl3 || bl4) {
				int p = bl2 ? 1 : -1;
				switch (itemStack.getUseAction()) {
					...
					case field_8953:
						...
						float lxx = (float)itemStack.getMaxUseTime() - ((float)this.client.player.getItemUseTimeLeft() - f + 1.0F);
						float mx = lxx / 20.0F;
						mx = (mx * mx + mx * 2.0F) / 3.0F;
						if (mx > 1.0F) {
							mx = 1.0F;
						}

						if (mx > 0.1F) {
							float n = MathHelper.sin((lxx - 0.1F) * 1.3F);
							float o = mx - 0.1F;
							float q = n * o;
							GlStateManager.translatef(q * 0.0F, q * 0.004F, q * 0.0F);
						}

						GlStateManager.translatef(mx * 0.0F, mx * 0.0F, mx * 0.04F);
						GlStateManager.scalef(1.0F, 1.0F, 1.0F + mx * 0.2F);
					...
					case field_8951:
						...
						GlStateManager.rotatef((float)p * 35.3F, 0.0F, 1.0F, 0.0F);
						GlStateManager.rotatef((float)p * -9.785F, 0.0F, 0.0F, 1.0F);
						float lx = (float)itemStack.getMaxUseTime() - ((float)this.client.player.getItemUseTimeLeft() - f + 1.0F);
						float m = lx / 10.0F;
						if (m > 1.0F) {
							m = 1.0F;
						}
						...
						if (m > 0.1F) {
							float n = MathHelper.sin((lx - 0.1F) * 1.3F);
							float o = m - 0.1F;
							float q = n * o;
							GlStateManager.translatef(q * 0.0F, q * 0.004F, q * 0.0F);
						}

						GlStateManager.translatef(0.0F, 0.0F, m * 0.2F);
						GlStateManager.scalef(1.0F, 1.0F, 1.0F + m * 0.2F);
						GlStateManager.rotatef((float)p * 45.0F, 0.0F, -1.0F, 0.0F);
				}
				...
				float kx = -0.4F * MathHelper.sin(MathHelper.sqrt(h) * (float) Math.PI);
				float lxxx = 0.2F * MathHelper.sin(MathHelper.sqrt(h) * (float) (Math.PI * 2));
				float mxx = -0.2F * MathHelper.sin(h * (float) Math.PI);
				int r = bl2 ? 1 : -1;
				GlStateManager.translatef((float)r * kx, lxxx, mxx);
				this.applyHandOffset(arm, i);
				this.method_3217(arm, h);
			}
