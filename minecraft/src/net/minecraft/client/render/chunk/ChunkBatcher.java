...
public class ChunkBatcher {
	...
	public ChunkBatcher(World world, WorldRenderer worldRenderer, Executor executor, boolean bl, BlockLayeredBufferBuilder blockLayeredBufferBuilder) {
		...
		int i = Math.max(
			1,
			(int)((double)Runtime.getRuntime().maxMemory() * 0.3) / (BlockRenderLayer.method_22720().stream().mapToInt(BlockRenderLayer::method_22722).sum() * 4) - 1
		);
				...
				if (chunkRendererRegion != null) {
					...
					for(BlockPos blockPos3 : BlockPos.iterate(blockPos, blockPos2)) {
						...
						if (blockState.getRenderType() != BlockRenderType.INVISIBLE) {
							...
							if (blockRenderManager.tesselateBlock(blockState, blockPos3, chunkRendererRegion, lv, bufferBuilder, true, random)) {
								chunkRenderData.empty = false;
								chunkRenderData.nonEmpty.add(blockRenderLayer);
							}
						}
