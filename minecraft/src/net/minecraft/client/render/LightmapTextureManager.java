...
public class LightmapTextureManager implements AutoCloseable {
	...
	public LightmapTextureManager(GameRenderer gameRenderer) {
		...
		this.textureIdentifier = this.client.method_1531().registerDynamicTexture("light_map", this.field_4138);
	...
	public void enable() {
		...
		this.client.method_1531().bindTexture(this.textureIdentifier);
	...
	public void update(float f) {
		if (this.isDirty) {
			...
			if (world != null) {
				...
				float j;
				if (this.client.field_1724.hasStatusEffect(StatusEffects.field_5925)) {
					j = this.worldRenderer.getNightVisionStrength(this.client.field_1724, f);
				} else if (i > 0.0F && this.client.field_1724.hasStatusEffect(StatusEffects.field_5927)) {
					j = i;
				} else {
					j = 0.0F;
				}
				...
				for(int k = 0; k < 16; ++k) {
					for(int l = 0; l < 16; ++l) {
						...
						float x = (float)this.client.field_1690.gamma;
						float y = 1.0F - u;
						float z = 1.0F - v;
						float aa = 1.0F - w;
						y = 1.0F - y * y * y * y;
