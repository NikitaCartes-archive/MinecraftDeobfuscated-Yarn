...
public class GameEventDebugRenderer implements DebugRenderer.Renderer {
		...
		if (world == null) {
			...
			for(GameEventDebugRenderer.Listener listener : this.listeners) {
				listener.getPos(world)
					.ifPresent(
						vec3dx -> {
							double gxx = vec3dx.getX() - (double)listener.getRange();
							double hxx = vec3dx.getY() - (double)listener.getRange();
							double ixx = vec3dx.getZ() - (double)listener.getRange();
							double jxx = vec3dx.getX() + (double)listener.getRange();
							...
							WorldRenderer.method_22983(
								matrices,
								vertexConsumer,
								VoxelShapes.cuboid(new Box(gxx, hxx, ixx, jxx, k, l)),
								-cameraX,
								-cameraY,
								-cameraZ,
								vec3f.getX(),
	...
	static class Listener implements GameEventListener {
		...
		public boolean isTooFar(World world, Vec3d to) {
			return this.positionSource.getPos(world).filter(from -> from.squaredDistanceTo(to) <= 1024.0).isPresent();
		}
