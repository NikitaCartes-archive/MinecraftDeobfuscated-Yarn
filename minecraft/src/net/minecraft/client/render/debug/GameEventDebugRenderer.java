		...
		if (world == null) {
			...
			for(GameEventDebugRenderer.Listener listener : this.listeners) {
				listener.getPos(world)
					.ifPresent(
						pos -> {
							double gxx = pos.getX() - (double)listener.getRange();
							double hxx = pos.getY() - (double)listener.getRange();
							double ixx = pos.getZ() - (double)listener.getRange();
							double jxx = pos.getX() + (double)listener.getRange();
							double k = pos.getY() + (double)listener.getRange();
							double l = pos.getZ() + (double)listener.getRange();
							WorldRenderer.drawShapeOutline(
								matrices, vertexConsumer, VoxelShapes.cuboid(new Box(gxx, hxx, ixx, jxx, k, l)), -cameraX, -cameraY, -cameraZ, 1.0F, 1.0F, 0.0F, 0.35F, true
							);
						}
					);
			}
