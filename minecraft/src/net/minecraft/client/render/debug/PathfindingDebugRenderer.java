...
public class PathfindingDebugRenderer implements DebugRenderer.DebugRenderer {
	...
	private final Map<Integer, Path> field_4616 = Maps.newHashMap();
		...
		if (!this.field_4616.isEmpty()) {
			...
			for(Integer integer : this.field_4616.keySet()) {
				...
				for(PathNode pathNode2 : path.method_37()) {
					if (!(this.method_3867(pathNode2) > 40.0F)) {
						DebugRenderer.method_3714(
							String.format("%s", pathNode2.type), (double)pathNode2.x + 0.5, (double)pathNode2.y + 0.75, (double)pathNode2.z + 0.5, f, -65536
						);
						DebugRenderer.method_3714(
				...
				for(PathNode pathNode2 : path.method_43()) {
					if (!(this.method_3867(pathNode2) > 40.0F)) {
						DebugRenderer.method_3714(
							String.format("%s", pathNode2.type), (double)pathNode2.x + 0.5, (double)pathNode2.y + 0.75, (double)pathNode2.z + 0.5, f, -16776961
						);
						DebugRenderer.method_3714(
							String.format(Locale.ROOT, "%.2f", pathNode2.field_43), (double)pathNode2.x + 0.5, (double)pathNode2.y + 0.25, (double)pathNode2.z + 0.5, f, -16776961
						);
					}
				}
