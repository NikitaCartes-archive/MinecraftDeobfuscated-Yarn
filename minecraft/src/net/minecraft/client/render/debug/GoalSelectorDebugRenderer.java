		...
		this.goalSelectors.forEach((integer, list) -> {
			for(int i = 0; i < list.size(); ++i) {
				...
				if (blockPos.isWithinDistance(goalSelector.pos, 160.0)) {
					double dxx = (double)goalSelector.pos.getX() + 0.5;
					double exx = (double)goalSelector.pos.getY() + 2.0 + (double)i * 0.25;
					double fxx = (double)goalSelector.pos.getZ() + 0.5;
					int j = goalSelector.field_18785 ? -16711936 : -3355444;
					DebugRenderer.drawString(goalSelector.name, dxx, exx, fxx, j);
				}
			}
		});
