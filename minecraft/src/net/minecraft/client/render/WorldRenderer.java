...
public class WorldRenderer implements AutoCloseable, ResourceReloadListener {
	...
	private long field_3998 = SystemUtil.getMeasuringTimeMili();
	...
	public WorldRenderer(MinecraftClient minecraftClient, ResourceManager resourceManager) {
	...
	private void loadShader(Identifier identifier) {
		...
		try {
			this.field_4024 = new class_279(this.client.getTextureManager(), this.resourceContainer, this.client.getFramebuffer(), identifier);
			this.field_4024.method_1259(this.client.window.getWindowWidth(), this.client.window.getWindowHeight());
	...
	public void method_3169(int i, int j) {
		if (GLX.usePostProcess) {
			...
			this.client.renderer.method_3242(i, j);
		}
	}
	...
	public void method_3190(float f) {
		...
		if (entity != null) {
			if (this.client.world != null) {
				this.client.getProfiler().begin("pick");
	...
	private void method_3185(float f) {
		this.field_4025 = (float)(this.client.options.viewDistance * 16);
	...
	private void method_3172(float f) {
		if (!this.field_4001) {
			...
			if (this.client.options.field_1850 == 0
				&& !bl
				&& !this.client.options.field_1842
	...
	public void method_3192(float f, long l, boolean bl) {
		if (!this.client.method_1569() && this.client.options.pauseOnLostFocus && (!this.client.options.touchscreen || !this.client.mouse.method_1609())) {
			if (SystemUtil.getMeasuringTimeMili() - this.field_3998 > 500L) {
				this.client.openInGameMenu();
			}
		} else {
			this.field_3998 = SystemUtil.getMeasuringTimeMili();
		}

		if (!this.client.field_1743) {
			int i = (int)(this.client.mouse.method_1603() * (double)this.client.window.getScaledWidth() / (double)this.client.window.method_4480());
			int j = (int)(this.client.mouse.method_1604() * (double)this.client.window.getScaledHeight() / (double)this.client.window.method_4507());
			int k = this.client.options.maxFps;
			if (bl && this.client.world != null) {
				this.client.getProfiler().begin("level");
				...
				if (this.client.method_1496() && this.field_4017 < SystemUtil.getMeasuringTimeMili() - 1000L) {
					this.field_4017 = SystemUtil.getMeasuringTimeMili();
				...
				if (GLX.usePostProcess) {
					this.client.renderer.drawFramebuffer();
				...
				this.client.getProfiler().endBegin("gui");
				if (!this.client.options.field_1842 || this.client.currentGui != null) {
			...
			if (this.client.currentGui != null) {
				...
				try {
					...
					CrashReportElement crashReportElement = crashReport.addElement("Screen render details");
					crashReportElement.add("Screen name", (ICrashCallable<String>)(() -> this.client.currentGui.getClass().getCanonicalName()));
					crashReportElement.add(
						"Mouse location",
						(ICrashCallable<String>)(() -> String.format(
								Locale.ROOT, "Scaled: (%d, %d). Absolute: (%f, %f)", i, j, this.client.mouse.method_1603(), this.client.mouse.method_1604()
							))
					);
					crashReportElement.add(
						"Screen size",
	...
	private void method_3176() {
		if (this.client.renderer.getChunkNumber() > 10 && this.client.renderer.method_3281() && !this.client.getServer().hasIconFile()) {
	...
	private boolean method_3202() {
		if (!this.field_4009) {
			...
			Entity entity = this.client.getCameraEntity();
			boolean bl = entity instanceof PlayerEntity && !this.client.options.field_1842;
	...
	private void method_3178(float f, long l) {
		...
		if (this.client.options.viewDistance >= 4) {
			...
			this.client.getProfiler().endBegin("sky");
			GlStateManager.matrixMode(5889);
			GlStateManager.loadIdentity();
			GlStateManager.multMatrix(
				Matrix4f.method_4929(
		...
		if (entity.y + (double)entity.getEyeHeight() < 128.0) {
			this.method_3206(renderer, f, d, e, g);
		}

		this.client.getProfiler().endBegin("prepareterrain");
		this.field_3990.method_3211(0, f);
		this.client.getTextureManager().bindTexture(SpriteAtlasTexture.BLOCK_ATLAS_TEX);
		class_308.method_1450();
		this.client.getProfiler().endBegin("terrain_setup");
		this.client.world.getChunkProvider().getLightingProvider().method_15563(Integer.MAX_VALUE, true, true);
		renderer.method_3273(entity, f, lv2, this.field_4021++, this.client.player.isSpectator());
		this.client.getProfiler().endBegin("updatechunks");
		this.client.renderer.method_3269(l);
		this.client.getProfiler().endBegin("terrain");
		GlStateManager.matrixMode(5888);
		GlStateManager.pushMatrix();
		GlStateManager.disableAlphaTest();
		renderer.method_3251(BlockRenderLayer.SOLID, (double)f, entity);
		GlStateManager.enableAlphaTest();
		renderer.method_3251(BlockRenderLayer.MIPPED_CUTOUT, (double)f, entity);
		this.client.getTextureManager().getTexture(SpriteAtlasTexture.BLOCK_ATLAS_TEX).pushFilter(false, false);
		renderer.method_3251(BlockRenderLayer.CUTOUT, (double)f, entity);
		...
		this.client.getProfiler().endBegin("entities");
		renderer.renderEntities(entity, lv2, f);
		class_308.method_1450();
		this.method_3187();
		GlStateManager.matrixMode(5888);
		GlStateManager.popMatrix();
		if (bl && this.client.hitResult != null) {
			...
			GlStateManager.disableAlphaTest();
			this.client.getProfiler().endBegin("outline");
			renderer.method_3294(playerEntity, this.client.hitResult, 0, f);
			GlStateManager.enableAlphaTest();
		}
		...
		this.client.getProfiler().endBegin("destroyProgress");
		...
		this.client.getTextureManager().getTexture(SpriteAtlasTexture.BLOCK_ATLAS_TEX).pushFilter(false, false);
		renderer.renderPartiallyBrokenBlocks(Tessellator.getInstance(), Tessellator.getInstance().getVertexBuffer(), entity, f);
		this.client.getTextureManager().getTexture(SpriteAtlasTexture.BLOCK_ATLAS_TEX).popFilter();
		GlStateManager.disableBlend();
		this.method_3180();
		this.client.getProfiler().endBegin("litParticles");
		particleManager.method_3060(entity, f);
		class_308.method_1450();
		this.field_3990.method_3211(0, f);
		this.client.getProfiler().endBegin("particles");
		particleManager.method_3049(entity, f);
		this.method_3187();
		GlStateManager.depthMask(false);
		GlStateManager.enableCull();
		this.client.getProfiler().endBegin("weather");
		this.method_3170(f);
		GlStateManager.depthMask(true);
		renderer.renderWorldBorder(entity, f);
		GlStateManager.disableBlend();
		GlStateManager.enableCull();
		GlStateManager.blendFuncSeparate(
		...
		this.client.getTextureManager().bindTexture(SpriteAtlasTexture.BLOCK_ATLAS_TEX);
		GlStateManager.shadeModel(7425);
		this.client.getProfiler().endBegin("translucent");
		renderer.method_3251(BlockRenderLayer.TRANSLUCENT, (double)f, entity);
		GlStateManager.shadeModel(7424);
		GlStateManager.depthMask(true);
		GlStateManager.enableCull();
		GlStateManager.disableBlend();
		...
		if (entity.y + (double)entity.getEyeHeight() >= 128.0) {
			this.client.getProfiler().endBegin("aboveClouds");
			this.method_3206(renderer, f, d, e, g);
		}

		this.client.getProfiler().endBegin("hand");
		if (this.field_3992) {
			GlStateManager.clear(256, MinecraftClient.isSystemMac);
			this.method_3172(f);
		}
	...
	private void method_3206(Renderer renderer, float f, double d, double e, double g) {
		if (this.client.options.getCloudRenderMode() != 0) {
			this.client.getProfiler().endBegin("clouds");
			GlStateManager.matrixMode(5889);
			GlStateManager.loadIdentity();
			GlStateManager.multMatrix(
				Matrix4f.method_4929(
					...
					if (biome.getPrecipitation() != Biome.Precipitation.NONE) {
						...
						if (v != w) {
							...
							if (y >= 0.15F) {
								...
								int af = ae >> 16 & 65535;
								int ag = ae & 65535;
								vertexBuffer.vertex((double)q - s + 0.5, (double)w, (double)p - t + 0.5)
								...
								vertexBuffer.vertex((double)q + s + 0.5, (double)w, (double)p + t + 0.5)
									...
									.texture(af, ag)
									.next();
								vertexBuffer.vertex((double)q + s + 0.5, (double)v, (double)p + t + 0.5)
	...
	public static void method_3179(FontRenderer fontRenderer, String string, float f, float g, float h, int i, float j, float k, boolean bl, boolean bl2) {
		...
		vertexBuffer.vertex((double)(-l - 1), (double)(-1 + i), 0.0).color(0.0F, 0.0F, 0.0F, 0.25F).next();
		vertexBuffer.vertex((double)(-l - 1), (double)(8 + i), 0.0).color(0.0F, 0.0F, 0.0F, 0.25F).next();
		vertexBuffer.vertex((double)(l + 1), (double)(8 + i), 0.0).color(0.0F, 0.0F, 0.0F, 0.25F).next();
		vertexBuffer.vertex((double)(l + 1), (double)(-1 + i), 0.0).color(0.0F, 0.0F, 0.0F, 0.25F).next();
		tessellator.draw();
	...
	private void method_3171(int i, int j, float f) {
		if (this.field_4006 != null && this.field_4007 > 0) {
			...
			this.client.getItemRenderer().renderItemWithTransformation(this.field_4006, ModelTransformations.Type.FIXED);
