...
public class WorldRenderer implements WorldListener, AutoCloseable, ResourceReloadListener {
	...
	private boolean doesEntityHaveOutline(Entity entity, Entity entity2, VisibleRegion visibleRegion) {
		...
		if (entity == entity2 && this.client.options.field_1850 == 0 && !bl) {
			...
			return true;
		} else if (this.client.player.isSpectator() && this.client.options.keySpectatorOutlines.isPressed() && entity instanceof PlayerEntity) {
			return entity.ignoreCameraFrustum || visibleRegion.intersects(entity.getBoundingBox()) || entity.method_5821(this.client.player);
		} else {
			return false;
		}
	}
	...
	public void updateChunks(long l) {
		this.terrainUpdateNecessary |= this.chunkBatcher.method_3631(l);
	...
	public void addParticle(ParticleParameters particleParameters, boolean bl, boolean bl2, double d, double e, double f, double g, double h, double i) {
		try {
			...
			crashReportSection.add("ID", Registry.PARTICLE_TYPE.getId(particleParameters.getType()));
	...
	public void onWorldEvent(PlayerEntity playerEntity, int i, BlockPos blockPos, int j) {
		...
		switch(i) {
			...
			case 1042:
				...
				break;
			case 1043:
				this.world.playSoundClient(blockPos, SoundEvents.field_17481, SoundCategory.field_15245, 1.0F, this.world.random.nextFloat() * 0.1F + 0.9F, false);
				break;
			case 2000:
