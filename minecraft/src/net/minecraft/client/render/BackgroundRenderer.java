	...
	public static void render(Camera camera, float tickDelta, ClientWorld world, int viewDistance, float skyDarkness) {
		...
		if (cameraSubmersionType == CameraSubmersionType.WATER) {
			...
			Vec3d vec3d3 = CubicSampler.sampleColor(vec3d2, (x, y, z) -> {
				...
				return world.getDimensionEffects().adjustFogColor(Vec3d.unpackRgb(registryEntry.value().getFogColor(registryEntry)), v);
			});
			...
			green += (t - green) * r;
			blue += (u - blue) * r;
			float w = world.getRainGradient(tickDelta);
			if (w > 0.0F) {
				float f = 1.0F - w * 0.5F;
				float g = 1.0F - w * 0.4F;
				red *= f;
