...
public class BackgroundRenderer {
	...
	public void render(Camera camera, float tickDelta, World world, int viewDistance, float skyDarkness) {
		...
		if (fluidState.matches(FluidTags.WATER)) {
			...
			float w = Math.min(1.0F / r, Math.min(1.0F / s, 1.0F / t));
			r = r * (1.0F - v) + r * w * v;
			s = s * (1.0F - v) + s * w * v;
			t = t * (1.0F - v) + t * w * v;
			...
			float v = GameRenderer.getNightVisionStrength((LivingEntity)camera.getFocusedEntity(), tickDelta);
			float w = Math.min(1.0F / r, Math.min(1.0F / s, 1.0F / t));
			r = r * (1.0F - v) + r * w * v;
			s = s * (1.0F - v) + s * w * v;
			t = t * (1.0F - v) + t * w * v;
