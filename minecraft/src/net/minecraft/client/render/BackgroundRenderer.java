	...
	public static void render(Camera camera, float tickDelta, ClientWorld world, int viewDistance, float skyDarkness) {
		...
		if (cameraSubmersionType == CameraSubmersionType.WATER) {
			if (entity instanceof ClientPlayerEntity) {
				t = ((ClientPlayerEntity)entity).getUnderwaterVisibility();
			} else {
				t = 1.0F;
			}
		} else if (entity instanceof LivingEntity && ((LivingEntity)entity).hasStatusEffect(StatusEffects.NIGHT_VISION)) {
			t = GameRenderer.getNightVisionStrength((LivingEntity)entity, tickDelta);
		} else {
			t = 0.0F;
		}
		...
		if (red != 0.0F && green != 0.0F && blue != 0.0F) {
			...
			red = red * (1.0F - t) + red * u * t;
			green = green * (1.0F - t) + green * u * t;
			blue = blue * (1.0F - t) + blue * u * t;
		}
