...
public class HeldItemRenderer {
			...
			if (player.isUsingItem() && player.getItemUseTimeLeft() > 0 && player.getActiveHand() == hand) {
				...
				switch(item.getUseAction()) {
					...
					case BOW:
						...
						matrices.translate((double)(f * 0.0F), (double)(f * 0.0F), (double)(f * 0.04F));
					...
					case SPEAR:
						...
						matrices.translate(0.0, 0.0, (double)(f * 0.2F));
						matrices.scale(1.0F, 1.0F, 1.0F + f * 0.2F);
						matrices.multiply(Vector3f.NEGATIVE_Y.getDegreesQuaternion((float)l * 45.0F));
				}
				...
				int o = bl2 ? 1 : -1;
				matrices.translate((double)((float)o * n), (double)m, (double)f);
				this.applyEquipOffset(matrices, arm, equipProgress);
				this.applySwingOffset(matrices, arm, swingProgress);
			}
	...
	public void updateHeldItems() {
		...
		if (clientPlayerEntity.isRiding()) {
			...
			this.equipProgressMainHand += MathHelper.clamp(
				(ItemStack.areEqualIgnoreDamage(this.mainHand, itemStack) ? f * f * f : 0.0F) - this.equipProgressMainHand, -0.4F, 0.4F
			);
			this.equipProgressOffHand += MathHelper.clamp(
				(float)(ItemStack.areEqualIgnoreDamage(this.offHand, itemStack2) ? 1 : 0) - this.equipProgressOffHand, -0.4F, 0.4F
			);
		}
