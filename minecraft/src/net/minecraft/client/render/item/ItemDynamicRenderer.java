...
public class ItemDynamicRenderer {
	...
	public void render(ItemStack itemStack) {
		...
		if (item instanceof BannerItem) {
			...
			if (itemStack.getSubTag("BlockEntityTag") != null) {
				...
				MinecraftClient.getInstance().method_1531().bindTexture(TextureCache.DEFAULT_SHIELD);
			}
			...
			if (SkullBlockEntityRenderer.INSTANCE != null) {
				...
				SkullBlockEntityRenderer.INSTANCE
					.render(0.0F, 0.0F, 0.0F, null, 180.0F, ((AbstractSkullBlock)((BlockItem)item).method_7711()).getSkullType(), gameProfile, -1, 0.0F);
				GlStateManager.enableCull();
				GlStateManager.popMatrix();
			}
		} else if (item == Items.field_8547) {
			MinecraftClient.getInstance().method_1531().bindTexture(TridentEntityModel.TEXTURE);
			GlStateManager.pushMatrix();
			GlStateManager.scalef(1.0F, -1.0F, -1.0F);
	...
	private void renderEnchantmentGlint(Runnable runnable) {
		...
		MinecraftClient.getInstance().method_1531().bindTexture(ItemRenderer.ENCHANTMENT_GLINT_TEX);
