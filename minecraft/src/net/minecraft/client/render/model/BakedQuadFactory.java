...
public class BakedQuadFactory {
	...
	private void method_3462(int[] is, Direction direction) {
		...
		for(int j = 0; j < 4; ++j) {
			int k = 7 * j;
			SomethingDirectionSomethingQuadBakery.class_755 lv = somethingDirectionSomethingQuadBakery.method_3162(j);
			float h = fs[lv.field_3975];
			...
			for(int n = 0; n < 4; ++n) {
				...
				if (MathHelper.equalsApproximate(h, p) && MathHelper.equalsApproximate(l, q) && MathHelper.equalsApproximate(m, r)) {
					is[k + 4] = js[o + 4];
					is[k + 4 + 1] = js[o + 4 + 1];
				}
			}
