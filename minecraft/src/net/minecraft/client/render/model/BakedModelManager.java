...
public class BakedModelManager implements ResourceReloader, AutoCloseable {
			...
			.thenCompose(
				blockStates -> {
					...
					for(Entry<Identifier, List<Resource>> entry : blockStates.entrySet()) {
						list.add(CompletableFuture.supplyAsync(() -> {
							List<Resource> listxx = (List)entry.getValue();
							List<ModelLoader.SourceTrackedData> list2 = new ArrayList(listxx.size());
		
							for(Resource resource : listxx) {
								try {
	...
	private BakedModelManager.BakingResult bake(Profiler profiler, Map<Identifier, SpriteAtlasManager.AtlasPreparation> preparations, ModelLoader modelLoader) {
		...
		CompletableFuture<Void> completableFuture = CompletableFuture.allOf(
			(CompletableFuture[])preparations.values().stream().map(SpriteAtlasManager.AtlasPreparation::whenComplete).toArray(i -> new CompletableFuture[i])
		);
		profiler.pop();
		profiler.endTick();
