...
public class WeightedUnbakedModel implements UnbakedModel {
	...
	public Collection<class_4730> getTextureDependencies(
		...
		return (Collection<class_4730>)this.getVariants()
			.stream()
			.map(ModelVariant::getLocation)
			.distinct()
			.flatMap(
				identifier -> ((UnbakedModel)unbakedModelGetter.apply(identifier)).getTextureDependencies(unbakedModelGetter, unresolvedTextureReferences).stream()
			)
