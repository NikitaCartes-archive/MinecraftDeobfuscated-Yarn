...
public class MultipartModelComponent {
	...
	public static class Deserializer implements JsonDeserializer<MultipartModelComponent> {
		public MultipartModelComponent method_3535(JsonElement element, Type type, JsonDeserializationContext context) throws JsonParseException {
		...
		static MultipartModelSelector deserializeSelector(JsonObject object) {
			...
			if (set.isEmpty()) {
				...
				if (object.has("OR")) {
					List<MultipartModelSelector> list = (List)Streams.stream(JsonHelper.getArray(object, "OR"))
					...
					List<MultipartModelSelector> list = (List)Streams.stream(JsonHelper.getArray(object, "AND"))
						.map(jsonElement -> deserializeSelector(jsonElement.getAsJsonObject()))
						.collect(Collectors.toList());
