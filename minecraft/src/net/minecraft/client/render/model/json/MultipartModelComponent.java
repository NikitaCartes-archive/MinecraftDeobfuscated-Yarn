		...
		static MultipartModelSelector deserializeSelector(JsonObject jsonObject) {
			...
			if (set.isEmpty()) {
				...
				if (jsonObject.has("OR")) {
					List<MultipartModelSelector> list = (List)Streams.stream(JsonHelper.getArray(jsonObject, "OR"))
					...
					List<MultipartModelSelector> list = (List)Streams.stream(JsonHelper.getArray(jsonObject, "AND"))
						.map(jsonElement -> deserializeSelector(jsonElement.getAsJsonObject()))
						.collect(Collectors.toList());
