	...
	public boolean method_3347(ExtendedBlockView extendedBlockView, BlockPos blockPos, VertexBuffer vertexBuffer, FluidState fluidState) {
		...
		if (!bl2 && !bl3 && !bl7 && !bl6 && !bl4 && !bl5) {
			...
			if (bl3) {
				...
				float ap = 0.5F * g;
				float af = 0.5F * h;
				vertexBuffer.vertex(d, e, r + 1.0).color(aax, ap, af, 1.0F).texture((double)tx, (double)zx).texture(an, ao).next();
				vertexBuffer.vertex(d, e, r).color(aax, ap, af, 1.0F).texture((double)tx, (double)xx).texture(an, ao).next();
				vertexBuffer.vertex(d + 1.0, e, r).color(aax, ap, af, 1.0F).texture((double)vx, (double)xx).texture(an, ao).next();
				vertexBuffer.vertex(d + 1.0, e, r + 1.0).color(aax, ap, af, 1.0F).texture((double)vx, (double)zx).texture(an, ao).next();
				bl8 = true;
			}
			...
			for (int aq = 0; aq < 4; aq++) {
				...
				if (bl9 && !method_3344(extendedBlockView, blockPos, direction, Math.max(vx, xx))) {
					...
					float bc = 1.0F * ba * g;
					float bd = 1.0F * ba * h;
					vertexBuffer.vertex(ar, e + (double)vx, at).color(bb, bc, bd, 1.0F).texture((double)av, (double)aj).texture(ay, az).next();
					vertexBuffer.vertex(as, e + (double)xx, au).color(bb, bc, bd, 1.0F).texture((double)aw, (double)ak).texture(ay, az).next();
