...
public class FluidRenderer {
	...
	protected void onResourceReload() {
		...
		this.field_4165[1] = spriteAtlasTexture.method_4608(ModelLoader.LAVA_FLOW);
		...
		this.field_4166[1] = spriteAtlasTexture.method_4608(ModelLoader.WATER_FLOW);
		this.field_4164 = spriteAtlasTexture.method_4608(ModelLoader.WATER_OVERLAY);
	}
	...
	public boolean tesselate(ExtendedBlockView extendedBlockView, BlockPos blockPos, BufferBuilder bufferBuilder, FluidState fluidState) {
		...
		if (!bl2 && !bl3 && !bl7 && !bl6 && !bl4 && !bl5) {
			...
			if (bl3) {
				...
				float ap = 0.5F * g;
				float af = 0.5F * h;
				bufferBuilder.vertex(d, e, r + 1.0).color(aa, ap, af, 1.0F).texture((double)t, (double)z).texture(an, ao).next();
				bufferBuilder.vertex(d, e, r).color(aa, ap, af, 1.0F).texture((double)t, (double)x).texture(an, ao).next();
				bufferBuilder.vertex(d + 1.0, e, r).color(aa, ap, af, 1.0F).texture((double)v, (double)x).texture(an, ao).next();
				bufferBuilder.vertex(d + 1.0, e, r + 1.0).color(aa, ap, af, 1.0F).texture((double)v, (double)z).texture(an, ao).next();
				bl8 = true;
			}
			...
			for(int aq = 0; aq < 4; ++aq) {
				...
				if (bl9 && !method_3344(extendedBlockView, blockPos, direction, Math.max(v, x))) {
					...
					float bc = 1.0F * ba * g;
					float bd = 1.0F * ba * h;
					bufferBuilder.vertex(ar, e + (double)v, at).color(bb, bc, bd, 1.0F).texture((double)av, (double)aj).texture(ay, az).next();
					bufferBuilder.vertex(as, e + (double)x, au).color(bb, bc, bd, 1.0F).texture((double)aw, (double)ak).texture(ay, az).next();
	...
	private float getNorthWestCornerFluidHeight(BlockView blockView, BlockPos blockPos, Fluid fluid) {
		...
		for(int j = 0; j < 4; ++j) {
			...
			} else if (!blockView.method_8320(blockPos2).method_11620().isSolid()) {
				++i;
			}
		}
