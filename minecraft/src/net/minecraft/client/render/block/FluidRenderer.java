	...
	public boolean tesselate(BlockRenderView blockRenderView, BlockPos blockPos, BufferBuilder bufferBuilder, FluidState fluidState) {
		...
		if (!bl2 && !bl3 && !bl7 && !bl6 && !bl4 && !bl5) {
			...
			if (bl3) {
				...
				float ap = 0.5F * g;
				float af = 0.5F * h;
				bufferBuilder.vertex(d, e, r + 1.0).color(aa, ap, af, 1.0F).texture((double)t, (double)z).texture(an, ao).normal(0.0F, 1.0F, 0.0F).next();
				bufferBuilder.vertex(d, e, r).color(aa, ap, af, 1.0F).texture((double)t, (double)x).texture(an, ao).normal(0.0F, 1.0F, 0.0F).next();
				bufferBuilder.vertex(d + 1.0, e, r).color(aa, ap, af, 1.0F).texture((double)v, (double)x).texture(an, ao).normal(0.0F, 1.0F, 0.0F).next();
				bufferBuilder.vertex(d + 1.0, e, r + 1.0).color(aa, ap, af, 1.0F).texture((double)v, (double)z).texture(an, ao).normal(0.0F, 1.0F, 0.0F).next();
				bl8 = true;
			}
			...
			for(int aq = 0; aq < 4; ++aq) {
				...
				if (bl9 && !method_3344(blockRenderView, blockPos, direction, Math.max(v, x))) {
					bl8 = true;
					BlockPos blockPos2 = blockPos.offset(direction);
					...
					float bc = 1.0F * ba * g;
					float bd = 1.0F * ba * h;
					bufferBuilder.vertex(ar, e + (double)v, at).color(bb, bc, bd, 1.0F).texture((double)av, (double)aj).texture(ay, az).normal(0.0F, 1.0F, 0.0F).next();
					bufferBuilder.vertex(as, e + (double)x, au).color(bb, bc, bd, 1.0F).texture((double)aw, (double)ak).texture(ay, az).normal(0.0F, 1.0F, 0.0F).next();
