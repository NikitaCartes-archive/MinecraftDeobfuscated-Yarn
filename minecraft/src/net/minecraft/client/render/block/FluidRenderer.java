...
public class FluidRenderer {
	...
	public boolean render(BlockRenderView world, BlockPos pos, VertexConsumer vertexConsumer, FluidState state) {
		...
		if (!bl2 && !bl3 && !bl7 && !bl6 && !bl4 && !bl5) {
			...
			for(int al = 0; al < 4; ++al) {
				...
				if (bl9 && !isSideCovered(world, pos, direction, Math.max(v, x))) {
					bl8 = true;
					...
					float aq = sprite2.getV((double)((1.0F - x) * 16.0F * 0.5F));
	...
	private int getLight(BlockRenderView world, BlockPos pos) {
		int i = world.getLightmapCoordinates(pos);
		int j = world.getLightmapCoordinates(pos.up());
		int k = i & 0xFF;
		int l = j & 0xFF;
		int m = i >> 16 & 0xFF;
		int n = j >> 16 & 0xFF;
