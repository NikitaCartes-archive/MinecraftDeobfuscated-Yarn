...
public class MobSpawnerBlockEntityRenderer extends BlockEntityRenderer<MobSpawnerBlockEntity> {
	...
	public static void method_3589(MobSpawnerLogic mobSpawnerLogic, double d, double e, double f, float g) {
		Entity entity = mobSpawnerLogic.method_8283();
		if (entity != null) {
			float h = 0.53125F;
			float i = Math.max(entity.width, entity.height);
			if ((double)i > 1.0) {
				h /= i;
			}
			...
			MinecraftClient.getInstance().getEntityRenderManager().method_3954(entity, 0.0, 0.0, 0.0, 0.0F, g, false);
		}
	}
}
