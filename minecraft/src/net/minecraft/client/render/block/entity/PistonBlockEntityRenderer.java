...
public class PistonBlockEntityRenderer extends BlockEntityRenderer<PistonBlockEntity> {
	...
	public void render(PistonBlockEntity pistonBlockEntity, double d, double e, double f, float g, int i) {
		BlockPos blockPos = pistonBlockEntity.getPos().method_10093(pistonBlockEntity.method_11506().getOpposite());
		...
		if (!blockState.isAir() && !(pistonBlockEntity.getProgress(g) >= 1.0F)) {
			...
			if (blockState.getBlock() == Blocks.field_10379 && pistonBlockEntity.getProgress(g) <= 4.0F) {
				...
				this.method_3575(blockPos, blockState, vertexBuffer, world, false);
				...
				this.method_3575(blockPos, blockState2, vertexBuffer, world, false);
				BlockPos blockPos2 = blockPos.method_10093(pistonBlockEntity.method_11506());
				vertexBuffer.setOffset(d - (double)blockPos2.getX(), e - (double)blockPos2.getY(), f - (double)blockPos2.getZ());
				blockState = blockState.with(PistonBlock.field_12191, Boolean.valueOf(true));
				this.method_3575(blockPos2, blockState, vertexBuffer, world, true);
			} else {
				this.method_3575(blockPos, blockState, vertexBuffer, world, false);
			}
	...
	private boolean method_3575(BlockPos blockPos, BlockState blockState, VertexBuffer vertexBuffer, World world, boolean bl) {
		return this.manager
			.getRenderer()
			.tesselate(world, this.manager.getModel(blockState), blockState, blockPos, vertexBuffer, bl, new Random(), blockState.getPosRandom(blockPos));
	}
}
