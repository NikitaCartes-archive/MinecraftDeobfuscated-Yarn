...
public class ItemFrameEntityRenderer extends EntityRenderer<ItemFrameEntity> {
	private static final Identifier field_4722 = new Identifier("textures/map/map_background.png");
	...
	public void method_3994(ItemFrameEntity itemFrameEntity, double d, double e, double f, float g, float h) {
		...
		this.renderManager.field_4685.method_4618(SpriteAtlasTexture.field_5275);
		...
		ModelIdentifier modelIdentifier = itemFrameEntity.method_6940().getItem() == Items.field_8204 ? field_4723 : field_4721;
		GlStateManager.pushMatrix();
		...
		if (itemFrameEntity.method_6940().getItem() == Items.field_8204) {
	...
	private void method_3992(ItemFrameEntity itemFrameEntity) {
		ItemStack itemStack = itemFrameEntity.method_6940();
		if (!itemStack.isEmpty()) {
			...
			if (bl) {
				...
				this.renderManager.field_4685.method_4618(field_4722);
				...
				this.field_4720.renderItem(itemStack, ModelTransformation.Type.FIXED);
			}
	...
	protected void method_3995(ItemFrameEntity itemFrameEntity, double d, double e, double f) {
		if (MinecraftClient.isHudEnabled()
			&& !itemFrameEntity.method_6940().isEmpty()
			&& itemFrameEntity.method_6940().hasDisplayName()
			&& this.renderManager.field_4678 == itemFrameEntity) {
			double g = itemFrameEntity.method_5707(this.renderManager.field_4686.method_19326());
			float h = itemFrameEntity.isSneaking() ? 32.0F : 64.0F;
			if (!(g >= (double)(h * h))) {
				String string = itemFrameEntity.method_6940().method_7964().getFormattedText();
