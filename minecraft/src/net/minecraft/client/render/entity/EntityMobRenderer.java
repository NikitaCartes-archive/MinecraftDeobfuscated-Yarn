...
public abstract class EntityMobRenderer<T extends MobEntity> extends LivingEntityRenderer<T> {
	public EntityMobRenderer(EntityRenderDispatcher entityRenderDispatcher, Model model, float f) {
		super(entityRenderDispatcher, model, f);
	}

	protected boolean method_4071(T mobEntity) {
		return super.method_4055(mobEntity) && (mobEntity.shouldRenderName() || mobEntity.hasCustomName() && mobEntity == this.renderManager.field_4678);
	}

	public boolean method_4068(T mobEntity, class_856 arg, double d, double e, double f) {
		if (super.method_3933(mobEntity, arg, d, e, f)) {
			return true;
		} else if (mobEntity.isLeashed() && mobEntity.getHoldingEntity() != null) {
			Entity entity = mobEntity.getHoldingEntity();
			return arg.method_3699(entity.method_5830());
		} else {
			return false;
		}
	}
	...
	public void method_4070(T mobEntity) {
	...
	protected void method_4073(T mobEntity, double d, double e, double f, float g, float h) {
		...
		if (entity != null) {
			...
			for (int aa = 0; aa <= 24; aa++) {
				...
				vertexBuffer.vertex(d + v * (double)ae + 0.0, e + w * (double)(ae * ae + ae) * 0.5 + (double)((24.0F - (float)aa) / 18.0F + 0.125F), f + x * (double)ae)
					.color(ab, ac, ad, 1.0F)
					.next();
				vertexBuffer.vertex(
