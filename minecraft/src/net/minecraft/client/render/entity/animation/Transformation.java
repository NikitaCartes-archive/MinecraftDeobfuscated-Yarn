...
public record Transformation(Transformation.Target target, Keyframe... keyframes) {
	...
	public static class Interpolations {
		...
		public static final Transformation.Interpolation field_37885 = (vec3f, delta, keyframes, start, end, f) -> {
			...
			vec3f.set(
				MathHelper.method_41303(delta, vec3f2.getX(), vec3f3.getX(), vec3f4.getX(), vec3f5.getX()) * f,
				MathHelper.method_41303(delta, vec3f2.getY(), vec3f3.getY(), vec3f4.getY(), vec3f5.getY()) * f,
				MathHelper.method_41303(delta, vec3f2.getZ(), vec3f3.getZ(), vec3f4.getZ(), vec3f5.getZ()) * f
			);
			return vec3f;
		};
	}
