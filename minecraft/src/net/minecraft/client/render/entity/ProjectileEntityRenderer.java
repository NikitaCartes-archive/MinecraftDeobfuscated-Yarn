...
public abstract class ProjectileEntityRenderer<T extends ProjectileEntity> extends EntityRenderer<T> {
	...
	public void method_3875(
		...
		matrixStack.push();
		matrixStack.multiply(Vector3f.POSITIVE_Y.getRotationQuaternion(MathHelper.lerp(h, projectileEntity.prevYaw, projectileEntity.yaw) - 90.0F, true));
		matrixStack.multiply(Vector3f.POSITIVE_Z.getRotationQuaternion(MathHelper.lerp(h, projectileEntity.prevPitch, projectileEntity.pitch), true));
		int i = 0;
		float j = 0.0F;
		float k = 0.5F;
		float l = 0.0F;
	...
	public void method_23153(Matrix4f matrix4f, VertexConsumer vertexConsumer, int i, int j, int k, float f, float g, int l, int m, int n, int o) {
		vertexConsumer.vertex(matrix4f, (float)i, (float)j, (float)k).color(255, 255, 255, 255).texture(f, g).light(o).normal((float)l, (float)n, (float)m).next();
	}
}
