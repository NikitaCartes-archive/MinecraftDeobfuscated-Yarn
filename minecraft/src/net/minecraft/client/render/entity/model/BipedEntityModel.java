...
public class BipedEntityModel extends Model {
	...
	public void setRotationAngles(float f, float g, float h, float i, float j, float k, Entity entity) {
		...
		if (this.swingProgress > 0.0F) {
			...
			m = 1.0F - this.swingProgress;
			m *= m;
			m *= m;
			m = 1.0F - m;
			float n = MathHelper.sin(m * (float) Math.PI);
			float o = MathHelper.sin(this.swingProgress * (float) Math.PI) * -(this.head.pitch - 0.7F) * 0.75F;
		...
		if (this.field_3396 > 0.0F) {
			float q = f % 26.0F;
			float m = this.swingProgress > 0.0F ? 0.0F : this.field_3396;
			if (q < 14.0F) {
