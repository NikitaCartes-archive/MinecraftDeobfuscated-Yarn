...
public class ModelPredicateProviderRegistry {
	...
	public static ModelPredicateProvider get(ItemStack stack, Identifier id) {
		...
		if (modelPredicateProvider != null) {
			...
			Map<Identifier, ModelPredicateProvider> map = (Map)ITEM_SPECIFIC.get(stack.getItem());
	...
	static {
		...
		ClampedModelPredicateProvider clampedModelPredicateProvider = (stack, world, entity, seed) -> {
			...
			return armorTrim != null ? ((ArmorTrimMaterial)armorTrim.getMaterial().value()).itemModelIndex() : Float.NEGATIVE_INFINITY;
		};
		register(ItemModelGenerator.TRIM_TYPE, clampedModelPredicateProvider);
		registerCustomModelData(
			(stack, world, entity, seed) -> (float)((CustomModelDataComponent)stack.getOrDefault(DataComponentTypes.CUSTOM_MODEL_DATA, CustomModelDataComponent.DEFAULT))
					.value()
		);
