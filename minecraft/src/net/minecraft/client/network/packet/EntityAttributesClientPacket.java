...
public class EntityAttributesClientPacket implements Packet<ClientPlayPacketListener> {
	private int field_12719;
	private final List<EntityAttributesClientPacket.Entry> entries = Lists.<EntityAttributesClientPacket.Entry>newArrayList();

	public EntityAttributesClientPacket() {
	}

	public EntityAttributesClientPacket(int i, Collection<EntityAttributeInstance> collection) {
		this.field_12719 = i;

		for (EntityAttributeInstance entityAttributeInstance : collection) {
			this.entries
				.add(
					new EntityAttributesClientPacket.Entry(
	...
	public void read(PacketByteBuf packetByteBuf) throws IOException {
		...
		for (int j = 0; j < i; j++) {
			...
			this.entries.add(new EntityAttributesClientPacket.Entry(string, d, list));
		}
	}
	...
	public void write(PacketByteBuf packetByteBuf) throws IOException {
		...
		for (EntityAttributesClientPacket.Entry entry : this.entries) {
	...
	@Environment(EnvType.CLIENT)
	public List<EntityAttributesClientPacket.Entry> getEntries() {
		return this.entries;
	}
