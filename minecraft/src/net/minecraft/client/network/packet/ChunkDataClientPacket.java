...
public class ChunkDataClientPacket implements Packet<ClientPlayPacketListener> {
	...
	private boolean containsVerticalStrip;
	...
	public ChunkDataClientPacket(WorldChunk worldChunk, int i) {
		...
		for (Entry<BlockPos, BlockEntity> entryx : worldChunk.getBlockEntityMap().entrySet()) {
			...
			if (this.containsVerticalStrip() || (i & 1 << j) != 0) {
	...
	public void read(PacketByteBuf packetByteBuf) throws IOException {
		...
		this.containsVerticalStrip = packetByteBuf.readBoolean();
	...
	public void write(PacketByteBuf packetByteBuf) throws IOException {
		...
		packetByteBuf.writeBoolean(this.containsVerticalStrip);
	...
	public int method_11529(PacketByteBuf packetByteBuf, WorldChunk worldChunk, int i) {
		...
		for (int l = chunkSections.length; k < l; k++) {
			...
			if (chunkSection != WorldChunk.EMPTY_SECTION && (!this.containsVerticalStrip() || !chunkSection.isEmpty()) && (i & 1 << k) != 0) {
				j |= 1 << k;
				chunkSection.toPacket(packetByteBuf);
			}
		}

		if (this.containsVerticalStrip()) {
	...
	public boolean containsVerticalStrip() {
		return this.containsVerticalStrip;
	}
