...
public abstract class ClientCommonNetworkHandler implements ClientCommonPacketListener {
	...
	private <T> void handleSynchronizedTagGroup(RegistryKey<? extends Registry<? extends T>> registryRef, TagPacketSerializer.Serialized tags) {
		if (!tags.isEmpty()) {
			Registry<T> registry = (Registry)this.getRegistryManager()
	...
	class ConfirmServerResourcePackScreen extends ConfirmScreen {
		...
		ConfirmServerResourcePackScreen(
			...
			super(
				confirmed -> {
					...
					for(ClientCommonNetworkHandler.ConfirmServerResourcePackScreen.Pack packxx : pack) {
						serverResourcePackLoader.addResourcePack(packxx.id, packxx.url, packxx.hash);
					}
		...
		public ClientCommonNetworkHandler.ConfirmServerResourcePackScreen add(
			...
			List<ClientCommonNetworkHandler.ConfirmServerResourcePackScreen.Pack> list = ImmutableList.builderWithExpectedSize(this.packs.size() + 1)
				.addAll(this.packs)
