...
public record ReporterEnvironment(String clientVersion, @Nullable ReporterEnvironment.Server server) {
	...
	public ThirdPartyServerInfo toThirdPartyServerInfo() {
		ReporterEnvironment.Server var2 = this.server;
		return var2 instanceof ReporterEnvironment.Server.ThirdParty thirdParty ? new ThirdPartyServerInfo(thirdParty.ip) : null;
	}

	@Nullable
	public RealmInfo toRealmInfo() {
		ReporterEnvironment.Server var2 = this.server;
		return var2 instanceof ReporterEnvironment.Server.Realm realm ? new RealmInfo(String.valueOf(realm.realmId()), realm.slotId()) : null;
	}
