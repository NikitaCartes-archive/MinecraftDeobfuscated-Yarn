...
public class ClientPlayNetworkHandler implements ClientPlayPacketListener {
	...
	public void onPlayerPositionLook(PlayerPositionLookS2CPacket packet) {
		...
		if (bl) {
			...
			playerEntity.prevRenderX += packet.getX();
		} else {
			d = 0.0;
			e = packet.getX();
		...
		if (bl2) {
			...
			playerEntity.prevRenderY += packet.getY();
		} else {
			f = 0.0;
			g = packet.getY();
		...
		if (bl3) {
			...
			playerEntity.prevRenderZ += packet.getZ();
		} else {
			h = 0.0;
			i = packet.getZ();
	...
	public void onChunkData(ChunkDataS2CPacket packet) {
		...
		WorldChunk worldChunk = this.world
			.method_2935()
			.loadChunkFromPacket(this.world, i, j, packet.method_22422(), packet.getReadBuffer(), packet.getHeightmaps(), packet.getVerticalStripBitmask());
