	...
	public void onServerMetadata(ServerMetadataS2CPacket packet) {
		...
		if (serverInfo != null) {
			...
			packet.getFavicon().ifPresent(favicon -> {
				...
				} catch (ParseException var3xx) {
					LOGGER.error("Invalid server icon", var3xx);
				}
			});
										...
										enabled -> {
											...
											if (enabled) {
												if (serverInfoxx != null) {
													serverInfoxx.setResourcePackPolicy(ServerInfo.ResourcePackPolicy.ENABLED);
												}
												...
												if (bl) {
													this.connection.disconnect(Text.translatable("multiplayer.requiredTexturePrompt.disconnect"));
												} else if (serverInfoxx != null) {
													serverInfoxx.setResourcePackPolicy(ServerInfo.ResourcePackPolicy.DISABLED);
												}
											}
