...
public class AllowedAddressResolver {
	public static final AllowedAddressResolver DEFAULT = new AllowedAddressResolver(
		AddressResolver.DEFAULT, RedirectResolver.createSrv(), getBlockListPredicate()
	);
	...
	private static Predicate<Address> getBlockListPredicate() {
		ImmutableList<Predicate<String>> immutableList = (ImmutableList)Streams.stream(ServiceLoader.load(BlockListSupplier.class))
