...
public class ChatPreviewer {
	...
	private boolean shouldQuery(long currentTime) {
		...
		if (clientPlayNetworkHandler == null) {
			return true;
		} else if (currentTime < this.getEarliestNextQueryTime()) {
			return false;
		} else {
			return this.pendingResponseQuery == null || currentTime >= this.getLatestNextQueryTime();
		}
	}
	...
	static record Response(long receivedTimeStamp, String query, @Nullable Text previewText) {
		public Response(long l, String string, @Nullable Text text) {
