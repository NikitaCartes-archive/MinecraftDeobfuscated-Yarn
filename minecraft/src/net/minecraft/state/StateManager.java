...
public class StateManager<O, S extends State<S>> {
	...
	protected <A extends AbstractState<O, S>> StateManager(O object, StateManager.Factory<O, S, A> factory, Map<String, Property<?>> map) {
		...
		stream.forEach(list2 -> {
			Map<Property<?>, Comparable<?>> map2xx = MapUtil.createMap(this.properties.values(), list2);
			A abstractStatexx = factory.create(object, ImmutableMap.copyOf(map2xx));
			map2.put(map2xx, abstractStatexx);
			list.add(abstractStatexx);
		});
