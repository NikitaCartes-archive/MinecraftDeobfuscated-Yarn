...
public class StateFactory<O, S extends PropertyContainer<S>> {
	...
	protected <A extends AbstractPropertyContainer<O, S>> StateFactory(O object, StateFactory.Factory<O, S, A> factory, Map<String, Property<?>> map) {
		...
		stream.forEach(list2 -> {
			Map<Property<?>, Comparable<?>> map2xx = MapUtil.createMap(this.propertyMap.values(), list2);
			A abstractPropertyContainerxx = factory.create(object, ImmutableMap.copyOf(map2xx));
			map2.put(map2xx, abstractPropertyContainerxx);
			list.add(abstractPropertyContainerxx);
		});
