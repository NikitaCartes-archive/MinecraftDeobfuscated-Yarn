	...
	private void method_23823() {
		...
		this.field_21533.values().forEach(arg -> {
			if (blockPos.isWithinDistance(arg.field_21543, 30.0)) {
				Set<UUID> set = (Set)map.get(arg.field_21543);
				this.method_23821(arg, (Collection<UUID>)(set == null ? Sets.newHashSet() : set));
			}
		});
	...
	private static String method_23825(Collection<UUID> collection) {
		if (collection.isEmpty()) {
			...
			return collection.size() > 3
				...
				: ((Set)collection.stream().map(VillagerNamer::name).collect(Collectors.toSet())).toString();
		}
	}
