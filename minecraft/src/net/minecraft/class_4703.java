...
public class class_4703 implements DebugRenderer.Renderer {
	...
	private final Map<UUID, class_4703.class_4704> field_21534 = Maps.newHashMap();
	...
	private void method_23826() {
		...
		this.field_21534.values().stream().filter(class_4703.class_4704::method_23836).forEach(arg -> {
			Set<UUID> set = (Set)map.get(arg.field_21541);
			if (set == null) {
				set = Sets.newHashSet();
		...
		map.entrySet().forEach(entry -> {
			...
			Set<String> set2 = (Set)set.stream().map(VillagerNamer::name).collect(Collectors.toSet());
			method_23816(set2.toString(), blockPos, 1, -256);
			method_23816("Flower", blockPos, 2, -1);
			float f = 0.05F;
	...
	private void method_23821(class_4703.class_4705 arg) {
		int i = 0;
		if (this.method_23825(arg).isEmpty()) {
			method_23815("Out: -", arg, i, -3355444);
		} else if (this.method_23825(arg).size() < 4) {
			method_23815("Out: " + this.method_23825(arg), arg, i, -3355444);
		} else {
			method_23815("Out: " + this.method_23825(arg).size() + " bees", arg, i, -3355444);
		}
	...
	private Set<String> method_23825(class_4703.class_4705 arg) {
		return (Set<String>)this.method_23822(arg.field_21543).stream().map(VillagerNamer::name).collect(Collectors.toSet());
	}
	...
	private Map<BlockPos, List<String>> method_23830() {
		...
		for(class_4703.class_4704 lv : this.field_21534.values()) {
			if (lv.field_21540 != null && !this.field_21533.containsKey(lv.field_21540)) {
				List<String> list = (List)map.get(lv.field_21540);
				if (list == null) {
