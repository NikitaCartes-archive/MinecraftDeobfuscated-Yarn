...
public class class_1184 extends DataFix {
	private static final String[] field_5679 = DataFixUtils.make(new String[256], strings -> {
		strings[1] = "Item";
		strings[2] = "XPOrb";
		strings[7] = "ThrownEgg";
		strings[8] = "LeashKnot";
	...
	public class_1184(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	public TypeRewriteRule makeRule() {
		...
		Type<?> type = schema.getType(class_1208.field_5712);
		...
		OpticFinder<?> opticFinder5 = DSL.typeFinder(schema.getTypeRaw(class_1208.field_5729));
		return this.fixTypeEverywhereTyped(
			...
			typed -> {
				...
				if (optional.isPresent() && Objects.equals(((Pair)optional.get()).getSecond(), "minecraft:spawn_egg")) {
					...
					Typed<?> typed2 = typed;
					String string = field_5679[s & 255];
					if (string != null && (!optional5.isPresent() || !Objects.equals(optional5.get(), string))) {
						...
						Typed<?> typed6 = (Typed)((Optional)this.getOutputSchema().getTypeRaw(class_1208.field_5729).readTyped(dynamic2).getSecond())
