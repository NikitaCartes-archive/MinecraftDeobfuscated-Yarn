...
public class Batches {
	...
	public static Collection<GameTestBatch> createBatches(Collection<TestFunction> testFunctions, ServerWorld world) {
		Map<String, List<TestFunction>> map = (Map)testFunctions.stream().collect(Collectors.groupingBy(TestFunction::batchId));
	...
	public static TestRunContext.Batcher defaultBatcher() {
		return states -> {
			Map<String, List<GameTestState>> map = (Map)states.stream()
			...
			return (Collection<GameTestBatch>)map.entrySet().stream().flatMap(entry -> {
				String string = (String)entry.getKey();
				Collection<GameTestState> collection = (Collection)entry.getValue();
				MutableInt mutableInt = new MutableInt();
