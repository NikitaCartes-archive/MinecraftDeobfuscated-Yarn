...
public class TestRunner {
	...
	public TestRunner(
		...
		this.sizeZ = i;
		this.batches = (List)collection.stream()
			.map(
				gameTestBatch -> {
					Collection<GameTest> collectionxx = (Collection)gameTestBatch.getTestFunctions()
						.stream()
						.map(testFunction -> new GameTest(testFunction, blockRotation, serverWorld))
						.collect(ImmutableList.toImmutableList());
					return Pair.of(gameTestBatch, collectionxx);
				}
			)
			.collect(ImmutableList.toImmutableList());
