...
public class TestRunner {
	...
	public TestRunner(Collection<GameTestBatch> batches, BlockPos pos, BlockRotation rotation, ServerWorld world, TestManager testManager, int sizeZ) {
		...
		this.batches = (List)batches.stream()
			.map(
				batch -> {
					Collection<GameTestState> collection = (Collection)batch.getTestFunctions()
						.stream()
	...
	private Map<GameTestState, BlockPos> alignTestStructures(Collection<GameTestState> gameTests) {
		...
		for(GameTestState gameTestState : gameTests) {
			...
			StructureBlockBlockEntity structureBlockBlockEntity = StructureTestUtil.initStructure(
				gameTestState.getTemplateName(), blockPos, gameTestState.getRotation(), this.world
			);
