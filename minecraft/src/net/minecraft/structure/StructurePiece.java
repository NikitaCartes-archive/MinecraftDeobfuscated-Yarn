...
public abstract class StructurePiece {
	...
	public abstract boolean generate(IWorld world, ChunkGenerator<?> generator, Random random, BlockBox box, ChunkPos pos, BlockPos blockPos);
	...
	protected void fillWithOutline(
		IWorld world, BlockBox blockBox, int i, int j, int k, int l, int m, int n, BlockState blockState, BlockState inside, boolean bl
	) {
		for(int o = j; o <= m; ++o) {
			for(int p = i; p <= l; ++p) {
				for(int q = k; q <= n; ++q) {
			...
			for(int p = i; p <= l; ++p) {
				for(int q = k; q <= n; ++q) {
					if (!(random.nextFloat() > f)
						&& (!bl || !this.getBlockAt(iWorld, p, o, q, blockBox).isAir())
						&& (!bl2 || this.isUnderSeaLevel(iWorld, p, o, q, blockBox))) {
						if (o != j && o != m && p != i && p != l && q != k && q != n) {
