						...
						if (structurePoolFeatureConfig.getSize() > 0) {
							...
							while(!structurePoolGenerator.structurePieces.isEmpty()) {
								...
								structurePoolGenerator.generatePiece(
									shapedPoolStructurePiece.piece, shapedPoolStructurePiece.pieceShape, shapedPoolStructurePiece.currentSize, bl, heightLimitView
								);
							}
	...
	public static void generate(
		...
		while(!structurePoolGenerator.structurePieces.isEmpty()) {
			StructurePoolBasedGenerator.ShapedPoolStructurePiece shapedPoolStructurePiece = (StructurePoolBasedGenerator.ShapedPoolStructurePiece)structurePoolGenerator.structurePieces
				.removeFirst();
			structurePoolGenerator.generatePiece(
				shapedPoolStructurePiece.piece, shapedPoolStructurePiece.pieceShape, shapedPoolStructurePiece.currentSize, false, world
			);
		}
	}
								...
								if (modifyBoundingBox && blockBox2.getBlockCountY() <= 16) {
									l = list2.stream().mapToInt(structureBlockInfox -> {
										if (!blockBox2.contains(structureBlockInfox.pos.offset(JigsawBlock.getFacing(structureBlockInfox.state)))) {
											...
											Optional<StructurePool> optional2xx = optionalxx.flatMap(structurePool -> this.registry.getOrEmpty(structurePool.getTerminatorsId()));
