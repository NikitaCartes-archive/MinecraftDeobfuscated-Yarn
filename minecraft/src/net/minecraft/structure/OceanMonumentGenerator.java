...
public class OceanMonumentGenerator {
	public static class Base extends OceanMonumentGenerator.Piece {
		...
		public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
			...
			for (int j = 0; j < 5; j++) {
				this.setAirAndWater(iWorld, blockBox, -1 - j, 0 + j * 2, -1 - j, -1 - j, 23, 58 + j);
				this.setAirAndWater(iWorld, blockBox, 58 + j, 0 + j * 2, -1 - j, 58 + j, 23, 58 + j);
				this.setAirAndWater(iWorld, blockBox, 0 - j, 0 + j * 2, -1 - j, 57 + j, 23, -1 - j);
				this.setAirAndWater(iWorld, blockBox, 0 - j, 0 + j * 2, 58 + j, 57 + j, 23, 58 + j);
			}
		...
		public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
			...
			for (int ix = 1; ix <= 6; ix++) {
				...
				for (int j = 0; j <= 15; j += 15) {
					this.fillWithOutline(iWorld, blockBox, j, ix, 0, j, ix, 1, blockState, blockState, false);
					this.fillWithOutline(iWorld, blockBox, j, ix, 6, j, ix, 9, blockState, blockState, false);
					this.fillWithOutline(iWorld, blockBox, j, ix, 14, j, ix, 15, blockState, blockState, false);
				}

				this.fillWithOutline(iWorld, blockBox, 1, ix, 0, 1, ix, 0, blockState, blockState, false);
				this.fillWithOutline(iWorld, blockBox, 6, ix, 0, 9, ix, 0, blockState, blockState, false);
				this.fillWithOutline(iWorld, blockBox, 14, ix, 0, 14, ix, 0, blockState, blockState, false);
				this.fillWithOutline(iWorld, blockBox, 1, ix, 15, 14, ix, 15, blockState, blockState, false);
			}

			this.fillWithOutline(iWorld, blockBox, 6, 3, 6, 9, 6, 9, DARK_PRISMARINE, DARK_PRISMARINE, false);
			this.fillWithOutline(iWorld, blockBox, 7, 4, 7, 8, 5, 8, Blocks.GOLD_BLOCK.getDefaultState(), Blocks.GOLD_BLOCK.getDefaultState(), false);

			for (int ix = 3; ix <= 6; ix += 3) {
				for (int k = 6; k <= 9; k += 3) {
					this.addBlock(iWorld, SEA_LANTERN, k, ix, 6, blockBox);
					this.addBlock(iWorld, SEA_LANTERN, k, ix, 9, blockBox);
				}
			}
	...
	public static class DoubleXRoom extends OceanMonumentGenerator.Piece {
		...
		public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
			...
			if (this.field_14479.field_14486 / 25 > 0) {
				this.method_14774(iWorld, blockBox, 8, 0, lv.field_14482[Direction.DOWN.getId()]);
				this.method_14774(iWorld, blockBox, 0, 0, lv2.field_14482[Direction.DOWN.getId()]);
			}
	...
	public static class DoubleYRoom extends OceanMonumentGenerator.Piece {
		...
		public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
			if (this.field_14479.field_14486 / 25 > 0) {
				this.method_14774(iWorld, blockBox, 0, 0, this.field_14479.field_14482[Direction.DOWN.getId()]);
			}
	...
	public static class DoubleYZRoom extends OceanMonumentGenerator.Piece {
		...
		public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
			...
			if (this.field_14479.field_14486 / 25 > 0) {
				this.method_14774(iWorld, blockBox, 0, 8, lv.field_14482[Direction.DOWN.getId()]);
				this.method_14774(iWorld, blockBox, 0, 0, lv2.field_14482[Direction.DOWN.getId()]);
			}
			...
			for (int i = 1; i <= 7; i++) {
				...
				this.fillWithOutline(iWorld, blockBox, 3, i, 7, 4, i, 8, blockState, blockState, false);
			}
	...
	public static class Penthouse extends OceanMonumentGenerator.Piece {
		...
		public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
			...
			for (int j = 0; j < 2; j++) {
				for (int k = 2; k <= 8; k += 3) {
					this.fillWithOutline(iWorld, blockBox, i, 0, k, i, 2, k, PRISMARINE_BRICKS, PRISMARINE_BRICKS, false);
				}
		...
		public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
			if (this.field_14481 == 0) {
				...
				for (int i = 18; i >= 7; i -= 3) {
					this.addBlock(iWorld, SEA_LANTERN, 6, 3, i, blockBox);
					this.addBlock(iWorld, SEA_LANTERN, 16, 3, i, blockBox);
				}
				...
				for (int l = 0; l < 2; l++) {
					this.addBlock(iWorld, PRISMARINE_BRICKS, i, 6, 20, blockBox);
					this.addBlock(iWorld, SEA_LANTERN, i, 5, 20, blockBox);
					this.addBlock(iWorld, PRISMARINE_BRICKS, i, 4, 20, blockBox);
					i = 13;
				}
