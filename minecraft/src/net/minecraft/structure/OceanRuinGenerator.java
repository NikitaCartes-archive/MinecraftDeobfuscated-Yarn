	...
	private static void method_14825(
		...
		for(int l = 0; l < k; ++l) {
			if (!list.isEmpty()) {
				...
				BlockPos blockPos4 = Structure.method_15168(new BlockPos(5, 0, 6), BlockMirror.NONE, blockRotation, BlockPos.ORIGIN).add(n, 0, o);
	...
	public static class Piece extends SimpleStructurePiece {
		...
		protected void handleMetadata(String metadata, BlockPos pos, IWorld world, Random random, BlockBox boundingBox) {
			if ("chest".equals(metadata)) {
				world.setBlockState(
					pos, Blocks.CHEST.getDefaultState().with(ChestBlock.WATERLOGGED, Boolean.valueOf(world.getFluidState(pos).matches(FluidTags.WATER))), 2
				);
