...
public class EndCityGenerator {
			...
			if (i > 8) {
				...
				if (j == 0) {
					piece2 = EndCityGenerator.addPiece(
						list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(-1, 4, -1), "base_roof", blockRotation, true)
					);
				} else if (j == 1) {
					piece2 = EndCityGenerator.addPiece(
	...
	private static final List<Pair<BlockRotation, BlockPos>> SMALL_TOWER_BRIDGE_ATTACHMENTS = Lists.<Pair<BlockRotation, BlockPos>>newArrayList(
		new Pair<>(BlockRotation.ROT_0, new BlockPos(1, -1, 0)),
		new Pair<>(BlockRotation.ROT_90, new BlockPos(6, -1, 1)),
		new Pair<>(BlockRotation.ROT_270, new BlockPos(0, -1, 5)),
		new Pair<>(BlockRotation.ROT_180, new BlockPos(5, -1, 6))
	);
				...
				for(Pair<BlockRotation, BlockPos> pair : EndCityGenerator.SMALL_TOWER_BRIDGE_ATTACHMENTS) {
					if (random.nextBoolean()) {
						EndCityGenerator.Piece piece4 = EndCityGenerator.addPiece(
							list, EndCityGenerator.createPiece(structureManager, piece3, pair.getRight(), "bridge_end", blockRotation.rotate((BlockRotation)pair.getLeft()), true)
						);
	...
	private static final EndCityGenerator.Part BRIDGE_PIECE = new EndCityGenerator.Part() {
		...
		public boolean create(StructureManager structureManager, int i, EndCityGenerator.Piece piece, BlockPos blockPos, List<StructurePiece> list, Random random) {
			...
			piece2 = EndCityGenerator.addPiece(
				list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(4, k, 0), "bridge_end", blockRotation.rotate(BlockRotation.ROT_180), true)
			);
			piece2.field_15316 = -1;
			return true;
		}
	...
	private static final List<Pair<BlockRotation, BlockPos>> FAT_TOWER_BRIDGE_ATTACHMENTS = Lists.<Pair<BlockRotation, BlockPos>>newArrayList(
		new Pair<>(BlockRotation.ROT_0, new BlockPos(4, -1, 0)),
		new Pair<>(BlockRotation.ROT_90, new BlockPos(12, -1, 4)),
		new Pair<>(BlockRotation.ROT_270, new BlockPos(0, -1, 8)),
		new Pair<>(BlockRotation.ROT_180, new BlockPos(8, -1, 12))
	);
				...
				for(Pair<BlockRotation, BlockPos> pair : EndCityGenerator.FAT_TOWER_BRIDGE_ATTACHMENTS) {
					if (random.nextBoolean()) {
						EndCityGenerator.Piece piece3 = EndCityGenerator.addPiece(
							list, EndCityGenerator.createPiece(structureManager, piece2, pair.getRight(), "bridge_end", blockRotation.rotate((BlockRotation)pair.getLeft()), true)
						);
