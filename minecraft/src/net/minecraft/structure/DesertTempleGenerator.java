...
public class DesertTempleGenerator extends StructurePieceWithDimensions {
	...
	public DesertTempleGenerator(Random random, int x, int z) {
		super(StructurePieceType.TE_D_P, random, x, 64, z, 21, 15, 21);
	}

	public DesertTempleGenerator(StructureManager manager, CompoundTag tag) {
		super(StructurePieceType.TE_D_P, tag);
	...
	public boolean generate(IWorld world, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
		...
		this.fillWithOutline(
			world, blockBox, 8, -10, 8, 12, -10, 12, Blocks.CHISELED_SANDSTONE.getDefaultState(), Blocks.CHISELED_SANDSTONE.getDefaultState(), false
		);
