			...
			if (this.method_14937(iWorld, blockBox)) {
				...
				for(int n = 0; n <= 2; ++n) {
					for(int o = 0; o <= m; ++o) {
						...
						if (blockState2.isAir() && this.isUnderSeaLevel(iWorld, n, -1, o, blockBox)) {
							int r = -1;
			...
			if (this.method_14937(iWorld, blockBox)) {
				...
				for(int i = this.boundingBox.minX; i <= this.boundingBox.maxX; ++i) {
					for(int j = this.boundingBox.minZ; j <= this.boundingBox.maxZ; ++j) {
						if (this.getBlockAt(iWorld, i, this.boundingBox.minY - 1, j, blockBox).isAir()
							&& this.isUnderSeaLevel(iWorld, i, this.boundingBox.minY - 1, j, blockBox)) {
		...
		public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos) {
			if (this.method_14937(iWorld, blockBox)) {
				...
				for(BlockBox blockBox2 : this.entrances) {
					this.fillWithOutline(
						iWorld, blockBox, blockBox2.minX, blockBox2.maxY - 2, blockBox2.minZ, blockBox2.maxX, blockBox2.maxY, blockBox2.maxZ, AIR, AIR, false
					);
				}
