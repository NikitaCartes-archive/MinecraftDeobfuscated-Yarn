...
public record BlockPredicate(Optional<RegistryEntryList<Block>> blocks, Optional<StatePredicate> state, Optional<NbtPredicate> nbt) {
	...
	public boolean test(ServerWorld world, BlockPos pos) {
		if (!world.canSetBlock(pos)) {
			...
			return !this.nbt.isPresent() || testBlockEntity(world, world.getBlockEntity(pos), (NbtPredicate)this.nbt.get());
		}
	}
	...
	public boolean test(CachedBlockPosition pos) {
		if (!this.testState(pos.getBlockState())) {
			...
			return !this.nbt.isPresent() || testBlockEntity(pos.getWorld(), pos.getBlockEntity(), (NbtPredicate)this.nbt.get());
		}
	}

	private boolean testState(BlockState state) {
		if (this.blocks.isPresent() && !state.isIn((RegistryEntryList<Block>)this.blocks.get())) {
			return false;
		} else {
