...
public class LocationPredicate {
	...
	public boolean method_9020(ServerWorld serverWorld, float f, float g, float h) {
		if (!this.x.matches(f)) {
			...
			if (this.field_9683 != null && this.field_9683 != serverWorld.method_8310(blockPos)) {
				return false;
			} else {
				return this.field_9687 == null || this.field_9687.method_14024(serverWorld, blockPos);
			}
		}
	}
	...
	public static LocationPredicate deserialize(@Nullable JsonElement jsonElement) {
		if (jsonElement != null && !jsonElement.isJsonNull()) {
			...
			DimensionType dimensionType = jsonObject.has("dimension")
				? DimensionType.method_12483(new Identifier(JsonHelper.getString(jsonObject, "dimension")))
				: null;
