				...
				entity2 -> {
					if (!entity2.isPushable()) {
						...
						if (collisionRule2 == AbstractScoreboardTeam.CollisionRule.field_1435) {
							...
							if ((collisionRule == AbstractScoreboardTeam.CollisionRule.field_1440 || collisionRule2 == AbstractScoreboardTeam.CollisionRule.field_1440) && bl) {
								return false;
							} else {
								return collisionRule != AbstractScoreboardTeam.CollisionRule.field_1434 && collisionRule2 != AbstractScoreboardTeam.CollisionRule.field_1434 || bl;
							}
						}
					} else {
						return false;
		...
		public boolean method_5916(@Nullable Entity entity) {
			if (!entity.isValid()) {
				...
				if (!livingEntity.getEquippedStack(equipmentSlot).isEmpty()) {
					...
					return !((ArmorStandEntity)livingEntity).method_6915(equipmentSlot);
				} else {
