	...
	public boolean test(@Nullable Entity entity) {
		if (this == ANY) {
			...
			if (!this.head.test(livingEntity.getEquippedStack(EquipmentSlot.HEAD))) {
				...
				return false;
			} else if (!this.mainhand.test(livingEntity.getEquippedStack(EquipmentSlot.HAND_MAIN))) {
				return false;
			} else {
				return this.offhand.test(livingEntity.getEquippedStack(EquipmentSlot.HAND_OFF));
			}
		}
	}
