	...
	public static class class_5033 extends Biome {
		public class_5033() {
			...
			Stream.of(ColoredBlockArrays.ALL)
				...
				.forEach(
					block -> {
						float fxx = 1.0F + random.nextFloat() * 5.0F;
						float gxx = Math.min(fxx + random.nextFloat() * 10.0F, 15.0F);
						list.add(
							Feature.SHAPE
								.configure(new class_5105(new SimpleBlockStateProvider(block.getDefaultState()), Util.method_26721(random, class_5105.class_5106.values()), fxx, gxx))
						);
					}
				);

			for(Block[] blocks : ColoredBlockArrays.ALL) {
				ImmutableList<BlockState> immutableList = (ImmutableList)Stream.of(blocks).map(Block::getDefaultState).collect(ImmutableList.toImmutableList());
