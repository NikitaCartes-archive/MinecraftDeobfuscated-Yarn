...
public abstract class BaseFluid extends Fluid {
		...
		try (BlockPos.PooledMutable pooledMutable = BlockPos.PooledMutable.get()) {
				...
				if (this.method_15748(fluidState)) {
					...
					if (f == 0.0F) {
						if (!world.getBlockState(pooledMutable).getMaterial().blocksMovement()) {
							BlockPos blockPos = pooledMutable.method_10074();
			...
			if (state.get(FALLING)) {
				for(Direction direction2 : Direction.Type.HORIZONTAL) {
					pooledMutable.method_10114(pos).method_10118(direction2);
	...
	protected FluidState getUpdatedState(WorldView worldView, BlockPos pos, BlockState state) {
		...
		for(Direction direction : Direction.Type.HORIZONTAL) {
			...
			if (fluidState.getFluid().matchesType(this) && this.receivesFlow(direction, worldView, pos, state, blockPos, blockState)) {
		...
		if (this.isInfinite() && j >= 2) {
			BlockState blockState2 = worldView.getBlockState(pos.method_10074());
		...
		if (!fluidState3.isEmpty() && fluidState3.getFluid().matchesType(this) && this.receivesFlow(Direction.UP, worldView, pos, state, blockPos2, blockState3)) {
		...
		for(Direction direction2 : Direction.Type.HORIZONTAL) {
			if (direction2 != direction) {
				...
				Pair<BlockState, FluidState> pair = short2ObjectMap.computeIfAbsent(s, ix -> {
					BlockState blockStatexx = worldView.getBlockState(blockPos3);
				...
				if (this.method_15746(worldView, this.getFlowing(), blockPos, blockState, direction2, blockPos3, blockState2, fluidState)) {
					boolean bl = short2BooleanMap.computeIfAbsent(s, ix -> {
						BlockPos blockPos2xx = blockPos3.method_10074();
						BlockState blockState2xx = worldView.getBlockState(blockPos2xx);
						return this.method_15736(worldView, this.getFlowing(), blockPos3, blockState2, blockPos2xx, blockState2xx);
					});
					if (bl) {
						return i;
					}
	...
	protected Map<Direction, FluidState> method_15726(WorldView worldView, BlockPos blockPos, BlockState blockState) {
		...
		for(Direction direction : Direction.Type.HORIZONTAL) {
			...
			Pair<BlockState, FluidState> pair = short2ObjectMap.computeIfAbsent(s, ix -> {
				BlockState blockStatexx = worldView.getBlockState(blockPos2);
				return Pair.of(blockStatexx, blockStatexx.getFluidState());
			});
			BlockState blockState2 = pair.getFirst();
			FluidState fluidState = pair.getSecond();
			...
			if (this.method_15746(worldView, fluidState2.getFluid(), blockPos, blockState, direction, blockPos2, blockState2, fluidState)) {
				...
				boolean bl = short2BooleanMap.computeIfAbsent(s, ix -> {
					BlockState blockState2xx = worldView.getBlockState(blockPos3);
					return this.method_15736(worldView, this.getFlowing(), blockPos2, blockState2, blockPos3, blockState2xx);
				});
				int j;
				if (bl) {
					j = 0;
