...
public abstract class BaseFluid extends Fluid {
	...
	private void method_15744(IWorld iWorld, BlockPos blockPos, FluidState fluidState, BlockState blockState) {
		...
		if (i > 0) {
			Map<Direction, FluidState> map = this.getSpread(iWorld, blockPos, blockState);
	...
	protected FluidState getUpdatedState(CollisionView world, BlockPos pos, BlockState state) {
		int i = 0;
		int j = 0;
	...
	private boolean receivesFlow(Direction face, BlockView world, BlockPos pos, BlockState state, BlockPos fromPos, BlockState fromState) {
		...
		boolean bl = !VoxelShapes.method_1080(voxelShape, voxelShape2, face);
		if (object2ByteLinkedOpenHashMap != null) {
		...
		for(Direction direction2 : Direction.Type.HORIZONTAL) {
			if (direction2 != direction) {
				...
				Pair<BlockState, FluidState> pair = short2ObjectMap.computeIfAbsent(s, ix -> {
					BlockState blockStatexx = collisionView.getBlockState(blockPos3);
					return Pair.of(blockStatexx, blockStatexx.getFluidState());
				});
				BlockState blockState2 = pair.getFirst();
				FluidState fluidState = pair.getSecond();
				if (this.canFlowThrough(collisionView, this.getFlowing(), blockPos, blockState, direction2, blockPos3, blockState2, fluidState)) {
					boolean bl = short2BooleanMap.computeIfAbsent(s, ix -> {
						BlockPos blockPos2xx = blockPos3.down();
						BlockState blockState2xx = collisionView.getBlockState(blockPos2xx);
						return this.method_15736(collisionView, this.getFlowing(), blockPos3, blockState2, blockPos2xx, blockState2xx);
					});
					if (bl) {
						return i;
					}

					if (i < this.method_15733(collisionView)) {
						int k = this.method_15742(collisionView, blockPos3, i + 1, direction2.getOpposite(), blockState2, blockPos2, short2ObjectMap, short2BooleanMap);
						if (k < j) {
							j = k;
						}
					}
	...
	private boolean method_15736(BlockView blockView, Fluid fluid, BlockPos blockPos, BlockState blockState, BlockPos blockPos2, BlockState blockState2) {
		if (!this.receivesFlow(Direction.DOWN, blockView, blockPos, blockState, blockPos2, blockState2)) {
			...
			return blockState2.getFluidState().getFluid().matchesType(this) ? true : this.canFill(blockView, blockPos2, blockState2, fluid);
		}
	}
	...
	protected abstract int method_15733(CollisionView collisionView);

	private int method_15740(CollisionView collisionView, BlockPos blockPos) {
		int i = 0;
	...
	protected Map<Direction, FluidState> getSpread(CollisionView world, BlockPos pos, BlockState state) {
		int i = 1000;
		...
		for(Direction direction : Direction.Type.HORIZONTAL) {
			...
			Pair<BlockState, FluidState> pair = short2ObjectMap.computeIfAbsent(s, ix -> {
				BlockState blockStatexx = world.getBlockState(blockPos);
				return Pair.of(blockStatexx, blockStatexx.getFluidState());
			});
	...
	protected abstract int getLevelDecreasePerBlock(CollisionView world);
	...
	public float getHeight(FluidState state, BlockView world, BlockPos pos) {
		return isFluidAboveEqual(state, world, pos) ? 1.0F : state.method_20785();
	}
