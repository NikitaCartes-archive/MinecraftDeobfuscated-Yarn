		...
		for (Direction direction2 : Direction.Type.HORIZONTAL) {
			if (direction2 != direction) {
				...
				Pair<BlockState, FluidState> pair = short2ObjectMap.computeIfAbsent(s, ix -> {
					BlockState blockStatex = collisionView.getBlockState(blockPos3);
				...
				if (this.canFlowThrough(collisionView, this.getFlowing(), blockPos, blockState, direction2, blockPos3, blockState2, fluidState)) {
					boolean bl = short2BooleanMap.computeIfAbsent(s, ix -> {
						...
						BlockState blockState2x = collisionView.getBlockState(blockPos2x);
						return this.method_15736(collisionView, this.getFlowing(), blockPos3, blockState2, blockPos2x, blockState2x);
					});
					if (bl) {
						return i;
					}
