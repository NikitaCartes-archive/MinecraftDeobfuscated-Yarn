...
public abstract class FlowableFluid extends Fluid {
	...
	protected FluidState getUpdatedState(WorldView world, BlockPos pos, BlockState state) {
		...
		if (!fluidState3.isEmpty()
			...
			&& this.receivesFlow(Direction.field_11036, world, pos, state, blockPos2, blockState3)) {
		...
		for(Direction direction2 : Direction.Type.field_11062) {
			if (direction2 != direction) {
				...
				Pair<BlockState, FluidState> pair = short2ObjectMap.computeIfAbsent(s, ix -> {
					BlockState blockStatexx = world.getBlockState(blockPos3);
				...
				if (this.canFlowThrough(world, this.getFlowing(), blockPos, blockState, direction2, blockPos3, blockState2, fluidState)) {
					boolean bl = short2BooleanMap.computeIfAbsent(s, ix -> {
						...
						BlockState blockState2xx = world.getBlockState(blockPos2xx);
						return this.method_15736(world, this.getFlowing(), blockPos3, blockState2, blockPos2xx, blockState2xx);
					});
					if (bl) {
						return i;
					}
