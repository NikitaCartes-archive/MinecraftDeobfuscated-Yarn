...
public class SerializableRegistries {
	private static final Map<RegistryKey<? extends Registry<?>>, SerializableRegistries.Info<?>> REGISTRIES = Util.make(() -> {
		...
		add(builder, Registry.BIOME_KEY, Biome.NETWORK_CODEC);
		add(builder, Registry.MESSAGE_TYPE_KEY, MessageType.CODEC);
		add(builder, Registry.DIMENSION_TYPE_KEY, DimensionType.CODEC);
	...
	private static <E> void add(
		...
		builder.put(key, new SerializableRegistries.Info<E>(key, networkCodec));
	}
	...
	public static Stream<DynamicRegistryManager.Entry<?>> streamRegistryManagerEntries(CombinedDynamicRegistries<ServerDynamicRegistryType> combinedRegistries) {
		...
		Stream<DynamicRegistryManager.Entry<?>> stream2 = stream(combinedRegistries.getSucceedingRegistryManagers(ServerDynamicRegistryType.WORLDGEN));
