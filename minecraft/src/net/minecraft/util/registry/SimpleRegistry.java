...
public class SimpleRegistry<T> extends MutableRegistry<T> {
	...
	private RegistryEntry<T> set(int rawId, RegistryKey<T> key, T value, Lifecycle lifecycle, boolean checkDuplicateKeys) {
		...
		if (this.valueToEntryFunction != null) {
			...
			RegistryEntry.Reference<T> reference2 = (RegistryEntry.Reference)this.keyToEntry.put(key, reference);
	...
	public RegistryEntry<T> replace(OptionalInt rawId, RegistryKey<T> key, T newEntry, Lifecycle lifecycle) {
		...
		RegistryEntry<T> registryEntry = (RegistryEntry)this.keyToEntry.get(key);
	...
	public Identifier getId(T value) {
		RegistryEntry.Reference<T> reference = (RegistryEntry.Reference)this.valueToEntry.get(value);
	...
	public T get(@Nullable Identifier id) {
		RegistryEntry.Reference<T> reference = (RegistryEntry.Reference)this.idToEntry.get(id);
	...
	public RegistryEntryList.Named<T> getOrCreateEntryList(TagKey<T> tag) {
		RegistryEntryList.Named<T> named = (RegistryEntryList.Named)this.tagToEntryList.get(tag);
		if (named == null) {
	...
	public void populateTags(Map<TagKey<T>, List<RegistryEntry<T>>> tagEntries) {
		...
		tagEntries.forEach((tag, entries) -> {
			for(RegistryEntry<T> registryEntry : entries) {
				...
				RegistryEntry.Reference<T> reference = (RegistryEntry.Reference)registryEntry;
		...
		if (!set.isEmpty()) {
			field_36635.warn(
				...
				this.getKey(),
				set.stream().map(tagKey -> tagKey.id().toString()).sorted().collect(Collectors.joining(", "))
			);
		}

		Map<TagKey<T>, RegistryEntryList.Named<T>> map2 = new IdentityHashMap(this.tagToEntryList);
		tagEntries.forEach((tagKey, list) -> ((RegistryEntryList.Named)map2.computeIfAbsent(tagKey, this::method_40562)).copyOf(list));
