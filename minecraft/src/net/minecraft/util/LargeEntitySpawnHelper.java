...
public class LargeEntitySpawnHelper {
	public static <T extends MobEntity> Optional<T> trySpawnAt(
		EntityType<T> entityType, ServerWorld world, BlockPos pos, int count, int range, int heightRequired
	) {
		...
		for(int i = 0; i < count; ++i) {
			...
			if (findSpawnPos(world, heightRequired, mutable.set(pos, j, heightRequired, k))) {
				T mobEntity = entityType.create(world, null, null, null, mutable, SpawnReason.MOB_SUMMONED, false, false);
				if (mobEntity != null) {
