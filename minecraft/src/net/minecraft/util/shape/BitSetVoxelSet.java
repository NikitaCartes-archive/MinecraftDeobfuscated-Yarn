...
public final class BitSetVoxelSet extends VoxelSet {
	...
	protected boolean isColumnFull(int i, int j, int k, int l) {
		if (k < 0 || l < 0 || i < 0) {
			return false;
		} else if (k < this.xSize && l < this.ySize && j <= this.zSize) {
	...
	static BitSetVoxelSet method_1040(
		...
		doubleListPair.forAllOverlappingSections((i, j, k) -> {
			...
			boolean bl = doubleListPair2.forAllOverlappingSections((l, m, n) -> {
				...
				boolean blxx = doubleListPair3.forAllOverlappingSections((o, p, q) -> {
					boolean blxxx = booleanBiFunction.apply(voxelSet.inBoundsAndContains(i, l, o), voxelSet2.inBoundsAndContains(j, m, p));
					if (blxxx) {
