...
public final class BitSetVoxelSet extends VoxelSet {
	...
	protected boolean isColumnFull(int minZ, int maxZ, int x, int y) {
		if (x < 0 || y < 0 || minZ < 0) {
			return false;
		} else if (x < this.xSize && y < this.ySize && maxZ <= this.zSize) {
	...
	static BitSetVoxelSet combine(
		...
		BitSetVoxelSet bitSetVoxelSet = new BitSetVoxelSet(
			xPoints.getMergedList().size() - 1, yPoints.getMergedList().size() - 1, zPoints.getMergedList().size() - 1
		);
		...
		xPoints.forAllOverlappingSections((i, j, k) -> {
			...
			boolean bl = yPoints.forAllOverlappingSections((l, m, n) -> {
				...
				boolean blxx = zPoints.forAllOverlappingSections((o, p, q) -> {
					boolean blxxx = function.apply(first.inBoundsAndContains(i, l, o), second.inBoundsAndContains(j, m, p));
					if (blxxx) {
