...
public final class VoxelShapes {
	...
	private static boolean matchesAnywhere(
		...
		return !mergedX.forAllOverlappingSections(
			(i, j, k) -> mergedY.forAllOverlappingSections(
					(kx, l, m) -> mergedZ.forAllOverlappingSections(
							(mx, n, o) -> !predicate.apply(shape1.inBoundsAndContains(i, kx, mx), shape2.inBoundsAndContains(j, l, n))
						)
				)
		);
	}
	...
	public static double calculateMaxOffset(Direction.Axis axis, Box box, Stream<VoxelShape> shapes, double d) {
		for(Iterator<VoxelShape> iterator = shapes.iterator(); iterator.hasNext(); d = ((VoxelShape)iterator.next()).method_1108(axis, box, d)) {
	...
	public static boolean unionCoversFullCube(VoxelShape shape1, VoxelShape shape2) {
		if (shape1 == fullCube() || shape2 == fullCube()) {
			...
			return !matchesAnywhere(fullCube(), combine(shape1, shape2, BooleanBiFunction.OR), BooleanBiFunction.ONLY_FIRST);
		}
	}
