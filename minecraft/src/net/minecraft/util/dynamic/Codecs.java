...
public class Codecs {
	...
	public static final Codec<Quaternionf> QUATERNIONF = Codec.FLOAT
		.listOf()
		.comapFlatMap(
			list -> Util.decodeFixedLengthList(list, 4).map(listx -> new Quaternionf(listx.get(0), listx.get(1), listx.get(2), listx.get(3))),
	...
	public static <A> ResultFunction<A> orElsePartial(A object) {
		return new ResultFunction<A>() {
			...
			public <T> DataResult<Pair<A, T>> apply(DynamicOps<T> ops, T input, DataResult<Pair<A, T>> result) {
				...
				return optional.isPresent() ? result : DataResult.error(() -> "(" + (String)mutableObject.getValue() + " -> using default)", Pair.of(object, input));
			}
		...
		return new MapCodec<V>() {
			...
			public <T> DataResult<V> decode(DynamicOps<T> ops, MapLike<T> input) {
				...
				return object == null ? DataResult.error(() -> "Missing \"" + typeKey + "\" in: " + input) : typeCodec.decode(ops, object).flatMap(pair -> {
					T objectxx = (T)Objects.requireNonNullElseGet(input.get(parametersKey), ops::emptyMap);
					return ((Codec)parametersCodecGetter.apply(pair.getFirst())).decode(ops, objectxx).map(Pair::getFirst);
				});
			}
