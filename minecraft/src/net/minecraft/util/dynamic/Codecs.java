...
public class Codecs {
	...
	public static final Codec<Vector3f> VECTOR_3F = Codec.FLOAT
		.listOf()
		.comapFlatMap(
			list -> Util.toArray(list, 3).map(listx -> new Vector3f(listx.get(0), listx.get(1), listx.get(2))), vec3f -> List.of(vec3f.x(), vec3f.y(), vec3f.z())
		);
	public static final Codec<Quaternionf> QUATERNIONF = Codec.FLOAT
		.listOf()
		.comapFlatMap(
			list -> Util.toArray(list, 4).map(listx -> new Quaternionf(listx.get(0), listx.get(1), listx.get(2), listx.get(3))),
	...
	public static final Codec<GameProfile> GAME_PROFILE = RecordCodecBuilder.create(
		instance -> instance.group(
					Codec.mapPair(
							Uuids.CODEC.xmap(Optional::of, optional -> (UUID)optional.orElse(null)).optionalFieldOf("id", Optional.empty()),
	...
	public static <P, I> Codec<I> createCodecForPairObject(
		...
		Codec<I> codec4 = new Codecs.Either<>(codec2, codec3)
			.xmap(either -> either.map(object -> object, object -> object), com.mojang.datafixers.util.Either::left);
