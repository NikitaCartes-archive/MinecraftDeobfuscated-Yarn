...
public class RegistryLoader {
	...
	<E> DataResult<RegistryEntry<E>> load(
		...
		DataResult<RegistryEntry<E>> dataResult = (DataResult)valueHolder.values.get(entryKey);
		if (dataResult != null) {
			return dataResult;
		} else {
			...
			if (optional.isEmpty()) {
				...
				DataResult<EntryLoader.Entry<E>> dataResult3 = (DataResult)optional.get();
				Optional<EntryLoader.Entry<E>> optional2 = dataResult3.result();
				if (optional2.isPresent()) {
					EntryLoader.Entry<E> entry = (EntryLoader.Entry)optional2.get();
