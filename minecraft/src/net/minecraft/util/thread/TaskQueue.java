...
public interface TaskQueue<T, F> {
	...
	public static final class PrioritizedQueueMailbox implements TaskQueue<TaskQueue.PrioritizedMessage, Runnable> {
		private final List<Queue<Runnable>> queues;

		public PrioritizedQueueMailbox(int priorityCount) {
			this.queues = (List)IntStream.range(0, priorityCount).mapToObj(i -> Queues.newConcurrentLinkedQueue()).collect(Collectors.toList());
		}
		...
		public boolean add(TaskQueue.PrioritizedMessage prioritizedMessage) {
	...
	public static final class QueueMailbox<T> implements TaskQueue<T, T> {
