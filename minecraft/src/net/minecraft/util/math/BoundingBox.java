...
public class BoundingBox {
	...
	public HitResult rayTrace(Vec3d vec3d, Vec3d vec3d2) {
		return this.rayTrace(vec3d, vec3d2, null);
	}

	@Nullable
	public HitResult rayTrace(Vec3d vec3d, Vec3d vec3d2, @Nullable BlockPos blockPos) {
		double[] ds = new double[]{1.0};
		Direction direction = null;
		double d = vec3d2.x - vec3d.x;
		double e = vec3d2.y - vec3d.y;
		double f = vec3d2.z - vec3d.z;
		direction = method_1007(blockPos == null ? this : this.offset(blockPos), vec3d, ds, direction, d, e, f);
		if (direction == null) {
			return null;
		} else {
			double g = ds[0];
			return new HitResult(vec3d.add(g * d, g * e, g * f), direction, blockPos == null ? BlockPos.ORIGIN : blockPos);
		}
	}

	@Nullable
	public static HitResult method_1010(Iterable<BoundingBox> iterable, Vec3d vec3d, Vec3d vec3d2, BlockPos blockPos) {
		double[] ds = new double[]{1.0};
		Direction direction = null;
		double d = vec3d2.x - vec3d.x;
		double e = vec3d2.y - vec3d.y;
		...
		if (direction == null) {
			...
			double g = ds[0];
			return new HitResult(vec3d.add(g * d, g * e, g * f), direction, blockPos);
		}
	}
