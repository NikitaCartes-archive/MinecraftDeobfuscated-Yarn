...
public class MathHelper {
	...
	private static final float[] SINE_TABLE = Util.make(new float[65536], sineTable -> {
		for(int ixx = 0; ixx < sineTable.length; ++ixx) {
			sineTable[ixx] = (float)Math.sin((double)ixx * Math.PI * 2.0 / 65536.0);
		}
	});
	...
	public static float method_37484(float f, float g, float h, float i, float j) {
		return method_37166(i, j, method_37491(f, g, h));
	}
	...
	public static <C> class_6468<C> method_37487(float f, float[] fs, List<class_6468<C>> list, float[] gs) {
		...
		if (i < 0) {
			...
			float h = fs[i + 1];
			float k = (f - g) / (h - g);
			class_6468<C> lv = (class_6468)list.get(i);
			class_6468<C> lv2 = (class_6468)list.get(i + 1);
			float l = gs[i];
			float m = gs[i + 1];
			return lv.method_37749(lv2, (kx, lx) -> {
				float mxx = l * (h - g) - (lx - kx);
				float n = -m * (h - g) + (lx - kx);
				return lerp(k, kx, lx) + k * (1.0F - k) * lerp(k, mxx, n);
			});
		}
	}
