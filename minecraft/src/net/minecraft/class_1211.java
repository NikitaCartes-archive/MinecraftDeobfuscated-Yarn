...
public abstract class class_1211 extends DataFix {
	...
	public TypeRewriteRule makeRule() {
		TaggedChoiceType<String> taggedChoiceType = this.getInputSchema().findChoiceType(class_1208.field_5729);
		TaggedChoiceType<String> taggedChoiceType2 = this.getOutputSchema().findChoiceType(class_1208.field_5729);
		...
		if (!Objects.equals(this.getOutputSchema().getType(class_1208.field_5719), type)) {
			...
			return TypeRewriteRule.seq(this.fixTypeEverywhere(this.field_5734, taggedChoiceType, taggedChoiceType2, dynamicOps -> pair -> pair.mapFirst(string -> {
						...
						Type<?> typexx = (Type)taggedChoiceType.types().get(string);
						Type<?> type2 = (Type)taggedChoiceType2.types().get(string2);
						if (!type2.equals(typexx, true, true)) {
							throw new IllegalStateException(String.format("Dynamic type check failed: %s not equal to %s", type2, typexx));
						} else {
							return string2;
						}
