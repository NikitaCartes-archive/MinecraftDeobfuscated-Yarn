						...
						while(true) {
							...
							if (nbtCompound2.contains(string, NbtElement.COMPOUND_TYPE)) {
								Util.ifPresentOrElse(
									...
									statType -> {
										NbtCompound nbtCompound2xx = nbtCompound2.getCompound(string);
	
										for(String string2 : nbtCompound2xx.getKeys()) {
											if (nbtCompound2xx.contains(string2, NbtElement.NUMBER_TYPE)) {
												Util.ifPresentOrElse(
												...
												LOGGER.warn("Invalid statistic value in {}: Don't know what {} is for key {}", this.file, nbtCompound2xx.get(string2), string2);
											}
										}
									},
