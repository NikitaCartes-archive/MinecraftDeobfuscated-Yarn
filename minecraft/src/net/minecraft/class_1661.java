...
public class class_1661 implements class_1263, class_1275 {
	...
	public int method_7369(Predicate<class_1799> predicate, int i) {
		...
		if (!this.field_7549.method_7960() && predicate.test(this.field_7549)) {
			int k = i <= 0 ? this.field_7549.method_7947() : Math.min(i - j, this.field_7549.method_7947());
			j += k;
			if (i != 0) {
				this.field_7549.method_7934(k);
	...
	public class_2499 method_7384(class_2499 arg) {
		...
		for(int i = 0; i < this.field_7548.size(); ++i) {
			if (!this.field_7548.get(i).method_7960()) {
				class_2487 lv = new class_2487();
				lv.method_10567("Slot", (byte)(i + 100));
				this.field_7548.get(i).method_7953(lv);
				arg.add(lv);
			}
		}

		for(int i = 0; i < this.field_7544.size(); ++i) {
			if (!this.field_7544.get(i).method_7960()) {
				class_2487 lv = new class_2487();
				lv.method_10567("Slot", (byte)(i + 150));
				this.field_7544.get(i).method_7953(lv);
				arg.add(lv);
			}
		}
	...
	public boolean method_5442() {
		...
		for(class_1799 lv : this.field_7548) {
			if (!lv.method_7960()) {
				return false;
			}
		}

		for(class_1799 lv : this.field_7544) {
			if (!lv.method_7960()) {
				return false;
			}
		}
		...
		if (!(f <= 0.0F)) {
			...
			for(int i = 0; i < this.field_7548.size(); ++i) {
				...
				if (lv.method_7909() instanceof class_1738) {
					lv.method_7956((int)f, this.field_7546);
				}
			}
		}
	}
	...
	public boolean method_5443(class_1657 arg) {
		if (this.field_7546.field_5988) {
			return false;
		} else {
			return !(arg.method_5858(this.field_7546) > 64.0);
		}
	}
