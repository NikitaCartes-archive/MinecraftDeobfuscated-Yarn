				...
				return TypeRewriteRule.seq(
					...
					this.fixTypeEverywhereTyped("Trapped Chest fix", type4, typed -> typed.updateTyped(opticFinder2, typedx -> {
							...
							if (!optional.isPresent()) {
								...
								TaggedChoiceType<String> taggedChoiceType = this.getInputSchema().findChoiceType(class_1208.field_5727);
								return typedx.updateTyped(opticFinder, typedxx -> typedxx.updateTyped(taggedChoiceType.finder(), typedxxx -> {
										Dynamic<?> dynamic = typedxxx.getOrCreate(DSL.remainderFinder());
										int kxx = dynamic.get("x").asInt(0) - (k << 4);
										int lxx = dynamic.get("y").asInt(0);
										int m = dynamic.get("z").asInt(0) - (l << 4);
										return intSet.contains(class_1191.method_5051(kxx, lxx, m)) ? typedxxx.update(taggedChoiceType.finder(), pair -> pair.mapFirst(string -> {
