...
public class ShapedRecipe implements Recipe {
	...
	public boolean matches(Inventory inventory, World world) {
		if (!(inventory instanceof CraftingInventory)) {
			...
			for (int i = 0; i <= inventory.getInvWidth() - this.width; i++) {
				for (int j = 0; j <= inventory.getInvHeight() - this.height; j++) {
	...
	private boolean matchesSmall(Inventory inventory, int i, int j, boolean bl) {
		for (int k = 0; k < inventory.getInvWidth(); k++) {
			for (int l = 0; l < inventory.getInvHeight(); l++) {
				int m = k - i;
				int n = l - j;
				...
				if (!ingredient.matches(inventory.getInvStack(k + l * inventory.getInvWidth()))) {
					return false;
				}
			}
		}
