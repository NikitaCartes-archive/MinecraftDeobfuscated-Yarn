...
public final class Ingredient implements Predicate<ItemStack> {
	...
	private Ingredient(Stream<? extends Ingredient.Entry> stream) {
		this.entries = (Ingredient.Entry[])stream.filter(field_9020).toArray(i -> new Ingredient.Entry[i]);
	}
	...
	private void createStackArray() {
		if (this.stackArray == null) {
			this.stackArray = (ItemStack[])Arrays.stream(this.entries).flatMap(entry -> entry.getStacks().stream()).distinct().toArray(i -> new ItemStack[i]);
		}
	}
