...
public class BrewingRecipeRegistry {
	...
	protected static boolean hasPotionRecipe(ItemStack input, ItemStack ingredient) {
		Optional<RegistryEntry<Potion>> optional = ((PotionContentsComponent)input.getOrDefault(DataComponentTypes.POTION_CONTENTS, PotionContentsComponent.DEFAULT))
			.potion();
		if (optional.isEmpty()) {
			return false;
		} else {
	...
	private static void registerItemRecipe(Item input, Item ingredient, Item output) {
		if (!(input instanceof PotionItem)) {
			...
			ITEM_RECIPES.add(new BrewingRecipeRegistry.Recipe<Item>(input.getRegistryEntry(), Ingredient.ofItems(ingredient), output.getRegistryEntry()));
		}
	}
