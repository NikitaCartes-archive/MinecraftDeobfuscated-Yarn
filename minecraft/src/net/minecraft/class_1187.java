...
public class class_1187 extends DataFix {
	private static final Int2ObjectMap<String> field_5681 = DataFixUtils.make(new Int2ObjectOpenHashMap(), int2ObjectOpenHashMap -> {
	...
	public class_1187(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	private Dynamic<?> method_5035(Dynamic<?> dynamic) {
		Optional<Dynamic<?>> optional = dynamic.get("ench")
			.asStreamOpt()
			.map(stream -> stream.map(dynamicx -> dynamicx.set("id", dynamicx.createString((String)field_5681.getOrDefault(dynamicx.get("id").asInt(0), "null")))))
		...
		return dynamic.update(
			...
			dynamicx -> DataFixUtils.orElse(
					dynamicx.asStreamOpt()
						.map(
							stream -> stream.map(dynamicxx -> dynamicxx.set("id", dynamicxx.createString((String)field_5681.getOrDefault(dynamicxx.get("id").asInt(0), "null"))))
						)
