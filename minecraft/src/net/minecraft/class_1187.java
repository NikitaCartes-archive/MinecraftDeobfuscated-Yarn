...
public class class_1187 extends DataFix {
	...
	private Dynamic<?> method_5035(Dynamic<?> dynamic) {
		Optional<Dynamic<?>> optional = dynamic.get("ench")
			.asStreamOpt()
			.map(stream -> stream.map(dynamicx -> dynamicx.set("id", dynamicx.createString(field_5681.getOrDefault(dynamicx.get("id").asInt(0), "null")))))
		...
		return dynamic.update(
			...
			dynamicx -> DataFixUtils.orElse(
					dynamicx.asStreamOpt()
						.map(stream -> stream.map(dynamicxx -> dynamicxx.set("id", dynamicxx.createString(field_5681.getOrDefault(dynamicxx.get("id").asInt(0), "null")))))
