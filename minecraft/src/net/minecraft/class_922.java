...
public abstract class class_922<T extends class_1309, M extends class_583<T>> extends class_897<T> implements class_3883<T, M> {
	...
	public void method_4054(T arg, double d, double e, double f, float g, float h) {
		...
		try {
			...
			this.method_4058(arg, l, 0.0F, h);
			float n = this.method_4060(arg, h);
			this.method_20283(d, f, arg, i);
			float o = 0.0F;
			float p = 0.0F;
	...
	protected void method_20283(double d, double e, T arg, float f) {
		double g = Math.atan2(e, d) / Math.PI * 180.0;
		double h = (double)f - g;
		double i = Math.floor(h / 45.0) * 45.0;
		GlStateManager.rotatef((float)g, 0.0F, 1.0F, 0.0F);
		GlStateManager.scalef(0.02F, 1.0F, 1.0F);
		GlStateManager.rotated(i, 0.0, 1.0, 0.0);
	}
