...
public class class_1214 extends DataFix {
	private static final Set<String> field_5739 = ImmutableSet.<String>builder()
		...
		.build();
	private static final Map<String, String> field_5737 = ImmutableMap.<String, String>builder()
	...
	private static final Map<String, String> field_5736 = ImmutableMap.<String, String>builder()
		...
		.build();
	private static final Map<String, String> field_5735 = ImmutableMap.<String, String>builder()
	...
	public class_1214(Schema schema, boolean bl) {
		super(schema, bl);
	}
	...
	public TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped(
			"StatsCounterFix",
			this.getInputSchema().getType(class_1208.field_5730),
			type,
			typed -> {
				...
				if (optional.isPresent()) {
					for (Entry<? extends Dynamic<?>, ? extends Dynamic<?>> entry : ((Map)optional.get()).entrySet()) {
						if (((Dynamic)entry.getValue()).asNumber().isPresent()) {
							...
							if (!field_5739.contains(string)) {
								String string2;
								String string3;
								if (field_5737.containsKey(string)) {
									string2 = "minecraft:custom";
									string3 = (String)field_5737.get(string);
								} else {
									...
									if ("stat.mineBlock".equals(string4)) {
										...
										String string5 = string.substring(i + 1).replace('.', ':');
										string3 = (String)field_5735.getOrDefault(string5, string5);
									}
								}
	...
	@Nullable
	protected String method_5172(String string) {
		return class_1188.method_5042(string, 0);
	}

	protected String method_5173(String string) {
		return class_3580.method_15600(string);
	}
}
