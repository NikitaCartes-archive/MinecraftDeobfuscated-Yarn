...
public class WardenAngerManager {
	private final Map<UUID, Integer> suspects = Maps.newHashMap();
	...
	public void tick() {
		if (this.field_36737.get()) {
			this.suspects.replaceAll((uuid, anger) -> Math.max(0, anger - this.angerDecreasePerTick));
			this.suspects.entrySet().removeIf(suspect -> suspect.getValue() <= 0);
		}
	}
	...
	public int getAnger(UUID uuid) {
		return !this.suspects.containsKey(uuid) ? 0 : this.suspects.get(uuid);
	}

	public int getPrimeSuspectAnger() {
		return this.getPrimeSuspect().map(Entry::getValue).orElse(0);
	}
