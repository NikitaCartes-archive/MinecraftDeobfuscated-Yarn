...
public class PounceAtTargetGoal extends Goal {
		...
		if (this.owner.hasPassengers()) {
			...
			if (this.target == null) {
				...
				if (d < 4.0 || d > 16.0) {
					...
					return this.owner.getRand().nextInt(5) == 0;
				}
			}
		}
	}
	...
	public void start() {
		...
		if ((double)f >= 1.0E-4) {
			this.owner.velocityX += d / (double)f * 0.5 * 0.8F + this.owner.velocityX * 0.2F;
			this.owner.velocityZ += e / (double)f * 0.5 * 0.8F + this.owner.velocityZ * 0.2F;
		}
