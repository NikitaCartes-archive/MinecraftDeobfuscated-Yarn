...
public class MoveToRaidCenterGoal<T extends RaiderEntity> extends Goal {
	...
	public void tick() {
		...
		if (!this.field_16597.method_6150() && this.field_16597.hasPatrolTarget()) {
			Vec3d vec3d = new Vec3d(this.field_16597.getPatrolTarget());
			...
			if (!this.field_16597.getNavigation().startMovingTo((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ(), 1.0)) {
	...
	private void moveToAlternativePosition() {
		...
		this.field_16597.getNavigation().startMovingTo((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ(), 1.0);
	}
}
