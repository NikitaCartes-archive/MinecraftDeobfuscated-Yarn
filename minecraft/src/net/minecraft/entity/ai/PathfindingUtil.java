	...
	private static Vec3d findTarget(
		...
		for(int n = 0; n < 10; ++n) {
			...
			if (blockPos != null) {
				...
				BlockPos blockPos2 = new BlockPos((double)o + mobEntityWithAi.x, (double)p + mobEntityWithAi.y, (double)q + mobEntityWithAi.z);
				if ((!bl2 || mobEntityWithAi.isInWalkTargetRange(blockPos2)) && entityNavigation.isValidPosition(blockPos2)) {
					if (!bl) {
						blockPos2 = method_6372(blockPos2, mobEntityWithAi);
						if (isWater(blockPos2, mobEntityWithAi)) {
							continue;
						}
					}

					double f = toDoubleFunction.applyAsDouble(blockPos2);
					if (f > e) {
						e = f;
						k = o;
						l = p;
