...
public class WaterPathNodeMaker extends PathNodeMaker {
	...
	public PathNode getPathNode(double d, double e, double f) {
		return super.getPathNode(
			MathHelper.floor(d - (double)(this.entity.width / 2.0F)), MathHelper.floor(e + 0.5), MathHelper.floor(f - (double)(this.entity.width / 2.0F))
		);
	}
	...
	public PathNodeType getPathNodeType(BlockView blockView, int i, int j, int k) {
		...
		if (fluidState.isEmpty() && blockState.canPlaceAtSide(blockView, blockPos.down(), PlacementEnvironment.field_48) && blockState.isAir()) {
			return PathNodeType.field_16;
		} else {
			return fluidState.matches(FluidTags.field_15517) && blockState.canPlaceAtSide(blockView, blockPos, PlacementEnvironment.field_48)
				? PathNodeType.WATER
	...
	private PathNodeType getPathNodeType(int i, int j, int k) {
		...
		for (int l = i; l < i + this.field_31; l++) {
			for (int m = j; m < j + this.field_30; m++) {
				for (int n = k; n < k + this.field_28; n++) {
					...
					if (fluidState.isEmpty() && blockState.canPlaceAtSide(this.blockView, mutable.down(), PlacementEnvironment.field_48) && blockState.isAir()) {
						return PathNodeType.field_16;
					}
		...
		return blockState2.canPlaceAtSide(this.blockView, mutable, PlacementEnvironment.field_48) ? PathNodeType.WATER : PathNodeType.BLOCKED;
	}
}
