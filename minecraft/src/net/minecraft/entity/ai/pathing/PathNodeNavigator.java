...
public class PathNodeNavigator {
	...
	public Path pathfind(ChunkCache chunkCache, MobEntity mobEntity, Set<BlockPos> set, float f, int i) {
		...
		Map<class_4459, BlockPos> map = (Map)set.stream()
			.collect(
				Collectors.toMap(
					blockPos -> this.pathNodeMaker.getPathNode((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()), Function.identity()
				)
			);
