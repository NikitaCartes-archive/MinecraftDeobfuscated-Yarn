...
public class SwimNavigation extends EntityNavigation {
	...
	protected Vec3d method_6347() {
		return new Vec3d(this.entity.x, this.entity.y + (double)this.entity.height * 0.5, this.entity.z);
	}
	...
	protected void method_6346(Vec3d vec3d) {
		...
		if (this.field_6681 != null && !this.field_6681.isFinished()) {
			...
			if (vec3d2.equals(this.field_6680)) {
				this.field_6670 += SystemUtil.getMeasuringTimeMili() - this.field_6669;
			} else {
	...
	protected boolean method_6341(Vec3d vec3d, Vec3d vec3d2, int i, int j, int k) {
		HitResult hitResult = this.world
			.rayTrace(vec3d, new Vec3d(vec3d2.x, vec3d2.y + (double)this.entity.height * 0.5, vec3d2.z), FluidRayTraceMode.NONE, true, false);
		return hitResult == null || hitResult.type == HitResult.Type.NONE;
	}
