...
public class LandPathNodeMaker extends PathNodeMaker {
	...
	protected PathNodeType getBasicPathNodeType(BlockView blockView, int i, int j, int k) {
		...
		if (blockState.isAir()) {
		...
		} else if (block instanceof DoorBlock && material == Material.WOOD && !blockState.method_11654(DoorBlock.field_10945)) {
			return PathNodeType.field_23;
		} else if (block instanceof DoorBlock && material == Material.METAL && !blockState.method_11654(DoorBlock.field_10945)) {
			return PathNodeType.field_8;
		} else if (block instanceof DoorBlock && blockState.method_11654(DoorBlock.field_10945)) {
			...
			&& (!(block instanceof FenceGateBlock) || blockState.method_11654(FenceGateBlock.field_11026))) {
