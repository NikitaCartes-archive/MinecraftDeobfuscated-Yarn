...
public class LandPathNodeMaker extends PathNodeMaker {
			...
			while(
				(
						this.blockView.getBlockState(blockPos).isAir()
							|| this.blockView.getBlockState(blockPos).canPlaceAtSide(this.blockView, blockPos, PlacementEnvironment.field_50)
					)
					&& blockPos.getY() > 0
			) {
		...
		if (e - d > 1.125) {
			...
			if (pathNodeType == PathNodeType.NORMAL) {
				...
				if (pathNode == null && l > 0 && pathNodeType != PathNodeType.FENCE && pathNodeType != PathNodeType.TRAPDOOR) {
					...
					if (pathNode != null && (pathNode.type == PathNodeType.AIR || pathNode.type == PathNodeType.NORMAL) && this.entity.width < 1.0F) {
						double h = (double)(i - direction.getOffsetX()) + 0.5;
						double m = (double)(k - direction.getOffsetZ()) + 0.5;
						BoundingBox boundingBox = new BoundingBox(
							h - g, (double)j + 0.001, m - g, h + g, (double)this.entity.height + method_60(this.blockView, blockPos.up()) - 0.002, m + g
						);
	...
	public static double method_60(BlockView blockView, BlockPos blockPos) {
		...
		VoxelShape voxelShape = blockView.getBlockState(blockPos2).method_11628(blockView, blockPos2);
	...
	public PathNodeType getPathNodeType(BlockView blockView, int i, int j, int k) {
		...
		if (pathNodeType == PathNodeType.AIR && j >= 1) {
			...
			pathNodeType = pathNodeType2 != PathNodeType.NORMAL
				...
				? PathNodeType.NORMAL
				: PathNodeType.AIR;
			if (pathNodeType2 == PathNodeType.FIRE || block == Blocks.field_10092) {
	...
	protected PathNodeType getBasicPathNodeType(BlockView blockView, int i, int j, int k) {
		...
		if (blockState.isAir()) {
		...
		} else if (block instanceof DoorBlock && material == Material.WOOD && !blockState.get(DoorBlock.field_10945)) {
			return PathNodeType.DOOR_WOOD;
		} else if (block instanceof DoorBlock && material == Material.METAL && !blockState.get(DoorBlock.field_10945)) {
			return PathNodeType.DOOR_METAL;
		} else if (block instanceof DoorBlock && blockState.get(DoorBlock.field_10945)) {
			...
			&& (!(block instanceof FenceGateBlock) || blockState.get(FenceGateBlock.field_11026))) {
			...
			if (fluidState.matches(FluidTags.field_15517)) {
				...
				return blockState.canPlaceAtSide(blockView, blockPos, PlacementEnvironment.field_50) ? PathNodeType.AIR : PathNodeType.BLOCKED;
			}
		} else {
			return PathNodeType.FENCE;
		}
