...
public class TargetFinder {
	...
	private static Vec3d method_21758(
		...
		return findTarget(
			...
			toDoubleFunction,
			!bl,
			blockPos -> mobEntityWithAi.world.getBlockState(blockPos).getMaterial().isSolid(),
			0,
			0,
			true
		);
	...
	private static Vec3d findTarget(
		...
		for(int l = 0; l < 10; ++l) {
			...
			if (blockPos2 != null) {
				...
				BlockPos blockPos3 = new BlockPos((double)m + mob.getX(), (double)n + mob.getY(), (double)o + mob.getZ());
				if ((!bl3 || mob.isInWalkTargetRange(blockPos3)) && (!bl2 || entityNavigation.isValidPosition(blockPos3))) {
					if (bl) {
						blockPos3 = method_21761(blockPos3, random.nextInt(j + 1) + k, mob.world.getHeight(), predicate);
					}

					if (anywhere || !isWater(blockPos3, mob)) {
						PathNodeType pathNodeType = LandPathNodeMaker.method_23476(mob.world, blockPos3.getX(), blockPos3.getY(), blockPos3.getZ());
	...
	static BlockPos method_21761(BlockPos blockPos, int i, int j, Predicate<BlockPos> predicate) {
		if (i < 0) {
			...
			BlockPos blockPos3;
			BlockPos blockPos4;
			for(blockPos3 = blockPos2; blockPos3.getY() < j && blockPos3.getY() - blockPos2.getY() < i; blockPos3 = blockPos4) {
				blockPos4 = blockPos3.up();
