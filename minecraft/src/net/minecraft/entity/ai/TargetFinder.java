	...
	private static Vec3d findTarget(
		...
		for(int n = 0; n < 10; ++n) {
			...
			if (blockPos2 != null) {
				...
				BlockPos blockPos3 = new BlockPos((double)o + mobEntityWithAi.getX(), (double)p + mobEntityWithAi.getY(), (double)q + mobEntityWithAi.getZ());
				if ((!bl4 || mobEntityWithAi.isInWalkTargetRange(blockPos3)) && (!bl3 || entityNavigation.isValidPosition(blockPos3))) {
					if (bl2) {
						blockPos3 = method_21761(blockPos3, random.nextInt(l + 1) + m, mobEntityWithAi.world.getHeight(), predicate);
					}

					if (bl || !isWater(blockPos3, mobEntityWithAi)) {
