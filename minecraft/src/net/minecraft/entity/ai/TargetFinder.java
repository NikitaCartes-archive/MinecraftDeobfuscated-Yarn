	...
	private static Vec3d findTarget(
		...
		for(int i = 0; i < 10; ++i) {
			...
			if (blockPos2 != null) {
				...
				BlockPos blockPos3 = new BlockPos((double)j + mob.getX(), (double)k + mob.getY(), (double)l + mob.getZ());
				if (blockPos3.getY() >= 0
					&& blockPos3.getY() <= mob.world.getHeight()
					&& (!bl || mob.isInWalkTargetRange(blockPos3))
					&& (!validPositionsOnly || entityNavigation.isValidPosition(blockPos3))) {
					if (aboveGround) {
						blockPos3 = findValidPositionAbove(
							blockPos3,
					...
					if (notInWater || !mob.world.getFluidState(blockPos3).matches(FluidTags.WATER)) {
						PathNodeType pathNodeType = LandPathNodeMaker.getPathNodeType(mob.world, blockPos3.getX(), blockPos3.getY(), blockPos3.getZ());
