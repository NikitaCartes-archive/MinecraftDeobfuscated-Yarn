...
public class NearestPlayersSensor extends Sensor<LivingEntity> {
	...
	protected void sense(ServerWorld world, LivingEntity entity) {
		List<PlayerEntity> list = (List)world.getPlayers()
			.stream()
		...
		List<PlayerEntity> list2 = (List)list.stream().filter(playerEntity -> testTargetPredicate(entity, playerEntity)).collect(Collectors.toList());
		brain.remember(MemoryModuleType.NEAREST_VISIBLE_PLAYER, list2.isEmpty() ? null : (PlayerEntity)list2.get(0));
		Optional<PlayerEntity> optional = list2.stream().filter(playerEntity -> method_36982(entity, playerEntity)).findFirst();
