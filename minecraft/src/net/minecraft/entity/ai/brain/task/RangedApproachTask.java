...
public class RangedApproachTask {
	...
	public static Task<MobEntity> create(Function<LivingEntity, Float> speed) {
		return TaskTriggerer.task(
			context -> context.<MemoryQueryResult, MemoryQueryResult, MemoryQueryResult, MemoryQueryResult>group(
					...
					.apply(
						...
						(walkTarget, lookTarget, attackTarget, visibleMobs) -> (world, entity, time) -> {
								...
								if (optional.isPresent()
									...
									walkTarget.remember(new WalkTarget(new EntityLookTarget(livingEntity, false), speed.apply(entity), 0));
								}
