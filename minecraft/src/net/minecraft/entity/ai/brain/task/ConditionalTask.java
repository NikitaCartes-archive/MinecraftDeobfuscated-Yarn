...
public class ConditionalTask<E extends LivingEntity> extends Task<E> {
	...
	private static Map<MemoryModuleType<?>, MemoryModuleState> merge(
		...
		Map<MemoryModuleType<?>, MemoryModuleState> map = Maps.<MemoryModuleType<?>, MemoryModuleState>newHashMap();
		map.putAll(first);
		map.putAll(second);
		return map;
	}

	public ConditionalTask(Predicate<E> predicate, Task<? super E> task, boolean bl) {
		this(ImmutableMap.of(), predicate, task, bl);
	}
	...
	public ConditionalTask(Map<MemoryModuleType<?>, MemoryModuleState> map, Task<? super E> task) {
		this(map, livingEntity -> true, task, false);
	}
