		...
		return TaskTriggerer.task(
			context -> context.<MemoryQueryResult, MemoryQueryResult, MemoryQueryResult>group(
					...
					.apply(context, (path, doorsToClose, mobs) -> (world, entity, time) -> {
							Path pathxx = context.getValue(path);
							Optional<Set<GlobalPos>> optional = context.getOptionalValue(doorsToClose);
							if (!pathxx.isStart() && !pathxx.isFinished()) {
								if (Objects.equals(mutableObject.getValue(), pathxx.getCurrentNode())) {
									mutableInt.setValue(20);
								} else if (mutableInt.decrementAndGet() > 0) {
									return false;
								}
		
								mutableObject.setValue(pathxx.getCurrentNode());
								PathNode pathNode = pathxx.getLastNode();
								PathNode pathNode2 = pathxx.getCurrentNode();
