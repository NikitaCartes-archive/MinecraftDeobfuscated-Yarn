...
public class LookTargetUtil {
	...
	public static boolean isTargetWithinAttackRange(MobEntity mob, LivingEntity target, int rangedWeaponReachReduction) {
		Item i = mob.getMainHandStack().getItem();
		if (i instanceof RangedWeaponItem rangedWeaponItem && mob.canUseRangedWeapon(rangedWeaponItem)) {
			int ix = rangedWeaponItem.getRange() - rangedWeaponReachReduction;
			return mob.isInRange(target, (double)ix);
		}
