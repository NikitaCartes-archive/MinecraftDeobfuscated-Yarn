...
public class RoarTask extends Task<WardenEntity> {
	...
	protected void keepRunning(ServerWorld serverWorld, WardenEntity wardenEntity, long l) {
		...
		optional.ifPresent(long_ -> {
			Optional<Long> optionalxx = wardenEntity.getBrain().getOptionalMemory(MemoryModuleType.LAST_ROAR);
			boolean bl = optionalxx.isEmpty() || l > optionalxx.get() + (long)WardenBrain.ROAR_DURATION;
			if (bl && l >= 25L + long_) {
