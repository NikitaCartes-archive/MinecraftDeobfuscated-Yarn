	...
	protected boolean shouldRun(ServerWorld world, LivingEntity entity) {
		if (entity.hasVehicle()) {
			...
			if (world.getRegistryKey() != globalPos.getDimension()) {
				...
				return globalPos.getPos().isWithinDistance(entity.getPos(), 2.0) && blockState.isIn(BlockTags.BEDS) && !blockState.get(BedBlock.OCCUPIED);
			}
		}
	}
	...
	protected void run(ServerWorld world, LivingEntity entity, long time) {
		if (time > this.startTime) {
			...
			if (brain.hasMemoryModule(MemoryModuleType.DOORS_TO_CLOSE)) {
				Set<GlobalPos> set = (Set)brain.getOptionalRegisteredMemory(MemoryModuleType.DOORS_TO_CLOSE).get();
