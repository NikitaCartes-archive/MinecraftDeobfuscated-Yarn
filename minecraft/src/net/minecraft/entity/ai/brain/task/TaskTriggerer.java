...
public class TaskTriggerer<E extends LivingEntity, M> implements App<TaskTriggerer.K1<E>, M> {
	...
	public static <E extends LivingEntity> SingleTickTask<E> predicate(Predicate<E> predicate) {
		return task(context -> context.point((TaskRunnable<LivingEntity>)(world, entity, time) -> predicate.test(entity)));
	}

	public static <E extends LivingEntity> SingleTickTask<E> predicate(BiPredicate<ServerWorld, E> predicate) {
		return task(context -> context.point((TaskRunnable<LivingEntity>)(world, entity, time) -> predicate.test(world, entity)));
	}
				...
				return TaskTriggerer.of(new TaskTriggerer.TaskFunction<E, R>() {
					...
					public R run(ServerWorld world, E entity, long time) {
						...
						if (object == null) {
							...
							Function<A, R> function = (Function)taskFunction2.run(world, entity, time);
				...
				public R run(ServerWorld world, E entity, long time) {
					...
					if (object == null) {
						...
						if (object2 == null) {
							...
							BiFunction<A, B, R> biFunction = (BiFunction)taskFunction3.run(world, entity, time);
