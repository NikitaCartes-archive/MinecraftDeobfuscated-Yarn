				...
				if (blockState.isAir() && block2 instanceof FarmlandBlock && villagerEntity.hasSeedToPlant()) {
					...
					for(int i = 0; i < simpleInventory.size(); ++i) {
						...
						if (!itemStack.isEmpty() && itemStack.isIn(ItemTags.VILLAGER_PLANTABLE_SEEDS)) {
							...
							if (blockState2 instanceof BlockItem blockItem) {
								BlockState blockState2x = blockItem.getBlock().getDefaultState();
								serverWorld.setBlockState(this.currentTarget, blockState2x);
								serverWorld.emitGameEvent(GameEvent.BLOCK_PLACE, this.currentTarget, GameEvent.Emitter.of(villagerEntity, blockState2x));
								bl = true;
							}
						}
