...
public class MeetVillagerTask extends Task<LivingEntity> {
	...
	protected void run(ServerWorld world, LivingEntity entity, long time) {
		...
		brain.getOptionalMemory(MemoryModuleType.VISIBLE_MOBS)
			.ifPresent(
				list -> list.stream()
						.filter(livingEntity -> EntityType.VILLAGER.equals(livingEntity.getType()))
						...
						.ifPresent(livingEntity -> {
							...
							brain.remember(MemoryModuleType.WALK_TARGET, new WalkTarget(new EntityLookTarget(livingEntity), 0.3F, 1));
						})
			);
	}
}
