...
public class MeetVillagerTask extends Task<LivingEntity> {
	...
	protected void run(ServerWorld world, LivingEntity entity, long time) {
		...
		brain.getOptionalMemory(MemoryModuleType.field_18442)
			.ifPresent(
				list -> list.stream()
						.filter(livingEntity -> EntityType.field_6077.equals(livingEntity.getType()))
						.filter(livingEntity2 -> livingEntity2.squaredDistanceTo(entity) <= 32.0)
						.findFirst()
						.ifPresent(livingEntity -> {
							brain.remember(MemoryModuleType.field_18447, livingEntity);
							brain.remember(MemoryModuleType.field_18446, new EntityLookTarget(livingEntity, true));
							brain.remember(MemoryModuleType.field_18445, new WalkTarget(new EntityLookTarget(livingEntity, false), 0.3F, 1));
						})
			);
	}
}
