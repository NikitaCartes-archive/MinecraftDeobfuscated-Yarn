...
public class EntityLookTarget implements LookTarget {
	...
	public boolean isSeenBy(LivingEntity entity) {
		...
		if (optional instanceof LivingEntity livingEntity) {
			if (!livingEntity.isAlive()) {
				return false;
			} else {
				Optional<LivingTargetCache> optionalx = entity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.VISIBLE_MOBS);
