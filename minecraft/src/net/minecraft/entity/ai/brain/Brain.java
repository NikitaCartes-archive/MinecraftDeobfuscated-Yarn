...
public class Brain<E extends LivingEntity> {
			...
			(new MapCodec<Brain<E>>() {
					...
					public <T> DataResult<Brain<E>> decode(DynamicOps<T> dynamicOps, MapLike<T> mapLike) {
						...
						mapLike.entries()
							.forEach(
								pair -> {
									DataResult<MemoryModuleType<?>> dataResult = Registry.MEMORY_MODULE_TYPE.method_39673().parse(dynamicOps, (T)pair.getFirst());
						...
						ImmutableList<Brain.MemoryEntry<?>> immutableList = (ImmutableList)mutableObjectx.getValue()
	...
	private void forgetIrrelevantMemories(Activity except) {
		for(Activity activity : this.possibleActivities) {
			if (activity != except) {
				Set<MemoryModuleType<?>> set = (Set)this.forgettingActivityMemories.get(activity);
				if (set != null) {
	...
	static final class MemoryEntry<U> {
		...
		public <T> void serialize(DynamicOps<T> ops, RecordBuilder<T> builder) {
			this.type
				...
				.ifPresent(
					codec -> this.data
							.ifPresent(memory -> builder.add(Registry.MEMORY_MODULE_TYPE.method_39673().encodeStart(ops, this.type), codec.encodeStart(ops, memory)))
				);
		}
	}
