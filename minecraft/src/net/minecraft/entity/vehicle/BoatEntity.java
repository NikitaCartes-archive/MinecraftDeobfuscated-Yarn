...
public class BoatEntity extends Entity {
					...
					if (p < j) {
						for(int q = m; q < n; ++q) {
							...
							if (fluidState.matches(FluidTags.field_15517)) {
								f = Math.max(f, (float)o + fluidState.method_15763());
							}
	...
	public float method_7548() {
		...
		BoundingBox boundingBox2 = new BoundingBox(
			boundingBox.minX, boundingBox.minY - 0.001, boundingBox.minZ, boundingBox.maxX, boundingBox.minY, boundingBox.maxZ
		);
			...
			for(int o = i; o < j; ++o) {
				for(int p = k; p < l; ++p) {
					for(int q = m; q < n; ++q) {
						...
						FluidState fluidState = this.world.getFluidState(pooledMutable);
						if (fluidState.matches(FluidTags.field_15517)) {
							float f = (float)p + fluidState.method_15763();
			...
			for(int o = i; o < j; ++o) {
				for(int p = k; p < l; ++p) {
					for(int q = m; q < n; ++q) {
						pooledMutable.method_10113(o, p, q);
						FluidState fluidState = this.world.getFluidState(pooledMutable);
						if (fluidState.matches(FluidTags.field_15517) && d < (double)((float)pooledMutable.getY() + fluidState.method_15763())) {
	...
	private void method_7534() {
		...
		if (this.field_7701 == BoatEntity.class_1691.field_7720
			...
			this.field_7697 = this.getBoundingBox().minY + (double)this.height;
			this.setPosition(this.x, (double)(this.method_7544() - this.height) + 0.101, this.z);
			this.velocityY = 0.0;
			this.field_7696 = 0.0;
			this.field_7702 = BoatEntity.class_1691.field_7718;
		} else {
			if (this.field_7702 == BoatEntity.class_1691.field_7718) {
				f = (this.field_7697 - this.getBoundingBox().minY) / (double)this.height;
			...
			this.velocityX *= (double)this.field_7692;
			this.velocityZ *= (double)this.field_7692;
	...
	private void method_7549() {
		if (this.hasPassengers()) {
			...
			this.velocityX += (double)(MathHelper.sin(-this.yaw * (float) (Math.PI / 180.0)) * f);
			this.velocityZ += (double)(MathHelper.cos(this.yaw * (float) (Math.PI / 180.0)) * f);
