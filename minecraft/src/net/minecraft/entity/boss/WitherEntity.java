...
public class WitherEntity extends HostileEntity implements RangedAttacker {
		...
		if (this.getInvulTimer() > 0) {
			...
			for(int i = 1; i < 3; ++i) {
				if (this.age >= this.field_7091[i - 1]) {
					...
					if ((this.field_6002.getDifficulty() == Difficulty.NORMAL || this.field_6002.getDifficulty() == Difficulty.HARD) && this.field_7092[i - 1]++ > 15) {
						float f = 10.0F;
						float g = 5.0F;
	...
	public int getTrackedEntityId(int i) {
		return this.field_6011.get((TrackedData)field_7087.get(i));
	}

	public void setTrackedEntityId(int i, int j) {
		this.field_6011.set((TrackedData)field_7087.get(i), j);
	}
