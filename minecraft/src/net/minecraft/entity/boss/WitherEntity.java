...
public class WitherEntity extends HostileEntity implements RangedAttacker {
		...
		if (!this.world.isClient && this.getTrackedEntityId(0) > 0) {
			...
			if (entity != null) {
				...
				if (f > 9.0) {
					...
					this.velocityX += (d / g * 0.5 - this.velocityX) * 0.6F;
					this.velocityZ += (e / g * 0.5 - this.velocityZ) * 0.6F;
				}
			}
		}
	...
	public int getTrackedEntityId(int i) {
		return this.dataTracker.get((TrackedData)field_7087.get(i));
	}

	public void setTrackedEntityId(int i, int j) {
		this.dataTracker.set((TrackedData)field_7087.get(i), j);
	}
