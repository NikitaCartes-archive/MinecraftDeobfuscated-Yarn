...
public class EnderDragonEntity extends MobEntity implements Monster {
	...
	private static final TargetPredicate field_18120 = new TargetPredicate().setBaseMaxDistance(64.0);
	...
	public void updateMovement() {
		...
		if (this.getHealth() <= 0.0F) {
			...
			this.world.addParticle(ParticleTypes.field_11236, this.x + (double)f, this.y + 2.0 + (double)g, this.z + (double)h, 0.0, 0.0, 0.0);
		} else {
			this.method_6830();
			Vec3d vec3d = this.getVelocity();
			float g = 2.0F / (MathHelper.sqrt(squaredHorizontalLength(vec3d)) + 1.0F);
	...
	private boolean method_6821(BoundingBox boundingBox) {
		...
		if (bl2) {
			...
			this.world.addParticle(ParticleTypes.field_11236, d, e, f, 0.0, 0.0, 0.0);
		}
