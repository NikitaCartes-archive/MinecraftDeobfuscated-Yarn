...
public class EnderDragonEntity extends MobEntity implements Monster {
	...
	public void tickMovement() {
		...
		if (this.getHealth() <= 0.0F) {
			...
			this.field_6002.addParticle(ParticleTypes.field_11236, this.x + (double)f, this.y + 2.0 + (double)g, this.z + (double)h, 0.0, 0.0, 0.0);
		} else {
			this.method_6830();
			Vec3d vec3d = this.method_18798();
			float g = 0.2F / (MathHelper.sqrt(method_17996(vec3d)) * 10.0F + 1.0F);
			...
			if (this.isAiDisabled()) {
				...
				this.partHead
					.setPositionAndAngles(this.x + (double)(aa * 6.5F * v), this.y + (double)m + (double)(w * 6.5F), this.z - (double)(ab * 6.5F * v), 0.0F, 0.0F);
				this.partNeck
					.setPositionAndAngles(this.x + (double)(aa * 5.5F * v), this.y + (double)m + (double)(w * 5.5F), this.z - (double)(ab * 5.5F * v), 0.0F, 0.0F);
