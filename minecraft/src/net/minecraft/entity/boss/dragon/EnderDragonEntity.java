			...
			if (this.isAiDisabled()) {
				...
				if (this.world.isClient) {
					...
					if (optional.isPresent()) {
						Object var38 = optional.get();
						if (var38 instanceof PlayerEntity playerEntity) {
					...
					if (vec3d2 != null) {
						...
						if (Math.abs(l) > 1.0E-5F || Math.abs(n) > 1.0E-5F) {
							...
							this.yawAcceleration += s * phase.getYawAcceleration();
				...
				for(int af = 0; af < 3; ++af) {
					...
					float r = MathHelper.cos(ag);
					float s = 1.5F;
					float t = (float)(af + 1) * 2.0F;
					this.movePart(enderDragonPart, (double)(-(aa * 1.5F + ah * t) * x), es[1] - ds[1] - (double)((t + 1.5F) * y) + 1.5, (double)((ab * 1.5F + r * t) * x));
				}
