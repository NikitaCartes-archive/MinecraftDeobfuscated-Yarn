...
public enum EnderDragonSpawnState {
			...
			if (bl || bl2) {
				...
				if (k < list.size()) {
					...
					if (bl) {
						...
						Feature.END_SPIKE
							.generate(
								world, world.getChunkManager().getChunkGenerator(), new Random(), new BlockPos(spike.getCenterX(), 45, spike.getCenterZ()), endSpikeFeatureConfig
							);
					}
				} else if (bl) {
	...
	public abstract void run(ServerWorld world, EnderDragonFight fight, List<EnderCrystalEntity> crystals, int i, BlockPos blockPos);
}
