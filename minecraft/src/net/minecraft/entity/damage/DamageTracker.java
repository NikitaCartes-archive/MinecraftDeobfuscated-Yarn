...
public class DamageTracker {
		...
		if (this.recentDamage.isEmpty()) {
			...
			if (damageRecord != null && deathMessageType == DeathMessageType.FALL_VARIANTS) {
				...
				if (damageSource2.isIn(DamageTypeTags.IS_FALL) || damageSource2.isIn(DamageTypeTags.ALWAYS_MOST_SIGNIFICANT_FALL)) {
					...
					ItemStack itemStack = entity2 instanceof LivingEntity livingEntity ? livingEntity.getMainHandStack() : ItemStack.EMPTY;
	...
	public LivingEntity getBiggestAttacker() {
		...
		for(DamageRecord damageRecord : this.recentDamage) {
			Entity var8 = damageRecord.getDamageSource().getAttacker();
			if (var8 instanceof PlayerEntity playerEntity2 && (playerEntity == null || damageRecord.getDamage() > g)) {
				g = damageRecord.getDamage();
				playerEntity = playerEntity2;
			}

			var8 = damageRecord.getDamageSource().getAttacker();
			if (var8 instanceof LivingEntity livingEntity2 && (livingEntity == null || damageRecord.getDamage() > f)) {
