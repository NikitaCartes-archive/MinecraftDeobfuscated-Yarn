...
public class FallingBlockEntity extends Entity {
	...
	public FallingBlockEntity(ServerPlayerEntity serverPlayerEntity, BlockState blockState, Direction direction) {
		...
		if (this.block.isOf(Blocks.END_PORTAL_FRAME)) {
			World blockPos = this.world;
			if (blockPos instanceof ServerWorld serverWorld) {
				BlockPos blockPosx = serverPlayerEntity.getBlockPos();
				BlockPos blockPos2 = serverWorld.locateStructure(ConfiguredStructureFeatureTags.EYE_OF_ENDER_LOCATED, blockPosx, 100, false);
				if (blockPos2 != null) {
					BlockPos blockPos3 = blockPos2.subtract(blockPosx);
								...
								if (this.blockEntityData != null && this.block.hasBlockEntity()) {
									...
									if (blockEntity != null) {
										...
										try {
											...
											field_36333.error("Failed to load block entity from falling block", var25);
										}
	...
	private boolean method_42819(BlockPos blockPos, Direction direction) {
		...
		optional2.ifPresent(blockStatex -> {
			for(BlockPos blockPos2xx : BlockPos.iterate(blockPos.add(-1, 0, -1), blockPos.add(1, 0, 1))) {
				if (!class_7320.method_42867(this.world.getBlockState(blockPos2xx)).isEmpty()) {
					this.world.setBlockState(blockPos2xx, blockStatex, Block.NOTIFY_ALL);
				}
			}
		});
