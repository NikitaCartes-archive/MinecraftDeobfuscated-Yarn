...
public abstract class HorseBaseEntity extends AnimalEntity implements InventoryListener, JumpingMount {
	...
	protected HorseInventory field_6962;
	...
	public void method_5617(boolean bl) {
		this.method_5616(bl ? this.method_6770() : 1.0F);
	}
	...
	public boolean canBeLeashedBy(PlayerEntity playerEntity) {
		return super.canBeLeashedBy(playerEntity) && this.method_6046() != class_1310.field_6289;
	}
	...
	public void handleFallDamage(float f, float g) {
		if (f > 1.0F) {
			this.playSoundAtEntity(SoundEvents.field_14783, 0.4F, 1.0F);
		}
	...
	protected void method_6721() {
		HorseInventory horseInventory = this.field_6962;
		this.field_6962 = new HorseInventory(this.getName(), this.getInventorySize());
		this.field_6962.setCustomName(this.getCustomName());
		if (horseInventory != null) {
			horseInventory.removeListener(this);
			int i = Math.min(horseInventory.getInvSize(), this.field_6962.getInvSize());

			for(int j = 0; j < i; ++j) {
				ItemStack itemStack = horseInventory.getInvStack(j);
	...
	public void onInvChange(Inventory inventory) {
		...
		if (this.age > 20 && !bl && this.isSaddled()) {
			this.playSoundAtEntity(SoundEvents.field_14704, 0.5F, 1.0F);
		}
	}
	...
	protected void playStepSound(BlockPos blockPos, BlockState blockState) {
		if (!blockState.getMaterial().method_15797()) {
			...
			if (this.hasPassengers() && this.field_6964) {
				...
				if (this.field_6975 > 5 && this.field_6975 % 3 == 0) {
					...
					this.playSoundAtEntity(SoundEvents.field_15061, blockSoundGroup.getVolume() * 0.15F, blockSoundGroup.getPitch());
				}
			} else if (blockSoundGroup == BlockSoundGroup.WOOD) {
				this.playSoundAtEntity(SoundEvents.field_15061, blockSoundGroup.getVolume() * 0.15F, blockSoundGroup.getPitch());
			} else {
				this.playSoundAtEntity(SoundEvents.field_14613, blockSoundGroup.getVolume() * 0.15F, blockSoundGroup.getPitch());
			}
		}
	}

	protected void method_6761(BlockSoundGroup blockSoundGroup) {
		this.playSoundAtEntity(SoundEvents.field_14987, blockSoundGroup.getVolume() * 0.15F, blockSoundGroup.getPitch());
	}
	...
	public void method_6722(PlayerEntity playerEntity) {
		if (!this.world.isRemote && (!this.hasPassengers() || this.hasPassenger(playerEntity)) && this.isTame()) {
			this.field_6962.setCustomName(this.getCustomName());
	...
	protected boolean method_6742(PlayerEntity playerEntity, ItemStack itemStack) {
		...
		if (item == Items.field_8861) {
			...
			i = 60;
			j = 5;
			if (this.isTame() && this.getBreedingAge() == 0 && !this.method_6479()) {
				bl = true;
	...
	public void update() {
		...
		if (this.isEating()) {
			this.field_6969 += (1.0F - this.field_6969) * 0.4F + 0.05F;
			if (this.field_6969 > 1.0F) {
				this.field_6969 = 1.0F;
			}
		} else {
			this.field_6969 += (0.0F - this.field_6969) * 0.4F - 0.05F;
		...
		if (this.method_6736()) {
			this.field_6969 = 0.0F;
			this.field_6966 = this.field_6969;
			this.field_6967 += (1.0F - this.field_6967) * 0.4F + 0.05F;
			if (this.field_6967 > 1.0F) {
				this.field_6967 = 1.0F;
			}
		} else {
			this.field_6960 = false;
			this.field_6967 += (0.8F * this.field_6967 * this.field_6967 * this.field_6967 - this.field_6967) * 0.6F - 0.05F;
		...
		if (this.getHorseFlag(64)) {
			this.field_6965 += (1.0F - this.field_6965) * 0.7F + 0.05F;
			if (this.field_6965 > 1.0F) {
				this.field_6965 = 1.0F;
			}
		} else {
			this.field_6965 += (0.0F - this.field_6965) * 0.7F - 0.05F;
	...
	public void method_6757() {
		...
		if (soundEvent != null) {
			this.playSoundAtEntity(soundEvent, this.getSoundVolume(), this.getSoundPitch());
		}
	}
		...
		if (this.hasPassengers() && this.method_5956() && this.isSaddled()) {
			...
			if (this.field_6976 > 0.0F && !this.method_6763() && this.onGround) {
				...
				if (this.hasPotionEffect(StatusEffects.field_5913)) {
					this.velocityY += (double)((float)(this.getPotionEffect(StatusEffects.field_5913).getAmplifier() + 1) * 0.1F);
				}
				...
				if (h > 0.0F) {
					...
					this.velocityX += (double)(-0.4F * i * this.field_6976);
					this.velocityZ += (double)(0.4F * j * this.field_6976);
					this.method_6723();
				}
			...
			this.field_6225 += (k - this.field_6225) * 0.4F;
	...
	protected void method_6723() {
		this.playSoundAtEntity(SoundEvents.field_14831, 0.4F, 1.0F);
	}
	...
	protected boolean method_6734() {
		return !this.hasPassengers() && !this.hasVehicle() && this.isTame() && !this.isChild() && this.getHealth() >= this.getHealthMaximum() && this.method_6479();
	}
	...
	public boolean method_5956() {
		return this.method_5642() instanceof LivingEntity;
	}
	...
	protected void method_6760(boolean bl) {
		Particle particle = bl ? ParticleTypes.field_11201 : ParticleTypes.field_11251;
