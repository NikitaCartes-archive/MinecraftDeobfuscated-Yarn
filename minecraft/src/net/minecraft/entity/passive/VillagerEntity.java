...
public class VillagerEntity extends AbstractTraderEntity implements class_4094, VillagerDataContainer {
	...
	protected void method_18008(TraderRecipe traderRecipe) {
		...
		this.field_18536 += traderRecipe.method_19279();
		...
		if (this.field_18533 < 12 && this.method_19189() != 0) {
			...
			for(int i = 0; i < this.getInventory().getInvSize(); ++i) {
				...
				if (!itemStack.isEmpty() || set.contains(item)) {
					...
					for(int k = j; k > 0; --k) {
						this.field_18533 = (byte)(this.field_18533 + field_18526.get(item));
	...
	private int method_19189() {
		...
		return field_18526.entrySet().stream().mapToInt(entry -> basicInventory.method_18861((Item)entry.getKey()) * entry.getValue()).sum();
	}

	@Override
	protected void fillRecipes() {
		VillagerData villagerData = this.getVillagerData();
		Int2ObjectMap<Trades.Factory[]> int2ObjectMap = (Int2ObjectMap)Trades.PROFESSION_TO_LEVELED_TRADE.get(villagerData.method_16924());
