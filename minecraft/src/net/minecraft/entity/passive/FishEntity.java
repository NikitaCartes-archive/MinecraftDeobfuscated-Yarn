...
public abstract class FishEntity extends WaterCreatureEntity {
	...
	public void updateMovement() {
		if (!this.isInsideWater() && this.onGround && this.verticalCollision) {
			this.velocityY += 0.4F;
			this.velocityX += (double)((this.random.nextFloat() * 2.0F - 1.0F) * 0.05F);
			this.velocityZ += (double)((this.random.nextFloat() * 2.0F - 1.0F) * 0.05F);
			this.onGround = false;
	...
	static class FishMoveControl extends MoveControl {
		...
		public void tick() {
			...
			if (this.state == MoveControl.State.field_6378 && !this.fish.getNavigation().isIdle()) {
				...
				this.fish.velocityY += (double)this.fish.getMovementSpeed() * e * 0.1;
			} else {
