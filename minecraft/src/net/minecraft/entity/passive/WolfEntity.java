...
public class WolfEntity extends TameableEntity {
	...
	protected void playStepSound(BlockPos blockPos, BlockState blockState) {
		this.playSoundAtEntity(SoundEvents.field_14772, 0.15F, 1.0F);
	}
	...
	public void update() {
		...
		if (this.method_6710()) {
			this.field_6952 += (1.0F - this.field_6952) * 0.4F;
		} else {
			this.field_6952 += (0.0F - this.field_6952) * 0.4F;
		}
		...
		if (this.method_5637()) {
			...
			if (this.field_6947 == 0.0F) {
				this.playSoundAtEntity(SoundEvents.field_15042, this.getSoundVolume(), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
			}
			...
			if (this.field_6947 > 0.4F) {
				...
				for(int j = 0; j < i; ++j) {
					...
					this.world
						.method_8406(ParticleTypes.field_11202, this.x + (double)g, (double)(f + 0.8F), this.z + (double)h, this.velocityX, this.velocityY, this.velocityZ);
				}
			}
		}
	}
		...
		if (this.isTamed()) {
			if (!itemStack.isEmpty()) {
				if (item instanceof FoodItem) {
					...
					if (foodItem.isWolfFood() && this.dataTracker.get(WOLF_HEALTH) < 20.0F) {
	...
	public boolean method_6474(AnimalEntity animalEntity) {
		if (animalEntity == this) {
			...
			if (!wolfEntity.isTamed()) {
				...
				return this.method_6479() && wolfEntity.method_6479();
			}
		}
	}
	...
	class WolfFleeGoal<T extends Entity> extends FleeEntityGoal<T> {
		...
		public boolean canStart() {
			if (super.canStart() && this.field_6390 instanceof LlamaEntity) {
				return !this.field_6954.isTamed() && this.method_6720((LlamaEntity)this.field_6390);
			} else {
				return false;
			}
		}
