...
public class BeeEntity extends AnimalEntity implements Flutterer {
	...
	protected void initGoals() {
		this.goalSelector.add(0, new BeeEntity.EnterHiveGoal());
		this.goalSelector.add(0, new BeeEntity.FindHiveGoal());
		this.goalSelector.add(1, new AnimalMateGoal(this, 1.0));
		this.goalSelector.add(2, new TemptGoal(this, 1.25, Ingredient.fromTag(ItemTags.SMALL_FLOWERS), false));
		this.goalSelector.add(3, new BeeEntity.PollinateGoal());
		this.goalSelector.add(4, new FollowParentGoal(this, 1.25));
		this.goalSelector.add(4, new BeeEntity.MoveToHiveGoal());
		this.goalSelector.add(5, new BeeEntity.MoveToFlowerGoal());
		this.goalSelector.add(6, new BeeEntity.StingGoal(this, 1.4F, true));
	...
	protected float getActiveEyeHeight(EntityPose entityPose, EntityDimensions entityDimensions) {
		return this.isBaby() ? entityDimensions.height * 0.95F : 0.6F;
	}
	...
	abstract class BeeMoveToTargetGoal extends BeeEntity.NotAngryGoal {
		...
		public void tick() {
			...
			if (BeeEntity.this.getNavigation().isIdle()) {
				...
				if (vec3d == null) {
					vec3d = PathfindingUtil.method_6373(BeeEntity.this, 3, 3, new Vec3d((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()));
				}

				if (vec3d != null && !bl && BeeEntity.this.world.getBlockState(new BlockPos(vec3d)).getBlock() != Blocks.WATER) {
					vec3d = PathfindingUtil.method_6373(BeeEntity.this, 8, 6, new Vec3d((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()));
				}
	...
	class BeeWanderAroundGoal extends Goal {
		...
		private Vec3d getRandomLocation() {
			...
			int i = 8;
			Vec3d vec3d2 = PathfindingUtil.method_21757(BeeEntity.this, 8, 7, vec3d, (float) (Math.PI / 2), 2, 1);
			return vec3d2 != null ? vec3d2 : PathfindingUtil.method_21756(BeeEntity.this, 8, 4, -2, vec3d, (float) (Math.PI / 2));
		}
	}
	...
	class PollinateGoal extends BeeEntity.NotAngryGoal {
		private final Predicate<BlockState> field_20617 = blockState -> {
			if (blockState.matches(BlockTags.SMALL_FLOWERS)) {
				return true;
			} else if (!blockState.matches(BlockTags.TALL_FLOWERS)) {
				return false;
			} else {
				return blockState.getBlock() == Blocks.SUNFLOWER
					&& blockState.contains(TallPlantBlock.HALF)
					&& blockState.get(TallPlantBlock.HALF) == DoubleBlockHalf.UPPER;
			}
		};
