...
public class BeeEntity extends AnimalEntity implements Flutterer {
	...
	public void writeCustomDataToTag(CompoundTag tag) {
		...
		tag.put("HivePos", NbtHelper.fromBlockPos(this.hivePos));
	...
	private Optional<BlockPos> findPointOfInterest(int range) {
		BlockPos blockPos = this.getBlockPos();
		if (this.world instanceof ServerWorld) {
			Optional<PointOfInterest> optional = ((ServerWorld)this.world)
				.getPointOfInterestStorage()
				.get(type -> type == PointOfInterestType.BEEHIVE || type == PointOfInterestType.BEE_NEST, blockPos, range, PointOfInterestStorage.OccupationStatus.ANY)
				.findFirst();
	...
	abstract class BeeMoveToTargetGoal extends BeeEntity.NotAngryGoal {
		...
		public BeeMoveToTargetGoal(int range) {
			this.range = range;
		...
		public void tick() {
			...
			if (BeeEntity.this.getNavigation().isIdle()) {
				...
				Vec3d vec3d2 = TargetFinder.findTargetTowards(BeeEntity.this, 8, 6, vec3d, (float) (Math.PI / 10));
				if (vec3d2 == null) {
	...
	class PollinateGoal extends BeeEntity.NotAngryGoal {
		...
		public boolean canBeeContinue() {
			if (this.completedPollination()) {
				...
				return BeeEntity.this.world.canSetBlock(BeeEntity.this.flowerPos)
					&& BeeEntity.this.world.getBlockState(BeeEntity.this.flowerPos).getBlock() instanceof FlowerBlock;
			}
		}
