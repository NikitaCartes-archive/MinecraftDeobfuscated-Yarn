...
public class SquidEntity extends WaterCreatureEntity {
	...
	public void updateMovement() {
		...
		if (this.isInsideWaterOrBubbleColumn()) {
			...
			this.field_6283 += (-((float)MathHelper.atan2(this.velocityX, this.velocityZ)) * (180.0F / (float)Math.PI) - this.field_6283) * 0.1F;
			this.yaw = this.field_6283;
			this.field_6903 = (float)((double)this.field_6903 + Math.PI * (double)this.field_6913 * 1.5);
			this.field_6907 += (-((float)MathHelper.atan2((double)f, this.velocityY)) * (180.0F / (float)Math.PI) - this.field_6907) * 0.1F;
		} else {
			...
			if (!this.world.isClient) {
				...
				if (this.hasPotionEffect(StatusEffects.field_5902)) {
					this.velocityY += 0.05 * (double)(this.getPotionEffect(StatusEffects.field_5902).getAmplifier() + 1) - this.velocityY;
