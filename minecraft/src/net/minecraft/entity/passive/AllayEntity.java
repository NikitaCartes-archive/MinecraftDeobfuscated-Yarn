...
public class AllayEntity extends PathAwareEntity implements InventoryOwner, Vibrations {
	...
	public AllayEntity(EntityType<? extends AllayEntity> entityType, World world) {
		...
		this.jukeboxEventHandler = new EntityGameEventHandler<>(
			new AllayEntity.JukeboxEventListener(this.vibrationCallback.getPositionSource(), ((GameEvent)GameEvent.JUKEBOX_PLAY.value()).notificationRadius())
		);
	}
	...
	public boolean damage(DamageSource source, float amount) {
		...
		if (optional instanceof PlayerEntity playerEntity) {
			Optional<UUID> optionalx = this.getBrain().getOptionalRegisteredMemory(MemoryModuleType.LIKED_PLAYER);
			if (optionalx.isPresent() && playerEntity.getUuid().equals(optionalx.get())) {
				return false;
			}
		}
	...
	private boolean shouldStopDancing() {
		return this.jukeboxPos == null
			|| !this.jukeboxPos.isWithinDistance(this.getPos(), (double)((GameEvent)GameEvent.JUKEBOX_PLAY.value()).notificationRadius())
