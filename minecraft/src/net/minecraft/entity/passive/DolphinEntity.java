...
public class DolphinEntity extends WaterCreatureEntity {
	...
	protected void method_5959() {
		this.goalSelector.add(0, new class_1342(this));
		this.goalSelector.add(0, new class_1393(this));
		...
		this.goalSelector.add(5, new class_1357(this, 10));
	...
	public boolean method_6121(Entity entity) {
		...
		if (bl) {
			...
			this.playSoundAtEntity(SoundEvents.field_14992, 1.0F, 1.0F);
		}
	...
	public void update() {
		...
		if (!this.isAiDisabled()) {
			if (this.method_5637()) {
				...
				if (this.onGround) {
					this.velocityY += 0.5;
					this.velocityX += (double)((this.random.nextFloat() * 2.0F - 1.0F) * 0.2F);
					this.velocityZ += (double)((this.random.nextFloat() * 2.0F - 1.0F) * 0.2F);
	...
	@Environment(EnvType.CLIENT)
	private void method_6492(Particle particle) {
		for(int i = 0; i < 7; ++i) {
	...
	protected boolean interactMob(PlayerEntity playerEntity, Hand hand) {
		...
		if (!itemStack.isEmpty() && itemStack.getItem().matches(ItemTags.field_15527)) {
			if (!this.world.isRemote) {
				this.playSoundAtEntity(SoundEvents.field_14590, 1.0F, 1.0F);
			}
	...
	public ItemEntity method_6490(ItemStack itemStack) {
		if (itemStack.isEmpty()) {
			...
			itemEntity.velocityX += (double)(MathHelper.cos(g) * f);
			itemEntity.velocityZ += (double)(MathHelper.sin(g) * f);
	...
	public boolean method_5979(IWorld iWorld, class_3730 arg) {
		return this.y > 45.0 && this.y < (double)iWorld.getSeaLevel() && iWorld.getBiome(new BlockPos(this)) != Biomes.field_9423
			|| iWorld.getBiome(new BlockPos(this)) != Biomes.field_9446 && super.method_5979(iWorld, arg);
	}
	...
	protected boolean method_6484() {
		...
		if (blockPos != null) {
			return this.squaredDistanceTo(blockPos) < 144.0;
		} else {
			return false;
		}
	}
		...
		public void tick() {
			...
			if (this.field_6374 == MoveControl.class_1336.field_6378 && !this.dolphin.getNavigation().method_6357()) {
				...
				if (g < 2.5000003E-7F) {
					...
					this.dolphin.headPitch = this.dolphin.yaw;
	...
	static class class_1435 extends Goal {
		...
		public void start() {
			...
			world.method_8421(this.field_6752, (byte)38);
		}
			...
			if (this.field_6752.method_6484() || this.field_6752.getNavigation().method_6357()) {
				...
				if (vec3d != null) {
					...
					if (!world.getFluidState(blockPos2).matches(FluidTags.field_15517)
						|| !world.getBlockState(blockPos2).canPlaceAtSide(world, blockPos2, PlacementEnvironment.field_48)) {
				...
				this.field_6752.getNavigation().method_6337(vec3d.x, vec3d.y, vec3d.z, 1.3);
	...
	static class class_1436 extends Goal {
		...
		public void tick() {
			...
			if (this.field_6755.squaredDistanceTo(this.field_6756) < 6.25) {
				...
				this.field_6755.getNavigation().method_6335(this.field_6756, this.field_6754);
			}
	...
	class class_1437 extends Goal {
		...
		public void start() {
			List<ItemEntity> list = DolphinEntity.this.world
				.getEntities(ItemEntity.class, DolphinEntity.this.getBoundingBox().expand(8.0, 8.0, 8.0), DolphinEntity.field_6748);
			if (!list.isEmpty()) {
				DolphinEntity.this.getNavigation().method_6335((Entity)list.get(0), 1.2F);
				DolphinEntity.this.playSoundAtEntity(SoundEvents.field_14972, 1.0F, 1.0F);
			}
