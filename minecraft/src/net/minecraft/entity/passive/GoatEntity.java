...
public class GoatEntity extends AnimalEntity {
	...
	public boolean canTarget(LivingEntity target) {
		return target instanceof PlayerEntity ? super.canTarget(target) : target.canTakeDamage();
	}
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (itemStack.isOf(Items.BUCKET) && !this.isBaby()) {
			...
			if (actionResult.isAccepted() && this.isBreedingItem(itemStack)) {
				this.world
					.playSoundFromEntity(null, this, this.getEatSound(itemStack), SoundCategory.NEUTRAL, 1.0F, MathHelper.nextBetween(this.world.random, 0.8F, 1.2F));
			}
