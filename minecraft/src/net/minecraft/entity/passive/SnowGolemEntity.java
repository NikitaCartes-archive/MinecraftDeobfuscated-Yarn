...
public class SnowGolemEntity extends GolemEntity implements RangedAttackMob {
	...
	public void tickMovement() {
		...
		if (!this.field_6002.isClient) {
			...
			if (this.field_6002.method_8310(new BlockPos(i, 0, k)).getTemperature(new BlockPos(i, j, k)) > 1.0F) {
				this.damage(DamageSource.ON_FIRE, 1.0F);
			}

			if (!this.field_6002.getGameRules().getBoolean(GameRules.field_19388)) {
				return;
			}
			...
			for (int l = 0; l < 4; l++) {
				...
				if (this.field_6002.method_8320(blockPos).isAir()
					&& this.field_6002.method_8310(blockPos).getTemperature(blockPos) < 0.8F
					&& blockState.canPlaceAt(this.field_6002, blockPos)) {
