...
public interface Hoglin {
	...
	static boolean tryAttack(LivingEntity attacker, LivingEntity target) {
		float f = (float)attacker.getAttributeValue(EntityAttributes.field_23721);
		float g;
	...
	static void knockback(LivingEntity attacker, LivingEntity target) {
		double d = attacker.getAttributeValue(EntityAttributes.field_23722);
		double e = target.getAttributeValue(EntityAttributes.field_23718);
		double f = d - e;
		if (!(f <= 0.0)) {
