...
public class HoglinBrain {
	...
	private static void addIdleTasks(Brain<HoglinEntity> brain) {
		brain.setTaskList(
			...
			ImmutableList.of(
				...
				new ConditionalTask<PathAwareEntity>(HoglinEntity::isAdult, GoToRememberedPositionTask.toEntity(MemoryModuleType.field_22345, 0.4F, 8, false)),
	...
	private static boolean hasMoreHoglinsAround(HoglinEntity hoglin) {
		if (hoglin.isBaby()) {
			...
			int i = hoglin.getBrain().getOptionalMemory(MemoryModuleType.field_22347).orElse(0);
			int j = hoglin.getBrain().getOptionalMemory(MemoryModuleType.field_22348).orElse(0) + 1;
			return i > j;
		}
	}
