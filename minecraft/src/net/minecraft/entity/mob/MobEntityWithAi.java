...
public abstract class MobEntityWithAi extends MobEntity {
	...
	public boolean isInAiRange(BlockPos blockPos) {
		if (this.aiRadius == -1.0F) {
			return true;
		} else {
			return this.aiHome.squaredDistanceTo(blockPos) < (double)(this.aiRadius * this.aiRadius);
		}
	}
	...
	protected void method_5995() {
		...
		if (this.isLeashed() && this.getHoldingEntity() != null && this.getHoldingEntity().world == this.world) {
			...
			if (f > 10.0F) {
				...
				this.velocityX += d * Math.abs(d) * 0.4;
				this.velocityY += e * Math.abs(e) * 0.4;
				this.velocityZ += g * Math.abs(g) * 0.4;
			} else {
