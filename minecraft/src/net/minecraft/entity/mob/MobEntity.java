...
public abstract class MobEntity extends LivingEntity {
	...
	public boolean method_26320(ItemStack itemStack, ItemStack itemStack2) {
		if (itemStack.getDamage() >= itemStack2.getDamage() && (!itemStack.hasTag() || itemStack2.hasTag())) {
			if (itemStack.hasTag() && itemStack2.hasTag()) {
				return itemStack.getTag().getKeys().stream().anyMatch(string -> !string.equals("Damage"))
					&& !itemStack2.getTag().getKeys().stream().anyMatch(string -> !string.equals("Damage"));
			} else {
				return false;
			}
		} else {
	...
	protected boolean interactMob(PlayerEntity player, Hand hand) {
		...
		if (!this.world.isClient && item instanceof SpawnEggItem) {
			...
			Optional<MobEntity> optional = spawnEggItem.spawnBaby(player, this, this.getType(), this.world, this.getPos(), itemStack);
