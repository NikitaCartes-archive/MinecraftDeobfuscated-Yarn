...
public abstract class MobEntity extends LivingEntity {
	...
	private static final TrackedData<Optional<BlockState>> field_38511 = DataTracker.registerData(
		MobEntity.class, TrackedDataHandlerRegistry.OPTIONAL_BLOCK_STATE
	);
	@Nullable
	...
	public BlockState method_42800() {
		return (BlockState)((Optional)this.dataTracker.get(field_38511)).orElse(null);
	}
	...
	public boolean prefersNewDamageableItem(ItemStack newStack, ItemStack oldStack) {
		if (newStack.getDamage() >= oldStack.getDamage() && (!newStack.hasNbt() || oldStack.hasNbt())) {
			if (newStack.hasNbt() && oldStack.hasNbt()) {
				return newStack.getNbt().getKeys().stream().anyMatch(string -> !string.equals("Damage"))
