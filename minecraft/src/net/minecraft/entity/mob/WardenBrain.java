...
public class WardenBrain {
	...
	private static void addCoreActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(
			...
			ImmutableList.of(
				...
				new UpdateAttackTargetTask<>(warden -> true, WardenBrain::getLastAttacker)
			)
		);
	}
	...
	private static boolean isAngryAt(WardenEntity warden, LivingEntity entity) {
		return getLastAttacker(warden).filter(attacker -> attacker == entity).isPresent()
			|| getPrimeSuspect(warden).filter(suspect -> suspect == entity).isPresent();
	}
