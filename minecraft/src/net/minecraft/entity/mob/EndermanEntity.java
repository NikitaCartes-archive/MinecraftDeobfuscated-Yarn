...
public class EndermanEntity extends HostileEntity {
	...
	private boolean method_7024(double d, double e, double f) {
		...
		if (bl) {
			...
			this.playSoundAtEntity(SoundEvents.field_14879, 1.0F, 1.0F);
		}
	...
	static class PickUpBlockGoal extends Goal {
		...
		public boolean canStart() {
			if (this.owner.getCarriedBlock() != null) {
				return false;
			} else if (!this.owner.world.getGameRules().getBoolean("mobGriefing")) {
				return false;
			} else {
		...
		public void tick() {
			...
			HitResult hitResult = world.rayTrace(
				new Vec3d((double)((float)MathHelper.floor(this.owner.x) + 0.5F), (double)((float)j + 0.5F), (double)((float)MathHelper.floor(this.owner.z) + 0.5F)),
				new Vec3d((double)((float)i + 0.5F), (double)((float)j + 0.5F), (double)((float)k + 0.5F)),
				FluidRayTraceMode.NONE,
				true,
				false
			);
			boolean bl = hitResult != null && hitResult.getBlockPos().equals(blockPos);
