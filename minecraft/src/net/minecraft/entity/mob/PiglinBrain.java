...
public class PiglinBrain {
	...
	private static boolean method_29276(LivingEntity livingEntity, LivingEntity livingEntity2) {
		if (livingEntity2.getType() != EntityType.field_21973) {
			return false;
		} else {
	...
	public static Optional<PlayerEntity> getNearestDetectedPlayer(AbstractPiglinEntity piglin) {
		return piglin.getBrain().hasMemoryModule(MemoryModuleType.field_22354)
			? piglin.getBrain().getOptionalMemory(MemoryModuleType.field_22354)
			: Optional.empty();
	}
	...
	private static boolean shouldRunAwayFromHoglins(PiglinEntity piglin) {
		...
		if (!brain.hasMemoryModule(MemoryModuleType.field_22357)) {
			...
			if (entityType == EntityType.field_21973) {
				...
				return !brain.method_29519(MemoryModuleType.field_22346, livingEntity);
			} else {
				return false;
			}
		}
	...
	private static boolean hasOutnumberedHoglins(PiglinEntity piglins) {
		int i = piglins.getBrain().getOptionalMemory(MemoryModuleType.field_22347).orElse(0) + 1;
		int j = piglins.getBrain().getOptionalMemory(MemoryModuleType.field_22348).orElse(0);
		return j > i;
	}
