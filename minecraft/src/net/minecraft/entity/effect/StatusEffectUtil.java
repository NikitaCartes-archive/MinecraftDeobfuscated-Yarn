...
public final class StatusEffectUtil {
	...
	public static List<ServerPlayerEntity> addEffectToPlayersWithinDistance(
		ServerWorld world, @Nullable Entity source, Vec3d position, double distance, StatusEffectInstance effect, int i
	) {
		StatusEffect statusEffect = effect.getEffectType();
		List<ServerPlayerEntity> list = world.getPlayers(
			player -> position.squaredDistanceTo(player.getPos()) < distance * distance
					&& player.interactionManager.isSurvivalLike()
					&& (
						!player.hasStatusEffect(statusEffect)
							|| player.getStatusEffect(statusEffect).getAmplifier() < effect.getAmplifier()
							|| player.getStatusEffect(statusEffect).getDuration() < i
					)
		);
		list.forEach(player -> player.addStatusEffect(new StatusEffectInstance(effect), source));
		return list;
	}
}
