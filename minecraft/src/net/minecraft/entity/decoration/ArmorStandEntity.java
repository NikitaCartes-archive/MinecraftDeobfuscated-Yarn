	...
	public boolean damage(DamageSource source, float amount) {
		if (this.getWorld().isClient || this.isRemoved()) {
			...
			if (!bl && !bl2) {
				...
				if (var6 instanceof PlayerEntity playerEntity && !playerEntity.getAbilities().allowModifyWorld) {
					return false;
				}
				...
				if (source.isSourceCreativePlayer()) {
					...
					this.kill();
					var6 = source.getSource();
					if (var6 instanceof PersistentProjectileEntity persistentProjectileEntity && persistentProjectileEntity.getPierceLevel() > 0) {
						return true;
					}
