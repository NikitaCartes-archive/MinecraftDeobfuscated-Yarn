...
public final class ProjectileUtil {
	...
	public static HitResult method_18074(Entity entity, Box box, Predicate<Entity> predicate, RayTraceContext.ShapeType shapeType, boolean bl) {
		return method_7482(entity, bl, false, null, shapeType, false, predicate, box);
	}

	@Nullable
	public static EntityHitResult method_18077(World world, Entity entity, Vec3d vec3d, Vec3d vec3d2, Box box, Predicate<Entity> predicate) {
		return method_18078(world, entity, vec3d, vec3d2, box, predicate, Double.MAX_VALUE);
	}
	...
	private static HitResult method_7482(
		...
		if (bl3 && !world.method_8590(entity, entity.method_5829(), (Set<Entity>)(!bl2 && entity2 != null ? getEntityAndRidingEntity(entity2) : ImmutableSet.of()))) {
			...
			HitResult hitResult = world.method_17742(new RayTraceContext(vec3d2, vec3d3, shapeType, RayTraceContext.FluidHandling.field_1348, entity));
			if (bl) {
				if (hitResult.getType() != HitResult.Type.field_1333) {
					vec3d3 = hitResult.method_17784();
				}

				HitResult hitResult2 = method_18077(world, entity, vec3d2, vec3d3, box, predicate);
				if (hitResult2 != null) {
	...
	public static EntityHitResult method_18075(Entity entity, Vec3d vec3d, Vec3d vec3d2, Box box, Predicate<Entity> predicate, double d) {
		World world = entity.field_6002;
		double e = d;
		Entity entity2 = null;
		Vec3d vec3d3 = null;

		for (Entity entity3 : world.method_8333(entity, box, predicate)) {
			Box box2 = entity3.method_5829().expand((double)entity3.getBoundingBoxMarginForTargeting());
			Optional<Vec3d> optional = box2.method_992(vec3d, vec3d2);
			if (box2.method_1006(vec3d)) {
				if (e >= 0.0) {
					entity2 = entity3;
	...
	@Nullable
	public static EntityHitResult method_18078(World world, Entity entity, Vec3d vec3d, Vec3d vec3d2, Box box, Predicate<Entity> predicate, double d) {
		double e = d;
		Entity entity2 = null;

		for (Entity entity3 : world.method_8333(entity, box, predicate)) {
			Box box2 = entity3.method_5829().expand(0.3F);
			Optional<Vec3d> optional = box2.method_992(vec3d, vec3d2);
