...
public class FishHookEntity extends Entity {
	...
	private void method_6953() {
		...
		this.velocityX *= 0.6 / (double)m + 0.5 + this.random.nextGaussian() * 0.0045;
		this.velocityY *= 0.6 / (double)m + 0.5 + this.random.nextGaussian() * 0.0045;
		this.velocityZ *= 0.6 / (double)m + 0.5 + this.random.nextGaussian() * 0.0045;
		...
		if (this.owner == null) {
			...
			if (this.state == FishHookEntity.State.field_7180) {
				...
				if (this.state == FishHookEntity.State.field_7178) {
					if (this.hookedEntity != null) {
						if (this.hookedEntity.invalid) {
							...
							double var10002 = (double)this.hookedEntity.height;
							this.y = this.hookedEntity.getBoundingBox().minY + var10002 * 0.8;
				...
				if (this.state == FishHookEntity.State.field_7179) {
					...
					this.velocityY -= d * (double)this.random.nextFloat() * 0.2;
	...
	private void method_6949(BlockPos blockPos) {
		...
		if (this.field_7173 > 0) {
			...
			if (this.field_7173 <= 0) {
				...
				this.velocityY -= 0.2 * (double)this.random.nextFloat() * (double)this.random.nextFloat();
			}
			...
			if (this.field_7172 > 0) {
				...
				this.playSoundAtEntity(SoundEvents.field_14660, 0.25F, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.4F);
				double m = this.getBoundingBox().minY + 0.5;
				serverWorld.method_14199(ParticleTypes.field_11247, this.x, m, this.z, (int)(1.0F + this.width * 20.0F), (double)this.width, 0.0, (double)this.width, 0.2F);
				serverWorld.method_14199(ParticleTypes.field_11244, this.x, m, this.z, (int)(1.0F + this.width * 20.0F), (double)this.width, 0.0, (double)this.width, 0.2F);
			...
			this.field_7174 -= this.field_7168 * 20 * 5;
		}
	}
