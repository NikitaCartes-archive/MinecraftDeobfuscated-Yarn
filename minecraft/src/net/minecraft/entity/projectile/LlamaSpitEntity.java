...
public class LlamaSpitEntity extends Entity implements Projectile {
	...
	public LlamaSpitEntity(World world, LlamaEntity llamaEntity) {
		...
		this.setPosition(
			llamaEntity.x - (double)(llamaEntity.width + 1.0F) * 0.5 * (double)MathHelper.sin(llamaEntity.field_6283 * (float) (Math.PI / 180.0)),
			llamaEntity.y + (double)llamaEntity.getEyeHeight() - 0.1F,
			llamaEntity.z + (double)(llamaEntity.width + 1.0F) * 0.5 * (double)MathHelper.cos(llamaEntity.field_6283 * (float) (Math.PI / 180.0))
		);
	}
	...
	public void method_7481(HitResult hitResult) {
		if (hitResult.entity != null && this.owner != null) {
