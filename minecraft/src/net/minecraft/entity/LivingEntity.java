...
public abstract class LivingEntity extends Entity {
	...
	public int field_6269 = 20;
	public float field_6286;
	public float field_6223;
	public float field_6244;
	public float field_6262;
	...
	protected void method_5623(double d, boolean bl, BlockState blockState, BlockPos blockPos) {
		...
		if (!this.world.isRemote && this.fallDistance > 3.0F && bl) {
			...
			if (!blockState.isAir()) {
				...
				int i = (int)(150.0 * e);
				((ServerWorld)this.world).method_14199(new BlockStateParticle(ParticleTypes.field_11217, blockState), this.x, this.y, this.z, i, 0.0, 0.0, 0.0, 0.15F);
			}
		}
	...
	public void updateLogic() {
		...
		super.updateLogic();
		this.world.getProfiler().begin("livingEntityBaseTick");
		...
		if (this.isFireImmune() || this.world.isRemote) {
			this.extinguish();
		}
		...
		if (this.isValid()) {
			if (this.method_5777(FluidTags.field_15517)
				...
				if (!this.method_6094() && !StatusEffectUtil.hasWaterBreathing(this) && !bl2) {
					...
					if (this.getBreath() == -20) {
						...
						for(int i = 0; i < 8; ++i) {
							...
							this.world
								.method_8406(ParticleTypes.field_11247, this.x + (double)f, this.y + (double)g, this.z + (double)h, this.velocityX, this.velocityY, this.velocityZ);
						}
				...
				if (!this.world.isRemote && this.hasVehicle() && this.getRiddenEntity() != null && !this.getRiddenEntity().method_5788()) {
					this.stopRiding();
				}
		...
		if (this.isValid() && this.method_5637()) {
			this.extinguish();
		}
	...
	protected void updatePostDeath() {
		...
		if (this.deathCounter == 20) {
			if (!this.world.isRemote && (this.method_6071() || this.playerHitTimer > 0 && this.canDropLootAndXp() && this.world.getGameRules().getBoolean("doMobLoot"))
				)
			 {
	...
	protected void onEquipStack(ItemStack itemStack) {
		if (!itemStack.isEmpty()) {
			...
			if (item instanceof ArmorItem) {
				soundEvent = ((ArmorItem)item).getMaterial().method_7698();
			} else if (item == Items.field_8833) {
				soundEvent = SoundEvents.field_14966;
			}

			this.playSoundAtEntity(soundEvent, 1.0F, 1.0F);
		}
	}
	...
	public void writeCustomDataToTag(CompoundTag compoundTag) {
		...
		for(EquipmentSlot equipmentSlot : EquipmentSlot.values()) {
			ItemStack itemStack = this.getEquippedStack(equipmentSlot);
			if (!itemStack.isEmpty()) {
				this.getAttributeContainer().method_6210(itemStack.getAttributeModifiers(equipmentSlot));
			}
		}
		...
		if (!this.activePotionEffects.isEmpty()) {
			...
			for(StatusEffectInstance statusEffectInstance : this.activePotionEffects.values()) {
				listTag.add((net.minecraft.nbt.Tag)statusEffectInstance.serialize(new CompoundTag()));
			}
	...
	public void readCustomDataFromTag(CompoundTag compoundTag) {
		...
		if (compoundTag.containsKey("Attributes", 9) && this.world != null && !this.world.isRemote) {
	...
	public boolean method_6016(StatusEffect statusEffect) {
	...
	public boolean damage(DamageSource damageSource, float f) {
		if (this.isInvulnerableTo(damageSource)) {
			...
			boolean bl2 = true;
			if ((float)this.field_6008 > (float)this.field_6269 / 2.0F) {
			...
			if (this instanceof ServerPlayerEntity) {
				CriterionCriterions.ENTITY_HURT_PLAYER.handle((ServerPlayerEntity)this, damageSource, g, f, bl);
			...
			if (entity2 instanceof ServerPlayerEntity) {
				CriterionCriterions.PLAYER_HURT_ENTITY.handle((ServerPlayerEntity)entity2, this, damageSource, g, f, bl);
			}
		...
		if (damageSource.doesDamageToCreative()) {
			...
			if (itemStack != null) {
				if (this instanceof ServerPlayerEntity) {
					...
					CriterionCriterions.USED_TOTEM.handle(serverPlayerEntity, itemStack);
				}
	...
	public void method_6045(ItemStack itemStack) {
		super.playSoundAtEntity(SoundEvents.field_15075, 0.8F, 0.8F + this.world.random.nextFloat() * 0.4F);
	...
	private boolean method_6077(BlockPos blockPos, BlockState blockState) {
		if (blockState.get(TrapdoorBlock.field_11631)) {
	...
	public void handleFallDamage(float f, float g) {
		...
		if (i > 0) {
			this.playSoundAtEntity(this.getFallSound(i), 1.0F, 1.0F);
			...
			if (!blockState.isAir()) {
				...
				this.playSoundAtEntity(blockSoundGroup.getFallSound(), blockSoundGroup.getVolume() * 0.5F, blockSoundGroup.getPitch() * 0.75F);
			}
		}
	}
	...
	public void method_5711(byte b) {
		...
		if (b == 2 || bl || bl2 || bl3) {
			...
			this.field_6271 = 0.0F;
			if (bl) {
				this.playSoundAtEntity(SoundEvents.field_14663, this.getSoundVolume(), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
			}
			...
			SoundEvent soundEvent = this.getHurtSound(damageSource);
			if (soundEvent != null) {
				this.playSoundAtEntity(soundEvent, this.getSoundVolume(), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
			}
			...
			if (soundEvent2 != null) {
				this.playSoundAtEntity(soundEvent2, this.getSoundVolume(), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
			}
			...
			this.playSoundAtEntity(SoundEvents.field_15150, 1.0F, 0.8F + this.world.random.nextFloat() * 0.4F);
		} else {
			super.method_5711(b);
		}
	}
	...
	public void method_6038(Entity entity) {
		if (!(entity instanceof BoatEntity) && !(entity instanceof HorseBaseEntity)) {
			double k = entity.x;
			double l = entity.getBoundingBox().minY + (double)entity.height;
			double e = entity.z;
			...
			if (direction != null) {
				...
				BoundingBox boundingBox = new BoundingBox(
					m - o / 2.0, entity.getBoundingBox().minY, n - p / 2.0, m + o / 2.0, Math.floor(entity.getBoundingBox().minY) + (double)this.height, n + p / 2.0
				);
				...
				for(int[] js : is) {
					...
					if (this.world.method_8587(this, boundingBox2)) {
						...
						if (this.world.method_8587(this, boundingBox2.offset(0.0, 1.0, 0.0)) && this.world.getBlockState(blockPos).hasSolidTopSurface(this.world, blockPos)) {
							k = s;
							l = this.y + 2.0;
							e = t;
						}
			...
			this.setPosition(i, entity.y + (double)entity.height + 0.001, j);
			if (!this.world.method_8587(this, this.getBoundingBox().union(entity.getBoundingBox()))) {
				this.setPosition(i, entity.y + (double)entity.height + 1.001, j);
				if (!this.world.method_8587(this, this.getBoundingBox().union(entity.getBoundingBox()))) {
					this.setPosition(entity.x, entity.y + (double)this.height + 0.001, entity.z);
				}
			}
		}
	}
	...
	public void method_6091(float f, float g, float h) {
				...
				if (!this.isTouchingLava() || this instanceof PlayerEntity && ((PlayerEntity)this).abilities.flying) {
					if (this.isFallFlying()) {
						...
						if (this.velocityY < 0.0 && m > 0.0) {
							...
							this.velocityY += q;
							this.velocityX += vec3d.x * q / m;
							this.velocityZ += vec3d.z * q / m;
						}
						...
						if (m > 0.0) {
							this.velocityX += (vec3d.x / m * n - this.velocityX) * 0.1;
							this.velocityZ += (vec3d.z / m * n - this.velocityZ) * 0.1;
						}
						...
						try (BlockPos.PooledMutable pooledMutable = BlockPos.PooledMutable.get(this.x, this.getBoundingBox().minY - 1.0, this.z)) {
							...
							this.move(MovementType.SELF, this.velocityX, this.velocityY, this.velocityZ);
							if ((this.field_5976 || this.field_6282) && this.canClimb()) {
								this.velocityY = 0.2;
							}

							if (this.hasPotionEffect(StatusEffects.field_5902)) {
								this.velocityY += (0.05 * (double)(this.getPotionEffect(StatusEffects.field_5902).getAmplifier() + 1) - this.velocityY) * 0.2;
								this.fallDistance = 0.0F;
							} else {
								pooledMutable.set(this.x, 0.0, this.z);
								if (!this.world.isRemote || this.world.isBlockLoaded(pooledMutable) && this.world.getChunk(pooledMutable).isLoadedToWorld()) {
					...
					if (this.field_5976 && this.method_5654(this.velocityX, this.velocityY + 0.6F - this.y + e, this.velocityZ)) {
						this.velocityY = 0.3F;
					}
				}
			} else {
		...
		double m = this instanceof class_1432 ? this.y - this.prevY : 0.0;
		float k = MathHelper.sqrt(d * d + m * m + e * e) * 4.0F;
		if (k > 1.0F) {
			k = 1.0F;
		}
	...
	public void update() {
		...
		this.world.getProfiler().begin("headTurn");
		h = this.method_6031(g, h);
		this.world.getProfiler().end();
		this.world.getProfiler().begin("rangeChecks");
	...
	public void updateMovement() {
		...
		this.world.getProfiler().begin("ai");
		if (this.method_6062()) {
			...
			this.world.getProfiler().begin("newAi");
			this.method_6023();
			this.world.getProfiler().end();
		}

		this.world.getProfiler().end();
		this.world.getProfiler().begin("jump");
		if (this.field_6282) {
		...
		this.world.getProfiler().end();
		this.world.getProfiler().begin("travel");
	...
	private void initAi() {
		...
		if (bl && !this.onGround && !this.hasVehicle()) {
			...
			if (itemStack.getItem() == Items.field_8833 && ElytraItem.isUsable(itemStack)) {
				bl = true;
				if (!this.world.isRemote && (this.field_6239 + 1) % 20 == 0) {
	...
	protected void method_6035(BoundingBox boundingBox, BoundingBox boundingBox2) {
		...
		if (!this.world.isRemote && this.field_6261 <= 0) {
	...
	public void stopRiding() {
		...
		super.stopRiding();
		if (entity != null && entity != this.getRiddenEntity() && !this.world.isRemote) {
	...
	public void method_6103(Entity entity, int i) {
		if (!entity.invalid && !this.world.isRemote) {
	...
	public float getYaw(float f) {
		return f == 1.0F ? this.headPitch : MathHelper.lerp(f, this.prevHeadPitch, this.headPitch);
	}
	...
	protected void method_5785() {
		this.field_6037 = this.random.nextDouble() >= this.getAttributeInstance(EntityAttributes.KNOCKBACK_RESISTANCE).getValue();
	}
	...
	protected void method_6076() {
		if (this.method_6115()) {
			if (this.getStackInHand(this.getActiveHand()) == this.activeItemStack) {
				...
				if (--this.field_6222 == 0 && !this.world.isRemote && !this.activeItemStack.method_7967()) {
					this.method_6040();
				}
			} else {
				this.method_6021();
	...
	public void onTrackedDataSet(TrackedData<?> trackedData) {
		...
		if (LIVING_FLAGS.equals(trackedData) && this.world.isRemote) {
	...
	protected void method_6098(ItemStack itemStack, int i) {
		if (!itemStack.isEmpty() && this.method_6115()) {
			if (itemStack.getUseAction() == UseAction.field_8946) {
				this.playSoundAtEntity(SoundEvents.field_14643, 0.5F, this.world.random.nextFloat() * 0.1F + 0.9F);
			}

			if (itemStack.getUseAction() == UseAction.field_8950) {
				this.method_6037(itemStack, i);
				this.playSoundAtEntity(
					SoundEvents.field_14544, 0.5F + 0.5F * (float)this.random.nextInt(2), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F
				);
			}
		}
	}
	...
	private void method_6037(ItemStack itemStack, int i) {
		for(int j = 0; j < i; ++j) {
			...
			this.world.method_8406(new ItemStackParticle(ParticleTypes.field_11218, itemStack), vec3d2.x, vec3d2.y, vec3d2.z, vec3d.x, vec3d.y + 0.05, vec3d.z);
		}
	}
	...
	public boolean method_6039() {
		if (this.method_6115() && !this.activeItemStack.isEmpty()) {
			...
			if (item.getUseAction(this.activeItemStack) != UseAction.field_8949) {
				...
				return item.getMaxUseTime(this.activeItemStack) - this.field_6222 >= 5;
			}
		} else {
			return false;
		}
		...
		if (!bl2) {
			...
			if (bl) {
				...
				for(int k = 0; k < 128; ++k) {
					...
					double p = MathHelper.lerp(l, g, this.x) + (random.nextDouble() - 0.5) * (double)this.width * 2.0;
					double q = MathHelper.lerp(l, h, this.y) + random.nextDouble() * (double)this.height;
					double r = MathHelper.lerp(l, i, this.z) + (random.nextDouble() - 0.5) * (double)this.width * 2.0;
