...
public class PlayerInventory implements Inventory, Nameable {
	...
	public int method_7369(Predicate<ItemStack> predicate, int i) {
		...
		if (!this.field_7549.isEmpty() && predicate.test(this.field_7549)) {
			int k = i <= 0 ? this.field_7549.getAmount() : Math.min(i - j, this.field_7549.getAmount());
			j += k;
			if (i != 0) {
				this.field_7549.subtractAmount(k);
	...
	public ListTag method_7384(ListTag listTag) {
		...
		for(int i = 0; i < this.field_7548.size(); ++i) {
			if (!this.field_7548.get(i).isEmpty()) {
				CompoundTag compoundTag = new CompoundTag();
				compoundTag.putByte("Slot", (byte)(i + 100));
				this.field_7548.get(i).method_7953(compoundTag);
				listTag.add(compoundTag);
			}
		}

		for(int i = 0; i < this.field_7544.size(); ++i) {
			if (!this.field_7544.get(i).isEmpty()) {
				CompoundTag compoundTag = new CompoundTag();
				compoundTag.putByte("Slot", (byte)(i + 150));
				this.field_7544.get(i).method_7953(compoundTag);
				listTag.add(compoundTag);
			}
		}
	...
	public boolean isInvEmpty() {
		...
		for(ItemStack itemStack : this.field_7548) {
			if (!itemStack.isEmpty()) {
				return false;
			}
		}

		for(ItemStack itemStack : this.field_7544) {
			if (!itemStack.isEmpty()) {
				return false;
			}
		}
	...
	public boolean method_5443(PlayerEntity playerEntity) {
		if (this.field_7546.invalid) {
			return false;
		} else {
			return !(playerEntity.squaredDistanceTo(this.field_7546) > 64.0);
		}
	}
