...
public abstract class PlayerEntity extends LivingEntity {
	...
	protected float field_7509 = 0.02F;
	...
	private void method_7300() {
		...
		if (blockState != null) {
			if (!this.field_7517 && !this.field_5953 && blockState.getBlock() == Blocks.field_10422 && !this.isSpectator()) {
				boolean bl = blockState.get(BubbleColumnBlock.field_10680);
				if (bl) {
	...
	public void updateMovement() {
		...
		this.field_6281 = this.field_7509;
		if (this.isSprinting()) {
			this.field_6281 = (float)((double)this.field_6281 + (double)this.field_7509 * 0.3);
		}
		...
		this.field_6223 += (g - this.field_6223) * 0.8F;
	...
	protected SoundEvent getHurtSound(DamageSource damageSource) {
		if (damageSource == DamageSource.ON_FIRE) {
			...
			return damageSource == DamageSource.DROWN ? SoundEvents.field_15205 : SoundEvents.field_15115;
		}
	}
	...
	public ItemEntity dropItem(ItemStack itemStack, boolean bl, boolean bl2) {
		if (itemStack.isEmpty()) {
			...
			if (bl) {
				...
				itemEntity.velocityX += Math.cos((double)g) * (double)f;
	...
	protected void method_6056(float f) {
		if (f >= 3.0F && this.activeItemStack.getItem() == Items.field_8255) {
			...
			if (this.activeItemStack.isEmpty()) {
				...
				this.playSoundAtEntity(SoundEvents.field_15239, 0.8F, 0.8F + this.world.random.nextFloat() * 0.4F);
			}
		}
	}
						...
						if (bl4) {
							...
							for(LivingEntity livingEntity : this.world.getVisibleEntities(LivingEntity.class, entity.getBoundingBox().expand(1.0, 0.25, 1.0))) {
								if (livingEntity != this
									...
									&& (!(livingEntity instanceof ArmorStandEntity) || !((ArmorStandEntity)livingEntity).method_6912())
						...
						if (entity instanceof LivingEntity) {
							...
							if (this.world instanceof ServerWorld && o > 2.0F) {
								...
								((ServerWorld)this.world)
									.method_14199(ParticleTypes.field_11209, entity.x, entity.y + (double)(entity.height * 0.5F), entity.z, p, 0.1, 0.0, 0.1, 0.2);
							}
						}
	...
	public void method_7263() {
		...
		if (this.world instanceof ServerWorld) {
			((ServerWorld)this.world).method_14199(ParticleTypes.field_11227, this.x + d, this.y + (double)this.height * 0.5, this.z + e, 0, d, 0.0, e, 0.0);
		}
	}
	...
	public void method_6091(float f, float g, float h) {
		...
		if (this.isSwimming() && !this.hasVehicle()) {
			...
			if (j <= 0.0 || this.field_6282 || !this.world.getBlockState(new BlockPos(this.x, this.y + 1.0 - 0.1, this.z)).getFluidState().isEmpty()) {
				this.velocityY += (j - this.velocityY) * k;
			}
		}
	...
	public void addExperience(int i) {
		this.addScore(i);
		this.experienceBarProgress += (float)i / (float)this.method_7349();
		...
		while(this.experienceBarProgress >= 1.0F) {
			...
			this.method_7316(1);
			this.experienceBarProgress /= (float)this.method_7349();
		}
	}
	...
	public float getEyeHeight() {
		...
		if (this.isSleeping()) {
			f = 0.2F;
		} else if (this.isSwimming() || this.isFallFlying() || this.height == 0.6F) {
			f = 0.4F;
	...
	public boolean canUnlock(LockContainer lockContainer) {
		if (lockContainer.isEmpty()) {
			return true;
		} else {
			ItemStack itemStack = this.getMainHandStack();
			return !itemStack.isEmpty() && itemStack.hasDisplayName() ? itemStack.getDisplayName().getString().equals(lockContainer.getKey()) : false;
		}
	}
	...
	public static enum ChatVisibility {
		...
		private static final PlayerEntity.ChatVisibility[] field_7534 = (PlayerEntity.ChatVisibility[])Arrays.stream(values())
			...
			.toArray(i -> new PlayerEntity.ChatVisibility[i]);
