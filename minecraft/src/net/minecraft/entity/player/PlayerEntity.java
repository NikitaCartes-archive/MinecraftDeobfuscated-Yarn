...
public abstract class PlayerEntity extends LivingEntity {
	...
	public float method_50659() {
		Entity var2 = this.getTransformedLook().entity();
		if (var2 instanceof LivingEntity livingEntity && livingEntity.canBreatheInWater()) {
	...
	public float getBlockBreakingSpeed(BlockState block) {
		...
		return f * class_8293.field_43563.method_50171();
	}
	...
	protected void dropEquipment(DamageSource source, int lootingMultiplier, boolean allowDrops) {
		...
		if (class_8293.field_43628.method_50116()) {
		...
		if (target.isAttackable()) {
			if (!target.handleAttack(this)) {
				...
				if (f > 0.0F || g > 0.0F) {
					...
					i = (int)((float)i * class_8293.field_43564.method_50171());
					if (bl6) {
						if (i > 0) {
							if (target instanceof LivingEntity) {
								((LivingEntity)target)
	...
	public static Optional<Vec3d> findRespawnPosition(ServerWorld world, BlockPos pos, float angle, boolean forced, boolean alive) {
		...
		if (block instanceof RespawnAnchorBlock && (forced || blockState.get(RespawnAnchorBlock.CHARGES) > 0) && RespawnAnchorBlock.isNether(world)) {
			...
			if (!forced && !alive && optional.isPresent()) {
				world.setBlockState(pos, blockState.with(RespawnAnchorBlock.CHARGES, Integer.valueOf(blockState.get(RespawnAnchorBlock.CHARGES) - 1)), Block.NOTIFY_ALL);
			}
	...
	public boolean isSwimming() {
		...
		if (entity != null && !entity.isPushedByFluids() && this.isTouchingWater()) {
			return true;
		} else {
