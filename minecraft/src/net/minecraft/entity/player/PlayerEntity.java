				...
				if (f > 0.0F || g > 0.0F) {
					...
					if (bl3) {
						...
						this.getWorld()
							...
							.forEach(
								livingEntity -> {
									Vec3d vec3dxx = livingEntity.getPos().subtract(target.getPos());
									double dxx = (2.5 - vec3dxx.length()) * 0.6F * (1.0 - livingEntity.getAttributeValue(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE));
									Vec3d vec3d2 = vec3dxx.normalize().multiply(dxx);
									if (dxx > 0.0) {
