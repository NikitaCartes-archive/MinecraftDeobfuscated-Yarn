...
public record EntityDamageS2CPacket(int entityId, RegistryEntry<DamageType> sourceType, int sourceCauseId, int sourceDirectId, Optional<Vec3d> sourcePosition)
	...
	private static final PacketCodec<RegistryByteBuf, RegistryEntry<DamageType>> DAMAGE_TYPE_PACKET_CODEC = PacketCodecs.registryEntry(RegistryKeys.DAMAGE_TYPE);
	...
	private EntityDamageS2CPacket(RegistryByteBuf buf) {
		this(
			buf.readVarInt(),
			DAMAGE_TYPE_PACKET_CODEC.decode(buf),
	...
	private void write(RegistryByteBuf buf) {
		...
		DAMAGE_TYPE_PACKET_CODEC.encode(buf, this.sourceType);
