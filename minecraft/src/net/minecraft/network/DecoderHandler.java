	...
	protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list) throws Exception {
		...
		if (i != 0) {
			...
			Packet<?> packet = ((NetworkState)channelHandlerContext.channel().attr(ClientConnection.PROTOCOL_ATTRIBUTE_KEY).get())
				.getPacketHandler(this.side, k, packetByteBuf);
			...
			if (packet == null) {
				...
				if (packetReceivedEvent.isEnabled() && packetReceivedEvent.shouldCommit()) {
					packetReceivedEvent.packetName = ((NetworkState)channelHandlerContext.channel().attr(ClientConnection.PROTOCOL_ATTRIBUTE_KEY).get()).getId()
						+ "/"
						+ k
						+ " ("
