...
public enum NetworkState implements PacketBundleHandler.BundlerGetter {
	HANDSHAKING(
		...
		createPacketHandlerInitializer()
			.setup(NetworkSide.SERVERBOUND, new NetworkState.PacketHandler().register(HandshakeC2SPacket.class, HandshakeC2SPacket::new))
	),
	PLAY(
		0,
		createPacketHandlerInitializer()
	...
	static class PacketHandler<T extends PacketListener> {
		...
		public Packet<?> createPacket(int id, PacketByteBuf buf) {
			Function<PacketByteBuf, ? extends Packet<T>> function = (Function)this.packetFactories.get(id);
