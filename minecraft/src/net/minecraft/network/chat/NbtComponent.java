...
public abstract class NbtComponent extends BaseComponent implements ComponentWithSelectors {
	...
	public Component resolve(@Nullable ServerCommandSource serverCommandSource, @Nullable Entity entity) throws CommandSyntaxException {
		if (serverCommandSource != null && this.parsedPath != null) {
			...
			return (Component)(this.componentJson
				? (Component)stream.flatMap(string -> {
					try {
						...
						LOGGER.warn("Failed to parse component: " + string, var4);
						return Stream.of();
					}
	...
	public static class BlockPosArgument extends NbtComponent {
		...
		public Component copyShallow() {
			return new NbtComponent.BlockPosArgument(this.path, this.parsedPath, this.componentJson, this.pos, this.parsedPos);
		}
		...
		public boolean equals(Object object) {
			if (this == object) {
				return true;
			} else if (!(object instanceof NbtComponent.BlockPosArgument)) {
				return false;
			} else {
				NbtComponent.BlockPosArgument blockPosArgument = (NbtComponent.BlockPosArgument)object;
	...
	public static class EntityNbtComponent extends NbtComponent {
		...
		@Override
		public Component copyShallow() {
			return new NbtComponent.EntityNbtComponent(this.path, this.parsedPath, this.componentJson, this.selector, this.parsedSelector);
		}
