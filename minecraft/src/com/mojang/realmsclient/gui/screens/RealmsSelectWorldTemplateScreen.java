...
public class RealmsSelectWorldTemplateScreen extends RealmsScreen {
	...
	public void init() {
		...
		ButtonWidget buttonWidget = new ButtonWidget(
			this.width / 2 + 6, this.height - 32, 100, 20, I18n.translate(string), buttonWidgetx -> this.backButtonClicked()
		);
		...
		(new Thread("realms-template-fetcher") {
				public void run() {
					...
					for(RealmsClient realmsClient = RealmsClient.createRealmsClient();
						...
						worldTemplatePaginatedListx = (WorldTemplatePaginatedList)RealmsSelectWorldTemplateScreen.this.client
							.submit(
								() -> {
	...
	public void render(int mouseX, int mouseY, float delta) {
		...
		if (this.displayWarning) {
			...
			for(int i = 0; i < strings.length; ++i) {
				...
				this.drawCenteredString(this.textRenderer, string, this.width / 2, row(-1 + i), k);
			}
		}
		...
		private void renderWorldTemplateItem(WorldTemplate worldTemplate, int x, int y, int mouseX, int mouseY) {
			...
			RealmsSelectWorldTemplateScreen.this.textRenderer
				.draw(
					...
					(float)(i + 227 - RealmsSelectWorldTemplateScreen.this.textRenderer.getStringWidth(worldTemplate.version)),
					(float)(y + 1),
					7105644
				);
