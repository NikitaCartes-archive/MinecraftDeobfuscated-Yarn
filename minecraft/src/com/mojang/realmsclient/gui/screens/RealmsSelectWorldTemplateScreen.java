...
public class RealmsSelectWorldTemplateScreen extends RealmsScreen {
	...
	public void init() {
		...
		this.selectButton = this.addButton(
			new ButtonWidget(
				this.width / 2 - 100, this.height - 32, 100, 20, new TranslatableText("mco.template.button.select"), buttonWidgetx -> this.selectTemplate()
			)
		);
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		if (this.displayWarning) {
			...
			for(int i = 0; i < texts.length; ++i) {
				...
				this.drawStringWithShadow(matrices, this.textRenderer, text, this.width / 2, row(-1 + i), k);
			}
		}
