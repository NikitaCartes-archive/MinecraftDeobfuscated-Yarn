...
public class RealmsBrokenWorldScreen extends RealmsScreen {
	...
	public void addButtons() {
		for(Entry<Integer, RealmsWorldOptions> entry : this.serverData.slots.entrySet()) {
			...
			boolean bl = entry.getKey() != this.serverData.activeSlot || this.serverData.worldType.equals(RealmsServer.WorldType.MINIGAME);
	...
	private void fetchServerData(long worldId) {
		(new Thread() {
			public void run() {
				...
				try {
					RealmsBrokenWorldScreen.this.serverData = realmsClient.getOwnWorld(worldId);
