...
public class RealmsMainScreen extends RealmsScreen {
	...
	public void addButtons() {
		...
		this.playButton = this.addButton(new ButtonWidget(this.width / 2 - 93, this.height - 32, 90, 20, I18n.translate("mco.selectServer.play"), buttonWidget -> {
			RealmsServer realmsServerxx = this.findServer(this.selectedServerId);
			if (realmsServerxx != null) {
				this.play(realmsServerxx, this);
			}
		}));
		...
		if (hasParentalConsent()) {
			...
			if (realmsDataFetcher.isFetchedSinceLastTry(RealmsDataFetcher.Task.SERVER_LIST)) {
				...
				if (list != null) {
					...
					for(RealmsServer realmsServer : this.realmsServers) {
						this.realmSelectionList.addEntry(new RealmsMainScreen.RealmSelectionListEntry(realmsServer));
					}
	...
	private void drawInvitationPendingIcon(int xm, int ym, int x, int y, boolean selectedOrHovered, boolean active) {
		...
		if (bl4) {
			...
			DrawableHelper.drawTexture(x + 4, y + 4 + l, (float)k, h, 8, 8, 48, 16);
		}
	...
	public static void method_23765(ResourceManager resourceManager) {
		...
		field_21517 = (List)collection.stream().filter(identifier -> identifier.getNamespace().equals("realms")).collect(ImmutableList.toImmutableList());
	}
	...
	class CloseButton extends ButtonWidget {
		...
		public void renderButton(int mouseX, int mouseY, float delta) {
			...
			drawTexture(this.x, this.y, 0.0F, f, 12, 12, 12, 24);
		...
		private void renderMcoServerItem(RealmsServer serverData, int x, int y, int mouseX, int mouseY) {
			if (serverData.state == RealmsServer.State.UNINITIALIZED) {
				...
				if (RealmsMainScreen.this.isSelfOwnedServer(serverData) && serverData.expired) {
					...
					int o = bl ? 2 : 1;
					DrawableHelper.drawTexture(m, n, 0.0F, (float)(46 + o * 20), k / 2, 8, 256, 256);
					DrawableHelper.drawTexture(m + k / 2, n, (float)(200 - k / 2), (float)(46 + o * 20), k / 2, 8, 256, 256);
					DrawableHelper.drawTexture(m, n + 8, 0.0F, (float)(46 + o * 20 + 12), k / 2, 8, 256, 256);
					DrawableHelper.drawTexture(m + k / 2, n + 8, (float)(200 - k / 2), (float)(46 + o * 20 + 12), k / 2, 8, 256, 256);
					RenderSystem.disableBlend();
					int p = y + 11 + 5;
					...
					if (serverData.worldType == RealmsServer.WorldType.MINIGAME) {
						int r = 13413468;
						String string2 = I18n.translate("mco.selectServer.minigame") + " ";
						int k = RealmsMainScreen.this.textRenderer.getStringWidth(string2);
						RealmsMainScreen.this.textRenderer.draw(string2, (float)(x + 2), (float)(y + 12), 13413468);
