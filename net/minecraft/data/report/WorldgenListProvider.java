...
public class WorldgenListProvider
implements DataProvider {
    ...
    public void run(DataCache cache) {
        ...
        WorldgenListProvider.writeRegistryEntries(path, cache, dynamicOps, Registry.DIMENSION_KEY, simpleRegistry2, DimensionOptions.CODEC);
    }
    ...
    private static <T> void writeRegistryEntries(DataCache cache, Path path, DynamicRegistryManager registryManager, DynamicOps<JsonElement> json, DynamicRegistryManager.Info<T> info) {
        WorldgenListProvider.writeRegistryEntries(path, cache, json, info.registry(), registryManager.getMutable(info.registry()), info.entryCodec());
    }
    ...
    private static <E, T extends Registry<E>> void writeRegistryEntries(Path path, DataCache cache, DynamicOps<JsonElement> json, RegistryKey<? extends T> registryKey, T registry, Encoder<E> encoder) {
        for (Map.Entry<RegistryKey<E>, E> entry : registry.getEntries()) {
            Path path2 = WorldgenListProvider.getPath(path, registryKey.getValue(), entry.getKey().getValue());
            WorldgenListProvider.writeToPath(path2, cache, json, encoder, entry.getValue());
        }
    }
    ...
    private static Path getPath(Path root, Identifier rootId, Identifier id) {
        return WorldgenListProvider.getRootPath(root).resolve(id.getNamespace()).resolve(rootId.getPath()).resolve(id.getPath() + ".json");
    }
