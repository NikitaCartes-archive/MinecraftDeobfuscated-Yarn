...
extends AbstractTagProvider<GameEvent> {
    ...
    protected void configure() {
        this.getOrCreateTagBuilder(GameEventTags.VIBRATIONS).add((GameEvent[])BASIC_GAME_EVENTS).add(GameEvent.FLAP);
        this.getOrCreateTagBuilder(GameEventTags.SHRIEKER_CAN_LISTEN).add(GameEvent.SCULK_SENSOR_TENDRILS_CLICKING);
        this.getOrCreateTagBuilder(GameEventTags.WARDEN_CAN_LISTEN).add((GameEvent[])BASIC_GAME_EVENTS).add(GameEvent.SHRIEK).addTag(GameEventTags.SHRIEKER_CAN_LISTEN);
        this.getOrCreateTagBuilder(GameEventTags.IGNORE_VIBRATIONS_SNEAKING).add((GameEvent[])new GameEvent[]{GameEvent.HIT_GROUND, GameEvent.PROJECTILE_SHOOT, GameEvent.STEP, GameEvent.SWIM, GameEvent.ITEM_INTERACT_START, GameEvent.ITEM_INTERACT_FINISH});
