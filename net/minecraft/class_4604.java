...
public class class_4604 {
    ...
    public class_4604(Matrix4f matrix4f, Matrix4f matrix4f2) {
    ...
    private boolean method_23090(float f, float g, float h, float i, float j, float k) {
        for (int l = 0; l < 6; ++l) {
            ...
            if (vector4f.method_23217(new Vector4f(f, g, h, 1.0f)) > 0.0f) continue;
            if (vector4f.method_23217(new Vector4f(i, g, h, 1.0f)) > 0.0f) continue;
            if (vector4f.method_23217(new Vector4f(f, j, h, 1.0f)) > 0.0f) continue;
            if (vector4f.method_23217(new Vector4f(i, j, h, 1.0f)) > 0.0f) continue;
            if (vector4f.method_23217(new Vector4f(f, g, k, 1.0f)) > 0.0f) continue;
            if (vector4f.method_23217(new Vector4f(i, g, k, 1.0f)) > 0.0f) continue;
            if (vector4f.method_23217(new Vector4f(f, j, k, 1.0f)) > 0.0f) continue;
            if (vector4f.method_23217(new Vector4f(i, j, k, 1.0f)) > 0.0f) continue;
            return false;
        }
        return true;
    }
