...
extends HostileEntity {
    ...
    public boolean damage(DamageSource damageSource, float f) {
        ...
        if (damageSource instanceof ProjectileDamageSource || damageSource == DamageSource.FIREWORKS) {
            for (int i = 0; i < 64; ++i) {
    ...
    extends Goal {
        ...
        private boolean method_7033(ViewableWorld viewableWorld, BlockPos blockPos, BlockState blockState, BlockState blockState2, BlockState blockState3, BlockPos blockPos2) {
            return blockState2.isAir() && !blockState3.isAir() && Block.isShapeFullCube(blockState3.getCollisionShape(viewableWorld, blockPos2)) && blockState.canPlaceAt(viewableWorld, blockPos);
        }
    }
    ...
    extends FollowTargetGoal<PlayerEntity> {
        ...
        public TeleportTowardsPlayerGoal(EndermanEntity endermanEntity) {
            ...
            this.enderman = endermanEntity;
            this.staringPlayerPredicate = new TargetPredicate().setBaseMaxDistance(this.getFollowRange()).setPredicate(livingEntity -> endermanEntity.isPlayerStaring((PlayerEntity)livingEntity));
        }
