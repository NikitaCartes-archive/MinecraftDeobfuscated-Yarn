...
extends HostileEntity {
        ...
        if (this.isAlive()) {
            if (this.world.isClient) {
                ...
                if (this.areSpikesRetracted() && this.isInsideWater()) {
                    ...
                    for (int i = 0; i < 2; ++i) {
                        this.world.addParticle(ParticleTypes.BUBBLE, this.method_23322(0.5) - vec3d.x * 1.5, this.method_23319() - vec3d.y * 1.5, this.method_23325(0.5) - vec3d.z * 1.5, 0.0, 0.0, 0.0);
                    }
                }
                if (this.hasBeamTarget()) {
                    ...
                    if ((livingEntity = this.getBeamTarget()) != null) {
                        ...
                        while (j < h) {
                            this.world.addParticle(ParticleTypes.BUBBLE, this.getX() + e * (j += 1.8 - d + this.random.nextDouble() * (1.7 - d)), this.method_23320() + f * j, this.getZ() + g * j, 0.0, 0.0, 0.0);
                        }
                    }
                }
            }
    ...
    public static boolean canSpawn(EntityType<? extends GuardianEntity> entityType, IWorld iWorld, SpawnType spawnType, BlockPos blockPos, Random random) {
        return !(random.nextInt(20) != 0 && iWorld.isSkyVisibleAllowingSea(blockPos) || iWorld.getDifficulty() == Difficulty.PEACEFUL || spawnType != SpawnType.SPAWNER && !iWorld.getFluidState(blockPos).matches(FluidTags.WATER));
    }
