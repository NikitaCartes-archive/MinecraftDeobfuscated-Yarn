...
public class BlockSenses {
    public static Optional<BlockPos> findBlock(BlockPos center, int horizontalRadius, int verticalRadius, Predicate<BlockPos> predicate) {
        ...
        for (int j = 1; j <= i; ++j) {
            for (int k = -j; k <= j; ++k) {
                if (k > horizontalRadius || k < -horizontalRadius) continue;
                boolean bl = k == -j || k == j;
                for (int l = -j; l <= j; ++l) {
                    if (l > verticalRadius || l < -verticalRadius) continue;
                    boolean bl2 = l == -j || l == j;
                    for (int m = -j; m <= j; ++m) {
                        boolean bl3;
                        if (m > horizontalRadius || m < -horizontalRadius) continue;
                        boolean bl4 = bl3 = m == -j || m == j;
                        if (!bl && !bl2 && !bl3 || !predicate.test(mutable.set(center).setOffset(k, l, m))) continue;
