...
extends Task<VillagerEntity> {
    ...
    protected void method_20645(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
        boolean bl;
        boolean bl2 = bl = PanicTask.wasHurt(villagerEntity) || PanicTask.isHostileNearby(villagerEntity) || StopPanicingTask.wasHurtByNearbyEntity(villagerEntity);
        if (!bl) {
    ...
    private static boolean wasHurtByNearbyEntity(VillagerEntity villagerEntity) {
        return villagerEntity.getBrain().getOptionalMemory(MemoryModuleType.HURT_BY_ENTITY).filter(livingEntity -> livingEntity.squaredDistanceTo(villagerEntity) <= 36.0).isPresent();
    }
}
