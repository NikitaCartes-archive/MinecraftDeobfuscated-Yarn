...
extends Task<MobEntity> {
    ...
    protected Set<Pair<MemoryModuleType<?>, MemoryModuleState>> getRequiredMemoryState() {
        return ImmutableSet.of(Pair.of(MemoryModuleType.PATH, MemoryModuleState.VALUE_ABSENT), Pair.of(MemoryModuleType.WALK_TARGET, MemoryModuleState.VALUE_PRESENT));
    }

    protected boolean method_18978(ServerWorld serverWorld, MobEntity mobEntity) {
        Brain<WalkTarget> brain = mobEntity.getBrain();
    ...
    protected void method_18983(ServerWorld serverWorld, MobEntity mobEntity, long l) {
        ...
        if (this.field_18369 != path) {
            ...
            mobEntity.getBrain().putMemory(MemoryModuleType.PATH, path);
        }
        if (path == null || this.field_18370 == null) {
            return;
        }
        WalkTarget walkTarget = mobEntity.getBrain().getOptionalMemory(MemoryModuleType.WALK_TARGET).get();
