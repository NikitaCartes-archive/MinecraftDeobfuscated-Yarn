...
public class LookTargetUtil {
    public static void lookAtAndWalkTowardsEachOther(LivingEntity livingEntity, LivingEntity livingEntity2) {
        LookTargetUtil.lookAtEachOther(livingEntity, livingEntity2);
        LookTargetUtil.walkTowardsEachOther(livingEntity, livingEntity2);
    }
    ...
    public static boolean canSee(Brain<?> brain, MemoryModuleType<? extends LivingEntity> memoryModuleType, EntityType<?> entityType) {
        return brain.getOptionalMemory(memoryModuleType).filter(livingEntity -> livingEntity.getType() == entityType).filter(LivingEntity::isAlive).filter(livingEntity -> LookTargetUtil.canSee(brain, livingEntity)).isPresent();
    }

    public static void lookAtEachOther(LivingEntity livingEntity, LivingEntity livingEntity2) {
        LookTargetUtil.lookAt(livingEntity, livingEntity2);
        LookTargetUtil.lookAt(livingEntity2, livingEntity);
    }
    ...
    public static void walkTowardsEachOther(LivingEntity livingEntity, LivingEntity livingEntity2) {
        int i = 2;
        LookTargetUtil.walkTowards(livingEntity, livingEntity2, 2);
        LookTargetUtil.walkTowards(livingEntity2, livingEntity, 2);
    }
    ...
    public static void give(LivingEntity livingEntity, ItemStack itemStack, LivingEntity livingEntity2) {
        double d = livingEntity.y - (double)0.3f + (double)livingEntity.getStandingEyeHeight();
    ...
    public static ChunkSectionPos getPosClosestToOccupiedPointOfInterest(ServerWorld serverWorld, ChunkSectionPos chunkSectionPos2, int i) {
        int j = serverWorld.getOccupiedPointOfInterestDistance(chunkSectionPos2);
        return ChunkSectionPos.stream(chunkSectionPos2, i).filter(chunkSectionPos -> serverWorld.getOccupiedPointOfInterestDistance((ChunkSectionPos)chunkSectionPos) < j).min(Comparator.comparingInt(serverWorld::getOccupiedPointOfInterestDistance)).orElse(chunkSectionPos2);
    }
}
