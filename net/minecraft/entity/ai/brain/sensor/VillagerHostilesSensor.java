...
extends NearestHostileSensor {
    ...
    protected Optional<LivingEntity> getNearestHostile(LivingEntity entity) {
        return this.getVisibleMobs(entity).flatMap(list -> list.stream().filter(this::isHostile).filter(livingEntity2 -> this.isCloseEnoughForDanger(entity, (LivingEntity)livingEntity2)).min(Comparator.comparingDouble(entity::squaredDistanceTo)));
    }
