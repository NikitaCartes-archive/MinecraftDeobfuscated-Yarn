    ...
    public void tick() {
        ...
        if (this.hasReached() && blockPos2 != null) {
            ...
            if (this.counter > 0) {
                ...
                if (!world.isClient) {
                    d = 0.08;
                    ((ServerWorld)world).spawnParticles(new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(Items.EGG)), (double)blockPos2.getX() + 0.5, (double)blockPos2.getY() + 0.7, (double)blockPos2.getZ() + 0.5, 3, ((double)abstractRandom.nextFloat() - 0.5) * 0.08, ((double)abstractRandom.nextFloat() - 0.5) * 0.08, ((double)abstractRandom.nextFloat() - 0.5) * 0.08, 0.15f);
                }
            }
            if (this.counter % 2 == 0) {
                vec3d = this.stepAndDestroyMob.getVelocity();
            ...
            if (this.counter > 60) {
                ...
                if (!world.isClient) {
                    for (int i = 0; i < 20; ++i) {
                        d = abstractRandom.nextGaussian() * 0.02;
