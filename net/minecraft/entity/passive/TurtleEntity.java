...
extends AnimalEntity {
    ...
    public static boolean canSpawn(EntityType<TurtleEntity> entityType, IWorld iWorld, SpawnType spawnType, BlockPos blockPos, Random random) {
        return blockPos.getY() < iWorld.getSeaLevel() + 4 && iWorld.getBlockState(blockPos.method_10074()).getBlock() == Blocks.SAND && iWorld.getBaseLightLevel(blockPos, 0) > 8;
    }
    ...
    protected SoundEvent getAmbientSound() {
        if (!this.isInsideWater() && this.onGround && !this.isBaby()) {
    ...
    public void tickMovement() {
        BlockPos blockPos;
        super.tickMovement();
        if (this.isAlive() && this.isDiggingSand() && this.sandDiggingCounter >= 1 && this.sandDiggingCounter % 5 == 0 && this.world.getBlockState((blockPos = new BlockPos(this)).method_10074()).getBlock() == Blocks.SAND) {
    ...
    extends MoveToTargetPosGoal {
        ...
        public boolean canStart() {
            ...
            if (!(this.turtle.isLandBound() || this.turtle.isInsideWater() || this.turtle.hasEgg())) {
                return super.canStart();
            }
            return false;
        }
    ...
    extends WanderAroundGoal {
        ...
        @Override
        public boolean canStart() {
            if (!(this.mob.isInsideWater() || this.turtle.isLandBound() || this.turtle.hasEgg())) {
                return super.canStart();
            }
            return false;
        }
    ...
    extends Goal {
        ...
        ApproachFoodHoldingPlayerGoal(TurtleEntity turtleEntity, double d, Item item) {
            this.turtle = turtleEntity;
            this.speed = d;
            this.attractiveItems = Sets.newHashSet(item);
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            if (this.turtle.getNavigation().isIdle()) {
                Vec3d vec3d = TargetFinder.findTargetTowards(this.turtle, 16, 3, new Vec3d(blockPos.getX(), blockPos.getY(), blockPos.getZ()), 0.3141592741012573);
                if (vec3d == null) {
                    vec3d = TargetFinder.findTargetTowards(this.turtle, 8, 7, new Vec3d(blockPos.getX(), blockPos.getY(), blockPos.getZ()));
                }
                if (vec3d != null && !bl && this.turtle.world.getBlockState(new BlockPos(vec3d)).getBlock() != Blocks.WATER) {
                    vec3d = TargetFinder.findTargetTowards(this.turtle, 16, 5, new Vec3d(blockPos.getX(), blockPos.getY(), blockPos.getZ()));
                }
                if (vec3d == null) {
                    this.noPath = true;
                    return;
