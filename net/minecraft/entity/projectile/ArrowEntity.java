...
extends ProjectileEntity {
    ...
    public void initFromStack(ItemStack itemStack) {
        if (itemStack.getItem() == Items.TIPPED_ARROW) {
            ...
            List<StatusEffectInstance> collection = PotionUtil.getCustomPotionEffects(itemStack);
            if (!collection.isEmpty()) {
                for (StatusEffectInstance statusEffectInstance : collection) {
                    this.effects.add(new StatusEffectInstance(statusEffectInstance));
                }
            }
            if ((i = ArrowEntity.getCustomPotionColor(itemStack)) == -1) {
                this.initColor();
            } else {
                this.setColor(i);
            }
    ...
    protected void onHit(LivingEntity livingEntity) {
        ...
        for (StatusEffectInstance statusEffectInstance : this.potion.getEffects()) {
            livingEntity.addPotionEffect(new StatusEffectInstance(statusEffectInstance.getEffectType(), Math.max(statusEffectInstance.getDuration() / 8, 1), statusEffectInstance.getAmplifier(), statusEffectInstance.isAmbient(), statusEffectInstance.shouldShowParticles()));
        }
        if (!this.effects.isEmpty()) {
            for (StatusEffectInstance statusEffectInstance : this.effects) {
