...
extends LivingEntity {
    ...
    private void method_7313() {
        ...
        double g = 10.0;
        if (d > 10.0) {
            this.field_7524 = this.field_7500 = this.getX();
        }
        if (f > 10.0) {
            this.field_7522 = this.field_7499 = this.getZ();
        }
        if (e > 10.0) {
            this.field_7502 = this.field_7521 = this.getY();
        }
        if (d < -10.0) {
            this.field_7524 = this.field_7500 = this.getX();
        }
    ...
    public void attack(Entity entity) {
        ...
        float g = entity instanceof LivingEntity ? EnchantmentHelper.getAttackDamage(this.getMainHandStack(), ((LivingEntity)entity).getGroup()) : EnchantmentHelper.getAttackDamage(this.getMainHandStack(), EntityGroup.DEFAULT);
        ...
        if ((f *= 0.2f + h * h * 0.8f) > 0.0f || g > 0.0f) {
            ...
            if (bl6) {
                if (i > 0) {
                    if (entity instanceof LivingEntity) {
                        ((LivingEntity)entity).takeKnockback(this, (float)i * 0.5f, MathHelper.sin(this.yaw * ((float)Math.PI / 180)), -MathHelper.cos(this.yaw * ((float)Math.PI / 180)));
                    } else {
                        entity.addVelocity(-MathHelper.sin(this.yaw * ((float)Math.PI / 180)) * (float)i * 0.5f, 0.1, MathHelper.cos(this.yaw * ((float)Math.PI / 180)) * (float)i * 0.5f);
                    }
    ...
    public boolean method_23668() {
        ItemStack itemStack;
        if (!this.onGround && !this.isFallFlying() && !this.isInsideWater() && (itemStack = this.getEquippedStack(EquipmentSlot.CHEST)).getItem() == Items.ELYTRA && ElytraItem.isUsable(itemStack)) {
            this.method_23669();
            return true;
        }
        return false;
    ...
    public boolean equip(int i, ItemStack itemStack) {
        ...
        if (equipmentSlot != null) {
            if (!itemStack.isEmpty() && (itemStack.getItem() instanceof ArmorItem || itemStack.getItem() instanceof ElytraItem ? MobEntity.getPreferredEquipmentSlot(itemStack) != equipmentSlot : equipmentSlot != EquipmentSlot.HEAD)) {
                return false;
            }
