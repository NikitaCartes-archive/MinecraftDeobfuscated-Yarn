...
extends LivingEntity {
    ...
    public void attack(Entity target) {
        ...
        if ((f *= 0.2f + h * h * 0.8f) > 0.0f || g > 0.0f) {
            ...
            boolean bl3 = bl && this.fallDistance > 0.0f && !this.onGround && !this.isClimbing() && !this.isInsideWater() && !this.hasStatusEffect(StatusEffects.BLINDNESS) && !this.hasVehicle() && target instanceof LivingEntity;
    ...
    private boolean isWithinSleepingRange(BlockPos sleepPos, Direction direction) {
        if (Math.abs(this.getX() - (double)sleepPos.getX()) <= 3.0 && Math.abs(this.getY() - (double)sleepPos.getY()) <= 2.0 && Math.abs(this.getZ() - (double)sleepPos.getZ()) <= 3.0) {
            return true;
        }
        BlockPos blockPos = sleepPos.offset(direction.getOpposite());
        return Math.abs(this.getX() - (double)blockPos.getX()) <= 3.0 && Math.abs(this.getY() - (double)blockPos.getY()) <= 2.0 && Math.abs(this.getZ() - (double)blockPos.getZ()) <= 3.0;
    }
