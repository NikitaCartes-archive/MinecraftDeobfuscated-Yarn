    ...
    public void grow(ServerWorld serverWorld, Random random, BlockPos blockPos, BlockState blockState) {
        ...
        block0: for (int i = 0; i < 128; ++i) {
            ...
            for (int j = 0; j < i / 16; ++j) {
                if (serverWorld.getBlockState((blockPos3 = blockPos3.add(random.nextInt(3) - 1, (random.nextInt(3) - 1) * random.nextInt(3) / 2, random.nextInt(3) - 1)).down()).getBlock() != this || serverWorld.getBlockState(blockPos3).isFullCube(serverWorld, blockPos3)) continue block0;
            }
