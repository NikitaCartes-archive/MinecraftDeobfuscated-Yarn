...
public enum SideShapeType {
    ...
    CENTER{
        ...
        private static final List<TagKey<Block>> HANGING_SIGNS_TAGS = Lists.newArrayList(BlockTags.CEILING_HANGING_SIGNS, BlockTags.WALL_HANGING_SIGNS);

        @Override
        public boolean matches(BlockState state, BlockView world, BlockPos pos, Direction direction) {
            for (TagKey<Block> tagKey : HANGING_SIGNS_TAGS) {
                if (!state.isIn(tagKey)) continue;
                return true;
            }
