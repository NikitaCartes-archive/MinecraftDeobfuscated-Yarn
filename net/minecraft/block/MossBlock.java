...
implements Fertilizable {
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        MossBlock.placePlantsAround(world, random, pos.up());
    }
    ...
    public static boolean placePlantsAround(StructureWorldAccess world, Random random, BlockPos pos) {
        ...
        for (int l = -j; l <= j; ++l) {
            for (int m = -k; m <= k; ++m) {
                ...
                i += MossBlock.placePlant(world, random, blockPos);
            }
        }
        return i > 0;
    }
    ...
    private static int placePlant(StructureWorldAccess world, Random random, BlockPos pos) {
        ...
        if (world.isAir(pos) && blockState.isSideSolidFullSquare(world, blockPos, Direction.UP)) {
            ...
            MossBlock.method_33631(world, random, pos.down());
            if (random.nextFloat() < 0.8f && (blockState2 = MossBlock.getPlant(random)).canPlaceAt(world, pos)) {
