...
extends SaplingGenerator {
    ...
    @Nullable
    protected abstract ConfiguredFeature<class_4636, ?> createLargeTreeFeature(Random var1);
    ...
    public static boolean canGenerateLargeTree(BlockState blockState, BlockView blockView, BlockPos blockPos, int i, int j) {
        ...
        return block == blockView.getBlockState(blockPos.add(i, 0, j)).getBlock() && block == blockView.getBlockState(blockPos.add(i + 1, 0, j)).getBlock() && block == blockView.getBlockState(blockPos.add(i, 0, j + 1)).getBlock() && block == blockView.getBlockState(blockPos.add(i + 1, 0, j + 1)).getBlock();
    }
}
