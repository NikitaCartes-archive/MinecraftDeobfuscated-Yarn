...
implements Fertilizable {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (random.nextInt(5) != 0 || state.get(PERSISTENT).booleanValue() || this.shouldDecay(state)) {
            return;
        }
        BlockPos blockPos = pos.down();
        if (world.getBlockState(blockPos).isAir() && world.getBlockState(blockPos.down()).isAir() && !MangroveLeavesBlock.hasNearbyPropagule(world, blockPos)) {
    ...
    private static boolean hasNearbyPropagule(WorldAccess world, BlockPos pos) {
        ...
        for (BlockPos blockPos : iterable) {
            if (!world.getBlockState(blockPos).isOf(Blocks.MANGROVE_PROPAGULE)) continue;
            return true;
        }
        return false;
    }
