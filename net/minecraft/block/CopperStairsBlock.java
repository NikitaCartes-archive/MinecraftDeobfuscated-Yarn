...
public class CopperStairsBlock
extends StairsBlock
implements Oxidizable {
    ...
    public BlockState getOxidationResult(BlockState state) {
        return (BlockState)((BlockState)((BlockState)((BlockState)this.oxidationResult.getDefaultState().with(FACING, state.get(FACING))).with(HALF, state.get(HALF))).with(SHAPE, state.get(SHAPE))).with(WATERLOGGED, state.get(WATERLOGGED));
    }
}
