...
extends Block {
    ...
    public void onScheduledTick(BlockState blockState, ServerWorld serverWorld, BlockPos blockPos, Random random) {
        ...
        if (bl && ChorusFlowerBlock.isSurroundedByAir(serverWorld, blockPos2, null) && serverWorld.isAir(blockPos.up(2))) {
            ...
            for (int l = 0; l < j; ++l) {
                ...
                if (!serverWorld.isAir(blockPos3) || !serverWorld.isAir(blockPos3.method_10074()) || !ChorusFlowerBlock.isSurroundedByAir(serverWorld, blockPos3, direction.getOpposite())) continue;
    ...
    private static boolean isSurroundedByAir(class_4538 arg, BlockPos blockPos, @Nullable Direction direction) {
        for (Direction direction2 : Direction.Type.HORIZONTAL) {
            if (direction2 == direction || arg.isAir(blockPos.offset(direction2))) continue;
            return false;
        }
        return true;
    }
    ...
    public static void generate(IWorld iWorld, BlockPos blockPos, Random random, int i) {
        ...
        ChorusFlowerBlock.generate(iWorld, blockPos, random, blockPos, i, 0);
    }
    ...
    private static void generate(IWorld iWorld, BlockPos blockPos, Random random, BlockPos blockPos2, int i, int j) {
        ...
        for (int l = 0; l < k; ++l) {
            ...
            if (!ChorusFlowerBlock.isSurroundedByAir(iWorld, blockPos3, null)) {
                return;
            }
        ...
        if (j < 4) {
            ...
            for (int n = 0; n < m; ++n) {
                ...
                if (Math.abs(blockPos4.getX() - blockPos2.getX()) >= i || Math.abs(blockPos4.getZ() - blockPos2.getZ()) >= i || !iWorld.isAir(blockPos4) || !iWorld.isAir(blockPos4.method_10074()) || !ChorusFlowerBlock.isSurroundedByAir(iWorld, blockPos4, direction.getOpposite())) continue;
                bl = true;
                ...
                ChorusFlowerBlock.generate(iWorld, blockPos4, random, blockPos2, i, j + 1);
            }
        }
        if (!bl) {
            iWorld.setBlockState(blockPos.up(k), (BlockState)Blocks.CHORUS_FLOWER.getDefaultState().with(AGE, 5), 2);
        }
    }
