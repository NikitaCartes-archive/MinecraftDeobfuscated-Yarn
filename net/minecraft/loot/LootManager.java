...
extends JsonDataLoader {
    ...
    protected void apply(Map<Identifier, JsonObject> map, ResourceManager resourceManager, Profiler profiler) {
        ...
        immutableMap.forEach((identifier, lootTable) -> LootManager.check(lootTableReporter, identifier, lootTable, immutableMap::get));
    ...
    public static void check(LootTableReporter lootTableReporter, Identifier identifier, LootTable lootTable, Function<Identifier, LootTable> function) {
        ImmutableSet<Identifier> set = ImmutableSet.of(identifier);
        lootTable.check(lootTableReporter.makeChild("{" + identifier.toString() + "}"), function, set, lootTable.getType());
    }
