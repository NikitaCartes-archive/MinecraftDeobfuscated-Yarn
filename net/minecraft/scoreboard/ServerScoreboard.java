...
extends Scoreboard {
    ...
    public void updateScore(ScoreboardPlayerScore scoreboardPlayerScore) {
        ...
        if (this.objectives.contains(scoreboardPlayerScore.getObjective())) {
            this.server.getPlayerManager().sendToAll(new ScoreboardPlayerUpdateS2CPacket(UpdateMode.CHANGE, scoreboardPlayerScore.getObjective().getName(), scoreboardPlayerScore.getPlayerName(), scoreboardPlayerScore.getScore()));
        }
    ...
    public void updatePlayerScore(String string, ScoreboardObjective scoreboardObjective) {
        ...
        if (this.objectives.contains(scoreboardObjective)) {
            this.server.getPlayerManager().sendToAll(new ScoreboardPlayerUpdateS2CPacket(UpdateMode.REMOVE, scoreboardObjective.getName(), string, 0));
        }
        this.runUpdateListeners();
    }
    ...
    public int getSlot(ScoreboardObjective scoreboardObjective) {
        int i = 0;
        for (int j = 0; j < 19; ++j) {
            if (this.getObjectiveForSlot(j) != scoreboardObjective) continue;
            ++i;
        }
        return i;
    }
