...
public class TestUtil {
    ...
    private static void sendMessage(ServerWorld world, Formatting formatting, String message) {
        world.getPlayers(serverPlayerEntity -> true).forEach(serverPlayerEntity -> serverPlayerEntity.sendSystemMessage(new LiteralText(message).formatted(formatting), Util.field_25140));
    }
    ...
    public static void clearTests(ServerWorld world, BlockPos pos, TestManager testManager, int radius) {
        ...
        BlockPos.stream(blockPos2, blockPos22).filter(blockPos -> world.getBlockState((BlockPos)blockPos).isOf(Blocks.STRUCTURE_BLOCK)).forEach(blockPos -> {
            ...
            BlockBox blockBox = StructureTestUtil.createArea(blockPos2, structureBlockBlockEntity.getSize(), 2);
