...
public class TestContext {
    ...
    public TimedTaskRunner method_35967(MobEntity entity, BlockPos pos, float f) {
        return this.createTimedTaskRunner().method_36077(2, () -> {
            ...
            entity.getNavigation().startMovingAlong(path, f);
        });
    }
    ...
    public void pushButton(BlockPos pos) {
        this.checkBlockState(pos, blockState -> blockState.isIn(BlockTags.BUTTONS), () -> "Expected button");
        BlockPos blockPos = this.getAbsolutePos(pos);
        BlockState blockState2 = this.getWorld().getBlockState(blockPos);
        AbstractButtonBlock abstractButtonBlock = (AbstractButtonBlock)blockState2.getBlock();
        abstractButtonBlock.powerOn(blockState2, this.getWorld(), blockPos);
    }
    ...
    public void expectEntityAt(Entity entity, BlockPos pos) {
        ...
        list.stream().filter(entity2 -> entity2 == entity).findFirst().orElseThrow(() -> new PositionedException("Expected " + entity.getType().getUntranslatedName(), blockPos, pos, this.test.getTick()));
    }
    ...
    public void addFinalTaskWithDuration(int duration, Runnable runnable) {
        this.markFinalCause();
        this.test.createTimedTaskRunner().createAndAdd(duration, runnable).method_36075();
    }
    ...
    public void method_36035(Runnable runnable) {
        LongStream.range(this.test.getTick(), this.test.getTicksLeft()).forEach(l -> this.test.runAtTick(l, runnable::run));
    }
