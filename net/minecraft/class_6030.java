...
extends Task<MobEntity> {
    ...
    private final List<class_6031> field_30142 = new ArrayList<class_6031>();
    private Optional<Vec3d> field_30143 = Optional.empty();
    private Optional<class_6031> field_30144 = Optional.empty();
    ...
    protected boolean shouldKeepRunning(ServerWorld serverWorld, MobEntity mobEntity, long l) {
        boolean bl;
        boolean bl2 = bl = this.field_30143.isPresent() && this.field_30143.get().equals(mobEntity.getPos()) && this.field_30145 > 0 && (this.field_30144.isPresent() || !this.field_30142.isEmpty());
    ...
    protected void run(ServerWorld serverWorld, MobEntity mobEntity, long l) {
        ...
        int j = blockPos.getY();
        int k = blockPos.getZ();
        Iterable<BlockPos> iterable = BlockPos.iterate(i - this.field_30140, j - this.field_30139, k - this.field_30140, i + this.field_30140, j + this.field_30139, k + this.field_30140);
        ...
        for (BlockPos blockPos2 : iterable) {
            ...
            optional.ifPresent(vec3d -> this.field_30142.add(new class_6031(new BlockPos(blockPos2), (Vec3d)vec3d, MathHelper.ceil(d))));
        }
    }
    ...
    protected void keepRunning(ServerWorld serverWorld, MobEntity mobEntity, long l) {
        if (this.field_30144.isPresent()) {
            if (l - this.field_30146 >= 40L) {
                ...
                mobEntity.setVelocity(this.field_30144.get().method_35085());
                mobEntity.getBrain().remember(MemoryModuleType.LONG_JUMP_MID_JUMP, true);
            }
        } else {
            --this.field_30145;
            Optional<class_6031> optional = WeightedPicker.getRandom(serverWorld.random, this.field_30142);
            if (optional.isPresent()) {
                this.field_30142.remove(optional.get());
                mobEntity.getBrain().remember(MemoryModuleType.LOOK_TARGET, new BlockPosLookTarget(optional.get().method_35084()));
                EntityNavigation entityNavigation = mobEntity.getNavigation();
                Path path = entityNavigation.method_35141(optional.get().method_35084(), 0, 7);
    ...
    private boolean method_35080(MobEntity mobEntity, Vec3d vec3d, Vec3d vec3d2) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (mobEntity.world.isSpaceEmpty(mobEntity, box)) continue;
            return false;
        }
        return true;
    }
    ...
    public static class class_6031
