...
extends ForwardingDynamicOps<T> {
    ...
    private <E> DataResult<Supplier<E>> readSupplier(RegistryKey<? extends Registry<E>> registryKey, MutableRegistry<E> mutableRegistry, Codec<E> codec, RegistryKey<E> registryKey2) {
        ...
        valueHolder.values.put(registryKey2, DataResult.success(RegistryOps.createLazyEntrySupplier(mutableRegistry, registryKey2)));
        Optional<DataResult<EntryLoader.Entry<E>>> optional = this.entryLoader.load(this.entryOps, registryKey, registryKey2, codec);
        if (optional.isEmpty()) {
            dataResult2 = mutableRegistry.contains(registryKey2) ? DataResult.success(RegistryOps.createEntrySupplier(mutableRegistry, registryKey2), Lifecycle.stable()) : DataResult.error("Missing referenced custom/removed registry entry for registry " + registryKey + " named " + registryKey2.getValue());
        } else {
            ...
            dataResult2 = dataResult3.map(entry -> RegistryOps.createEntrySupplier(mutableRegistry, registryKey2));
        }
    ...
    private static <E> Supplier<E> createLazyEntrySupplier(MutableRegistry<E> registry, RegistryKey<E> key) {
        return Suppliers.memoize(() -> {
            Object object = registry.get(key);
            if (object == null) {
    ...
    private static <E> Supplier<E> createEntrySupplier(final Registry<E> registry, final RegistryKey<E> key) {
