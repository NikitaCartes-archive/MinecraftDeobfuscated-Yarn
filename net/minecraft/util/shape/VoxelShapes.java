...
public final class VoxelShapes {
    ...
    public static VoxelShape cuboid(double d, double e, double f, double g, double h, double i) {
        return VoxelShapes.cuboid(new BoundingBox(d, e, f, g, h, i));
    }

    public static VoxelShape cuboid(BoundingBox boundingBox) {
        int i = VoxelShapes.findRequiredBitResolution(boundingBox.minX, boundingBox.maxX);
        int j = VoxelShapes.findRequiredBitResolution(boundingBox.minY, boundingBox.maxY);
        int k = VoxelShapes.findRequiredBitResolution(boundingBox.minZ, boundingBox.maxZ);
        if (i < 0 || j < 0 || k < 0) {
            return new ArrayVoxelShape(VoxelShapes.FULL_CUBE.voxels, new double[]{boundingBox.minX, boundingBox.maxX}, new double[]{boundingBox.minY, boundingBox.maxY}, new double[]{boundingBox.minZ, boundingBox.maxZ});
        }
        if (i == 0 && j == 0 && k == 0) {
            return boundingBox.contains(0.5, 0.5, 0.5) ? VoxelShapes.fullCube() : VoxelShapes.empty();
        }
        int l = 1 << i;
        int m = 1 << j;
        int n = 1 << k;
    ...
    public static double method_17945(Direction.Axis axis, BoundingBox boundingBox, ViewableWorld viewableWorld, double d, EntityContext entityContext, Stream<VoxelShape> stream) {
        return VoxelShapes.method_17944(boundingBox, viewableWorld, d, entityContext, AxisCycleDirection.between(axis, Direction.Axis.Z), stream);
    }

    private static double method_17944(BoundingBox boundingBox, ViewableWorld viewableWorld, double d, EntityContext entityContext, AxisCycleDirection axisCycleDirection, Stream<VoxelShape> stream) {
        ...
        stream.forEach(voxelShape -> {
            ds[0] = voxelShape.method_1108(axis3, boundingBox, ds[0]);
        });
        return ds[0];
    }
