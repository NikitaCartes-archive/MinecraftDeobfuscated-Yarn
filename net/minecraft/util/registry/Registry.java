...
IndexedIterable<T> {
    ...
    public static final RegistryKey<Registry<ChunkGeneratorType>> NOISE_SETTINGS_WORLDGEN = Registry.createRegistryKey("worldgen/noise_settings");
    ...
    public static final RegistryKey<Registry<ImmutableList<StructureProcessor>>> PROCESSOR_LIST_WORLDGEN = Registry.createRegistryKey("worldgen/processor_list");
    ...
    public static final RegistryKey<Registry<Decorator<?>>> DECORATOR_KEY = Registry.createRegistryKey("worldgen/decorator");
    ...
    public static final RegistryKey<Registry<BlockPlacerType<?>>> BLOCK_PLACER_TYPE_KEY = Registry.createRegistryKey("worldgen/block_placer_type");
    ...
    private static <T> Registry<T> create(RegistryKey<? extends Registry<T>> registryKey, Supplier<T> defaultEntry) {
        return Registry.create(registryKey, Lifecycle.experimental(), defaultEntry);
    }
    ...
    private static <T> DefaultedRegistry<T> create(RegistryKey<? extends Registry<T>> registryKey, String defaultId, Supplier<T> defaultEntry) {
        return Registry.create(registryKey, defaultId, Lifecycle.experimental(), defaultEntry);
    }
    ...
    public <U> DataResult<U> encode(T object, DynamicOps<U> dynamicOps, U object2) {
        ...
        return dynamicOps.mergeToPrimitive(object2, dynamicOps.createString(identifier.toString())).setLifecycle(this.lifecycle);
    }

    public <U> Stream<U> keys(DynamicOps<U> dynamicOps) {
        return this.getIds().stream().map((? super T identifier) -> dynamicOps.createString(identifier.toString()));
    }
