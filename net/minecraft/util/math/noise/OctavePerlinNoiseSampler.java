...
implements NoiseSampler {
    ...
    public double sample(double d, double e, double f, double g, double h, boolean bl) {
        ...
        for (PerlinNoiseSampler perlinNoiseSampler : this.octaveSamplers) {
            i += perlinNoiseSampler.sample(OctavePerlinNoiseSampler.maintainPrecision(d * j), bl ? -perlinNoiseSampler.originY : OctavePerlinNoiseSampler.maintainPrecision(e * j), OctavePerlinNoiseSampler.maintainPrecision(f * j), g * j, h * j) / j;
            j /= 2.0;
        }
        return i;
    }
