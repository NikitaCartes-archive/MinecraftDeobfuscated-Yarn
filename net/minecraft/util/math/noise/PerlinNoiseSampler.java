...
public final class PerlinNoiseSampler {
    ...
    public double sample(double x, double y, double z, double yScale, double yMax) {
        ...
        if (yScale != 0.0) {
            double p = yMax < 0.0 ? 0.0 : Math.min(yMax, h);
    ...
    public double sample(int sectionX, int sectionY, int sectionZ, double localX, double localY, double localZ, double fadeLocalX, double fadeLocalY, double fadeLocalZ) {
        ...
        double d = PerlinNoiseSampler.grad(this.getGradient(j), localX, localY, localZ);
        double e = PerlinNoiseSampler.grad(this.getGradient(m), localX - 1.0, localY, localZ);
        double f = PerlinNoiseSampler.grad(this.getGradient(k), localX, localY - 1.0, localZ);
        double g = PerlinNoiseSampler.grad(this.getGradient(n), localX - 1.0, localY - 1.0, localZ);
        double h = PerlinNoiseSampler.grad(this.getGradient(j + 1), localX, localY, localZ - 1.0);
        double o = PerlinNoiseSampler.grad(this.getGradient(m + 1), localX - 1.0, localY, localZ - 1.0);
        double p = PerlinNoiseSampler.grad(this.getGradient(k + 1), localX, localY - 1.0, localZ - 1.0);
        double q = PerlinNoiseSampler.grad(this.getGradient(n + 1), localX - 1.0, localY - 1.0, localZ - 1.0);
