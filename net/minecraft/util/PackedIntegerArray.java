...
public class PackedIntegerArray {
    ...
    public PackedIntegerArray(int i, int j, long[] ls) {
        ...
        if (ls.length != k) {
            throw SystemUtil.method_22320(new RuntimeException("Invalid length given for storage, got: " + ls.length + " but expected: " + k));
        }
    }
    ...
    public int setAndGetOldValue(int i, int j) {
        ...
        this.storage[l] = this.storage[l] & (this.maxValue << n ^ 0xFFFFFFFFFFFFFFFFL) | ((long)j & this.maxValue) << n;
        if (l != m) {
            int p = 64 - n;
