...
extends DataFix {
    ...
    private Dynamic<?> update(Dynamic<?> advancements) {
        return advancements.update(this.advancementId, advancement -> advancement.update("criteria", criteria -> criteria.updateMapValues(pair -> pair.mapFirst(key -> DataFixUtils.orElse(key.asString().map(keyString -> key.createString((String)this.renamer.apply((String)keyString))).result(), key)))));
    }
}
