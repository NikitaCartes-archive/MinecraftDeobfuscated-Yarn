...
extends DataFix {
    ...
    private <T> Dynamic<T> fixSections(Dynamic<T> dynamic2) {
        return dynamic2.update("Sections", dynamic -> dynamic.updateMapValues(pair -> pair.mapSecond(this::fixRecords)));
    }
    ...
    private <T> Dynamic<T> method_44188(Dynamic<T> dynamic) {
        return DataFixUtils.orElse(dynamic.asStreamOpt().result().map(stream -> dynamic.createList(this.method_44186((Stream)stream))), dynamic);
    }

    protected abstract <T> Stream<Dynamic<T>> method_44186(Stream<Dynamic<T>> var1);
}
