...
extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("EffectDurationEntity", schema.getType(TypeReferences.ENTITY), typed -> typed.update(DSL.remainderFinder(), this::fixEntityStatusEffects)), this.fixTypeEverywhereTyped("EffectDurationPlayer", schema.getType(TypeReferences.PLAYER), typed -> typed.update(DSL.remainderFinder(), this::fixEntityStatusEffects)), this.fixTypeEverywhereTyped("EffectDurationItem", type, typed -> {
            Optional optional2;
            Optional optional = typed.getOptional(opticFinder);
            if (optional.filter(POTION_ITEM_IDS::contains).isPresent() && (optional2 = typed.getOptionalTyped(opticFinder2)).isPresent()) {
                Dynamic<?> dynamic = optional2.get().get(DSL.remainderFinder());
                Typed<?> typed2 = optional2.get().set(DSL.remainderFinder(), dynamic.update("CustomPotionEffects", this::fixPotionEffects));
