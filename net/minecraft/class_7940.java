...
extends ClickableWidget {
    ...
    private class_7940(MultilineText multilineText, TextRenderer textRenderer, Text text, boolean bl) {
        ...
        this.field_41342 = textRenderer.fontHeight;
        this.field_41343 = bl;
        this.active = false;
    }

    public static class_7940 method_47617(int i, TextRenderer textRenderer, Text text) {
        MultilineText multilineText = MultilineText.create(textRenderer, (StringVisitable)text, i);
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (this.field_41343) {
            this.field_41341.drawCenterWithShadow(matrices, this.getX() + this.getWidth() / 2, this.getY(), this.field_41342, 0xFFFFFF);
        } else {
            this.field_41341.drawWithShadow(matrices, this.getX(), this.getY(), this.field_41342, 0xFFFFFF);
        }
    }
}
