...
extends IllagerEntityRenderer<IllusionerEntity> {
    ...
    public IllusionerEntityRenderer(EntityRenderDispatcher entityRenderDispatcher) {
        ...
        this.addFeature(new HeldItemFeatureRenderer<IllusionerEntity, EvilVillagerEntityModel<IllusionerEntity>>((FeatureRendererContext)this){
            ...
            public void method_17149(class_4587 arg, class_4597 arg2, int i, IllusionerEntity illusionerEntity, float f, float g, float h, float j, float k, float l, float m) {
    ...
    public void method_3991(IllusionerEntity illusionerEntity, double d, double e, double f, float g, float h, class_4587 arg, class_4597 arg2) {
        if (illusionerEntity.isInvisible()) {
            ...
            for (int j = 0; j < vec3ds.length; ++j) {
                arg.method_22903();
                arg.method_22904(vec3ds[j].x + (double)MathHelper.cos((float)j + i * 0.5f) * 0.025, vec3ds[j].y + (double)MathHelper.cos((float)j + i * 0.75f) * 0.0125, vec3ds[j].z + (double)MathHelper.cos((float)j + i * 0.7f) * 0.025);
