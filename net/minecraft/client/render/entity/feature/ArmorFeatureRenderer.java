...
extends FeatureRenderer<T, M> {
    ...
    private void renderArmor(MatrixStack matrices, VertexConsumerProvider vertexConsumers, T entity, float limbAngle, float limbDistance, float tickDelta, float customAngle, float headYaw, float headPitch, EquipmentSlot slot, int light, A armorModel) {
        ItemStack itemStack = ((LivingEntity)entity).getEquippedStack(slot);
        ...
        ((BipedEntityModel)this.getContextModel()).setAttributes(armorModel);
        ((BipedEntityModel)armorModel).animateModel(entity, limbAngle, limbDistance, tickDelta);
        this.setVisible(armorModel, slot);
        ((BipedEntityModel)armorModel).setAngles(entity, limbAngle, limbDistance, customAngle, headYaw, headPitch);
