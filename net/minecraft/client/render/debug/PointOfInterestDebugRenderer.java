...
implements DebugRenderer.Renderer {
    ...
    private void method_23135() {
        ...
        this.field_18788.forEach(chunkSectionPos -> {
            if (blockPos.isWithinDistance(chunkSectionPos.getCenterPos(), 60.0)) {
                PointOfInterestDebugRenderer.method_23143(chunkSectionPos);
            }
        });
        this.field_18921.values().forEach(arg -> {
            if (this.method_23147((class_4232)arg)) {
                this.method_23140((class_4232)arg);
            }
        });
        for (BlockPos blockPos22 : this.pointsOfInterest.keySet()) {
            if (!blockPos.isWithinDistance(blockPos22, 30.0)) continue;
            PointOfInterestDebugRenderer.method_23138(blockPos22);
        }
        this.pointsOfInterest.values().forEach(arg -> {
            if (blockPos.isWithinDistance(arg.field_18931, 30.0)) {
                this.method_23137((class_4233)arg);
            }
        });
        this.method_23144().forEach((blockPos2, list) -> {
            if (blockPos.isWithinDistance((Vec3i)blockPos2, 30.0)) {
                this.method_23131((BlockPos)blockPos2, (List<String>)list);
            }
        });
    }
    ...
    private void method_23131(BlockPos blockPos, List<String> list) {
        float f = 0.05f;
        DebugRenderer.method_23103(blockPos, 0.05f, 0.2f, 0.2f, 1.0f, 0.3f);
        PointOfInterestDebugRenderer.method_23134("" + list, blockPos, 0, -256);
        PointOfInterestDebugRenderer.method_23134("Ghost POI", blockPos, 1, -65536);
    }

    private void method_23137(class_4233 arg) {
        int i = 0;
        if (this.method_23141(arg).size() < 4) {
            PointOfInterestDebugRenderer.method_23133("" + this.method_23141(arg), arg, i, -256);
        } else {
            PointOfInterestDebugRenderer.method_23133("" + this.method_23141(arg).size() + " ticket holders", arg, i, -256);
        }
        PointOfInterestDebugRenderer.method_23133("Free tickets: " + arg.field_18933, arg, ++i, -256);
        PointOfInterestDebugRenderer.method_23133(arg.field_18932, arg, ++i, -1);
    }

    private void method_23136(class_4232 arg) {
        if (arg.field_19330 != null) {
            PathfindingDebugRenderer.method_23118(this.method_23139(), arg.field_19330, 0.5f, false, false);
        }
    }

    private void method_23140(class_4232 arg) {
        boolean bl = this.method_23145(arg);
        int i = 0;
        PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, arg.field_19328, -1, 0.03f);
        ++i;
        if (bl) {
            PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, arg.field_18925 + " " + arg.field_19329 + "xp", -1, 0.02f);
            ++i;
        }
        if (bl && !arg.field_19372.equals("")) {
            PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, arg.field_19372, -98404, 0.02f);
            ++i;
        }
        if (bl) {
            for (String string : arg.field_18928) {
                PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, string, -16711681, 0.02f);
                ++i;
            }
        }
        if (bl) {
            for (String string : arg.field_18927) {
                PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, string, -16711936, 0.02f);
                ++i;
            }
        }
        if (arg.field_19373) {
            PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, "Wants Golem", -23296, 0.02f);
            ++i;
        }
        if (bl) {
            for (String string : arg.field_19375) {
                if (string.startsWith(arg.field_19328)) {
                    PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, string, -1, 0.02f);
                } else {
                    PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, string, -23296, 0.02f);
                }
                ++i;
            }
        }
        if (bl) {
            for (String string : Lists.reverse(arg.field_19374)) {
                PointOfInterestDebugRenderer.method_23132(arg.field_18926, i, string, -3355444, 0.02f);
                ++i;
            }
        }
        if (bl) {
    ...
    private static void method_23133(String string, class_4233 arg, int i, int j) {
        BlockPos blockPos = arg.field_18931;
        PointOfInterestDebugRenderer.method_23134(string, blockPos, i, j);
    }
    ...
    private Set<String> method_23141(class_4233 arg) {
        return this.method_23142(arg.field_18931).stream().map(VillagerNamer::name).collect(Collectors.toSet());
    }

    private boolean method_23145(class_4232 arg) {
        return Objects.equals(this.field_18922, arg.field_18923);
    }

    private boolean method_23147(class_4232 arg) {
    ...
    private Collection<UUID> method_23142(BlockPos blockPos) {
        return this.field_18921.values().stream().filter(arg -> ((class_4232)arg).method_23151(blockPos)).map(class_4232::method_23149).collect(Collectors.toSet());
    }
    ...
    private void method_23146() {
        DebugRenderer.method_23101(this.field_18786.getCameraEntity(), 8).ifPresent(entity -> {
