...
public class ChunkBuilder {
    ...
    public boolean runTasksSync(long l) {
        ...
        } while (l != 0L && bl2 && l >= Util.getMeasuringTimeNano());
        return bl;
    }
    ...
    public void reset() {
        this.clear();
        ArrayList<BlockBufferBuilderStorage> list = Lists.newArrayList();
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public ListenableFuture<Void> upload(RenderLayer renderLayer, BufferBuilder bufferBuilder, ChunkRenderer chunkRenderer, ChunkRenderData chunkRenderData, double d) {
        if (MinecraftClient.getInstance().isOnThread()) {
            if (GLX.useVbo()) {
                this.uploadVbo(bufferBuilder, chunkRenderer.getGlBuffer(renderLayer.ordinal()));
            } else {
                this.uploadDisplayList(bufferBuilder, ((DisplayListChunkRenderer)chunkRenderer).method_3639(renderLayer, chunkRenderData));
            }
            bufferBuilder.setOffset(0.0, 0.0, 0.0);
            return Futures.immediateFuture(null);
        }
        ListenableFutureTask<Object> listenableFutureTask = ListenableFutureTask.create(() -> this.upload(renderLayer, bufferBuilder, chunkRenderer, chunkRenderData, d), null);
        Queue<ChunkUploadTask> queue = this.uploadQueue;
        synchronized (queue) {
            this.uploadQueue.add(new ChunkUploadTask(listenableFutureTask, d));
        }
    ...
    private void uploadVbo(BufferBuilder bufferBuilder, VertexBuffer vertexBuffer) {
        this.vboBufferRenderer.setGlBuffer(vertexBuffer);
