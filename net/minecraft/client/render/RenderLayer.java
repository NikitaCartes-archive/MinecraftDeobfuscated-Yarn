...
extends RenderPhase {
    ...
    public static RenderLayer getItemEntityTranslucentCull(Identifier texture) {
        MultiPhaseParameters multiPhaseParameters = MultiPhaseParameters.builder().texture(new RenderPhase.Texture(texture, false, false)).transparency(ITEM_TRANSPARENCY).diffuseLighting(ENABLE_DIFFUSE_LIGHTING).alpha(ONE_TENTH_ALPHA).lightmap(ENABLE_LIGHTMAP).overlay(ENABLE_OVERLAY_COLOR).writeMaskState(RenderPhase.ALL_MASK).build(true);
    ...
    public static RenderLayer getEntityAlpha(Identifier texture, float alpha) {
        MultiPhaseParameters multiPhaseParameters = MultiPhaseParameters.builder().transparency(DRAGON_EXPLOSION_TRANSPARENCY).texture(new RenderPhase.Texture(texture, false, false)).alpha(new RenderPhase.Alpha(alpha)).cull(DISABLE_CULLING).build(true);
