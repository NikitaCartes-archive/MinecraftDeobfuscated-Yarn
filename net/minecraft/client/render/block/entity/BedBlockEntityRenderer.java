...
extends class_4576<BedBlockEntity> {
    public static final Identifier[] TEXTURES = (Identifier[])Arrays.stream(DyeColor.values()).sorted(Comparator.comparingInt(DyeColor::getId)).map(dyeColor -> new Identifier("entity/bed/" + dyeColor.getName())).toArray(Identifier[]::new);
    ...
    protected void method_3557(BedBlockEntity bedBlockEntity, double d, double e, double f, float g, int i, BlockRenderLayer blockRenderLayer, BufferBuilder bufferBuilder, int j, int k) {
        Identifier identifier = i >= 0 ? ModelLoader.field_20848.get(i) : TEXTURES[bedBlockEntity.getColor().getId()];
