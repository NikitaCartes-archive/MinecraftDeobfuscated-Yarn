        ...
        static MultipartModelSelector deserializeSelector(JsonObject jsonObject) {
            ...
            if (set.size() == 1) {
                if (jsonObject.has("OR")) {
                    List list = Streams.stream(JsonHelper.getArray(jsonObject, "OR")).map(jsonElement -> Deserializer.deserializeSelector(jsonElement.getAsJsonObject())).collect(Collectors.toList());
                    return new OrMultipartModelSelector(list);
                }
                if (jsonObject.has("AND")) {
                    List list = Streams.stream(JsonHelper.getArray(jsonObject, "AND")).map(jsonElement -> Deserializer.deserializeSelector(jsonElement.getAsJsonObject())).collect(Collectors.toList());
