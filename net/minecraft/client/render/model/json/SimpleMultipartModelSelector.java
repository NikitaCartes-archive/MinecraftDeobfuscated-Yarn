...
implements MultipartModelSelector {
    ...
    private Predicate<BlockState> createPredicate(StateFactory<Block, BlockState> stateFactory, Property<?> property, String string) {
        ...
        if (!optional.isPresent()) {
            throw new RuntimeException(String.format("Unknown value '%s' for property '%s' on '%s' in '%s'", string, this.key, stateFactory.getBaseObject().toString(), this.valueString));
        }
