...
implements Toast {
    ...
    public Toast.Visibility draw(MatrixStack matrixStack, ToastManager toastManager, long l) {
        ...
        if (advancementDisplay != null) {
            ...
            if (list.size() == 1) {
                toastManager.getGame().textRenderer.draw(matrixStack, I18n.translate("advancements.toast." + advancementDisplay.getFrame().getId(), new Object[0]), 30.0f, 7.0f, i | 0xFF000000);
                toastManager.getGame().textRenderer.draw(matrixStack, list.get(0), 30.0f, 18.0f, -1);
            } else {
                int j = 1500;
                float f = 300.0f;
                if (l < 1500L) {
                    int k = MathHelper.floor(MathHelper.clamp((float)(1500L - l) / 300.0f, 0.0f, 1.0f) * 255.0f) << 24 | 0x4000000;
                    toastManager.getGame().textRenderer.draw(matrixStack, I18n.translate("advancements.toast." + advancementDisplay.getFrame().getId(), new Object[0]), 30.0f, 11.0f, i | k);
                } else {
                    ...
                    for (Text text : list) {
                        toastManager.getGame().textRenderer.draw(matrixStack, text, 30.0f, (float)m, 0xFFFFFF | k);
