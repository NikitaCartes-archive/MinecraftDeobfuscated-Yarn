...
public class ResourceIndex {
    ...
    public Collection<String> getFilesRecursively(String string3, int i, Predicate<String> predicate) {
        return this.index.keySet().stream().filter(string -> !string.endsWith(".mcmeta")).map(Identifier::new).map(Identifier::getPath).filter(string2 -> string2.startsWith(string3 + "/")).filter(predicate).collect(Collectors.toList());
    }
}
