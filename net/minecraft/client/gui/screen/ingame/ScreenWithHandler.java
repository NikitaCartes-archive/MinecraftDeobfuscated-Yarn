...
public abstract class ScreenWithHandler<T extends ScreenHandler>
extends Screen
implements ScreenHandlerProvider<T> {
    ...
    private void drawSlot(Slot slot) {
        ...
        if (itemStack.isEmpty() && slot.doDrawHoveringEffect() && (pair = slot.getBackgroundSprite()) != null) {
            ...
            ScreenWithHandler.blit(i, j, this.getZOffset(), 16, 16, sprite);
            bl2 = true;
        }
        if (!bl2) {
            if (bl) {
                ScreenWithHandler.fill(i, j, i + 16, j + 16, -2130706433);
            }
            RenderSystem.enableDepthTest();
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        ...
        if (this.isDoubleClicking && slot != null && button == 0 && ((ScreenHandler)this.handler).canInsertIntoSlot(ItemStack.EMPTY, slot)) {
            if (ScreenWithHandler.hasShiftDown()) {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (this.focusedSlot != null && this.focusedSlot.hasStack()) {
            if (this.client.options.keyPickItem.matchesKey(keyCode, scanCode)) {
                ...
                this.onMouseClick(this.focusedSlot, this.focusedSlot.id, ScreenWithHandler.hasControlDown() ? 1 : 0, SlotActionType.THROW);
            }
        }
        return true;
    }
