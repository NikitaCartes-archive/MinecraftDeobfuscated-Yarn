...
extends AbstractCommandBlockScreen {
    ...
    protected void init() {
        ...
        }).values((CommandBlockBlockEntity.Type[])CommandBlockBlockEntity.Type.values()).omitKeyText().initially(this.mode).build(this.width / 2 - 50 - 100 - 4, 165, 100, 20, Text.method_43471("advMode.mode"), (button, mode) -> {
            this.mode = mode;
        }));
        this.conditionalModeButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(Text.method_43471("advMode.mode.conditional"), Text.method_43471("advMode.mode.unconditional")).omitKeyText().initially(this.conditional).build(this.width / 2 - 50, 165, 100, 20, Text.method_43471("advMode.type"), (button, conditional) -> {
            this.conditional = conditional;
        }));
        this.redstoneTriggerButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(Text.method_43471("advMode.mode.autoexec.bat"), Text.method_43471("advMode.mode.redstoneTriggered")).omitKeyText().initially(this.autoActivate).build(this.width / 2 + 50 + 4, 165, 100, 20, Text.method_43471("advMode.triggering"), (button, autoActivate) -> {
