...
extends Screen {
    ...
    private void initWidgets() {
        ...
        this.addDrawableChild(new ButtonWidget(this.width / 2 - 102, this.height / 4 + 48 + -16, 98, 20, Text.method_43471("gui.advancements"), button -> this.client.setScreen(new AdvancementsScreen(this.client.player.networkHandler.getAdvancementHandler()))));
        this.addDrawableChild(new ButtonWidget(this.width / 2 + 4, this.height / 4 + 48 + -16, 98, 20, Text.method_43471("gui.stats"), button -> this.client.setScreen(new StatsScreen(this, this.client.player.getStatHandler()))));
        String string = SharedConstants.getGameVersion().isStable() ? JAVA_FEEDBACK_URL : SNAPSHOT_FEEDBACK_URL;
        this.addDrawableChild(new ButtonWidget(this.width / 2 - 102, this.height / 4 + 72 + -16, 98, 20, Text.method_43471("menu.sendFeedback"), button -> this.client.setScreen(new ConfirmChatLinkScreen(confirmed -> {
            if (confirmed) {
                Util.getOperatingSystem().open(string);
            }
            this.client.setScreen(this);
        }, string, true))));
        ButtonWidget buttonWidget = this.addDrawableChild(new ButtonWidget(this.width / 2 + 4, this.height / 4 + 72 + -16, 98, 20, Text.method_43471("menu.reportBugs"), button -> this.client.setScreen(new ConfirmChatLinkScreen(confirmed -> {
            if (confirmed) {
        ...
        this.addDrawableChild(new ButtonWidget(this.width / 2 - 102, this.height / 4 + 96 + -16, 98, 20, Text.method_43471("menu.options"), button -> this.client.setScreen(new OptionsScreen(this, this.client.options))));
        ButtonWidget buttonWidget2 = this.addDrawableChild(new ButtonWidget(this.width / 2 + 4, this.height / 4 + 96 + -16, 98, 20, Text.method_43471("menu.shareToLan"), button -> this.client.setScreen(new OpenToLanScreen(this))));
        buttonWidget2.active = this.client.isIntegratedServerRunning() && !this.client.getServer().isRemote();
        MutableText text = this.client.isInSingleplayer() ? Text.method_43471("menu.returnToMenu") : Text.method_43471("menu.disconnect");
