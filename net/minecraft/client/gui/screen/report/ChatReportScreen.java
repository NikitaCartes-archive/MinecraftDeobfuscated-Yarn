...
extends Screen {
    ...
    private void send() {
        this.report.finalizeReport(this.context).ifLeft(report -> {
            CompletableFuture<Unit> completableFuture = this.context.getSender().send(report.id(), report.report());
    ...
    extends WarningScreen {
        ...
        protected void initButtons(int yOffset) {
            int i = 150;
            this.addDrawableChild(ButtonWidget.createBuilder(RETURN_BUTTON_TEXT, button -> this.close()).setPositionAndSize(this.width / 2 - 245, 100 + yOffset, 150, 20).build());
