...
extends Screen {
    ...
    private void updateCommand() {
        ...
        if (stringReader.canRead() && stringReader.peek() == '/') {
            ...
            if (this.parseResults == null) {
                this.parseResults = commandDispatcher.parse(stringReader, (CommandSource)this.minecraft.player.networkHandler.getCommandSource());
            }
    ...
    private void updateCommandFeedback() {
        if (this.suggestionsFuture.join().isEmpty() && !this.parseResults.getExceptions().isEmpty() && this.chatField.getCursor() == this.chatField.getText().length()) {
            int i = 0;
            for (Map.Entry<CommandNode<CommandSource>, CommandSyntaxException> entry : this.parseResults.getExceptions().entrySet()) {
    ...
    private String getRenderText(String string, int i) {
        if (this.parseResults != null) {
            return ChatScreen.getRenderText(this.parseResults, string, i);
        }
        return string;
    }
    ...
    public void render(int i, int j, float f) {
        ...
        if (this.suggestionsWindow != null) {
            ...
            for (String string : this.commandExceptions) {
                ChatScreen.fill(this.commandExceptionsX - 1, this.height - 14 - 13 - 12 * k, this.commandExceptionsX + this.commandExceptionsWidth + 1, this.height - 2 - 13 - 12 * k, -16777216);
                this.font.drawWithShadow(string, this.commandExceptionsX, this.height - 14 - 13 + 2 - 12 * k, -1);
                ++k;
            }
        }
    ...
    class SuggestionWindow {
        ...
        public boolean mouseScrolled(double d) {
            int j;
            int i = (int)(ChatScreen.this.minecraft.mouse.getX() * (double)ChatScreen.this.minecraft.method_22683().getScaledWidth() / (double)ChatScreen.this.minecraft.method_22683().getWidth());
            if (this.area.contains(i, j = (int)(ChatScreen.this.minecraft.mouse.getY() * (double)ChatScreen.this.minecraft.method_22683().getScaledHeight() / (double)ChatScreen.this.minecraft.method_22683().getHeight()))) {
