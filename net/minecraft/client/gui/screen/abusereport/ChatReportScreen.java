...
extends Screen {
        ...
        this.report.finalizeReport(this.reporter).left().ifPresent(report -> {
            ...
            completableFuture.handleAsync((unit, throwable) -> {
                if (throwable == null) {
                    ...
                    this.onSubmissionError(runningTaskScreen, (Throwable)throwable);
                }
                return null;
    ...
    extends WarningScreen {
        protected DiscardWarningScreen() {
            super(Text.translatable("gui.chatReport.discard.title"), Text.translatable("gui.chatReport.discard.content"), Text.translatable("gui.chatReport.discard.content"));
        }

        @Override
        protected void initButtons(int yOffset) {
            this.addDrawableChild(new ButtonWidget(this.width / 2 - 155, 100 + yOffset, 150, 20, Text.translatable("gui.chatReport.discard.return"), button -> this.close()));
            this.addDrawableChild(new ButtonWidget(this.width / 2 + 5, 100 + yOffset, 150, 20, Text.translatable("gui.chatReport.discard.discard"), button -> this.client.setScreen(ChatReportScreen.this.parent)));
        }
