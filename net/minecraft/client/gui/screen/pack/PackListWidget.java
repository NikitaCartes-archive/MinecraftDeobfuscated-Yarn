...
extends AlwaysSelectedEntryListWidget<ResourcePackEntry> {
    ...
    extends AlwaysSelectedEntryListWidget.Entry<ResourcePackEntry> {
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            if ((i = this.client.textRenderer.getWidth(text)) > 157) {
                StringRenderable stringRenderable2 = StringRenderable.concat(this.client.textRenderer.trimToWidth(text, 157 - this.client.textRenderer.getWidth("...")), StringRenderable.plain("..."));
                this.client.textRenderer.drawWithShadow(matrices, stringRenderable2, (float)(x + 32 + 2), (float)(y + 1), 0xFFFFFF);
            } else {
                this.client.textRenderer.drawWithShadow(matrices, text, (float)(x + 32 + 2), (float)(y + 1), 0xFFFFFF);
            }
            this.client.textRenderer.drawWithShadow(matrices, text, (float)(x + 32 + 2), (float)(y + 1), 0xFFFFFF);
            List<StringRenderable> list = this.client.textRenderer.wrapLines(stringRenderable, 157);
            for (int k = 0; k < 2 && k < list.size(); ++k) {
                this.client.textRenderer.drawWithShadow(matrices, list.get(k), (float)(x + 32 + 2), (float)(y + 12 + 10 * k), 0x808080);
            }
        }
