...
public class WorldGenerationProgressScreen
extends Screen {
    ...
    public void render(int i, int j, float f) {
        ...
        int m = this.height / 2;
        int n = 30;
        WorldGenerationProgressScreen.drawChunkMap(this.progressProvider, k, m + 30, 2, 0);
    ...
    public static void drawChunkMap(WorldGenerationProgressTracker worldGenerationProgressTracker, int i, int j, int k, int l) {
        ...
        int u = -16772609;
        if (l != 0) {
            WorldGenerationProgressScreen.fill(i - t, j - t, i - t + 1, j + t, -16772609);
            WorldGenerationProgressScreen.fill(i + t - 1, j - t, i + t, j + t, -16772609);
            WorldGenerationProgressScreen.fill(i - t, j - t, i + t, j - t + 1, -16772609);
            WorldGenerationProgressScreen.fill(i - t, j + t - 1, i + t, j + t, -16772609);
        }
        for (int v = 0; v < p; ++v) {
            for (int w = 0; w < p; ++w) {
                ...
                int x = r + v * m;
                int y = s + w * m;
                WorldGenerationProgressScreen.fill(x, y, x + k, y + k, STATUS_TO_COLOR.getInt(chunkStatus) | 0xFF000000);
            }
        }
    }
}
