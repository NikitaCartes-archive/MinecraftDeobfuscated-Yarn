...
RecipeGridAligner<Ingredient> {
    ...
    private final List<GroupButtonWidget> tabButtons = Lists.newArrayList();
    ...
    public void reset(boolean bl) {
        ...
        if (this.currentTab != null) {
            this.currentTab = this.tabButtons.stream().filter(groupButtonWidget -> groupButtonWidget.getCategory().equals((Object)this.currentTab.getCategory())).findFirst().orElse(null);
        }
        if (this.currentTab == null) {
    ...
    public int findLeftEdge(boolean bl, int i, int j) {
        int k = this.isOpen() && !bl ? 177 + (i - j - 200) / 2 : (i - j) / 2;
        return k;
    }
    ...
    private void refreshTabButtons() {
        ...
        for (GroupButtonWidget groupButtonWidget : this.tabButtons) {
            ...
            if (!groupButtonWidget.hasKnownRecipes(this.recipeBook)) continue;
            groupButtonWidget.setPos(i, j + 27 * l++);
    ...
    public boolean mouseClicked(double d, double e, int i) {
        ...
        for (GroupButtonWidget groupButtonWidget : this.tabButtons) {
            if (!groupButtonWidget.mouseClicked(d, e, i)) continue;
    ...
    public void showGhostRecipe(Recipe<?> recipe, List<Slot> list) {
        ...
        this.ghostSlots.addSlot(Ingredient.ofStacks(itemStack), list.get((int)0).xPosition, list.get((int)0).yPosition);
