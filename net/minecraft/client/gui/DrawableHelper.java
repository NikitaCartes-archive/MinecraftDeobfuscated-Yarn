...
public abstract class DrawableHelper {
    ...
    protected void fillGradient(MatrixStack matrices, int xStart, int yStart, int xEnd, int yEnd, int colorStart, int colorEnd) {
        DrawableHelper.method_33284(matrices, xStart, yStart, xEnd, yEnd, colorStart, colorEnd, this.zOffset);
    }
    ...
    protected static void method_33284(MatrixStack matrixStack, int i, int j, int k, int l, int m, int n, int o) {
        ...
        bufferBuilder.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
        DrawableHelper.fillGradient(matrixStack.peek().getModel(), bufferBuilder, i, j, k, l, o, m, n);
        tessellator.draw();
