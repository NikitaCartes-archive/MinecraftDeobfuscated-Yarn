...
public class GlyphRenderer {
    ...
    public void draw(TextureManager textureManager, boolean bl, float f, float g, BufferBuilder bufferBuilder, float h, float i, float j, float k) {
        ...
        bufferBuilder.vertex(m + s, q, 0.0).texture(this.uMin, this.vMin).color(h, i, j, k).next();
        bufferBuilder.vertex(m + t, r, 0.0).texture(this.uMin, this.vMax).color(h, i, j, k).next();
        bufferBuilder.vertex(n + t, r, 0.0).texture(this.uMax, this.vMax).color(h, i, j, k).next();
        bufferBuilder.vertex(n + s, q, 0.0).texture(this.uMax, this.vMin).color(h, i, j, k).next();
    }
