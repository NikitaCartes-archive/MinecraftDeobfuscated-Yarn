...
public abstract class Option {
    ...
    }, (gameOptions, option) -> option.getPercentValueText(option.getRatio(option.get((GameOptions)gameOptions))));
    ...
    public static final DoubleOption FOV_EFFECT_SCALE = new DoubleOption("options.fovEffectScale", 0.0, 1.0, 0.0f, gameOptions -> Math.pow(gameOptions.fovEffectScale, 2.0), (gameOptions, fovEffectScale) -> {
        ...
        double d = option.getRatio(option.get((GameOptions)gameOptions));
        if (d == 0.0) {
            return option.getGenericValueText(ScreenTexts.OFF);
        }
        return option.getPercentValueText(d);
    }, client -> value -> minecraftClient.textRenderer.wrapLines(FOV_EFFECT_SCALE_TOOLTIP, 200));
    ...
    public static final DoubleOption DISTORTION_EFFECT_SCALE = new DoubleOption("options.screenEffectScale", 0.0, 1.0, 0.0f, gameOptions -> gameOptions.distortionEffectScale, (gameOptions, distortionEffectScale) -> {
        ...
        return option.getPercentValueText(d);
    }, client -> value -> minecraftClient.textRenderer.wrapLines(DISTORTION_EFFECT_SCALE_TOOLTIP, 200));
    public static final DoubleOption FRAMERATE_LIMIT = new DoubleOption("options.framerateLimit", 10.0, 260.0, 10.0f, gameOptions -> gameOptions.maxFps, (gameOptions, maxFps) -> {
        ...
        double d = option.get((GameOptions)gameOptions);
        if (d == option.getMax()) {
            return option.getGenericValueText(new TranslatableText("options.framerateLimit.max"));
        }
        return option.getGenericValueText(new TranslatableText("options.framerate", (int)d));
    });
    public static final DoubleOption GAMMA = new DoubleOption("options.gamma", 0.0, 1.0, 0.0f, gameOptions -> gameOptions.gamma, (gameOptions, gamma) -> {
        ...
        if (i == 100) {
            return option.getGenericValueText(new TranslatableText("options.gamma.max"));
        }
        return option.getGenericValueText(i);
    });
    public static final DoubleOption MIPMAP_LEVELS = new DoubleOption("options.mipmapLevels", 0.0, 4.0, 1.0f, gameOptions -> gameOptions.mipmapLevels, (gameOptions, mipmapLevels) -> {
        ...
        return option.getGenericValueText((int)d);
    });
    ...
    public static final DoubleOption SENSITIVITY = new DoubleOption("options.sensitivity", 0.0, 1.0, 0.0f, gameOptions -> gameOptions.mouseSensitivity, (gameOptions, mouseSensitivity) -> {
        ...
        if (d == 1.0) {
            return option.getGenericValueText(new TranslatableText("options.sensitivity.max"));
        }
        return option.getPercentValueText(2.0 * d);
    });
    ...
    public static final CyclingOption<Integer> GUI_SCALE = CyclingOption.create("options.guiScale", () -> IntStream.rangeClosed(0, MinecraftClient.getInstance().getWindow().calculateScaleFactor(0, MinecraftClient.getInstance().forcesUnicodeFont())).boxed().collect(Collectors.toList()), guiScale -> guiScale == 0 ? new TranslatableText("options.guiScale.auto") : new LiteralText(Integer.toString(guiScale)), gameOptions -> gameOptions.guiScale, (gameOptions, option, guiScale) -> {
    ...
    public static final CyclingOption<Boolean> DIRECTIONAL_AUDIO = CyclingOption.create("options.directionalAudio", gameOptions -> gameOptions.directionalAudio, (gameOptions, option, directionalAudio) -> {
        gameOptions.directionalAudio = directionalAudio;
        SoundManager soundManager = MinecraftClient.getInstance().getSoundManager();
        soundManager.reloadSounds();
        soundManager.play(PositionedSoundInstance.master(SoundEvents.UI_BUTTON_CLICK, 1.0f));
    }).tooltip(client -> value -> value != false ? minecraftClient.textRenderer.wrapLines(DIRECTIONAL_AUDIO_ON_TEXT, 200) : minecraftClient.textRenderer.wrapLines(DIRECTIONAL_AUDIO_OFF_TEXT, 200));
