...
implements Recorder {
    ...
    private final Map<SamplingRecorder, List<Sample>> field_33891 = new Object2ObjectOpenHashMap<SamplingRecorder, List<Sample>>();
    ...
    public void read() {
        ...
        for (SamplingRecorder samplingRecorder2 : this.field_33893) {
            samplingRecorder2.sample(this.ticks);
            if (!samplingRecorder2.method_37174()) continue;
            Sample sample = new Sample(Instant.now(), this.ticks, this.profiler.getResult());
            this.field_33891.computeIfAbsent(samplingRecorder2, samplingRecorder -> Lists.newArrayList()).add(sample);
        }
    ...
    private void execute(ProfileResult profileResult) {
        HashSet<SamplingRecorder> hashSet = new HashSet<SamplingRecorder>(this.field_33893);
