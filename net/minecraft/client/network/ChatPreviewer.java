...
public class ChatPreviewer {
     ...
     */
    @Nullable
    private Query pendingRequestQuery;
    /**
    ...
    public void tryQuery() {
        Query query = this.pendingRequestQuery;
        if (query == null) {
            return;
        }
     ...
     */
    private void query(Query query, long currentTime) {
    ...
    private boolean shouldQuery(long currentTime) {
        ...
        if (currentTime >= this.getEarliestNextQueryTime()) {
    ...
    public void onInput(String query) {
        Query query2;
        if ((query = ChatPreviewer.normalize(query)).isEmpty()) {
            this.lastResponse = new Response(Util.getMeasuringTimeMs(), query, null);
            this.pendingRequestQuery = null;
            this.pendingResponseQuery = null;
            return;
        }
        Query query3 = query2 = this.pendingRequestQuery != null ? this.pendingRequestQuery : this.pendingResponseQuery;
        if (query2 == null || !query2.messageEquals(query)) {
            this.pendingRequestQuery = new Query(this.idIncrementor.next(), query);
        }
    }
    ...
    public void onResponse(int id, @Nullable Text response) {
        ...
        if (this.pendingResponseQuery != null && this.pendingResponseQuery.idEquals(id)) {
            Text text = response != null ? response : Text.literal(this.pendingResponseQuery.message());
            this.lastResponse = new Response(Util.getMeasuringTimeMs(), this.pendingResponseQuery.message(), text);
            this.pendingResponseQuery = null;
        } else {
            this.lastResponse = null;
        }
    ...
    public Text getPreviewText() {
        return Util.map(this.lastResponse, Response::previewText);
    }
    ...
    @Environment(value=EnvType.CLIENT)
    record Response(long receivedTimeStamp, String query, @Nullable Text previewText) {
        public Response {
