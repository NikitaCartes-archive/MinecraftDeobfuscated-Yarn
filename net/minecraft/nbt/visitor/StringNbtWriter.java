...
implements class_5627 {
    ...
    public void method_32292(CompoundTag compoundTag) {
        this.result.append('{');
        ArrayList<String> list = Lists.newArrayList(compoundTag.getKeys());
        Collections.sort(list);
        for (String string : list) {
            if (this.result.length() != 1) {
                this.result.append(',');
            }
            this.result.append(StringNbtWriter.escapeName(string)).append(':').append(new StringNbtWriter().apply(compoundTag.get(string)));
        }
        this.result.append('}');
    }
