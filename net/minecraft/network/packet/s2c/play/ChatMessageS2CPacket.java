 ...
 * 
 *  * @param content the unwrapped chat message
 */
public record ChatMessageS2CPacket(Text content, MessageType type, ChatMessageSender sender, Instant timeStamp, NetworkEncryptionUtils.SignatureData saltSignature) implements Packet<ClientPlayPacketListener>
{
    ...
    public ChatMessageS2CPacket(PacketByteBuf buf) {
        this(buf.readText(), MessageType.byId(buf.readByte()), new ChatMessageSender(buf), Instant.ofEpochSecond(buf.readLong()), new NetworkEncryptionUtils.SignatureData(buf));
    }
