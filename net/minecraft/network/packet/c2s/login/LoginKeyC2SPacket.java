...
implements Packet<ServerLoginPacketListener> {
    ...
    public boolean verifySignedNonce(byte[] nonce, SignatureVerifier verifier) {
        return this.nonce.map(encrypted -> false, signature -> verifier.validate(updater -> {
    ...
    public boolean verifyEncryptedNonce(byte[] nonce, PrivateKey privateKey) {
        ...
        try {
            return optional.isPresent() && Arrays.equals(nonce, NetworkEncryptionUtils.decrypt(privateKey, optional.get()));
