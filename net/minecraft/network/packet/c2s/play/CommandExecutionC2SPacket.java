 ...
 */
public record CommandExecutionC2SPacket(String command, Instant timestamp, long salt, ArgumentSignatureDataMap argumentSignatures, boolean signedPreview) implements Packet<ServerPlayPacketListener>
{
    public CommandExecutionC2SPacket(String command, Instant timestamp, long salt, ArgumentSignatureDataMap argumentSignatures, boolean signedPreview) {
    ...
    public CommandExecutionC2SPacket(PacketByteBuf buf) {
        this(buf.readString(256), buf.readInstant(), buf.readLong(), new ArgumentSignatureDataMap(buf), buf.readBoolean());
    }
    ...
    public SignedCommandArguments createSignedArguments(ServerPlayerEntity player) {
        ...
        return new SignedCommandArguments.Impl(player.networkHandler.getMessageUnpacker(), messageMetadata, this.argumentSignatures, this.signedPreview);
    }
}
