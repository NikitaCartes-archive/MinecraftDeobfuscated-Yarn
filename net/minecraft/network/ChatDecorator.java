...
public interface ChatDecorator {
     ...
     */
    public static final ChatDecorator NOOP = (sender, message) -> message;
    ...
    public static ChatDecorator testRainbowChat() {
        return (sender, message) -> {
            ...
            MutableText mutableText = Text.literal(String.valueOf(string.charAt(0))).fillStyle(Style.EMPTY.withColor(MathHelper.hsvToRgb(Math.nextDown(1.0f), 1.0f, 1.0f)));
            for (int j = 1; j < i; ++j) {
     ...
     */
    default public SignedChatMessage decorate(@Nullable ServerPlayerEntity sender, Text message, ChatMessageSignature signature, boolean previewed) {
        Text text = this.decorate(sender, message);
        if (message.equals(text)) {
            return SignedChatMessage.of(message, signature);
        }
        if (!previewed) {
            return SignedChatMessage.of(message, signature).withUnsigned(text);
        }
        return SignedChatMessage.of(text, signature);
    }
     ...
     */
    default public SignedChatMessage decorate(@Nullable ServerPlayerEntity sender, SignedChatMessage message) {
