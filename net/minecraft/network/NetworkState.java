...
public enum NetworkState implements PacketBundleHandler.BundlerGetter
{
    ...
    public PacketBundleHandler getBundler(NetworkSide side) {
        return this.packetHandlers.get((Object)side).getBundler();
    }
    ...
    static {
        ...
        for (NetworkState networkState : NetworkState.values()) {
            ...
            networkState.packetHandlers.forEach((side, handler) -> handler.forEachPacketType(packetClass -> {
                if (HANDLER_STATE_MAP.containsKey(packetClass) && HANDLER_STATE_MAP.get(packetClass) != networkState) {
                    throw new IllegalStateException("Packet " + packetClass + " is already assigned to protocol " + HANDLER_STATE_MAP.get(packetClass) + " - can't reassign to " + networkState);
                }
    ...
    static class PacketHandler<T extends PacketListener> {
        ...
        private final Set<Class<? extends Packet<T>>> bundlePacketTypes = new HashSet<Class<? extends Packet<T>>>();
        ...
        public <P extends BundlePacket<T>> PacketHandler<T> registerBundlePacket(Class<P> bundlePacketType, Function<Iterable<Packet<T>>, P> bundleFunction) {
            if (this.bundler != PacketBundleHandler.NOOP) {
                throw new IllegalStateException("Bundle packet already configured");
            }
            BundleSplitterPacket bundleSplitterPacket = new BundleSplitterPacket();
            this.register(BundleSplitterPacket.class, buf -> bundleSplitterPacket);
