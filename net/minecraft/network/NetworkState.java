...
public enum NetworkState {
    ...
    protected NetworkState addPacket(NetworkSide networkSide, Class<? extends Packet<?>> class_) {
        BiMap<Integer, Class<Packet<?>>> biMap = this.packetHandlerMap.get((Object)networkSide);
        if (biMap == null) {
            biMap = HashBiMap.create();
            this.packetHandlerMap.put(networkSide, biMap);
        }
        if (biMap.containsValue(class_)) {
            String string = (Object)((Object)networkSide) + " packet " + class_ + " is already known to ID " + biMap.inverse().get(class_);
    ...
    public Integer getPacketId(NetworkSide networkSide, Packet<?> packet) throws Exception {
        return (Integer)this.packetHandlerMap.get((Object)networkSide).inverse().get(packet.getClass());
    }

    @Nullable
    public Packet<?> getPacketHandler(NetworkSide networkSide, int i) throws IllegalAccessException, InstantiationException {
        Class class_ = (Class)this.packetHandlerMap.get((Object)networkSide).get(i);
        if (class_ == null) {
            return null;
        }
