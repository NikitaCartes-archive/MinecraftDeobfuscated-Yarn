...
public class NetherFossilGenerator {
    ...
    public static void addPieces(StructureManager manager, List<StructurePiece> pieces, Random random, BlockPos pos) {
        ...
        pieces.add(new Piece(manager, FOSSILS[random.nextInt(FOSSILS.length)], pos, blockRotation));
    }
    ...
    extends SimpleStructurePiece {
        ...
        private void initializeStructureData(StructureManager manager) {
            ...
            StructurePlacementData structurePlacementData = new StructurePlacementData().setRotation(this.structureRotation).setMirrored(BlockMirror.NONE).addProcessor(BlockIgnoreStructureProcessor.IGNORE_AIR_AND_STRUCTURE_BLOCKS);
        ...
        public boolean generate(IWorld world, ChunkGenerator<?> generator, Random random, BlockBox box, ChunkPos pos) {
            while (world.getBlockState(this.pos.up()).getBlock() != Blocks.BEDROCK && this.pos.getY() < 128) {
                ...
                if (world.getBlockState(this.pos).isAir() && (blockState.getBlock() == Blocks.SOUL_SAND || blockState.isSideSolidFullSquare(world, blockPos, Direction.UP))) break;
