...
public class EndCityGenerator {
    ...
    private static final Part BUILDING = new Part(){
        ...
        @Override
        public boolean create(StructureManager structureManager, int i, Piece piece, BlockPos blockPos, List<StructurePiece> list, Random random) {
            if (i > 8) {
                return false;
            }
            BlockRotation blockRotation = piece.placementData.getRotation();
            Piece piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece, blockPos, "base_floor", blockRotation, true));
            ...
            if (j == 0) {
                ...
                piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(-1, 0, -1), "second_floor_2", blockRotation, false));
                piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(-1, 8, -1), "second_roof", blockRotation, false));
                EndCityGenerator.createPart(structureManager, SMALL_TOWER, i + 1, piece2, null, list, random);
            } else if (j == 2) {
                piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(-1, 0, -1), "second_floor_2", blockRotation, false));
                piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(-1, 4, -1), "third_floor_2", blockRotation, false));
                piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(-1, 8, -1), "third_roof", blockRotation, true));
                EndCityGenerator.createPart(structureManager, SMALL_TOWER, i + 1, piece2, null, list, random);
            }
            return true;
        }
    };
    private static final List<Pair<BlockRotation, BlockPos>> SMALL_TOWER_BRIDGE_ATTACHMENTS = Lists.newArrayList(new Pair<BlockRotation, BlockPos>(BlockRotation.NONE, new BlockPos(1, -1, 0)), new Pair<BlockRotation, BlockPos>(BlockRotation.CLOCKWISE_90, new BlockPos(6, -1, 1)), new Pair<BlockRotation, BlockPos>(BlockRotation.COUNTERCLOCKWISE_90, new BlockPos(0, -1, 5)), new Pair<BlockRotation, BlockPos>(BlockRotation.CLOCKWISE_180, new BlockPos(5, -1, 6)));
    private static final Part SMALL_TOWER = new Part(){
        ...
        public boolean create(StructureManager structureManager, int i, Piece piece, BlockPos blockPos, List<StructurePiece> list, Random random) {
            BlockRotation blockRotation = piece.placementData.getRotation();
            Piece piece2 = piece;
            piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(3 + random.nextInt(2), -3, 3 + random.nextInt(2)), "tower_base", blockRotation, true));
            ...
            if (piece3 != null) {
                for (Pair pair : SMALL_TOWER_BRIDGE_ATTACHMENTS) {
                    ...
                    Piece piece4 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece3, (BlockPos)pair.getRight(), "bridge_end", blockRotation.rotate((BlockRotation)((Object)pair.getLeft())), true));
                    EndCityGenerator.createPart(structureManager, BRIDGE_PIECE, i + 1, piece4, null, list, random);
                }
                ...
                return EndCityGenerator.createPart(structureManager, FAT_TOWER, i + 1, piece2, null, list, random);
            }
            return true;
        }
    };
    private static final Part BRIDGE_PIECE = new Part(){
        ...
        public boolean create(StructureManager structureManager, int i, Piece piece, BlockPos blockPos, List<StructurePiece> list, Random random) {
            ...
            Piece piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece, new BlockPos(0, 0, -4), "bridge_piece", blockRotation, true));
            piece2.field_15316 = -1;
            int k = 0;
            for (int l = 0; l < j; ++l) {
                if (random.nextBoolean()) {
                    piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(0, k, -4), "bridge_piece", blockRotation, true));
                    k = 0;
                    continue;
                }
                piece2 = random.nextBoolean() ? EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(0, k, -4), "bridge_steep_stairs", blockRotation, true)) : EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(0, k, -8), "bridge_gentle_stairs", blockRotation, true));
                k = 4;
            }
            if (this.shipGenerated || random.nextInt(10 - i) != 0) {
                if (!EndCityGenerator.createPart(structureManager, BUILDING, i + 1, piece2, new BlockPos(-3, k + 1, -11), list, random)) {
                    return false;
                }
            } else {
                EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(-8 + random.nextInt(8), k, -70 + random.nextInt(10)), "ship", blockRotation, true));
                this.shipGenerated = true;
            }
            piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(4, k, 0), "bridge_end", blockRotation.rotate(BlockRotation.CLOCKWISE_180), true));
            piece2.field_15316 = -1;
            return true;
        }
    };
    ...
    private static final Part FAT_TOWER = new Part(){
        ...
        public boolean create(StructureManager structureManager, int i, Piece piece, BlockPos blockPos, List<StructurePiece> list, Random random) {
            BlockRotation blockRotation = piece.placementData.getRotation();
            Piece piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece, new BlockPos(-3, 4, -3), "fat_tower_base", blockRotation, true));
            piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(0, 4, 0), "fat_tower_middle", blockRotation, true));
            for (int j = 0; j < 2 && random.nextInt(3) != 0; ++j) {
                piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(0, 8, 0), "fat_tower_middle", blockRotation, true));
                for (Pair pair : FAT_TOWER_BRIDGE_ATTACHMENTS) {
                    if (!random.nextBoolean()) continue;
                    Piece piece3 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, (BlockPos)pair.getRight(), "bridge_end", blockRotation.rotate((BlockRotation)((Object)pair.getLeft())), true));
                    EndCityGenerator.createPart(structureManager, BRIDGE_PIECE, i + 1, piece3, null, list, random);
                }
            }
            piece2 = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece2, new BlockPos(-2, 8, -2), "fat_tower_top", blockRotation, true));
            return true;
        }
    };

    private static Piece createPiece(StructureManager structureManager, Piece piece, BlockPos blockPos, String string, BlockRotation blockRotation, boolean bl) {
        Piece piece2 = new Piece(structureManager, string, piece.pos, blockRotation, bl);
    ...
    public static void addPieces(StructureManager structureManager, BlockPos blockPos, BlockRotation blockRotation, List<StructurePiece> list, Random random) {
        ...
        BRIDGE_PIECE.init();
        SMALL_TOWER.init();
        Piece piece = EndCityGenerator.addPiece(list, new Piece(structureManager, "base_floor", blockPos, blockRotation, true));
        piece = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece, new BlockPos(-1, 0, -1), "second_floor_1", blockRotation, false));
        piece = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece, new BlockPos(-1, 4, -1), "third_floor_1", blockRotation, false));
        piece = EndCityGenerator.addPiece(list, EndCityGenerator.createPiece(structureManager, piece, new BlockPos(-1, 8, -1), "third_roof", blockRotation, true));
        EndCityGenerator.createPart(structureManager, SMALL_TOWER, 1, piece, null, list, random);
    }

    private static Piece addPiece(List<StructurePiece> list, Piece piece) {
        list.add(piece);
        return piece;
    }

    private static boolean createPart(StructureManager structureManager, Part part, int i, Piece piece, BlockPos blockPos, List<StructurePiece> list, Random random) {
        if (i > 8) {
            return false;
        }
        ...
        if (part.create(structureManager, i, piece, blockPos, list2, random)) {
            ...
            for (StructurePiece structurePiece : list2) {
                ...
                if (structurePiece2 == null || structurePiece2.field_15316 == piece.field_15316) continue;
                bl = true;
                break;
            }
            if (!bl) {
