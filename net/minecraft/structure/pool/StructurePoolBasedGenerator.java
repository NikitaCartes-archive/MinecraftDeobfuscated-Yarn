...
public class StructurePoolBasedGenerator {
    ...
    public static void addPieces(Identifier identifier, int i, PieceFactory pieceFactory, ChunkGenerator<?> chunkGenerator, StructureManager structureManager, BlockPos blockPos, List<StructurePiece> list, Random random) {
        StructureFeatures.initialize();
        new class_4182(identifier, i, pieceFactory, chunkGenerator, structureManager, blockPos, list, random);
    }
    ...
    public static interface PieceFactory {
        public PoolStructurePiece create(StructureManager var1, StructurePoolElement var2, BlockPos var3, int var4, BlockRotation var5, MutableIntBoundingBox var6);
    }
    ...
    static final class class_4182 {
        ...
        public class_4182(Identifier identifier, int i, PieceFactory pieceFactory, ChunkGenerator<?> chunkGenerator, StructureManager structureManager, BlockPos blockPos, List<StructurePiece> list, Random random) {
            ...
            int m = 80;
            BoundingBox boundingBox = new BoundingBox(j - 80, l - 80, k - 80, j + 80 + 1, l + 80 + 1, k + 80 + 1);
            this.field_18706.addLast(new class_4181(poolStructurePiece, new AtomicReference<VoxelShape>(VoxelShapes.combineAndSimplify(VoxelShapes.cuboid(boundingBox), VoxelShapes.cuboid(BoundingBox.from(mutableIntBoundingBox)), BooleanBiFunction.ONLY_FIRST)), l + 80, 0));
            ...
            block0: for (Structure.StructureBlockInfo structureBlockInfo2 : structurePoolElement.getStructureBlockInfos(this.field_18703, blockPos, blockRotation, this.field_18705)) {
                ...
                while (iterator.hasNext() && (structurePoolElement2 = (StructurePoolElement)iterator.next()) != EmptyPoolElement.INSTANCE) {
                    for (BlockRotation blockRotation2 : BlockRotation.randomRotationOrder(this.field_18705)) {
                        ...
                        int o = mutableIntBoundingBox2.getBlockCountY() > 16 ? 0 : list2.stream().mapToInt(structureBlockInfo -> {
                            if (!mutableIntBoundingBox2.contains(structureBlockInfo.pos.offset(structureBlockInfo.state.get(JigsawBlock.FACING)))) {
                                return 0;
                            }
                        ...
                        for (Structure.StructureBlockInfo structureBlockInfo22 : list2) {
                            ...
                            if (o > 0) {
                                u = Math.max(o + 1, mutableIntBoundingBox4.maxY - mutableIntBoundingBox4.minY);
                                mutableIntBoundingBox4.maxY = mutableIntBoundingBox4.minY + u;
                            }
                            if (VoxelShapes.matchesAnywhere((VoxelShape)atomicReference3.get(), VoxelShapes.cuboid(BoundingBox.from(mutableIntBoundingBox4).contract(0.25)), BooleanBiFunction.ONLY_SECOND)) continue;
                            atomicReference3.set(VoxelShapes.combine((VoxelShape)atomicReference3.get(), VoxelShapes.cuboid(BoundingBox.from(mutableIntBoundingBox4)), BooleanBiFunction.ONLY_FIRST));
                            u = poolStructurePiece.getGroundLevelDelta();
                            int v = bl3 ? u - r : structurePoolElement2.method_19308();
                            PoolStructurePiece poolStructurePiece2 = this.field_18701.create(this.field_18703, structurePoolElement2, blockPos6, v, blockRotation2, mutableIntBoundingBox4);
                            if (bl) {
                                w = k + l;
                            } else if (bl3) {
                                w = s + q;
