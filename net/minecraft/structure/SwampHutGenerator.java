...
extends StructurePieceWithDimensions {
    ...
    public boolean generate(IWorld iWorld, Random random, BlockBox blockBox, ChunkPos chunkPos) {
        ...
        this.addBlock(iWorld, (BlockState)blockState.with(StairsBlock.SHAPE, StairShape.OUTER_RIGHT), 0, 4, 1, blockBox);
        this.addBlock(iWorld, (BlockState)blockState.with(StairsBlock.SHAPE, StairShape.OUTER_LEFT), 6, 4, 1, blockBox);
        this.addBlock(iWorld, (BlockState)blockState4.with(StairsBlock.SHAPE, StairShape.OUTER_LEFT), 0, 4, 8, blockBox);
        this.addBlock(iWorld, (BlockState)blockState4.with(StairsBlock.SHAPE, StairShape.OUTER_RIGHT), 6, 4, 8, blockBox);
        for (i = 2; i <= 7; i += 5) {
            for (j = 1; j <= 5; j += 4) {
                this.method_14936(iWorld, Blocks.OAK_LOG.getDefaultState(), j, -1, i, blockBox);
            }
        }
        if (!this.hasWitch && blockBox.contains(new BlockPos(i = this.applyXTransform(2, 5), j = this.applyYTransform(2), k = this.applyZTransform(2, 5)))) {
            this.hasWitch = true;
