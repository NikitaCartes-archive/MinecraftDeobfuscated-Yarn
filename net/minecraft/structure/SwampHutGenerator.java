...
extends StructurePieceWithDimensions {
    ...
    public boolean generate(IWorld iWorld, ChunkGenerator<?> chunkGenerator, Random random, MutableIntBoundingBox mutableIntBoundingBox, ChunkPos chunkPos) {
        ...
        this.addBlock(iWorld, (BlockState)blockState.with(StairsBlock.SHAPE, StairShape.OUTER_RIGHT), 0, 4, 1, mutableIntBoundingBox);
        this.addBlock(iWorld, (BlockState)blockState.with(StairsBlock.SHAPE, StairShape.OUTER_LEFT), 6, 4, 1, mutableIntBoundingBox);
        this.addBlock(iWorld, (BlockState)blockState4.with(StairsBlock.SHAPE, StairShape.OUTER_LEFT), 0, 4, 8, mutableIntBoundingBox);
        this.addBlock(iWorld, (BlockState)blockState4.with(StairsBlock.SHAPE, StairShape.OUTER_RIGHT), 6, 4, 8, mutableIntBoundingBox);
        for (i = 2; i <= 7; i += 5) {
            for (j = 1; j <= 5; j += 4) {
                this.method_14936(iWorld, Blocks.OAK_LOG.getDefaultState(), j, -1, i, mutableIntBoundingBox);
            }
        }
        if (!this.hasWitch && mutableIntBoundingBox.contains(new BlockPos(i = this.applyXTransform(2, 5), j = this.applyYTransform(2), k = this.applyZTransform(2, 5)))) {
            this.hasWitch = true;
    ...
    private void method_16181(IWorld iWorld, MutableIntBoundingBox mutableIntBoundingBox) {
        ...
        int j;
        int i;
        if (!this.hasCat && mutableIntBoundingBox.contains(new BlockPos(i = this.applyXTransform(2, 5), j = this.applyYTransform(2), k = this.applyZTransform(2, 5)))) {
            this.hasCat = true;
