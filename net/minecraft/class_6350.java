...
public interface class_6350 {
    ...
    public static class_6350 method_36382(ChunkPos chunkPos, DoublePerlinNoiseSampler doublePerlinNoiseSampler, DoublePerlinNoiseSampler doublePerlinNoiseSampler2, DoublePerlinNoiseSampler doublePerlinNoiseSampler3, ChunkGeneratorSettings chunkGeneratorSettings, NoiseColumnSampler noiseColumnSampler, int i, int j) {
        return new AquiferSampler(chunkPos, doublePerlinNoiseSampler, doublePerlinNoiseSampler2, doublePerlinNoiseSampler3, chunkGeneratorSettings, noiseColumnSampler, i, j);
    }

    public static class_6350 method_36381(final int i, final BlockState blockState) {
        return new class_6350(){

            @Override
            public BlockState apply(BlockSource blockSource, int i2, int j, int k, double d) {
                if (d > 0.0) {
                    return blockSource.sample(i2, j, k);
                }
                if (j >= i) {
    ...
    public static class AquiferSampler
    implements class_6350 {
        ...
        private final class_6351[] waterLevels;
        ...
        AquiferSampler(ChunkPos chunkPos, DoublePerlinNoiseSampler doublePerlinNoiseSampler, DoublePerlinNoiseSampler doublePerlinNoiseSampler2, DoublePerlinNoiseSampler doublePerlinNoiseSampler3, ChunkGeneratorSettings chunkGeneratorSettings, NoiseColumnSampler noiseColumnSampler, int i, int j) {
            ...
            this.waterLevels = new class_6351[o];
        ...
        public BlockState apply(BlockSource blockSource, int i, int j, int k, double d) {
            if (d <= 0.0) {
                ...
                if (this.method_35324(j)) {
                        ...
                        for (int v = -1; v <= 1; ++v) {
                            for (int w = 0; w <= 1; ++w) {
                                ...
                                if (ab != Long.MAX_VALUE) {
                                    ...
                                    this.blockPositions[aa] = ac = BlockPos.asLong(x * 16 + chunkRandom.nextInt(10), y * 12 + chunkRandom.nextInt(9), z * 16 + chunkRandom.nextInt(10));
                                }
                    ...
                    class_6351 lv = this.getWaterLevel(r);
                    class_6351 lv2 = this.getWaterLevel(s);
                    class_6351 lv3 = this.getWaterLevel(t);
                    ...
                    if (lv.field_33576 >= j && lv.field_33577.isOf(Blocks.WATER) && this.method_35324(j - 1)) {
                        ...
                        double ah = 1.0 + (this.edgeDensityNoise.sample(i, j, k) + 0.05) / 4.0;
                    ...
                    BlockState blockState2 = blockState = j >= lv.field_33576 ? Blocks.AIR.getDefaultState() : lv.field_33577;
                }
                if (d + e <= 0.0) {
        ...
        private double calculateDensity(int y, double noise, class_6351 arg, class_6351 arg2) {
        ...
        private class_6351 getWaterLevel(long pos) {
            class_6351 lv2;
            int n;
            int m;
            ...
            class_6351 lv = this.waterLevels[o];
            if (lv != null) {
                return lv;
            }
        ...
        private class_6351 getWaterLevel(int x, int y, int z) {
            int i = this.settings.getSeaLevel();
            if (y > 30) {
                return new class_6351(i, Blocks.WATER.getDefaultState());
            }
            int j = 64;
            int k = -10;
            int l = 40;
            ...
            if (m == -20) {
                double e = this.field_33575.sample(Math.floorDiv(x, 64), (double)Math.floorDiv(y, 40) / 1.4, Math.floorDiv(z, 64));
                bl = Math.abs(e) > (double)0.22f;
            }
            return new class_6351(Math.min(56, n), bl ? Blocks.LAVA.getDefaultState() : Blocks.WATER.getDefaultState());
        }
