    ...
    public void sendChatMessage(SignedMessage message, MessageSender sender, RegistryKey<MessageType> typeKey) {
        if (this.acceptsChatMessage()) {
            ...
            if (message.method_44781(sender)) {
                this.networkHandler.sendPacket(new ChatMessageS2CPacket(message.signedContent(), message.unsignedContent(), i, sender, message.signature().timestamp(), message.signature().saltSignature()));
            } else {
