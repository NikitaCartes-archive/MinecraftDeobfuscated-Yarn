...
public class CommandFunction {
    ...
    public static CommandFunction create(Identifier identifier, CommandFunctionManager commandFunctionManager, List<String> list) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            try {
                ParseResults<ServerCommandSource> parseResults = commandFunctionManager.getServer().getCommandManager().getDispatcher().parse(stringReader, commandFunctionManager.getFunctionCommandSource());
    ...
    implements Element {
        ...
        public void execute(CommandFunctionManager commandFunctionManager, ServerCommandSource serverCommandSource, ArrayDeque<CommandFunctionManager.Entry> arrayDeque, int i) throws CommandSyntaxException {
            commandFunctionManager.getDispatcher().execute(new ParseResults<ServerCommandSource>(this.parsed.getContext().withSource(serverCommandSource), this.parsed.getReader(), this.parsed.getExceptions()));
        }
