...
public class ResetChunksCommand {
    ...
    private static int executeResetChunks(ServerCommandSource source, int radius, boolean skipOldChunks) {
        ...
        source.sendFeedback(Text.literal(String.format("%d chunks have been reset. This took %d ms for %d chunks, or %02f ms per chunk", p, q, p, Float.valueOf((float)q / (float)p))), true);
        return 1;
    }
}
