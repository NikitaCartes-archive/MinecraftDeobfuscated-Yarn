...
public class ItemCommand {
    static final Dynamic3CommandExceptionType NOT_A_CONTAINER_TARGET_EXCEPTION = new Dynamic3CommandExceptionType((x, y, z) -> new TranslatableText("commands.item.target.not_a_container", x, y, z));
    private static final Dynamic3CommandExceptionType NOT_A_CONTAINER_SOURCE_EXCEPTION = new Dynamic3CommandExceptionType((x, y, z) -> new TranslatableText("commands.item.source.not_a_container", x, y, z));
    static final DynamicCommandExceptionType NO_SUCH_SLOT_TARGET_EXCEPTION = new DynamicCommandExceptionType(slot -> new TranslatableText("commands.item.target.no_such_slot", slot));
    ...
    private static int executeEntityModify(ServerCommandSource source, Collection<? extends Entity> targets, int slot, LootFunction modifier) throws CommandSyntaxException {
        ...
        for (Entity entity : targets) {
            ...
            if (commandItemSlot == CommandItemSlot.EMPTY || !commandItemSlot.set(itemStack = ItemCommand.getStackWithModifier(source, modifier, commandItemSlot.get().copy()))) continue;
    ...
    private static int executeEntityReplace(ServerCommandSource source, Collection<? extends Entity> targets, int slot, ItemStack stack) throws CommandSyntaxException {
        ...
        for (Entity entity : targets) {
            CommandItemSlot commandItemSlot = entity.getCommandItemSlot(slot);
            if (commandItemSlot == CommandItemSlot.EMPTY || !commandItemSlot.set(stack.copy())) continue;
            list.add(entity);
            if (!(entity instanceof ServerPlayerEntity)) continue;
            ((ServerPlayerEntity)entity).currentScreenHandler.sendContentUpdates();
        }
