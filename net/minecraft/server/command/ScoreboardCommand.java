...
public class ScoreboardCommand {
    ...
    private static int executeGet(ServerCommandSource serverCommandSource, String string, ScoreboardObjective scoreboardObjective) throws CommandSyntaxException {
        ...
        serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.get.success", string, scoreboardPlayerScore.getScore(), scoreboardObjective.getTextComponent()), false);
    ...
    private static int executeOperation(ServerCommandSource serverCommandSource, Collection<String> collection, ScoreboardObjective scoreboardObjective, OperationArgumentType.Operation operation, Collection<String> collection2, ScoreboardObjective scoreboardObjective2) throws CommandSyntaxException {
        ...
        if (collection.size() == 1) {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.operation.success.single", scoreboardObjective.getTextComponent(), collection.iterator().next(), i), true);
        } else {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.operation.success.multiple", scoreboardObjective.getTextComponent(), collection.size()), true);
        }
        return i;
    }
    ...
    private static int executeEnable(ServerCommandSource serverCommandSource, Collection<String> collection, ScoreboardObjective scoreboardObjective) throws CommandSyntaxException {
        ...
        if (collection.size() == 1) {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.enable.success.single", scoreboardObjective.getTextComponent(), collection.iterator().next()), true);
        } else {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.enable.success.multiple", scoreboardObjective.getTextComponent(), collection.size()), true);
        }
        return i;
    }
    ...
    private static int executeReset(ServerCommandSource serverCommandSource, Collection<String> collection, ScoreboardObjective scoreboardObjective) {
        ...
        if (collection.size() == 1) {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.reset.specific.single", scoreboardObjective.getTextComponent(), collection.iterator().next()), true);
        } else {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.reset.specific.multiple", scoreboardObjective.getTextComponent(), collection.size()), true);
        }
        return collection.size();
    }
    ...
    private static int executeSet(ServerCommandSource serverCommandSource, Collection<String> collection, ScoreboardObjective scoreboardObjective, int i) {
        ...
        if (collection.size() == 1) {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.set.success.single", scoreboardObjective.getTextComponent(), collection.iterator().next(), i), true);
        } else {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.set.success.multiple", scoreboardObjective.getTextComponent(), collection.size(), i), true);
        }
    ...
    private static int executeAdd(ServerCommandSource serverCommandSource, Collection<String> collection, ScoreboardObjective scoreboardObjective, int i) {
        ...
        if (collection.size() == 1) {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.add.success.single", i, scoreboardObjective.getTextComponent(), collection.iterator().next(), j), true);
        } else {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.add.success.multiple", i, scoreboardObjective.getTextComponent(), collection.size()), true);
        }
        return j;
    }
    ...
    private static int executeRemove(ServerCommandSource serverCommandSource, Collection<String> collection, ScoreboardObjective scoreboardObjective, int i) {
        ...
        if (collection.size() == 1) {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.remove.success.single", i, scoreboardObjective.getTextComponent(), collection.iterator().next(), j), true);
        } else {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.remove.success.multiple", i, scoreboardObjective.getTextComponent(), collection.size()), true);
        }
        return j;
    }
    ...
    private static int executeListPlayers(ServerCommandSource serverCommandSource) {
        ...
        if (collection.isEmpty()) {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.list.empty", new Object[0]), false);
        } else {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.list.success", collection.size(), Components.sortedJoin(collection)), false);
        }
        return collection.size();
    }
    ...
    private static int executeListScores(ServerCommandSource serverCommandSource, String string) {
        ...
        if (map.isEmpty()) {
            ...
            for (Map.Entry<ScoreboardObjective, ScoreboardPlayerScore> entry : map.entrySet()) {
                serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.players.list.entity.entry", entry.getKey().getTextComponent(), entry.getValue().getScore()), false);
            }
        }
        return map.size();
    }
    ...
    private static int executeSetDisplay(ServerCommandSource serverCommandSource, int i, ScoreboardObjective scoreboardObjective) throws CommandSyntaxException {
        ...
        serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.objectives.display.set", Scoreboard.getDisplaySlotNames()[i], scoreboardObjective.getDisplayName()), true);
        return 0;
    }
    ...
    private static int executeModifyObjective(ServerCommandSource serverCommandSource, ScoreboardObjective scoreboardObjective, Component component) {
        if (!scoreboardObjective.getDisplayName().equals(component)) {
            ...
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.objectives.modify.displayname", scoreboardObjective.getName(), scoreboardObjective.getTextComponent()), true);
        }
        return 0;
    }
    ...
    private static int executeListObjectives(ServerCommandSource serverCommandSource) {
        ...
        if (collection.isEmpty()) {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.objectives.list.empty", new Object[0]), false);
        } else {
            serverCommandSource.sendFeedback(new TranslatableComponent("commands.scoreboard.objectives.list.success", collection.size(), Components.join(collection, ScoreboardObjective::getTextComponent)), false);
        }
        return collection.size();
    }
}
