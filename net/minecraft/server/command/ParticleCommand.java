...
public class ParticleCommand {
    private static final SimpleCommandExceptionType FAILED_EXCPETION = new SimpleCommandExceptionType(new TranslatableComponent("commands.particle.failed", new Object[0]));
    ...
    private static int execute(ServerCommandSource serverCommandSource, ParticleEffect particleEffect, Vec3d vec3d, Vec3d vec3d2, float f, int i, boolean bl, Collection<ServerPlayerEntity> collection) throws CommandSyntaxException {
        ...
        serverCommandSource.sendFeedback(new TranslatableComponent("commands.particle.success", Registry.PARTICLE_TYPE.getId(particleEffect.getType()).toString()), true);
        return j;
    }
}
