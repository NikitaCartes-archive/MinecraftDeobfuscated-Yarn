...
SaveProperties {
    ...
    private void updateProperties(DynamicRegistryManager registryManager, CompoundTag levelTag, @Nullable CompoundTag playerTag) {
        ...
        GeneratorOptions.CODEC.encodeStart(registryReadingOps, this.generatorOptions).resultOrPartial(Util.addPrefix("WorldGenSettings: ", LOGGER::error)).ifPresent(tag -> levelTag.put("WorldGenSettings", (Tag)tag));
        ...
        DataPackSettings.CODEC.encodeStart(NbtOps.INSTANCE, this.levelInfo.getDataPackSettings()).result().ifPresent(tag -> levelTag.put("DataPacks", (Tag)tag));
