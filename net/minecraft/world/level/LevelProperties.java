...
public class LevelProperties {
    ...
    public LevelProperties(CompoundTag tag, DataFixer dataFixer, int dataVersion, @Nullable CompoundTag playerData) {
        ...
        if (tag.contains("generatorName", 8)) {
            ...
            Object levelGeneratorType = LevelGeneratorType.getTypeFromName(string);
            ...
            } else if (((LevelGeneratorType)levelGeneratorType).isVersioned()) {
                int j = 0;
                if (tag.contains("generatorVersion", 99)) {
                    j = tag.getInt("generatorVersion");
                }
                levelGeneratorType = ((LevelGeneratorType)levelGeneratorType).getTypeForVersion(j);
            }
            ...
            Dynamic<CompoundTag> dynamic2 = LevelProperties.updateGeneratorOptionsData((LevelGeneratorType)levelGeneratorType, dynamic, dataVersion, dataFixer);
            this.generatorOptions = ((LevelGeneratorType)levelGeneratorType).loadOptions(dynamic2);
        }
        ...
        this.structures = tag.contains("MapFeatures", 99) ? tag.getBoolean("MapFeatures") : true;
        this.spawnX = tag.getInt("SpawnX");
        this.spawnY = tag.getInt("SpawnY");
        this.spawnZ = tag.getInt("SpawnZ");
        this.time = tag.getLong("Time");
        this.timeOfDay = tag.contains("DayTime", 99) ? tag.getLong("DayTime") : this.time;
        ...
        this.initialized = tag.contains("initialized", 99) ? tag.getBoolean("initialized") : true;
        this.commandsAllowed = tag.contains("allowCommands", 99) ? tag.getBoolean("allowCommands") : this.gameMode == GameMode.CREATIVE;
    ...
    private void updateProperties(CompoundTag levelTag, CompoundTag playerTag) {
        ...
        for (Map.Entry<DimensionType, CompoundTag> entry : this.worldData.entrySet()) {
            compoundTag3.put(String.valueOf(entry.getKey().getRawId()), entry.getValue());
        }
    ...
    public void populateCrashReport(CrashReportSection section) {
        section.add("Level name", () -> this.levelName);
        section.add("Level seed", () -> String.valueOf(this.randomSeed));
        section.add("Level generator", () -> String.format("ID %02d - %s, ver %d. Features enabled: %b", this.generatorOptions.getType().getId(), this.generatorOptions.getType().getName(), this.generatorOptions.getType().getVersion(), this.structures));
        section.add("Level generator options", () -> this.generatorOptions.getDynamic().toString());
        section.add("Level spawn location", () -> CrashReportSection.createPositionString(this.spawnX, this.spawnY, this.spawnZ));
        section.add("Level time", () -> String.format("%d game time, %d day time", this.time, this.timeOfDay));
        section.add("Known server brands", () -> String.join((CharSequence)", ", this.serverBrands));
        section.add("Level was modded", () -> Boolean.toString(this.modded));
        ...
        section.add("Level weather", () -> String.format("Rain time: %d (now: %b), thunder time: %d (now: %b)", this.rainTime, this.raining, this.thunderTime, this.thundering));
        section.add("Level game mode", () -> String.format("Game mode: %s (ID %d). Hardcore: %b. Cheats: %b", this.gameMode.getName(), this.gameMode.getId(), this.hardcore, this.commandsAllowed));
    }

    public CompoundTag getWorldData(DimensionType type) {
        CompoundTag compoundTag = this.worldData.get(type);
        if (compoundTag == null) {
