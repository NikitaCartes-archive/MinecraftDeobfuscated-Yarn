...
public class SaveVersionInfo {
    ...
    public static SaveVersionInfo fromDynamic(Dynamic<?> dynamic) {
        ...
        if (optionalDynamic.result().isPresent()) {
            return new SaveVersionInfo(i, l, optionalDynamic.get("Name").asString(SharedConstants.getGameVersion().getName()), optionalDynamic.get("Id").asInt(SharedConstants.getGameVersion().getSaveVersion().getId()), optionalDynamic.get("Series").asString(SaveVersion.MAIN_SERIES), optionalDynamic.get("Snapshot").asBoolean(!SharedConstants.getGameVersion().isStable()));
        }
        return new SaveVersionInfo(i, l, "", 0, SaveVersion.MAIN_SERIES, false);
    }
