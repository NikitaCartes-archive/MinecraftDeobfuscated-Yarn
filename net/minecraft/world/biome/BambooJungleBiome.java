...
extends Biome {
    public BambooJungleBiome() {
        super(new Biome.Settings().configureSurfaceBuilder(SurfaceBuilder.DEFAULT, SurfaceBuilder.GRASS_CONFIG).precipitation(Biome.Precipitation.RAIN).category(Biome.Category.JUNGLE).depth(0.1f).scale(0.2f).temperature(0.95f).downfall(0.9f).effects(new BiomeEffects.Builder().waterColor(4159204).waterFogColor(329011).fogColor(12638463).moodSound(SoundEvents.AMBIENT_CAVE).build()).parent(null).noises(ImmutableList.of(new Biome.MixedNoisePoint(0.5f, 0.5f, 0.0f, 0.2f, 1.0f))));
