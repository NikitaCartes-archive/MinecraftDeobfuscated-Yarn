...
extends BiomeSource {
    ...
    public MultiNoiseBiomeSource(long seed, List<Pair<Biome.MixedNoisePoint, Biome>> list, Optional<Preset> optional) {
    ...
    private static MultiNoiseBiomeSource method_28467(long l) {
        ...
        return new MultiNoiseBiomeSource(l, immutableList.stream().flatMap(biome -> biome.streamNoises().map(point -> Pair.of(point, biome))).collect(ImmutableList.toImmutableList()), Optional.of(Preset.NETHER));
    }
