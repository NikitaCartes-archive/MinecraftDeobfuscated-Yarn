...
extends BiomeSource {
    ...
    public static MultiNoiseBiomeSource method_27986(long l, List<Biome> list) {
        return new MultiNoiseBiomeSource(l, list.stream().flatMap(biome -> biome.streamNoises().map(mixedNoisePoint -> Pair.of(mixedNoisePoint, biome))).collect(ImmutableList.toImmutableList()));
    }
