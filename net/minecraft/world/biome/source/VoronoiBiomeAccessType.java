...
public enum VoronoiBiomeAccessType implements BiomeAccessType
{
    ...
    public Biome getBiome(long l, int i, int j, int k, BiomeAccess.Storage storage) {
        ...
        for (s = 0; s < 8; ++s) {
            ...
            ds[s] = VoronoiBiomeAccessType.calcChance(l, t, u, v, g, h, w);
        }
        s = 0;
        double x = ds[0];
        for (y = 1; y < 8; ++y) {
        ...
        t = (s & 2) == 0 ? q : q + 1;
        u = (s & 1) == 0 ? r : r + 1;
        return storage.getBiomeForNoiseGen(y, t, u);
    }
    ...
    private static double calcChance(long l, int i, int j, int k, double d, double e, double f) {
        ...
        return VoronoiBiomeAccessType.sqr(f + n) + VoronoiBiomeAccessType.sqr(e + h) + VoronoiBiomeAccessType.sqr(d + g);
    }
