...
extends Biome {
    public ShatteredSavannaBiome() {
        super(new Biome.Settings().configureSurfaceBuilder(ConfiguredSurfaceBuilders.SHATTERED_SAVANNA).precipitation(Biome.Precipitation.NONE).category(Biome.Category.SAVANNA).depth(0.3625f).scale(1.225f).temperature(1.1f).downfall(0.0f).effects(new BiomeEffects.Builder().waterColor(4159204).waterFogColor(329011).fogColor(12638463).moodSound(BiomeMoodSound.CAVE).build()).parent("savanna"));
