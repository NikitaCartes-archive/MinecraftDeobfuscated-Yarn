...
public abstract class StructureType {
    public static final Codec<StructureType> STRUCTURE_TYPE_CODEC = Registry.STRUCTURE_TYPE.getCodec().dispatch(StructureType::getType, net.minecraft.structure.StructureType::codec);
    ...
    public static <S extends StructureType> Codec<S> createCodec(Function<Config, S> featureCreator) {
        return RecordCodecBuilder.create(instance -> instance.group(StructureType.configCodecBuilder(instance)).apply((Applicative)instance, featureCreator));
    }

    protected StructureType(Config config) {
        this.config = config;
    }
    ...
    protected static int getMinCornerHeight(Context context, int width, int height) {
        ...
        return StructureType.getMinCornerHeight(context, i, j, width, height);
    }

    protected static int getMinCornerHeight(Context context, int x, int z, int width, int height) {
        int[] is = StructureType.getCornerHeights(context, x, width, z, height);
    ...
    protected BlockPos getShiftedPos(Context context, BlockRotation rotation) {
        ...
        return new BlockPos(k, StructureType.getMinCornerHeight(context, k, l, i, j), l);
    }
