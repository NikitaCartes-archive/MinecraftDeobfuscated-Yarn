...
extends FoliagePlacer {
    ...
    protected void generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, AbstractRandom abstractRandom, TreeFeatureConfig config, int trunkHeight, FoliagePlacer.TreeNode treeNode, int foliageHeight, int radius, int offset) {
        ...
        for (int i = 0; i < this.leafPlacementAttempts; ++i) {
            mutable.set(blockPos, abstractRandom.nextInt(radius) - abstractRandom.nextInt(radius), abstractRandom.nextInt(foliageHeight) - abstractRandom.nextInt(foliageHeight), abstractRandom.nextInt(radius) - abstractRandom.nextInt(radius));
            RandomSpreadFoliagePlacer.placeFoliageBlock(world, replacer, abstractRandom, config, mutable);
        }
    }
