...
extends ChunkGenerator {
    ...
    public SurfaceChunkGenerator(BiomeSource biomeSource, long seed, T config, int horizontalNoiseResolution, int verticalNoiseResolution, int worldHeight, boolean useSimplexNoise) {
        super(biomeSource, ((class_5284)config).getConfig());
        this.verticalNoiseResolution = verticalNoiseResolution;
        this.horizontalNoiseResolution = horizontalNoiseResolution;
        this.defaultBlock = ((class_5284)config).getDefaultBlock();
        this.defaultFluid = ((class_5284)config).getDefaultFluid();
        ...
        this.bedrockFloorHeight = ((class_5284)config).getBedrockFloorY();
        this.bedrockCeilingHeight = ((class_5284)config).getBedrockCeilingY();
    }
    ...
    protected void sampleNoiseColumn(double[] buffer, int x, int z, double horizontalScale, double verticalScale, double horizontalStretch, double verticalStretch, int topInterpolationFactor, int topEndValue) {
        ...
        for (int i = 0; i < this.getNoiseSizeY(); ++i) {
            ...
            if ((double)i > f) {
                h = MathHelper.clampedLerp(h, topEndValue, ((double)i - f) / (double)topInterpolationFactor);
            } else if ((double)i < g) {
