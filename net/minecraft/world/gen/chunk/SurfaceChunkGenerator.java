...
extends ChunkGenerator<T> {
    ...
    public SurfaceChunkGenerator(IWorld iWorld, BiomeSource biomeSource, int i, int j, int k, T chunkGeneratorConfig, boolean bl) {
        ...
        this.defaultBlock = ((ChunkGeneratorConfig)chunkGeneratorConfig).getDefaultBlock();
        this.defaultFluid = ((ChunkGeneratorConfig)chunkGeneratorConfig).getDefaultFluid();
        ...
        this.surfaceDepthNoise = bl ? new OctaveSimplexNoiseSampler(this.random, 4) : new OctavePerlinNoiseSampler(this.random, 4);
    }
    ...
    private double sampleNoise(int i, int j, int k, double d, double e, double f, double g) {
        ...
        for (int o = 0; o < 16; ++o) {
            ...
            if (o < 8) {
                m += this.field_16575.getOctave(o).sample(OctavePerlinNoiseSampler.maintainPrecision((double)i * f * n), OctavePerlinNoiseSampler.maintainPrecision((double)j * g * n), OctavePerlinNoiseSampler.maintainPrecision((double)k * f * n), g * n, (double)j * g * n) / n;
            }
            n /= 2.0;
        }
    ...
    public int getHeightOnGround(int i, int j, Heightmap.Type type) {
        ...
        for (int p = this.noiseSizeY - 1; p >= 0; --p) {
            ...
            for (int v = this.verticalNoiseResolution - 1; v >= 0; --v) {
                ...
                if (!type.getBlockPredicate().test(blockState)) continue;
                return y + 1;
            }
        }
        return 0;
