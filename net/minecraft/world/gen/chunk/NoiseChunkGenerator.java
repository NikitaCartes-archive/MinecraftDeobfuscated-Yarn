...
extends ChunkGenerator {
    ...
    private NoiseChunkGenerator(BiomeSource populationSource, BiomeSource biomeSource, long seed, Supplier<ChunkGeneratorSettings> settings) {
        ...
        this.noiseColumnSampler = new NoiseColumnSampler(populationSource, this.horizontalNoiseResolution, this.verticalNoiseResolution, this.noiseSizeY, generationShapeConfig, interpolatedNoise, simplexNoiseSampler, octavePerlinNoiseSampler);
    }
    ...
    private OptionalInt sampleHeightmap(int x, int z, @Nullable BlockState[] states, @Nullable Predicate<BlockState> predicate, int minY, int noiseSizeY) {
        ...
        double[][] ds = new double[][]{this.sampleNoiseColumn(i, j, minY, noiseSizeY), this.sampleNoiseColumn(i, j + 1, minY, noiseSizeY), this.sampleNoiseColumn(i + 1, j, minY, noiseSizeY), this.sampleNoiseColumn(i + 1, j + 1, minY, noiseSizeY)};
    ...
    protected BlockState getBlockState(StructureWeightSampler structures, int x, int y, int z, double noise) {
        ...
        d = d / 2.0 - d * d * d / 24.0;
        BlockState blockState = (d += structures.getWeight(x, y, z)) > 0.0 ? this.defaultBlock : (y < this.getSeaLevel() ? this.defaultFluid : AIR);
        return blockState;
    }
    ...
    private void buildBedrock(Chunk chunk, Random random) {
        ...
        boolean bl3 = bl2 = k + 5 - 1 >= chunk.getBottomY() && k < chunk.getTopY();
        if (!bl && !bl2) {
            return;
        }
