...
extends ChunkGenerator {
    public static final Codec<NoiseChunkGenerator> CODEC = RecordCodecBuilder.create(instance -> instance.group(RegistryOps.createRegistryCodec(Registry.NOISE_WORLDGEN).forGetter(noiseChunkGenerator -> noiseChunkGenerator.noiseRegistry), RegistryOps.createRegistryCodec(Registry.CONFIGURED_STRUCTURE_FEATURE_KEY).forGetter(noiseChunkGenerator -> noiseChunkGenerator.field_36536), ((MapCodec)BiomeSource.CODEC.fieldOf("biome_source")).forGetter(noiseChunkGenerator -> noiseChunkGenerator.populationSource), ((MapCodec)Codec.LONG.fieldOf("seed")).stable().forGetter(noiseChunkGenerator -> noiseChunkGenerator.seed), ((MapCodec)ChunkGeneratorSettings.REGISTRY_CODEC.fieldOf("settings")).forGetter(noiseChunkGenerator -> noiseChunkGenerator.settings)).apply((Applicative<NoiseChunkGenerator, ?>)instance, instance.stable(NoiseChunkGenerator::new)));
    ...
    public NoiseChunkGenerator(Registry<DoublePerlinNoiseSampler.NoiseParameters> noiseRegistry, Registry<ConfiguredStructureFeature<?, ?>> registry, BiomeSource biomeSource, long l, RegistryEntry<ChunkGeneratorSettings> registryEntry) {
        this(noiseRegistry, registry, biomeSource, biomeSource, l, registryEntry);
    }

    private NoiseChunkGenerator(Registry<DoublePerlinNoiseSampler.NoiseParameters> noiseRegistry, Registry<ConfiguredStructureFeature<?, ?>> registry, BiomeSource biomeSource, BiomeSource biomeSource2, long l2, RegistryEntry<ChunkGeneratorSettings> registryEntry) {
        super(registry, biomeSource, biomeSource2, registryEntry.value().getStructuresConfig(), l2);
        ...
        this.field_36571 = class_6954.method_40544(generationShapeConfig, chunkGeneratorSettings.hasNoiseCaves(), chunkGeneratorSettings.hasNoodleCaves(), l2, noiseRegistry, chunkGeneratorSettings.getRandomProvider());
        this.noiseColumnSampler = new MultiNoiseUtil.MultiNoiseSampler(this.field_36571.temperature(), this.field_36571.humidity(), this.field_36571.continentalness(), this.field_36571.erosion(), this.field_36571.depth(), this.field_36571.weirdness(), this.field_36571.spawnTarget());
        ...
        this.surfaceBuilder = new SurfaceBuilder(noiseRegistry, this.defaultBlock, i, l2, chunkGeneratorSettings.getRandomProvider());
    }
    ...
    private void populateBiomes(Blender blender, StructureAccessor structureAccessor, Chunk chunk) {
        ChunkNoiseSampler chunkNoiseSampler = chunk.getOrCreateChunkNoiseSampler(this.field_36571, () -> new StructureWeightSampler(structureAccessor, chunk), this.settings.value(), this.fluidLevelSampler, blender);
    ...
    public void method_40450(List<String> list, BlockPos blockPos) {
        ...
        list.add("NoiseRouter T: " + decimalFormat.format(this.field_36571.temperature().method_40464(lv)) + " H: " + decimalFormat.format(this.field_36571.humidity().method_40464(lv)) + " C: " + decimalFormat.format(this.field_36571.continentalness().method_40464(lv)) + " E: " + decimalFormat.format(this.field_36571.erosion().method_40464(lv)) + " D: " + decimalFormat.format(this.field_36571.depth().method_40464(lv)) + " W: " + decimalFormat.format(d) + " PV: " + decimalFormat.format(VanillaTerrainParameters.getNormalizedWeirdness((float)d)) + " AS: " + decimalFormat.format(this.field_36571.initialDensityNoJaggedness().method_40464(lv)) + " N: " + decimalFormat.format(this.field_36571.fullNoise().method_40464(lv)));
    }
    ...
    public void buildSurface(ChunkRegion region, StructureAccessor structures, Chunk chunk) {
        ...
        ChunkGeneratorSettings chunkGeneratorSettings = this.settings.value();
        ChunkNoiseSampler chunkNoiseSampler = chunk.getOrCreateChunkNoiseSampler(this.field_36571, () -> new StructureWeightSampler(structures, chunk), chunkGeneratorSettings, this.fluidLevelSampler, Blender.getBlender(region));
    ...
    public void carve(ChunkRegion chunkRegion, long seed, BiomeAccess biomeAccess, StructureAccessor structureAccessor, Chunk chunk, GenerationStep.Carver generationStep) {
        ...
        ChunkNoiseSampler chunkNoiseSampler = chunk.getOrCreateChunkNoiseSampler(this.field_36571, () -> new StructureWeightSampler(structureAccessor, chunk), this.settings.value(), this.fluidLevelSampler, Blender.getBlender(chunkRegion));
        ...
        for (int j = -8; j <= 8; ++j) {
            for (int k = -8; k <= 8; ++k) {
                ...
                GenerationSettings generationSettings = chunk2.setBiomeIfAbsent(() -> this.populationSource.getBiome(BiomeCoords.fromBlock(chunkPos2.getStartX()), 0, BiomeCoords.fromBlock(chunkPos2.getStartZ()), this.getMultiNoiseSampler())).value().getGenerationSettings();
    ...
    private Chunk populateNoise(Blender blender, StructureAccessor structureAccessor, Chunk chunk, int i, int j) {
        ChunkGeneratorSettings chunkGeneratorSettings = this.settings.value();
        ChunkNoiseSampler chunkNoiseSampler = chunk.getOrCreateChunkNoiseSampler(this.field_36571, () -> new StructureWeightSampler(structureAccessor, chunk), chunkGeneratorSettings, this.fluidLevelSampler, blender);
                    ...
                    for (int t = n - 1; t >= 0; --t) {
                        ...
                        for (int x = 0; x < m; ++x) {
                            ...
                            for (int aa = 0; aa < m; ++aa) {
                                ...
                                if ((blockState = this.getBlockState(chunkNoiseSampler, y, u, ab, blockState)) == AIR || SharedConstants.method_37896(chunk.getPos())) continue;
    ...
    @Deprecated
    public Optional<BlockState> applyMaterialRule(CarverContext context, Function<BlockPos, RegistryEntry<Biome>> posToBiome, Chunk chunk, ChunkNoiseSampler chunkNoiseSampler, BlockPos pos, boolean hasFluid) {
