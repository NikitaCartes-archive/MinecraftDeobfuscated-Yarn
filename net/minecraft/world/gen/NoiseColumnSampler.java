...
implements MultiNoiseUtil.MultiNoiseSampler {
    ...
    public NoiseColumnSampler(GenerationShapeConfig config, boolean hasNoiseCaves, long seed, Registry<DoublePerlinNoiseSampler.NoiseParameters> noiseRegistry, ChunkRandom.RandomProvider randomProvider) {
        ...
        this.initialNoiseSampler = chunkNoiseSampler -> chunkNoiseSampler.createNoiseInterpolator((x, y, z) -> this.sampleNoiseColumn(x, y, z, chunkNoiseSampler.createMultiNoisePoint(BiomeCoords.fromBlock(x), BiomeCoords.fromBlock(z)).terrainInfo(), chunkNoiseSampler.getBlender()));
        ...
        this.amplified = config.amplified();
        ...
        if (randomProvider != ChunkRandom.RandomProvider.LEGACY) {
            ...
            this.temperatureNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, bl ? NoiseParametersKeys.TEMPERATURE_LARGE : NoiseParametersKeys.TEMPERATURE);
            this.humidityNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, bl ? NoiseParametersKeys.VEGETATION_LARGE : NoiseParametersKeys.VEGETATION);
            this.shiftNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.OFFSET);
        } else {
            this.terrainNoise = new InterpolatedNoiseSampler(randomProvider.create(seed), config.sampling(), config.horizontalBlockSize(), config.verticalBlockSize());
        ...
        this.aquiferFluidLevelFloodednessNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.AQUIFER_FLUID_LEVEL_FLOODEDNESS);
        ...
        this.aquiferFluidLevelSpreadNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.AQUIFER_FLUID_LEVEL_SPREAD);
        ...
        this.continentalnessNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, bl ? NoiseParametersKeys.CONTINENTALNESS_LARGE : NoiseParametersKeys.CONTINENTALNESS);
        this.erosionNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, bl ? NoiseParametersKeys.EROSION_LARGE : NoiseParametersKeys.EROSION);
        this.weirdnessNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.RIDGE);
        this.oreFrequencyNoiseSamplerFactory = NoiseColumnSampler.createNoiseSamplerFactory(NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.ORE_VEININESS), j, k, 0, 1.5);
        this.firstOrePlacementNoiseFactory = NoiseColumnSampler.createNoiseSamplerFactory(NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.ORE_VEIN_A), j, k, 0, 4.0);
        this.secondOrePlacementNoiseFactory = NoiseColumnSampler.createNoiseSamplerFactory(NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.ORE_VEIN_B), j, k, 0, 4.0);
        this.oreGapNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.ORE_GAP);
        this.noodleNoiseFactory = NoiseColumnSampler.createNoiseSamplerFactory(NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.NOODLE), l, m, -1, 1.0);
        this.noodleThicknessNoiseFactory = NoiseColumnSampler.createNoiseSamplerFactory(NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.NOODLE_THICKNESS), l, m, 0, 1.0);
        this.noodleRidgeFirstNoiseFactory = NoiseColumnSampler.createNoiseSamplerFactory(NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.NOODLE_RIDGE_A), l, m, 0, 2.6666666666666665);
        this.noodleRidgeSecondNoiseFactory = NoiseColumnSampler.createNoiseSamplerFactory(NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.NOODLE_RIDGE_B), l, m, 0, 2.6666666666666665);
        this.jaggedNoise = NoiseParametersKeys.createNoiseSampler(noiseRegistry, randomDeriver, NoiseParametersKeys.JAGGED);
    }
    ...
    private double sampleNoiseColumn(int x, int y, int z, TerrainNoisePoint point, double noise, boolean hasNoNoiseCaves, boolean bl, Blender blender) {
        ...
        j = this.applySlides(j, y / this.config.verticalBlockSize());
    ...
    private double applySlides(double noise, int y) {
        int i = y - this.config.minimumBlockY();
        noise = this.config.topSlide().method_38414(noise, this.config.verticalBlockCount() - i);
    ...
    protected int method_38383(int x, int z, TerrainNoisePoint point) {
        for (int i = this.config.minimumBlockY() + this.config.verticalBlockCount(); i >= this.config.minimumBlockY(); --i) {
            int j = i * this.config.verticalBlockSize();
            double d = -0.703125;
    ...
    protected AquiferSampler createAquiferSampler(ChunkNoiseSampler chunkNoiseSampler, int x, int z, int minimumY, int height, AquiferSampler.FluidLevelSampler fluidLevelSampler, boolean hasAquifers) {
        ...
        return AquiferSampler.aquifer(chunkNoiseSampler, new ChunkPos(i, j), this.aquiferBarrierNoise, this.aquiferFluidLevelFloodednessNoise, this.aquiferFluidLevelSpreadNoise, this.aquiferLavaNoise, this.aquiferRandomDeriver, minimumY * this.config.verticalBlockSize(), height * this.config.verticalBlockSize(), fluidLevelSampler);
    }

    @Debug
    public class_6747 method_39330(int i, int j, Blender blender) {
    ...
    public TerrainNoisePoint createTerrainNoisePoint(int x, int z, float continentalness, float weirdness, float erosion, Blender blender) {
        ...
        TerrainNoisePoint terrainNoisePoint = new TerrainNoisePoint(f, g, h);
    ...
    private double sampleSpaghetti2dNoise(int x, int y, int z) {
        ...
        int l = this.config.minimumBlockY();
        int m = 8;
