...
public class StructureAccessor {
    ...
    public List<StructureStart> method_41035(ChunkPos chunkPos, Predicate<Structure> predicate) {
        ...
        for (Map.Entry<Structure, LongSet> entry : map.entrySet()) {
            ...
            this.method_41032(structure, entry.getValue(), builder::add);
        }
    ...
    public StructureStart getStructureContaining(BlockPos pos, TagKey<Structure> structureTag) {
        ...
        for (StructureStart structureStart : this.method_41035(new ChunkPos(pos), structure -> registry.getEntry(registry.getRawId((Structure)structure)).map(registryEntry -> registryEntry.isIn(structureTag)).orElse(false))) {
