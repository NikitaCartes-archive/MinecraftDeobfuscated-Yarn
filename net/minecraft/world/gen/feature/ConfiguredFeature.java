...
public record ConfiguredFeature<FC extends FeatureConfig, F extends Feature<FC>>(F feature, FC config) {
    ...
    public static final Codec<ConfiguredFeature<?, ?>> NETWORK_CODEC = Codec.unit(() -> new ConfiguredFeature<DefaultFeatureConfig, Feature<DefaultFeatureConfig>>(Feature.NO_OP, DefaultFeatureConfig.INSTANCE));
    ...
    public boolean generate(StructureWorldAccess structureWorldAccess, ChunkGenerator chunkGenerator, Random random, BlockPos origin) {
        return ((Feature)this.feature).method_40163(this.config, structureWorldAccess, chunkGenerator, random, origin);
    }
