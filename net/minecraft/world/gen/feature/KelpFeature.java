...
extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(ServerWorldAccess serverWorldAccess, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockPos blockPos, DefaultFeatureConfig defaultFeatureConfig) {
        ...
        if (serverWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            ...
            for (int l = 0; l <= k; ++l) {
                if (serverWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER) && serverWorldAccess.getBlockState(blockPos2.up()).isOf(Blocks.WATER) && blockState2.canPlaceAt(serverWorldAccess, blockPos2)) {
                    if (l == k) {
                        serverWorldAccess.setBlockState(blockPos2, (BlockState)blockState.with(KelpBlock.AGE, random.nextInt(4) + 20), 2);
                        ++i;
                    } else {
                    ...
                    if (!blockState.canPlaceAt(serverWorldAccess, blockPos3) || serverWorldAccess.getBlockState(blockPos3.down()).isOf(Blocks.KELP)) break;
                    serverWorldAccess.setBlockState(blockPos3, (BlockState)blockState.with(KelpBlock.AGE, random.nextInt(4) + 20), 2);
                    ++i;
                    break;
                }
