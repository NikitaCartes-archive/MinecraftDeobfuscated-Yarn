...
extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            ...
            int k = 1 + abstractRandom.nextInt(10);
            for (int l = 0; l <= k; ++l) {
                if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER) && structureWorldAccess.getBlockState(blockPos2.up()).isOf(Blocks.WATER) && blockState2.canPlaceAt(structureWorldAccess, blockPos2)) {
                    if (l == k) {
                        structureWorldAccess.setBlockState(blockPos2, (BlockState)blockState.with(KelpBlock.AGE, abstractRandom.nextInt(4) + 20), Block.NOTIFY_LISTENERS);
                        ++i;
                    } else {
                    ...
                    structureWorldAccess.setBlockState(blockPos3, (BlockState)blockState.with(KelpBlock.AGE, abstractRandom.nextInt(4) + 20), Block.NOTIFY_LISTENERS);
                    ++i;
                    break;
                }
