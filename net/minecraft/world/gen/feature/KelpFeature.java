...
extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(IWorld iWorld, StructureAccessor structureAccessor, ChunkGenerator<? extends ChunkGeneratorConfig> chunkGenerator, Random random, BlockPos blockPos, DefaultFeatureConfig defaultFeatureConfig) {
        ...
        if (iWorld.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            ...
            for (int l = 0; l <= k; ++l) {
                if (iWorld.getBlockState(blockPos2).isOf(Blocks.WATER) && iWorld.getBlockState(blockPos2.up()).isOf(Blocks.WATER) && blockState2.canPlaceAt(iWorld, blockPos2)) {
                    if (l == k) {
                    ...
                    if (!blockState.canPlaceAt(iWorld, blockPos3) || iWorld.getBlockState(blockPos3.down()).isOf(Blocks.KELP)) break;
