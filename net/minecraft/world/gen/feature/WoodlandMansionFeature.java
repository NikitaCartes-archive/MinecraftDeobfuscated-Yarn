...
extends StructureFeature<DefaultFeatureConfig> {
    ...
    private static void method_38694(StructureWorldAccess structureWorldAccess, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox blockBox, ChunkPos chunkPos, class_6624 arg) {
        ...
        for (int k = blockBox.getMinX(); k <= blockBox.getMaxX(); ++k) {
            block1: for (int l = blockBox.getMinZ(); l <= blockBox.getMaxZ(); ++l) {
                mutable.set(k, j, l);
                if (structureWorldAccess.isAir(mutable) || !blockBox2.contains(mutable) || !arg.method_38710(mutable)) continue;
                for (int m = j - 1; m > i; --m) {
                    mutable.setY(m);
