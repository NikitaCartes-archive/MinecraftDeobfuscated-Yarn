...
public class AbstractTreeFeatureConfig
implements FeatureConfig {
    ...
    public static <T> AbstractTreeFeatureConfig deserialize(Dynamic<T> dynamic2) {
        ...
        return new AbstractTreeFeatureConfig((StateProvider)stateProviderType.deserialize(dynamic2.get("trunk_provider").orElseEmptyMap()), (StateProvider)stateProviderType2.deserialize(dynamic2.get("leaves_provider").orElseEmptyMap()), dynamic2.get("decorators").asList(dynamic -> Registry.TREE_DECORATOR_TYPE.get(new Identifier(dynamic.get("type").asString().orElseThrow(RuntimeException::new))).method_23472((Dynamic<?>)dynamic)), dynamic2.get("base_height").asInt(0));
    }
