...
public record NoiseConfig(RandomDeriver random, long legacyWorldSeed, Registry<DoublePerlinNoiseSampler.NoiseParameters> noiseParametersRegistry, NoiseRouter router, MultiNoiseUtil.MultiNoiseSampler sampler, SurfaceBuilder surfaceBuilder, RandomDeriver aquiferRandom, RandomDeriver oreRandom, Map<RegistryKey<DoublePerlinNoiseSampler.NoiseParameters>, DoublePerlinNoiseSampler> noiseIntances, Map<Identifier, RandomDeriver> randomDerivers) {
    ...
    public static NoiseConfig create(ChunkGeneratorSettings chunkGeneratorSettings, Registry<DoublePerlinNoiseSampler.NoiseParameters> noiseParametersRegistry, long legacyWorldSeed) {
        ...
        NoiseRouter noiseRouter = chunkGeneratorSettings.method_41542(noiseParametersRegistry, new class_7139(randomDeriver, chunkGeneratorSettings.usesLegacyRandom(), legacyWorldSeed));
        MultiNoiseUtil.MultiNoiseSampler multiNoiseSampler = new MultiNoiseUtil.MultiNoiseSampler(noiseRouter.temperature(), noiseRouter.vegetation(), noiseRouter.continents(), noiseRouter.erosion(), noiseRouter.depth(), noiseRouter.ridges(), chunkGeneratorSettings.spawnTarget());
        return new NoiseConfig(randomDeriver, legacyWorldSeed, noiseParametersRegistry, noiseRouter, multiNoiseSampler, new SurfaceBuilder(noiseParametersRegistry, chunkGeneratorSettings.defaultBlock(), chunkGeneratorSettings.seaLevel(), randomDeriver), randomDeriver.createRandom(new Identifier("aquifer")).createRandomDeriver(), randomDeriver.createRandom(new Identifier("ore")).createRandomDeriver(), new ConcurrentHashMap<RegistryKey<DoublePerlinNoiseSampler.NoiseParameters>, DoublePerlinNoiseSampler>(), new ConcurrentHashMap<Identifier, RandomDeriver>());
    }
    ...
    public DoublePerlinNoiseSampler getOrCreateSampler(RegistryKey<DoublePerlinNoiseSampler.NoiseParameters> noiseParametersKey) {
        return this.noiseIntances.computeIfAbsent(noiseParametersKey, key -> NoiseParametersKeys.createNoiseSampler(this.noiseParametersRegistry, this.random, noiseParametersKey));
    }

    public RandomDeriver getOrCreateRandomDeriver(Identifier id) {
        return this.randomDerivers.computeIfAbsent(id, id2 -> this.random.createRandom(id).createRandomDeriver());
    }
}
