...
extends TreeDecorator {
    ...
    public <T> AlterGroundTreeDecorator(Dynamic<T> dynamic) {
        this((class_4651)Registry.BLOCK_STATE_PROVIDER_TYPE.get(new Identifier(dynamic.get("provider").get("type").asString().orElseThrow(RuntimeException::new))).method_23456(dynamic.get("provider").orElseEmptyMap()));
    }
    ...
    private void method_23463(ModifiableTestableWorld modifiableTestableWorld, Random random, BlockPos blockPos) {
        for (int i = 2; i >= -3; --i) {
            ...
            if (!AbstractTreeFeature.isAir(modifiableTestableWorld, blockPos2) && i < 0) break;
        }
    }
