...
public class AquiferSampler {
    ...
    public AquiferSampler(int x, int z, DoublePerlinNoiseSampler edgeDensityNoise, DoublePerlinNoiseSampler waterLevelNoise, ChunkGeneratorSettings chunkGeneratorSettings, NoiseColumnSampler columnSampler, int i, int j) {
    ...
    protected void apply(int x, int y, int z) {
        ...
        if (this.waterLevel >= y && AquiferSampler.method_35324(y - this.settings.getGenerationShapeConfig().getMinimumY() - 1)) {
            this.densityAddition = 1.0;
        } else if (d > -1.0) {
            double g = 1.0 + (this.edgeDensityNoise.sample(x, y, z) + 0.05) / 4.0;
