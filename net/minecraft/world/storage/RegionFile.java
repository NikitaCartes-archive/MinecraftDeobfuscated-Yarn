...
implements AutoCloseable {
    ...
    public RegionFile(Path path, class_4486 arg) throws IOException {
        ...
        if (i != -1) {
            if (i != 8192) {
                field_20434.warn("Region file {} has truncated header: {}", (Object)path, (Object)i);
            }
            for (int j = 0; j < 1024; ++j) {
                int k = this.field_20439.get(j);
                if (k == 0) continue;
                int l = RegionFile.method_21878(k);
                int m = RegionFile.method_21871(k);
    ...
    public synchronized DataInputStream method_21873(ChunkPos chunkPos) throws IOException {
        ...
        int j = RegionFile.method_21878(i);
        int k = RegionFile.method_21871(i);
        int l = k * 4096;
        ByteBuffer byteBuffer = ByteBuffer.allocate(l);
        this.field_20436.read(byteBuffer, j * 4096);
        byteBuffer.flip();
        if (byteBuffer.remaining() < 5) {
            field_20434.error("Chunk {} header is truncated: expected {} but read {}", (Object)chunkPos, (Object)l, (Object)byteBuffer.remaining());
            return null;
        }
        int m = byteBuffer.getInt();
        byte b = byteBuffer.get();
        if (m == 0) {
            field_20434.warn("Chunk {} is allocated, but stream is missing", (Object)chunkPos);
            return null;
        }
        int n = m - 1;
        if (n > byteBuffer.remaining()) {
            field_20434.error("Chunk {} stream is truncated: expected {} but read {}", (Object)chunkPos, (Object)n, (Object)byteBuffer.remaining());
            return null;
        }
        if (n < 0) {
            field_20434.error("Declared size {} of chunk {} is negative", (Object)m, (Object)chunkPos);
            return null;
        }
        class_4486 lv = class_4486.method_21883(b);
        if (lv == null) {
            field_20434.error("Chunk {} has invalid chunk stream version {}", (Object)chunkPos, (Object)b);
            return null;
        }
        return new DataInputStream(new BufferedInputStream(lv.method_21885(RegionFile.method_21876(byteBuffer, n))));
    }
    ...
    public DataOutputStream method_21881(ChunkPos chunkPos) throws IOException {
        return new DataOutputStream(new BufferedOutputStream(this.field_20437.method_21886(new ChunkBuffer(chunkPos))));
    }

    protected synchronized void method_21874(ChunkPos chunkPos, ByteBuffer byteBuffer) throws IOException {
        int i = RegionFile.getPackedRegionRelativePosition(chunkPos);
        int j = this.field_20439.get(i);
        int k = RegionFile.method_21878(j);
        int l = RegionFile.method_21871(j);
        int m = RegionFile.method_21880(byteBuffer.remaining());
        if (m >= 256) {
        ...
        this.field_20436.write(byteBuffer, n * 4096);
    ...
    private int getOffset(ChunkPos chunkPos) {
        return this.field_20439.get(RegionFile.getPackedRegionRelativePosition(chunkPos));
    }
    ...
    private void method_21877() throws IOException {
        ...
        if (i != (j = RegionFile.method_21880(i) * 4096)) {
            ByteBuffer byteBuffer = field_20435.duplicate();
            byteBuffer.position(0);
            this.field_20436.write(byteBuffer, j - 1);
        }
    }
